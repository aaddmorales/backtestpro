...
def _protetor_posicao(sb, bot, snap, det):
    ...
    try:
        if lado_pos and lucro is not None and lucro > 0 and cs5 and len(cs5) >= 8:
            fechados5 = cs5[:-1]
            ...
            if eh and el and len(fechados5) >= 3:
                ...
                if lado_pos == "long" and float(c_ant["c"]) > eh[-2] and float(c_ult["c"]) <= eh[-1]:
                    if _prot_cmd(sb, bot, "close_all", {},
                                 f"saida estrutural M5: fechamento {float(c_ult['c']):g} de volta pro canal contra a compra (lucro {lucro:+.2f})"):
                        st["barra_acao"] = bucket
                    return
                if lado_pos == "short" and float(c_ant["c"]) < el[-2] and float(c_ult["c"]) >= el[-1]:
                    if _prot_cmd(sb, bot, "close_all", {},
                                 f"saida estrutural M5: fechamento {float(c_ult['c']):g} de volta pro canal contra a venda (lucro {lucro:+.2f})"):
                        st["barra_acao"] = bucket
                    return
    except Exception:
        pass
    ...
    try:
        ...
        for a in abertas:
            ...
            if not a["lado"] or a["entrada"] is None or not a.get("ticket"):
                continue
            avanco = (preco - a["entrada"]) if a["lado"] == "long" else (a["entrada"] - preco)
            if avanco < 0.6 * atr:
                continue
            alvo_sl = a["entrada"]
            if enviados is None:
            ...
            if _prot_cmd(sb, bot, "mover_sl", {"ticket": a["ticket"], "sl": round(alvo_sl, 5)},
                         f"breakeven: avanco {avanco:.1f} pts >= 0.6 x ATR ({0.6*atr:.1f}) — SL pra entrada"):
                enviados[a["ticket"]] = alvo_sl
                movidos += 1
                acao = True
        if acao:
