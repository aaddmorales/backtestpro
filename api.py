        ...
        if not abertas:
            ...
            try:
                ...
                orfas_r = (sb.table("babymachine_operacoes")
                           .select("id,decisao,ts_entrada,resultado").eq("bot_id", bot.get("id"))
                           .eq("status", "orfa").gte("ts_saida", corte_r)
                           .order("ts_saida", desc=True).limit(10).execute().data or [])
                alvo_r = next((o for o in orfas_r if _bm_eh_orfa_reconciliada(o.get("resultado"))), None)
                if alvo_r:
                    abertas = [alvo_r]
