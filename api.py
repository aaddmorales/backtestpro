# ============================================================
#  BotTested API — v7.49 | Build: 2026-08-16b-bt-cascata-v3 | Deploy: Railway
...
API_VERSAO = "7.49 - BT_CASCATA_V3, O CICLO INTEIRO (decreto do dono 16/ago: 'segue a construcao 1,2,3,4, nao quero deixar nada para tras, so testo quando terminar tudo'. Os 4 itens do desenho viram estrutura, TODOS DORMENTES: (1) REENTRADA POS-CORTE governada pelo 15m (InpCasReentrada) — apos qualquer corte (corte_cascata, corte_ciclo, corte_ciclo2) a janela arma por InpCasReentValid=8 barras de 15m: mesma direcao recusa cascata_pos_corte_aguarda; direcao nova so entra com o 15m confirmando a reversao (cascata_reentrada_sem_15m) — o quadro verde REVERSAO CONFIRMADA NO 15M do desenho. (2) ANDARES VOTAM: InpCas30Conf exige 30m confirmando a tendencia (cascata_30m_nao_confirma); InpCas1hCtx exige 1h nao-contra e sem exaustao (cascata_1h_contra / cascata_1h_exaustao). (3) CORTE DO CICLO 2 (InpCas2Corte): barra FECHADA de H4/D1/W1/MN1 com estrutura quebrada CONTRA a posicao fecha por ticket com motivo=corte_ciclo2 e o TF carimbado. (4) FILTROS ADICIONAIS do desenho: sessao GMT (InpCasSessao 1=Asia 2=Londres 3=NY 4=Lon+NY, recusa filtro_sessao), noticia alto impacto -15/+45min via calendario nativo do MT5 com degradacao honesta se indisponivel (InpCasNoticia, filtro_noticia), volume anormal >=3x media20 (InpCasVolAnormal, filtro_volume_anormal), piso de ATR14 (InpCasAtrPiso, filtro_atr_piso) e correlacao entre ativos por impulso M15 (InpCasCorrSimbolo/InpCasCorrAlinha, filtro_correlacao). TELEMETRIA: casses= (sessao A/L/X/N/F) carimbada em todo aberto — alimenta a probabilidade por sessao do Professor v5 de graca. CONFLITOS REGISTRADOS (sem decisao minha, dono decide com o v5): volume-como-gate ja REPROVOU medido (volume e telemetria, nao voto) e o gate de entrada segue em conflito com a lei do v7.45 — por isso TUDO nasce false. Marcador BT_CASCATA_V3 nas 5 validacoes -> frota regenera no proximo envio sem kill-cache. _bt_base_ligar intocado: nenhuma decisao nova nasce ligada. | 7.48 - BT_CASCATA_V2, AS DECISOES DA CASCATA (Fase 2, construcao completa pedida pelo dono em 16/ago: 'tudo completo antes de testar, com decisoes prontas'. TRES DECISOES no template, TODAS DORMENTES (inputs false — nada sobe sem medicao; ligar e decisao do dono por celula): (a) GATE DE ENTRADA InpCasEntrada — maquina de estado por barra FECHADA do TF da celula: quebra de estrutura (fechamento alem do topo/fundo das 20) ou candidato a EXAUSTAO no extremo ARMA o gatilho na extremidade do candle-gatilho (validade InpCasValid=4); a entrada SO passa quando a barra SEGUINTE rompe essa extremidade — a LEI DE CONFIRMACAO DE ENTRADA de 12/ago a risca; recusas cascata_sem_oportunidade / cascata_sem_confirmacao carimbadas na LM; gatilho consumido no uso. (b) CORTE-5M InpCasCorte (BTCasTick, relogio proprio independente do corte_ciclo): M1 E M5 com estrutura quebrada CONTRA a posicao, sustentado InpCasCorteNJ=2 barras de M5 fechadas -> fecha por ticket com motivo=corte_cascata. (c) TELEMETRIA SEMPRE ATIVA: todo evento aberto carimba casop= (0/Carm/Cconf/Varm/Vconf) + casgat= + casorig= (quebra/exaustao) somando aos cas1/cas2/casconf/casqa da Fase 1 — o acervo pro Professor v5 cresce ligado ou desligado. FIACAO: porteiro nos 2 lados dos wrappers (depois do Ciclo, antes do Nivel), BTCasTick() injetado no OnTick junto do BTCicloTick (so EAs nativos), marcador BT_CASCATA_V2 nas 5 validacoes de cache -> frota regenera sozinha no proximo envio, sem kill-cache. CONFLITO REGISTRADO (sem decisao minha): a lei MEDIDA do v7.45 — atrasar entrada de rompimento destroi celula de rompimento, 4 medicoes — bate de frente com o gate (a) aplicado a Cruzamento/Donchian; o dono decide com o numero do Professor v5 na mao. LEI DO CABECALHO VERSIONADO (decreto do dono 16/ago, REVOGA a Lei do Numero Unico de 11/ago): TODO arquivo carrega o numero da versao atualizada no CABECALHO, sempre sincronizado com API_VERSAO/BUILD_TAG (e APP_VERSAO no front) — cabecalho e constantes atualizados JUNTOS em toda entrega; a ausencia do numero no topo causou confusao real e a fonte-unica-invisivel morre aqui. | 7.47 - BT_CASCATA_V1 FASE 1, OS 2 CICLOS DO DONO (a reconstrucao: o desenho do Adriano — leitura continua da sequencia em formacao, Ciclo 1 de baixo pra cima M1->H1 com antecipacao, Ciclo 2 de cima pra baixo MN1->H4 com contexto — entra FIEL, sob a regra nova desenhado/implementado/testado com aval por coluna. FASE 1 = TELEMETRIA PURA, zero mandato: modulo BTCasTF/BTCasTag nos wrappers le, por TF, impulso (U/D/L nas 5 fechadas + deslocamento), estrutura das 20 fechadas com QUEBRADA (!) e IMINENTE (^, <=0.3xATR no sentido — a quebra de estrutura ANTECIPADA do desenho), e candidato a EXAUSTAO (x, range>=2x media10 + corpo<=35%). Todo evento aberto carimba cas1= (M1.M5.M15.M30.H1) + cas2= (MN1.W1.D1.H4 — as cegueiras Mensal/Semanal caem) + casconf= (maior andar confirmando a historia do M1) + casqa= (quebra antecipada: 1m e 5m quebraram, 30m ainda nao). LEI DE CONFIRMACAO DE ENTRADA registrada (12/ago, memoria permanente): rompimento/exaustao NUNCA entra sem a barra seguinte romper a extremidade do candle-gatilho — vira mandato universal na Fase 3. AJUSTES APROVADOS das 3 divergencias: candidato precoce por exaustao + promocao pelo degrau de baixo; Professor v5 com probabilidade por estado E por sessao na 1a rodada; corte com peso de evidencia + contrafactual. PROXIMO: Professor v5 (simulador FIEL dos 2 Ciclos nos 12m — corte-5m, reentrada 15m/30m confirmada, por celula e por sessao) -> numeros na mao do dono -> mandato por decisao dele. Marcador BT_CASCATA_V1 nas 5 validacoes -> frota regenera no proximo envio sem kill-cache. LEI DO NUMERO UNICO (decreto de 11/ago apos o header trair a versao): a partir desta versao o numero existe SO em API_VERSAO/BUILD_TAG — a linha 2 do arquivo nao carrega mais numero nenhum. | 7.46 - A BASE NASCE LIGADA (a correcao constitucional de 11/ago, depois de TRES frotas seguidas — 71-74, 75-79, 80-83 — nascerem com o Ciclo dormente por falha da corrente manual de ativacao: flag do formulario que nao pegou, Supabase nunca gravado, Properties nao conferidas; o _80 vendeu XAU 2x num foguete de alta e o stop do _83 nasceu 60 em vez de 3000. O decreto do dono: a base e soberana e universal — dormente por padrao virou contradicao com a propria constituicao. QUATRO CIRURGIAS: (1) _bt_base_ligar no /mt5/enviar — as celulas da CASA nascem com as chaves viradas NO CODIGO do EA final (fresco ou cache): Cruzamento/E2 = CicloAtivo+SoFavor true; Donchian BTC = CicloAtivo+CorteCond+NivelBloq true (mandatos medidos 20/27, 7/9, B -4966 -> +351); Donchian nao-BTC so CicloAtivo (NivelBloq reprovou no XAU — lei por celula); PISO DE DESASTRE por ativo no InpStopLoss (BTC 3000 / XAU 150, nunca afrouxa valor maior ja enviado) — mata na raiz o stop 60 que picou _74 e _83; inputs continuam visiveis pra desligar conscientemente, mas o PADRAO e a constituicao; EA custom/IA intocado. (2) MANDATO GRAVADO SOZINHO: o /mt5/enviar grava veto_fase true nas celulas XAU (Cruzamento/E2) e protecao false no Donchian BTC direto no config_operacional — o ritual manual do Supabase deixa de existir; ETH sem veto (nunca medido la). (3) DEDUP DA BABYMACHINE POR TICKET: a reentrada real de 20s do BTC (stop-out + reentrada no mesmo sinal, 10/ago 21:36:51) foi engolida pelo dedup por direcao+30s da v6.89 — History 9 x LM 8; agora ticket diferente = operacao NOVA nos dois estagios do dedup (memoria e banco), duplicata de verdade (mesmo ticket ou sem ticket) segue barrada. (4) build 2026-08-11a. RITUAL DA NOITE: push -> /versao 2026-08-11a -> remover 80-83 -> 4 envios novos -> prova de nascimento nos Inputs do BTC (InpCicloAtivo/InpCorteCond/InpNivelBloq TRUE, stop 3000) -> conferir Supabase gravado automaticamente. Zero SQL, zero Conector; template intocado (o replace e pos-geracao, entao zero kill-cache e zero regeneracao forcada). | 7.45 - BT_NIVEIS_V1, O MAPA DE TETO E CHAO (decreto do dono 09/ago a noite: 'qualquer operacao aberta o sistema tem que saber onde e o teto e onde e o chao' — e a ordem que inverteu o fluxo: medir ANTES de embarcar. O professor v4.6/v4.7 rodou a camada NIVEIS no walk-forward de 12m (XAU/ETH/BTC): detector de pivos fractais k=2 nas 60 fechadas, cluster 0.3xATR, nivel vivo = 2+ toques. VEREDITOS: (a) autopsia derrubou a premissa — entrada colada em nivel NAO rende menos em geral (no ETH, NO_NIVEL foi o MELHOR balde: 1.13/1.02 vs LIVRE 0.87/0.58; a entrada do _68 que motivou tudo pertence ao melhor grupo); (b) espera-confirmacao REPROVADA 0/6 e rompe-degrau (confirmacao no M5) reprovado nas 6 versoes SO-FAVOR — 4a medicao independente da mesma lei, agora PROMULGADA: atrasar entrada de rompimento destroi celula de rompimento, nenhuma forma de espera sobe nunca mais; (c) E2-ancorada reprovada na celula recordista (XAU A 1748->1144); (d) A PEPITA: BLOQUEIO de entrada colada/contra nivel de 2+ toques ATRAVESSOU no Donchian cripto — BTC H1 (celula da frota): base A 1846 / B -4966 -> bloqueio A 13014 / B +351, a metade cega saiu do vermelho pro azul, caso-simbolo mais forte que o do corte; ETH Don H1 tambem atravessou (padrao Donchian-cripto, igual ao corte 7/9); XAU NAO atravessou (nao liga la — lei por celula, nunca universal). O MODULO NO TEMPLATE (wrappers, todos os EAs nativos): BTNivPerto (pivos+cluster via iHigh/iLow, zero lookahead — pivo so conta 2 barras depois de fechado, zero nuvem), BTNivEstado (rotulos identicos ao professor: NO_NIVEL ±0.3 ATR / NIVEL_A_FRENTE 0.3-1.5 a frente / ROMPEU_RECENTE 0.3-1.0 atras / LIVRE), gate InpNivelBloq consultado nos BTBuy/BTSell DEPOIS do porteiro do Ciclo (recusa nivel_bloqueio_* carimbada na LM), e TELEMETRIA SEMPRE ATIVA: todo evento aberto ganha nv=/nv_px=/nv_tq=/nv_atr= (estado no TF da celula) + nv2_* (nivel do DEGRAU DE BAIXO — M15 pro M30, M30 pro H1: o estudo multi-TF do dono acumulando de graca pra proxima medicao). DORMENTE POR PADRAO (InpNivelBloq=false, InpNivelLook=60): deploy nao muda a frota; MANDATO MEDIDO = ligar InpNivelBloq SO no Donchian BTC H1 no reenvio. Painel on-chart do nivel = pendente cosmetico registrado. Marcador BT_NIVEIS_V1 nas 5 validacoes de cache -> EAs regeneram no proximo envio sem kill-cache. Par: professor_v4.py v4.7 entregue ao dono (camada NIVEIS + rompe-degrau; relatorios professor_v46/v47_* arquivados como evidencia). | 7.44 - BT_CICLO_V1 NO TEMPLATE (a lei do fim de semana vira modulo do chassi; decreto de 08/ago: 'primeiro o Ciclo, depois o resto'). Walk-forward 12m em 9 ativos, metade cega: contexto D1xH4 atravessou 20/27 celulas (cruzamento 9/9) e corte CONDICIONAL 16/27 (Donchian 7/9; caso-simbolo Donchian BTC -4966 -> -1803). O MODULO: (a) camada direcional — BTvCicloPermite consultada pelos wrappers BTBuy/BTSell como 2o porteiro depois da disciplina: bloqueia entrada nova com D1xH4 alinhado CONTRA (InpCicloAtivo) e tem modo estrito so-a-favor (InpCicloSoFavor); recusas carimbadas na LM via BTvRecusada (ciclo_contexto_contra / ciclo_exige_favor); (b) corte condicional — BTCicloTick por barra de M5 fechada: com contexto alinhado CONTRA a posicao, fechamento alem do extremo das 20 de M5 sustentado por InpCorteNJ barras (nivel congela na 1a quebra) fecha a posicao por ticket com evento motivo=corte_ciclo; (c) estados D1/H4 = canal EMA20 High/Low na barra FECHADA (iClose/iHigh/iLow multi-TF, zero nuvem — funciona no apagao). DORMENTE POR PADRAO: todos os inputs false — deploy NAO muda comportamento da frota; ativacao e decisao por celula (inputs no MT5 ou proxima geracao com flags). Injecao via wrappers = TODOS os EAs nativos (frota, 4 geradores, cards); caminho da IA fica pra V1.1 (clamp BTTrade). Marcador BT_CICLO_V1 nas 5 validacoes de cache -> proxima geracao/envio regenera com o modulo sem kill-cache. | 7.43 - PLACAR EM DINHEIRO REAL (defeito de produto achado pelo dono em 07/ago: o placar da Learning Machine soma PONTOS entre ativos, mas ponto de BTC vale $0.01 e ponto de ouro $1.00 no lote 0.01 — o agregado mostrou saldo -43.8 pts num dia que o History fechou +$63.59; numero certo, unidade errada, leitura enganosa. FIX: helper _lm_usd le o LUCRO em moeda da conta que o proprio EA carimba no evento de fechamento (OTT/BT_EVT — campo lucro do detalhe_ea), a UNICA soma honesta entre ativos; o /babymachine/estatisticas passa a devolver saldo_usd e usd_medidas em GERAL, POR BOT e POR ATIVO — None quando a linha nao tem lucro registrado (linhas antigas), cobertura honesta em vez de zero mentiroso. Classificacao acerto/erro SEGUE por pontos (regua de medicao intacta); o dinheiro e display, nao criterio. PAR DO FRONT v10.28 que mostra o $ como manchete do placar geral e ao lado dos pts por bot/ativo. Zero SQL, zero template, zero Conector — deploy nao mexe na frota. | 7.42 - ISOLAMENTO POR MAGIC, DOIS BOTS NO MESMO SIMBOLO (observacao do dono na janela de reenvio de 06/ago — a posicao no MT5 e da CONTA, todo grafico do simbolo desenha as mesmas posicoes; com 62 Cruzamento e 63 Estrategia 2 AMBOS em XAUUSD M30, o magic vira o UNICO separador, e a auditoria do template mostrou que duas pecas o ignoravam. A premissa um-simbolo-por-bot deixou de valer de proposito: as duas celulas XAU M30 sao as aprovadas. TRES CIRURGIAS: (1) CLOSE_ALL DO CANAL FILTRA MAGIC — o loop fechava TODA posicao do simbolo; um veto de fase, Guardiao ou R2c disparado no 62 fecharia tambem a posicao do 63 e vice-versa (capital de um bot morto pela regra do outro + deal de fechamento sairia com o magic do executor = telemetria cruzada envenenando a aderencia dos dois); agora so fecha POSITION_MAGIC == InpMagic; o comando close por ticket ja era cirurgico e segue igual. (2) SNAPSHOT POR BOT — BTvPosSimb/BTvLucroSimb contavam/somavam por simbolo: card do 62 diria EM TRADE com posicao do 63, reconciliacao v6.91 nunca drenaria orfa (posicoes_abertas nunca zera com o vizinho posicionado) e o guard de lucro do R2/R2c julgaria flutuante alheio; agora simbolo+magic — cada card, cada reconciliacao e cada guard enxerga SO o proprio bot; BTvDD segue da conta (drawdown e da conta mesmo). (3) MARCADOR BT_MAGIC_ISO_V1 no template + exigido nas 5 validacoes de cache junto com BT_OTT_V2 — regeneracao garantida no proximo envio mesmo se algum cache tiver nascido no intervalo v7.41, sem kill-cache. Acumula o v7.41 (OTT V2 sl no aberto + painel por magic) — e o PUSH E UM SO: quem sobe e este v7.42, o v7.41 nunca precisou ir pro ar. RITUAL INALTERADO: push -> /versao 2026-08-06d -> reenviar os 3 com NOME NOVO -> Supabase (protecao false no Donchian BTC; veto_fase true no Cruzamento e no E2). Zero SQL, zero IA, zero Conector. | 7.41 - OTT V2 + PAINEL POR MAGIC, O TEMPLATE DA PROXIMA JANELA (item 2 da fila de 06/ago, preparado de vespera pra o reenvio da frota. DUAS CIRURGIAS NO TEMPLATE: (1) SL NO EVENTO ABERTO DO OTT — fecha NA RAIZ o gap SEM STOP registrado que o v7.36/v7.37 contornavam por estimativa: no OnTradeTransaction, deal de ENTRADA (IN e a perna aberta do INOUT) seleciona a posicao por ticket (PositionSelectByTicket) e carimba |sl=|tp= no evento aberto quando existem — MESMAS chaves do BT_EVT_V2, entao o backend consome sem mudar uma linha: decisao.sl nasce preenchido, o R0b do Protetor compara DIRETO em vez de estimar por sl_envio_pts, os cards da Learning Machine param de dizer SEM STOP, e a validacao do stop-no-fundo da Estrategia 2 (item 1 da fila, bot 63) sai da propria telemetria em vez de print; posicao sem SL ou select falho = campo omitido com honestidade, comportamento antigo intacto. (2) PAINEL DO EA POR MAGIC — os tres contadores do selo (BTposSimb/BTlucroSimb/BTladoSimb) filtravam SO por simbolo: posicao manual ou legada no mesmo simbolo pintava Posicoes/Lucro/COMPRADO no card de um bot zerado; agora filtram simbolo + POSITION_MAGIC == InpMagic (seguro por construcao: a visao injetada no MESMO bloco ja exige InpMagic, entao todo EA com selo tem o input) — o selo mostra SO as posicoes DESTE bot; marcador BT_PAINEL_MAGIC_V1 no template. UPGRADE DE FROTA: o marcador injetado vira BT_OTT_V2 e as 5 validacoes de cache (3 geradores deterministicos + estrategia2 + caminho da IA/aquecedor) passam a exigir V2 — cache velho regenera SOZINHO no proximo envio, sem kill-cache (padrao v7.30/v7.38); a regeneracao carrega as DUAS cirurgias de uma vez. RITUAL DA JANELA: push + reenviar 61/62/63 com NOME NOVO quando a janela abrir (ate la a frota atual segue operando como esta; o EA no grafico so ganha o template novo em envio novo). Zero SQL, zero IA, zero mudanca no Conector. | 7.40 - ESTRATEGIA 2, A LEITURA DO DONO VIRA CELULA DA CASA (06/ago, aval na sequencia do Professor v2 — o dia em que a tese do produto fechou o circulo: 4 leituras anotadas pelo dono AO VIVO no mesmo dia (XAU 05:55 prevendo a queda confirmada as 06:20; BTC a tarde com a caixa vazia desenhada ANTES do preco chegar) viraram regra escrita, a regra entrou na bancada do Professor v2 junto com os simuladores novos (Donchian + Estrategia 2), e os 6 meses de CSV real julgaram: XAU M30 PF 1.88 sem veto / 2.55 COM VETO DE FASE = O MELHOR NUMERO JA MEDIDO NA PLATAFORMA, acima do Donchian H1 2.08; XAU M5 1.42/1.77 aprovada; BTC M15 1.17/1.32 no fio; BTC M30 1.08 abaixo da regua — e o veto de fase aprovou nas 4 bancadas (+0.34/+0.67/+0.07/+0.15), coerente: a estrategia E leitura de fase, o filtro de fase e o par natural. A REGRA v1 (deliberadamente simples, uma variavel por vez): FUNDO = minima das 12 barras fechadas; ARMA gatilho = maxima da barra do fundo, validade 4 barras; DISPARA no tick ao romper (entrada intrabarra, a leitura do dono); STOP NASCE ESTRUTURAL no fundo - 0.3xATR14 (BT_STOPE_V1 por definicao — aqui o stop estrutural nao e ajuste, e a propria regra) com InpStopLoss de teto de desastre; SAIDA no fechamento alem do canal EMA20 oposto (G1 espelhado), sem alvo fixo; topo simetrico pra venda. ENTREGA: (1) card estrategia2_reversao na Vitrine (casa=True, ambar #f59e0b, ABRE a galeria — a recordista na frente; codigo backtesting.py sandbox-safe com BT_MARCADOR; desc com os numeros e os reprovados, honestidade padrao v7.25); (2) gerador EA NATIVO deterministico BT_ESTRATEGIA2 (zero IA: cache por hash, instrumentacao completa da frota, validacao exigindo os 4 marcadores, magic/SL re-injetados — mesmo chassi do cruzamento/donchian); (3) linhagem automatica Reversao no Extremo quando o marcador chega sem nome. REFINAMENTOS REGISTRADOS pra depois da aderencia (nunca antes): E2 adicao no rompimento do canal, filtro de fundo duplo, Professor com veto por celula desta estrategia. PROTOCOLO DO PRIMEIRO BOT: XAU M30, lote 0.01, stop teto 150, veto_fase true (mandato +0.67, o mais forte medido), dias de aderencia na BabyMachine comparando vivo x simulado ANTES de qualquer conclusao. Par: professor_fases.py v2 standalone entregue junto (simuladores cruzamento+donchian+estrategia2, --celula/--stop). Zero SQL, zero kill-cache. | 7.39 - VETO DE FASE COM MANDATO, A COLHEITA DO PROFESSOR (06/ago, aval do dono; o primeiro mandato MEDIDO da historia da plataforma — o ciclo completo da pergunta de 05/ago fechou em 24h: sensibilidade do dono -> Maquina de Fases v7.34 (telemetria) -> Professor re-simula nos 6 meses de CSV real -> numero decide -> mandato POR CELULA. VEREDITOS IMPRESSOS pelo professor_fases.py standalone rodado no PC do dono: XAU M30 Cruzamento PF 1.44 -> 1.52 com veto, mesmo liquido (+1932 vs +1915) com 63 trades a MENOS (312->249, 488 vetadas — o stop-and-reverse ADIA entrada vetada pro momento com fase em vez de perde-la) = APROVADO; BTC M30 Cruzamento 0.95 -> 0.96 = celula reprovada segue reprovada, veto corta 35 por cento do prejuizo (-8695 -> -5708) mas menos-ruim nao e aprovado — fase nao conserta estrategia errada pro ativo, mandato NAO liga. IMPLEMENTACAO: criterio (d) no Guardiao de Aderencia, GATED por config_operacional.veto_fase (default DESLIGADO em toda a frota — o dono liga celula a celula, hoje so na aprovada): politica IDENTICA a testada — fase LATERAL condena entrada nativa fresca; entrada CONTRA o IMPULSO oposto condena (excecao da REVERSAO na direcao ja embutida na gramatica: REVERSAO_x e outra fase). Motivo carimbado VETO DE FASE (mandato medido pelo Professor) no close_all e no evento bot_guardiao_aderencia — auditoria distingue veto de fase dos criterios a/b/c. A isencao defensiva do v7.37b (RESPIRO/RETOMADA/IMPULSO a favor protege pullback legitimo do criterio c) continua intacta e ortogonal. ACAO DO DONO: push + gravar veto_fase: true no config_operacional do bot 59 (XAU M30 Cruzamento) no Supabase — mesmo ritual do protecao=false. BabyMachine passa a medir o mandato NO VIVO: cada veto vira evento auditavel, e em semanas o placar compara era com/sem veto na celula. PROXIMOS DEGRAUS registrados: Professor v2 com simulador do Donchian (decidir veto nas celulas BTC H1/M30 aprovadas); Estrategia 2 (exaustao + segundo topo fraco + rompimento — caso-modelo carimbado: par de prints do dono 06/ago 05:55 prevendo e 06:20 confirmando) desenhar -> Professor julgar -> so entao bot. Zero SQL, zero mq5_cache, EA intocado. | 7.38 - STOP ESTRUTURAL DE FABRICA, BT_STOPE_V1 (a pergunta final do dono na noite de 05/ago: ele nao pode colocar esse stop onde esta sendo colocado? — e a ordem: faz agora. RESPOSTA: pode, e agora poe. O ciclo backend v7.35-v7.37 conserta o stop em 15-45s; esta entrega conserta NO NASCIMENTO, dentro do EA, no ato da ordem. CIRURGIA NO TEMPLATE dos 2 EAs deterministicos com entrada nativa (Cruzamento do Canal e Donchian 20): (1) helper BTStopEstrutural(compra) — ATR14 manual das fechadas + fundo das 8 fechadas com folga de 0.3xATR pra compra (topo pra venda); (2) o SL da ordem vira sl = mais justo entre ESTRUTURAL e GEOMETRICO: MathMax na compra / MathMin na venda entre slEst e slGeo — numa expressao a catraca nasce pronta E o InpStopLoss vira o que sempre deveria ser, TETO DE DESASTRE (o stop nunca nasce alem dele; no BTC onde o geometrico e mais justo que a estrutura, o geometrico prevalece — nada afrouxa); estrutural invalido ou acima do preco = cai no geometrico puro, comportamento antigo intacto; (3) marcador BT_STOPE_V1 no esqueleto + validacao de cache dos DOIS marcadores passa a exigir BT_STOPE_V1 — caches velhos regeneram SOZINHOS no proximo envio, SEM kill-cache (padrao v7.30); executor puro e caminho da IA intocados (nao tem entrada nativa geometrica). CADEIA COMPLETA a partir daqui: stop nasce estrutural (EA) -> Protetor v7.35-37 aperta em segundos se a estrutura mudar (cinto) -> trailing persegue -> InpStopLoss teto de desastre (lei). ACAO DO DONO: push + reenvio do bot (a regeneracao automatica pega o template novo) — a primeira entrada ja nasce com a linha no fundo do pullback. | 7.37 - O STOP DO ENVIO VIRA CONHECIMENTO DO CEREBRO (fecha de vez o caso do stop la embaixo, na 3a iteracao do dia — e a licao assumida: a trava generica de 1.2xATR do v7.36 era chute com premissa falsa, porque os EAs deterministicos usam InpStopLoss CRU sem piso de ATR: 60 pts no XAU e 4-6x ATR largo, mas 60 pts no BTC e 0.23x ATR APERTADO — nenhum threshold universal existe sem conhecer o stop. A VERDADE SEMPRE ESTEVE NO ENVIO: o /mt5/enviar recebe req.stop_loss e re-injeta no EA; faltava so persistir. DUAS CIRURGIAS: (1) /mt5/enviar grava sl_envio_pts e tp_envio_pts no config_operacional (merge best-effort, todo envio) — o cerebro passa a conhecer o stop de fabrica de CADA bot; (2) R0b do Protetor, quando decisao.sl esta ausente (gap do OTT), ESTIMA: sl_estimado = entrada -/+ sl_envio_pts e compara DIRETO com o nivel estrutural — aperta o XAU largo, respeita o BTC apertado (nivel mais fundo que o stop = nao mexe, catraca intacta); a trava 1.2xATR vira ULTIMO fallback so pra bot sem sl_envio persistido (enviado antes desta versao). ACAO DO DONO: bots 54/55 foram enviados antes — proximo reenvio grava o sl_envio; ate la vale o fallback. PENDENTE mantido: OTT carregar SL no evento (raiz, template+ritual). Zero SQL (JSONB merge), EA intocado. | 7.36 - STOP ESTRUTURAL V2, O FALLBACK DO SL AUSENTE (print do dono minutos apos o deploy do v7.35: a entrada nova do 55 continuou com o stop la embaixo. CAUSA CONFIRMADA, a regra-irma prevista: o R0b exigia decisao.sl gravado pra garantir que so aperta — e o evento aberto do OTT v7.30 NAO carrega o SL (o gap SEM STOP registrado que apareceu em todos os cards da Learning Machine); registro sem stop, R0b pulava com honestidade, linha ficava embaixo. FIX: o aperto passa a funcionar COM ou SEM sl registrado — (a) sl gravado: compara direto, catraca intacta; (b) sl ausente mas ENTRADA gravada (e esta, os cards mostram entrou X): o nivel estrutural so e aplicado se a distancia entrada->nivel for MENOR que 1.2xATR — como o piso de fabrica do EA (R6) garante stop inicial >= 1.5xATR, mover pra menos de 1.2xATR e aperto MATEMATICO sem precisar conhecer o stop; (c) sem sl e sem entrada: pula com honestidade. Freios mantidos: nunca cola a menos de 0.5xATR do preco, uma vez por posicao, ate 3 tickets. PENDENTE ESTRUTURAL registrado: o OTT carregar o SL no evento aberto (PositionGetDouble no OnTradeTransaction) fecha o gap na raiz — mexe template+ritual, entra na proxima janela de reenvio da frota. Zero SQL, zero mq5_cache, EA intocado. | 7.35 - STOP ESTRUTURAL DE ENTRADA (cobranca do dono 05/ago com o print do 54 recem-criado: stop geometrico ~60 pts abaixo da entrada, abaixo ate do topo rompido — o EA calcula o SL inicial por multiplo de ATR cego a estrutura, e num dia de 4 por cento o ATR inflado fabrica risco maximo exatamente nas entradas. CORRECAO DE ROTA da v7.33: eu tinha dito que stop de entrada nao dava backend-only — conclusao ERRADA: a BabyMachine grava decisao.sl da entrada e o canal move stop; o cerebro conhece a estrutura E o stop registrado, entao pode apertar segundos depois da ordem nascer). REGRA R0b no Protetor, entre o Guardiao e o R2c: posicao FRESCA (<=15 min) com sl e ticket registrados ganha stop ESTRUTURAL — fundo das ultimas 8 barras fechadas do TF de operacao com folga de 0.3xATR pra compra (topo pra venda) — via mover_sl, com TRES freios: (1) SO SE APERTA o stop registrado (catraca de risco: nunca afrouxa — se o geometrico ja era mais justo que a estrutura, nada acontece); (2) nunca cola a menos de 0.5xATR do preco (nao vira stop de tick); (3) uma vez por posicao (marca por id na _PROT_STATE), ate 3 tickets por rodada. No caso do print: fundo do pullback ~4230 - folga ~= stop em ~4227 no lugar de 4182 — risco de 60 pts vira ~14, e o trailing assume dali. Zero SQL, zero mq5_cache, EA intocado. | 7.34 - MAQUINA DE FASES V1, TELEMETRIA (a pergunta do dono 05/ago: voce le respiro na hora e o sistema nao — como transferir essa sensibilidade? Diagnostico em tres pecas: eu leio FASES de uma historia e o sistema detecta PECAS isoladas; eu tenho MEMORIA narrativa e cada snapshot nasce orfao; eu leio TRANSICOES e o sistema le estados. A resposta e dar ao sistema a mesma gramatica). ENGINE _fase_mercado deterministica, custo zero de IA, com MEMORIA de sequencia por chave magic+simbolo+tf (_FASE_STATE): LATERAL (range dentro ou canal <0.6 ATR — mata historia direcional), IMPULSO alta/baixa (4+ de 5 fechadas na direcao ou barra >=1.8x o passo, com deslocamento >=1.5x; grava IMPULSO_REF: direcao, passo das barras do impulso, extremo, validade 60 barras), RESPIRO (recuo raso 5-61 por cento do swing com barras contra <=0.7x o passo do IMPULSO de referencia — o contraste e a definicao, exatamente o que o analista faz), RETOMADA (vindo de RESPIRO, fechada a favor rompendo a barra anterior — a entrada classica do dono, e renova o extremo), EXAUSTAO (esticado >2x ATR do canal + fechada de indecisao) e REVERSAO alta/baixa (o classificador do Ciclo v7.32 confirma contra o impulso — as pecas viram frases da mesma gramatica). Saida com CODIGO universal + transicao (RESPIRO->RETOMADA e gatilho semantico) + detalhe com numeros + rotulos i18n pt/en/es prontos (_FASE_I18N — codigo decide e mede igual nos 3 idiomas; texto so na borda, padrao vitrine v6.78). FIACAO: a fase viaja no _scanner_pacote — flui de graca pro /monitor/leitura (front pode pintar), pra IA do cockpit e pro Guardiao; e a BabyMachine CARIMBA fase+transicao em toda entrada via _bm_extrair_contexto — nasce a pergunta que fecha o ciclo de autoconhecimento: entradas em RETOMADA pagam quanto vs em RESPIRO contra, por celula. MANDATO DESLIGADO DE PROPOSITO (regra da casa, alinhada com o dono: narrativa sozinha muda zero por cento; fase com mandato so entra depois que o PROFESSOR re-simular as celulas nos CSVs de 6 meses com o veto de fase ligado e o delta de PF sair impresso — estimativa honesta discutida: PF 1.44 do XAU pra faixa 1.7-2.0 cortando erros de fase, numero final sai da medicao, nao da opiniao). Par: script professor_fases.py standalone entregue junto (le CSV do MT5, roda a mesma maquina, imprime distribuicao de fases e delta de PF da celula com/sem veto). Zero SQL, zero mq5_cache, EA intocado. | 7.33 - GUARDIAO DE ADERENCIA + SAIDA PELA CASCATA (ordem do dono 05/ago, apos o dia dos tres episodios com a mesma raiz — o gatilho nativo do EA no tick operando sem mandato do cerebro: 11 entradas em lateralizacao no BTC que o proprio card nomeou com 719 pts de range, giro cego de 16 segundos comprando XAU dentro de cascata de baixa confirmada com stop geometrico de 60 pts, e saida devolvendo 20 pts que M1/M5 tinham anunciado enquanto o corte esperava o M30. A frase do dono: elimine de uma vez por todas a interferencia; reversao confirmada no 5m corta, nao espera 15min. DUAS CIRURGIAS NO PROTETOR: (1) GUARDIAO DE ADERENCIA — prioridade maxima, roda ANTES do gate de protecao (toggle proprio config_operacional.guardiao default LIGADO; protecao=false desliga so a GESTAO, nunca a legitimidade da entrada — por isso o BTC com protecao off continua guardado): posicao FRESCA (linha aberta da BabyMachine com idade <= 10 min, fonte estrategia_compilada — entradas do detector passaram no portao e sao imunes) e julgada contra a leitura ATUAL do cerebro; entrada ilegitima = close_all MESMO NEGATIVA, porque perda de segundos e menor que stop de 60 pts depois. Tres criterios, qualquer um condena: (a) LATERAL — _detectar_range_lateral diz dentro (dentro do range nao ha trade — a frase do proprio card do 53 hoje); (b) CASCATA CONTRA — cascata de reversao confirmando contra o lado da posicao; (c) CONTRA SEM REVERSAO — alinhamento contra o lado E o classificador do Ciclo v7.32 NAO diz REVERSAO_EM_CURSO na direcao da posicao (comprar faca caindo sem confirmacao, o giro das 18:57). Carimbo em agente_eventos tipo bot_guardiao_aderencia com lado, motivo e preco — toda execucao do Guardiao e auditavel. Select da BabyMachine ganha fonte e created_at pro julgamento de frescor. (2) R2c SAIDA PELA CASCATA — entre o Guardiao e o R9: com lucro no simbolo, cascata de reversao confirmando contra OU M5 fechado ALEM do canal EMA20 oposto = close_all na cadencia do M5 (o R2 original so pegava a transicao suave de-fora-pra-dentro e deixava passar a barra gigante que atravessa o canal inteiro e fecha do lado oposto — o caso de hoje); NEGATIVA exige as duas condicoes juntas (cascata confirmada E fechamento alem do canal) — reversao identificada corta mesmo no vermelho, ruido isolado nao. HONESTIDADE DE ARQUITETURA: o backend nao impede a ordem nativa de NASCER — o Guardiao a mata na proxima leitura (ate ~45s de vida); a eliminacao TOTAL da entrada nativa e reenviar os bots como EA Executor Puro (BT_EXECUTOR_PURO, v7.24) com o detector decidindo em modo automatico, ou a aderencia bt_gate no template — decisao de frota do dono, registrada como proxima. Zero SQL, zero mq5_cache, EA intocado — deploy puro de backend, vale pros 52/53 como estao. | 7.32 - CICLO CIRCULAR V1 (decisao do dono 04/ago apos o caso do Brent/XBRUSD: barra gigante de reversao com 4 pontos de queda — 14x o passo medio de 0.28 — bloqueada pelo portao de tendencia como entrada contra, enquanto 30 barras de M1 contavam a historia em tempo real; a frase do dono: contra a tendencia e rotulo falso quando o sistema distingue reversao de pullback, tempo de sobra existe). TRES CIRURGIAS: (1) _ciclo_reversao_pullback — classificador deterministico com 5 criterios estruturais, custo zero de IA: ESTRUTURA (fechamento quebrou o ultimo swing relevante das 12 barras pre-impulso), ESFORCO (barra contra com range >= 1.8x o passo medio das 12 anteriores, na direcao), CANAL (fechou alem do canal EMA20 H/L calculado das fechadas), CASCATA (cascata de reversao dos TFs menores do scanner v6.82 confirmando na direcao) e PROFUNDIDADE (retrace >= 62 por cento do swing de 30 barras = alem do territorio de pullback). Veredicto: REVERSAO_EM_CURSO com n>=4 ou n>=3 incluindo quebra de estrutura; TENDENCIA_INTACTA com n<=1; senao INDEFINIDO. (2) ITEM 2 DO CHECKLIST REFORMADO: quando a direcao proposta e CONTRA o alinhamento e o ciclo diz REVERSAO_EM_CURSO, o item 2 vira VERDE com detalhe reversao confirmada N/5 criterios e marca contra_tendencia=true no item e no payload — a entrada contra e a primeira da tendencia nova, e a BabyMachine mede se paga (telemetria, nao dogma; se INDEFINIDO o detalhe mostra o placar N/5 aguardando, bloqueio mantido). O payload do checklist ganha campo ciclo completo (veredicto + criterios com detalhe cada) = a FICHA DE DECISAO do ciclo, carimbada na oportunidade via chk como sempre. PULLBACK continua bloqueando de proposito — pullback a favor e territorio de reentrada, nao de contra. (3) ESPERA CARIMBADA (fecha a pendencia (b) aberta desde a v7.12): toda recusa do portao da PLATAFORMA vira evento bot_recusada_plataforma em agente_eventos com bot_nome, simbolo, direcao, motivos, veredicto+n_ok do ciclo, prontidao e estrelas — acaba a autopsia por print: em semanas a BabyMachine responde com numero quanto cada portao salvou vs custou por ativo. Throttle de 1 evento por bot+motivo por barra do TF (_REC_PLAT_STATE, mesma filosofia do debounce v7.19), non-blocking. NOTA DE ARQUITETURA: o relogio do ciclo ja existia — o detector roda a cada snapshot de 15s; o que mudou e que o andar de contexto parou de responder ALTA/BAIXA e passou a responder pullback ou reversao. Zero SQL, zero mq5_cache, EA intocado — deploy puro de backend, vale pros bots 50/51 como estao. | 7.31 - TUTOR V1, O RADAR GANHA PERNAS (aval do dono: quero ver isso funcionando mesmo). Endpoint POST /tutor/momentos: le as fechadas/orfas recentes da Learning Machine (com a razao gravada pelo BT_OTT_V1 em resultado.detalhe_ea) e devolve MOMENTOS TUTORAVEIS deterministicos, custo ZERO de IA, cada um com ancora de UI + titulo + texto com os numeros reais do usuario: stop_apertado (2+ saidas no stop com prejuizo no mesmo ativo; cita referencias das celulas XAU 120-150 / BTC 3000), acerto_catraca (saiu pelo stop EM LUCRO = catraca travou), acerto_regra (melhor operacao positiva), fechamento_manual (coaching de disciplina: a saida e da regra), sem_telemetria (bot online + placar vazio => reenviar bot pos-v7.30) e telemetria_parcial (2+ orfas). Funcao _tutor_gerar_momentos e PURA de proposito (testavel sem Supabase, smoke com linhas sinteticas). Stateless: dispensa/dedup e do front via localStorage. Par do front v10.26 (avatar do Radar anda ate a ancora com spotlight e balao). | 7.30 - BT_OTT_V1, A CURA DA LEARNING MACHINE CEGA (par do diagnostico v7.29; escolhido pelo dono como proxima entrega). Buraco de meses fechado: fechamento por SL/TP acontece no servidor da corretora e o EA nunca emitia evento; e os EAs deterministicos (executor puro, cruzamento, donchian) nem BTEvento tinham — abertura tambem ficava muda. Agora a instrumentacao injeta OnTradeTransaction + BTOttGravar em todo EA SEM BTEvento proprio (nativos de preambulo seguem no canal antigo, zero duplicata; exige InpMagic): captura TODO deal do simbolo+magic e emite aberto/fechado no formato BOTTESTED_EVENTO que o Conector ja le, com preco, lado, ticket, deal, razao (sl/tp/expert/stopout/manual) e lucro — o casamento v6.98 do backend (ticket para simbolo para FIFO) ja consome. Reversao INOUT emite fechado+aberto. Validacao de cache dos 3 marcadores deterministicos E do caminho da IA agora exige BT_OTT_V1: caches velhos regeneram sozinhos no proximo envio, sem kill-cache. ACAO DO DONO: reenviar os bots das celulas (o EA no grafico so ganha OTT em envio novo) — XAU M30 stop 120 protecao ligada; BTC stop 3000 e protecao=false no Supabase. | 7.29 - DIAGNOSTICO NA TELA DA LEARNING MACHINE (par do front v10.25; pedido do dono apos 2 bots EM TRADE com a aba vazia). CONTEXTO: o card diz EM TRADE via SNAPSHOT (/conector/snapshot, canal read-only sempre vivo), mas a BabyMachine so grava linha na chegada do evento 'aberto' em /conector/evento — dois canais distintos; quando o de eventos fura, a aba fica vazia sem dizer por que. AGORA o /babymachine/operacoes, SO quando o periodo vem VAZIO (total==0), devolve 'diagnostico' com o MOTIVO: (a) ha_fora_periodo = ha operacao registrada deste bot nos ultimos 365d mas fora da janela selecionada -> ampliar filtro; (b) eventos_sem_linha = chegaram eventos bot_aberto na janela (casados por bot_nome, fallback simbolo) mas nenhuma linha foi gravada -> falha de casamento/backend; (c) sem_eventos = nenhum evento de abertura chegou -> Conector/EA desatualizado, reenviar bot com NOME NOVO + confirmar Conector atual. Custo zero no caminho quente: so 1-3 queries indexadas quando ha o que diagnosticar. Mesma filosofia do BT-G01 e da IA morta (v6.93): a tela entrega a causa-raiz sem cacar log do Railway. | 7.28 - ECONOMIA DE IA NA LEITURA (prioridade maxima do dono 28/jul: ~US$12/DIA de API so de leitura = inviavel por assinante). Diagnostico: _narrar_bot_analitico re-chamava a IA a cada 300s MESMO SEM MUDANCA (288+/dia/bot parado) e imediatamente a cada mudanca de assinatura FINA (padrao de vela, momentum e % do range mudam o tempo todo em mercado vivo), com system prompt de ~2k tokens por chamada. Fix: (1) assinatura identica => reusa pra sempre, zero chamada; (2) assinatura GROSSA nova (_bot_assinatura_grossa: regime/posicoes/range/gatilho/cascata/swing) muda => re-narra na hora; (3) mudanca so fina => re-narra no maximo 1x por barra do TF de operacao (_narr_tf_seg, piso 300s teto 3600s); (4) kill-switch env BT_IA_LEITURA=off => leitura IA desliga e painel cai no texto deterministico, custo zero. Estimativa: corte de 90-97 por cento do custo diario; leitura ja rodava em Haiku. Proximos da fila se a fatura persistir: _analisar_ia (mentor) e checklist. | 7.27 - DONCHIAN 20 DOIS LADOS vira estrategia da casa medida: pedido do dono na noite de 28/jul apos varredura web (consenso: tendencia/rompimento domina BTC) + medicao nos CSVs reais da IC Markets. Numeros liquidos de spread: XAUUSD positivo em TODOS os TFs (H1 PF 2.08 +1944 pts DD -462, M30 1.57, H4 1.71, M15 1.28), BTCUSD aprovado em H1 (PF 1.49 +33591) e M30 (1.34, 6/7 meses+), BTC H4 reprova (0.63), US500 reprova em todos (3a estrategia consecutiva reprovada nesse indice). Card rompimento_donchian promovido: casa=True, cor_destaque ciano, regra reescrita pros DOIS LADOS (a antiga era Turtle so comprada), codigo sandbox-safe sem imports com BT_MARCADOR, celula_real BTC/USD 1h com lista de ativos medidos (prego da vitrine agora aceita lista), desc pt/en/es com os numeros e os reprovados. Marcador BT_DONCHIAN_20 no gerador = EA MQL5 deterministico (bandas das 20 barras fechadas recalculadas por barra, gatilho no tick fiel a medicao, sem TP, InpStopLoss stop de desastre, cache por hash, instrumentacao da frota, SEM kill-cache). Stops de desastre recomendados (pior trade dos 6 meses): BTC 3000, XAU 150. | 7.26 - FIX SANDBOX DO TESTE (bug latente da frota inteira, achado ao testar o card novo): SAFE_BUILTINS nunca teve __import__, entao TODO codigo de card com import estourava ImportError no exec e o /backtest/custom caia CALADO no motor padrao — que nao conhece a estrategia do card — devolvendo 0 trades e 0 candles (era o resultado zerado que o dono viu duas vezes). Fix: o ns ja fornece pd/np/Strategy/crossover, entao as linhas de import sao removidas antes do exec; import perigoso continua barrado pelo AST. Bonus: codigo do card cruzamento_canal reescrito sem imports e sem comecar com cerquilha (dois caminhos internos tratam codigo iniciando em cerquilha como vazio) — o marcador virou atribuicao BT_MARCADOR, o gerador de EA continua achando o texto BT_CRUZAMENTO_CANAL normalmente. | 7.25 - CRUZAMENTO DO CANAL: a estrategia da casa que o dono validou em 28/jul virou card + EA nativo deterministico. Medicao em 6 meses de dado REAL do MT5 (35 mil barras M5 exportadas da IC Markets): XAUUSD M30 stop-and-reverse fez PF 1.44, +1916 pts, 312 trades, positiva em 6 de 7 meses, ganho medio 61.8 x perda media 20.5. A regra: fechou ABAIXO do canal EMA20 High/Low = vende; fechou ACIMA = compra; sai e INVERTE no cruzamento oposto; decide SO em barra fechada; sem take profit por desenho; InpStopLoss vira stop de DESASTRE em pontos de preco. Card cruzamento_canal com cor_destaque violeta na vitrine (casa=True) + codigo backtesting.py pro Teste + marcador BT_CRUZAMENTO_CANAL que o gerador transforma em EA MQL5 deterministico (zero IA, mesmo padrao do executor puro: cache por hash, magic/SL forcados depois, instrumentacao completa da frota, SEM kill-cache). Linhagem automatica quando o marcador chega sem nome. MEDICAO MULTI-ATIVO ANTES DO DEPLOY (CSVs M30/M15/M5 de 6 meses dos 4 ativos): XAUUSD M30 APROVADO (PF 1.44, liquido +1904 pts apos spread, 6/7 meses positivos); BTCUSD REPROVADO em todos os TFs (PF 0.98 bruto e o spread de ~13 pts/trade transforma em -9084 liquido no M30); US500 REPROVADO (PF 0.85, 1/7 meses positivo — indice que sobe em serrote pica o stop-and-reverse simetrico); NASDAQ pendente (export veio com o simbolo errado, acao NDAQ em vez do indice USTEC). A regra NAO viaja de ativo — e a vitrine diz isso na cara: mercados do card = so XAUUSD, desc registra os reprovados. Protocolo: 1 bot XAUUSD M30 lote 0.01; USTEC decide quando o CSV certo chegar. | 7.24 - REMESSA 2: EA EXECUTOR PURO (o chassi da doutrina — fecha o LOOP COMPLETO pela primeira vez: leitura do dono -> entrada -> gestao -> saida. Diagnostico do veredito de 28/jul: o buraco nunca foi leitura nem gestao — a ENTRADA sempre veio de EA nativo executando regra local burra que ignora tudo que a plataforma le; e o canal ja executava buy/sell com lote+SL+TP, o modo automatico do detector ja criava a ordem quando o portao abria, a Estrategia 1 (Caso 0c v7.18) ja era gatilho formal — faltava SO o EA sem estrategia nativa. AGORA: codigo enviado pelo Editor contendo o marcador BT_EXECUTOR_PURO gera um EA DETERMINISTICO (zero IA, zero custo, zero risco de compilacao criativa): esqueleto minimo com OnInit/OnTick/OnDeinit VAZIOS de estrategia + a instrumentacao BotTested completa injetada pelo mesmo _instrumentar_log_mql5 de sempre — visao (snapshot/eventos), canal de comando (BTLerComando executa buy/sell/close/close_all/mover_sl), disciplina e painel. O EA nao decide NADA: quem decide a entrada e o detector com a leitura do dono (portao camada 1: checklist + tendencia + Caso 0c), quem gerencia e o Protetor v7.22, quem executa e o canal. estrategia_nome registrado automaticamente como Executor BotTested quando o marcador chega sem nome (linhagem nunca vazia). SEM mudanca de template = SEM kill-cache, SEM aquecedor, frota intocada. RITUAL DO TESTE (protocolo cientifico congelado com o dono): 1 celula, XAUUSD M5, lote 0.01, modo AUTOMATICO no cockpit, 3-5 dias SEM MEXER; julgar ADERENCIA primeiro (cada entrada/saida com motivo auditavel), resultado depois; uma variavel por vez. - VITRINE CURADA PASSO A (o problema de produto nomeado pelo dono em 27/jul: usuario escolhe card, Teste da negativo, escolhe outro, negativo de novo — a plataforma e honesta ou eu sou uma merda? Honestidade sem caminho mata a conversao. A Vitrine deixa de ser catalogo neutro e vira CURADORIA com o dado que JA existe, sem esquenta: (1) regua de aprovacao POR CELULA ativo x TF — estrelas >= 4 (mesma _vit_estrelas da escada, com dedup anti-evidencia) + melhor combo do TF com retorno positivo, PF >= 1.3 e sharpe > 0; (2) cada card ganha celulas_aprovadas (ate 3, colapsadas: so estrelas + faixa, anti-vazamento mantido) e o campo aprovacao (aprovada / observacao / em_medicao); (3) resposta ganha melhores_celulas — as ate 6 melhores celulas aprovadas do banco inteiro, prontas pro topo da Vitrine com 1 clique no Probar pre-configurado; (4) ordenacao: cards com celula aprovada primeiro, depois medidos, depois em medicao. O usuario nunca mais precisa cacar celula boa na mao — a experiencia vira escolhi -> o Teste CONFIRMOU -> enviei. PASSO B ja desenhado: quando a doutrina entrar no simulador (BT_FABRICA_V2, junto do chassi executor) o esquenta remede o banco e a mesma regua reaprova sozinha — a auditoria estrategia-a-estrategia sai de graca da medicao. - DOUTRINA REMESSA 1 (Parametros de Seguranca fechados com o dono 27/jul — a consciencia do bot; esta remessa cobre tudo que NAO mexe no EA). (R9 REVERSAO IDENTIFICADA, regra 9 da doutrina, funde a antiga R4/aborto): janela de 6 barras de M1 confirmando pro M5 — SEM 15m de proposito, 6 minutos identificam; identificar NAO e esperar os 6 minutos: fecha assim que os TRES confirmadores batem CONTRA a posicao: (a) ESTRUTURA pin bar no M5 fechado ou cluster de M1 (4+ de 5 fechamentos contra), (b) VOLUME alterado (media das 6 M1 da janela >= 1.4x a media das anteriores), (c) LOCALIZACAO preco a <=0.2xATR de nivel de Fibonacci 38.2/50/61.8 do swing recente do M5. Com os tres -> close_all MESMO NEGATIVA (o aborto que o dono exigiu: bot que espera o stop perdendo e inaceitavel); exigir os tres e o equilibrio pra nao fechar demais. (ESCADA DEGRAU 2, regra 3): depois do breakeven o SL PERSEGUE o canal EMA20 do M5 — cola no low do canal pra compra (high pra venda) e sobe a cada vela fechada a favor; catraca mantida, nunca cruza o preco. (BREAKEVEN POR FECHAMENTO, ajuste do dono na regra 7): o avanco de 0.6xATR passa a ser medido no FECHAMENTO da ultima M5 fechada, nao no tick — espeto que encosta e volta nao move mais nada. (ESCOPO DO SIMBOLO na R2): o guard de lucro da saida estrutural passa a usar o lucro em PONTOS das posicoes registradas do proprio simbolo, nao o flutuante da conta — corrige o disparo das 05:31:49 de 27/jul em que o verde de outro simbolo abriu o guard com as posicoes do simbolo negativas. Prioridade nova: R9 aborto > R2 saida estrutural > R3 pin aperta > R1 breakeven > R5 perseguicao. - PROTETOR DE POSICAO (aval do dono na noite de 26/jul, apos o dia de 19h: 3 buys no 41 em BTCUSD — 64680/64742/64737, topo ~64781 — melhor excursao ~39 pts contra vela media de M15 de ~123 pts; o trailing por barra de M15 MATEMATICAMENTE nao podia armar, o sistema VIU a virada (pin bar no checklist, cascata no scanner) e nenhum componente tinha mandato de agir: o detector so nasce ENTRADA e a saida era 100 por cento do EA compilado. O PROTETOR fecha esse buraco usando o canal de comando v6.73 que ja existe ponta a ponta — zero mudanca no EA, zero kill-cache, zero reenvio. Roda no /conector/snapshot SO com posicao aberta, deterministico, custo zero de IA. REGRAS, em ordem de prioridade: (R2 SAIDA ESTRUTURAL M5 — a regra do dono da v7.18) fechamento de M5 de VOLTA pra dentro do canal EMA20 H/L contra a posicao, com flutuante positivo -> close_all: a saida sai na cadencia do M5, nao na do M15; exige transicao real (barra anterior fora, fechada dentro) pra nao disparar em posicao que nunca esteve fora. (R3 SINAL DE REVERSAO) pin bar/estrela cadente na ultima barra FECHADA do TF de operacao contra a posicao -> aperta o SL pro extremo da barra do sinal, SO quando esse extremo ja esta alem da entrada (garante breakeven-ou-melhor sem precisar conhecer o SL atual). (R1 BREAKEVEN) avanco >= 0.6 x ATR do TF de operacao a favor da posicao -> mover_sl pra entrada. FREIOS DE SEGURANCA: so age com lucro ou reduzindo risco (SL do protetor nunca e pior que a entrada; anti-afrouxo cross-restart consultando os mover_sl anteriores do proprio bot na tabela mt5_comandos antes de enviar); 1 acao por barra de M5 por bot (_PROT_STATE); ate 3 mover_sl por acao; respeita o teto diario MAX_COMANDOS_DIA_POR_BOT e o bot online; toggle config_operacional.protecao (default LIGADO; false desliga) — o protetor NAO respeita o pausado de proposito: pausado desliga ENTRADA nova, protecao de posicao viva e reducao de risco e continua (decisao registrada). Entrada e ticket por posicao vem das linhas abertas da babymachine_operacoes (v6.89 grava entrada/sl/tp/ticket/lote); posicao sem ticket registrado e pulada com honestidade no mover_sl (close_all nao precisa de ticket). Comandos auditaveis na mt5_comandos com params.motivo (origem auto_detector). No cenario de hoje: breakeven armaria nos +74 pts (0.6 x ATR15), a pin bar do topo apertaria o SL e o M5 fechando de volta no canal fecharia no verde. | 7.20 - R:R CONDICIONAL DO PLANO EM FORMACAO (analise do checklist com o dono, print do BOTTESTED_41 em BTCUSD: o BOT narrava estrutura de swing FORMANDO com plano completo — neckline, fundo P2, projecao por measured move — e o item 5 do checklist mostrava 0 por cento sem projecao, porque o _chk_leitura v7.04 so derivava R:R de RANGE ROMPIDO. A cegueira: o plano condicional da estrutura em formacao E um plano medido (entrada no rompimento da neckline, stop na invalidacao P2, alvo na projecao) e o card ignorava. FIX: quando nao ha rompimento de range, o R:R do card passa a ser derivado da ESTRUTURA DE SWING do extras — estado formando: entrada=neckline, risco=|neckline-P2|, alvo=projecao; estado rompeu: entrada=preco agora, invalidacao=neckline, alvo=projecao — com a MESMA regua da escada v7.09 promovendo o alvo pros niveis medidos alem do risco. O detalhe do item 5 ganha o rotulo (plano condicional - no rompimento da neckline X / plano do rompimento da estrutura) pra honestidade: e o R:R do plano que SO vale se disparar, nao de posicao aberta. A direcao do card tambem ganha o swing como ultimo fallback da cascata. Zero mudanca no gate real do detector (que ja usa o rr dos niveis), zero SQL, EA intocado. Par do front v10.22 (fix do estrategia_nome perdido no envio via Vitrine: indicadorMudou anulava a _estrategiaAtiva e o payload ia vazio — agora ha copia duravel window._estrategiaNomeEnvio). | 7.19 - ANTI-FLOOD DA RECUSADA (BT_DISCIPLINA_V3; causa-raiz do OFFLINE intermitente do BOTTESTED_40 achada pela aba Experts em 26/jul: com o teto de posicoes atingido (R3, InpBTMaxPosicoes=2) a estrategia tenta entrar a CADA TICK e o BTvRecusada emitia Print + gravacao no bt_ev_ POR TICK — dezenas de eventos recusada por segundo. O Conector afogava drenando a enxurrada e o SNAPSHOT chegava com 45-60s de idade; OFFLINE_APOS_SEGUNDOS=45 derrubava o card = offline intermitente sem queda real. A pasta de dados provou: bt_ev_1670310741 fresco, bt_snap_ parado. FIX: debounce no BTvRecusada — evento sai UMA vez por barra do TF por lado+motivo (static btRecBarra/btRecChave); repeticao na mesma barra e silenciosa. A BabyMachine nao perde informacao (a recusa repetida da mesma barra e identica) e o canal de eventos volta a ter espaco pro snapshot. MARCADOR DE FROTA vira BT_DISCIPLINA_V3: HIT do cache e aquecedor exigem V3 — toda entrada V2 e descartada e regenerada sozinha no proximo envio, SEM kill-cache manual. DEPLOY: Railway deploy + reenviar bot com NOME NOVO (o EA no grafico continua V2 metralhando ate ser trocado). | 7.18 - CANAL EMA 20 H/L + BOLLINGER: A METODOLOGIA DO DONO VIRA CASO FORMAL DO DETECTOR (fecha a pendencia (d), aberta desde 25/jul). Capturada em 26/jul por dois prints anotados + explicacao dele, e a descoberta que reorganizou o desenho: CRUZAMENTO DE MEDIAS E REVERSAO SAO A MESMA ESTRATEGIA — uma operacao com DUAS entradas. (G1 REVERSAO, nasce a operacao) barra A fecha com exaustao (extremo tocando a banda de Bollinger) -> barra B fecha CONFIRMANDO o sentido -> ENTRADA no ROMPIMENTO DO EXTREMO DA BARRA B, intrabarra (nao espera fechamento: e o que resolve o entrei-atrasado), stop no extremo da exaustao. (G2 DOUBLE) fechamento alem do canal EMA20 H/L no mesmo sentido = sinal de continuidade, dobra a posicao. (PERMANENCIA) enquanto NAO fechar de volta pra dentro do canal, PERMANECE — SEM ALVO, SEM TAKE PROFIT; a saida e estrutural, nao numerica. (SAIDA) fechamento de volta pro canal ou G1 no sentido oposto (sai e inverte). (VETO) preco dentro do canal = lateral, nao opera; canal comprimido (largura < 0.6x ATR14) tambem nao. TIMEFRAMES (decisao do dono): M5 DECIDE — menos ruido que o 1m, mais limpo de confirmacao; M1 mede o volume; M15 confirma quem esta no controle e so entao autoriza o DOUBLE (o M15 NUNCA filtra a ENTRADA: numa reversao ele aponta a direcao velha por definicao, entao exigi-lo na entrada impediria toda reversao). NUMEROS DO CASO REAL que motivou tudo: a mesma OP1 rendeu ~200 pts com a regra do dono e teria rendido 18%% disso com o alvo fixo de 36 pts que o bot usava. A entrada nunca foi o problema — a SAIDA era. Implementacao: _detectar_canal_ema_exaustao roda no TF de operacao junto do range e do swing (mesmo padrao arquitetural), entra em fatos.canal_ema_setup, e o Caso 0c do _detectar_direcao_potencial le dali; deterministico, custo ZERO de IA. | 7.17 - REFUNDACAO DA BIBLIOTECA (4 itens numa remedicao so, porque os quatro tocam o mesmo motor e entregar em separado obrigaria remedir o banco quatro vezes). O ACHADO QUE REORDENOU TUDO: o _matriz_calcular media com stop_loss=50/take_profit=100 e o simulador usa params/10000, ou seja STOP DE 0,50%% DO PRECO e ALVO DE 1,00%% (R:R 2,0) — no BTCUSD a 64k isso e stop de ~320 pontos, enquanto o EA em producao arriscava ~56. A biblioteca nao media uma calibragem diferente: media OUTRO SISTEMA. Fazer amostragem antes de consertar a regua seria medir melhor a coisa errada. (1) REGRAS DE FABRICA NA MEDICAO — _bib_risco_fabrica converte o R6/R7 do EA pras unidades do simulador: stop = max(0,50%%, 1.5x ATR do TF) e alvo = 1.5 x risco real, exatamente o BT_DISCIPLINA_V2; ZERO mudanca no simulador (so os params de entrada), entao backtest do usuario segue intocado. (2) JANELA MOVEL — _BIB_COMBOS deixa de ser lista de periodos e vira 3 JANELAS NAO SOBREPOSTAS por TF, fatiadas do df ja baixado: acaba a duplicata de 15m/30m (o proprio baixar_dados forca 60d nesses TFs, entao 6 meses e 1 ano baixavam o MESMO arquivo) E acaba o aninhamento do 1d (2y dentro de 3y dentro de 5y media quase o mesmo mercado 3x). Agora TODO timeframe tem n=3 independente, cada janela num regime diferente, com o mesmo dado e UM download por TF. (3) RECEITA GRAVADA — cada linha leva parametros (jsonb), codigo_hash e regras_versao (BT_FABRICA_V1): o numero do card vira reproduzivel e a Vitrine passa a distinguir medicao velha de nova. (4) CAPTURA_PCT — retorno / amplitude disponivel na janela: a metrica que faltava e que deixou passar alvo de 36 pts numa perna de 300. MIGRACAO SUAVE: a vitrine prefere, POR CELULA, as linhas da regras_versao atual quando existem, e cai nas antigas quando nao — durante a remedicao nenhum card fica vazio e nenhuma media mistura as duas eras. Requer o ALTER TABLE das 5 colunas novas (ja rodado). | 7.16 - DEDUP NA RAIZ DA BIBLIOTECA (query do dono confirmou a suspeita e foi pior que o esperado: 594 de 594 celulas de 15m e 30m — 18 estrategias x 33 ativos, o banco INTEIRO nesses TFs — tinham duas linhas com numeros IDENTICOS e periodo diferente, enquanto 1h/4h/1d tinham so 7-10 coincidencias em 594, ruido normal. Causa: o yfinance corta intradiario em ~60 dias, entao 6 meses e 1 ano baixam o MESMO arquivo. Eram ~1.188 linhas de eco num banco de ~7.100 = 17% do estudo, entrando com PESO DOBRADO em sharpe_medio/winrate_medio/pf_medio de TODO card e inflando o n das estrelas — o problema nunca foi so das estrelas. FIX EM DOIS PONTOS: (1) DEDUP NA RAIZ, logo apos a paginacao da estudo_biblioteca e antes de qualquer acumulo — chave (estrategia_id,ativo,timeframe,sharpe,win_rate,trades,retorno); vale pra medias, top_ativos, melhor_combo e escada de uma vez; (2) _BIB_COMBOS deixa de PEDIR o periodo inutil em 15m/30m (so 6 meses) — mesma evidencia, metade do tempo de esquenta, e para de fabricar copia. PENDENTE (unica saida real pro intradiario): JANELA MOVEL — o motor recebe rotulo de periodo e nao datas; fatiar os ~60 dias disponiveis em janelas nao sobrepostas daria 3 medicoes INDEPENDENTES por celula, cada uma num regime diferente, com o mesmo dado. | 7.15b - ESTRELAS JUSTAS + ANTI-EVIDENCIA DUPLICADA (o fix das estrelas relativas abriu um flanco: o yfinance corta intradiario em ~60 dias, entao 15m/30m pedindo 6 meses E 1 ano devolvem a MESMA janela e gravam duas linhas identicas com periodo diferente — contadas como 2 medicoes, virariam 5 estrelas em cima de evidencia que nao existe. Agora _vit_escada_tf DEDUPLICA por assinatura (tf+sharpe+win_rate+trades+retorno): medicao repetida vale UMA, e a celula cai pro teto honesto sozinha, sem precisar saber os limites do provedor. Nota operacional: rodar o esquenta N vezes NAO gera dado novo — o gravador e upsert na chave (ativo,periodo,timeframe,estrategia_id) e o backtest e deterministico; o que aumenta evidencia de verdade e JANELA MOVEL (fatiar o historico disponivel em varias janelas nao sobrepostas), nao repeticao. | 7.15 - ESTRELAS JUSTAS ENTRE TIMEFRAMES (observacao do dono depois de subir a v7.14: nao tem NENHUMA estrategia de 15m/30m na vitrine, so D1 e H1. CAUSA-RAIZ, e era regra minha: _vit_estrelas exigia n>=3 medicoes pra 5 estrelas, mas o _BIB_COMBOS deste mesmo arquivo so consegue medir 3 periodos no 1d — 15m/30m/1h/4h tem 2 e o 5m tem 1, por limitacao de historico intradiario do yfinance. Resultado: SO o D1 podia chegar a 5 estrelas, nunca por ser melhor, e sim por ser o unico com 3 medicoes possiveis; o intradiario estava estruturalmente tetado em 4 pra sempre. O vies ainda entrava DUAS vezes no mesmo card, porque o ranking de top_ativos usa max(sharpe) e mais medicoes = mais bilhetes na rifa. FIX: a exigencia vira RELATIVA AO OBTENIVEL — _vit_periodos_tf le o proprio _BIB_COMBOS e _vit_estrelas passa a exigir n >= max(2, min(3, periodos_disponiveis)): 1d segue precisando de 3, 15m/30m/1h/4h passam a precisar de 2 (e podem enfim chegar a 5), e o 5m fica honestamente tetado em 4 porque UMA medicao nao e evidencia — isso e limite real do dado, nao vies. Regra nao afrouxa nada mais: sharpe>=2.5 e aproveitamento cheio continuam valendo. | 7.14 - VITRINE POR TIMEFRAME (desenho do dono apos o achado do dia: BTCUSD em M5 e cemiterio — NENHUMA das 18 estrategias tem resultado bom em 5m, e o BOTTESTED_32 rodava justamente ali; a Escalonada que deu -4,39 em 5m da sharpe 1,01 em 1d, como o nome dela diz. O TIMEFRAME e o dado que decide e era o unico que o card nao mostrava, enquanto o percentual gigante gerava expectativa que o Teste nao consegue honrar). MUDANCAS: (1) _vit_escada_tf monta, por estrategia+ativo, a ESCADA DE TIMEFRAMES com ESTRELAS 1-5 — rank relativo entre os TFs daquele ativo com PISO ABSOLUTO por cima: 5 estrelas exige sharpe>=2.5 + todos os combos bons + n>=3; teto de 4 com menos de 3 medicoes (senao card ruim ganhava 5 estrelas so por ter o menos pior). Ordenacao: aproveitamento bons/n primeiro, sharpe como desempate; (2) FAIXA DE ACERTO em vez de numero cravado — win rate min-max arredondado em 5 (ex. 65-75%): com 2 medicoes por celula, uma casa decimal e falsa precisao, e a faixa absorve a diferenca ate a biblioteca ser remedida; (3) ANTI-VAZAMENTO DO BANCO (decisao do dono, que vetou o seletor de ativo no topo): a escada sai COLAPSADA DO BACKEND — no maximo 3 TFs individuais + uma linha agregada demais; a API nunca devolve a matriz TFxativo inteira, nem pra quem chamar o endpoint direto; (4) tf_destaque = melhor celula do card (tf + ativo + faixa) pra frente do card. Tudo group by no que ja estava na estudo_biblioteca — zero SQL novo, zero IA, dentro do cache de 1h da vitrine. PENDENTE (estudo pedido pelo dono, NAO codar antes de decidir): DESCOMPASSO VITRINE x TESTE — a estudo_biblioteca nao guarda parametros nem codigo_hash, entao o numero do card nao e reproduzivel pelo Teste nem em principio; proposto card virar RECIBO (estrategia+ativo+TF+periodo+parametros) com o botao Probar abrindo o Teste pre-configurado, estudo_biblioteca ganhar parametros jsonb + codigo_hash + versao das regras, e a biblioteca ser REMEDIDA pelo mesmo caminho de producao (a v7.13 mudou o alvo e invalidou os win rates atuais) — na remedicao entra tambem a coluna captura_pct. | 7.13 - ALVO PROPORCIONAL AO RISCO (BT_DISCIPLINA_V2; caso real 26/jul no BOTTESTED_32 em BTCUSD M5: 5 operacoes, TODAS com S/L preenchido (o EA Disciplinado da v7.12 funcionou), 3 acertos em 5 — e o dia fechou NEGATIVO (-16,74 pts). CAUSA-RAIZ medida: o R6 da v7.12 passou a ALARGAR o SL ate 1.5x ATR(14), mas o TP continuou no valor FIXO do input InpTakeProfit (36 pts) — stop calculado, alvo digitado. Resultado: risco medio 56,4 pts contra alvo medio 38,6 = R:R 0,58 nas 5 entradas (realizado 0,58; com 40,44%% de win rate da estrategia o R:R minimo pra EMPATAR e 1,47 — expectativa matematica de -20 pts POR OPERACAO). Consertamos um lado da balanca na v7.12 e desequilibramos o outro. FIX (R7 — ALVO PROPORCIONAL): BTvDisciplina passa a receber o tp POR REFERENCIA e, DEPOIS de o R1/R6 fecharem o SL definitivo, recalcula o alvo como MULTIPLO DO RISCO REAL: alvo = preco +/- k * |preco - sl|. So ESTICA, nunca encurta (mesma regra da v7.09: promove alvo, nunca corta). k = InpBTAlvoRR (1.5) em condicao normal e InpBTAlvoTendRR (3.0) quando a BTvTendencia detecta o TF do grafico a favor da direcao da ordem (fechamentos 1>5>10 pra compra, 1<5<10 pra venda) — em tendencia o alvo sai da frente e quem manda e o trailing por barra da v7.12, que ja provou funcionar (2 das 3 saidas boas do dia 26/jul foram trailing, nao alvo). Sem indicador novo, sem handle, sem chamada de IA. MARCADOR DE FROTA BT_DISCIPLINA_V2 (fecha a pendencia (a)): o HIT do mq5_cache e o aquecedor passam a exigir BT_DISCIPLINA_V2 alem de BT_EVT_V2 — entrada sem o marcador e descartada e REGENERADA sozinha, entao bot velho nao nasce mais com R:R invertido nem sem as regras de fabrica. DEPLOY RITUAL: kill-cache + aquecedor + reenviar bots com nome novo. PENDENTE NA SESSAO: (b) backend registrar tipo=recusada na BabyMachine; (c) aderencia bt_gate; (d) caso EMA-canal; (e) ABORTO EM DOIS ESTAGIOS (M1 arma / M5 decide, contra a ESTRUTURA) — exige o catalogo de ponto de entrada por estrategia; (f) coluna captura_pct no estudo (pontos feitos / amplitude disponivel), porque nenhuma metrica atual mede CAPTURA e foi essa ausencia que produziu alvo de 36 pts numa perna de 300. | 7.12 - EA DISCIPLINADO (sessao de correcao, contrato firmado com o dono apos 24-25/jul: metralhadora de 14 ops contra tendencia + 2 ordens de 1 lote SEM STOP = -6.6k. Regras de FABRICA no template — nenhuma estrategia desliga, todas viram inputs visiveis no envio): bloco BT_DISCIPLINA_V1 definido UMA vez no _BT_VISAO_MQL5 (injetado em TODO EA) e consultado pelas 5 portas de entrada — BTBuy/BTSell nativos + BTTrade.Buy/Sell/PositionOpen da IA. (R1) STOP OBRIGATORIO: sem SL = aplica automatico; (R6) PISO DE VOLATILIDADE: SL nunca menor que 1.5x ATR(14) do TF (InpBTStopMinATR) — SL fixo 36pts < passo medio da vela matou as 14 ops no ruido; (R2) CADENCIA POR BARRA (desenho do dono): UMA decisao de entrada por barra do TF do grafico (static btUltBarra; recusa nao queima a barra), e ADICAO so com a barra anterior fechada A FAVOR (R2b); espaco medido/fibo entram com a aderencia (backend); (R3) TETO DE POSICOES por magic (InpBTMaxPosicoes=2, recusa teto_posicoes); (R4) TETO DE LOTE (clamp em InpBTLoteMax=1.0); (R6 gestao) TRAILING POR BARRA FECHADA (metodologia TrailingBot do dono): na virada de cada barra, stop sobe pro low[1] (buy) / desce pro high[1] (sell), SO aperta e SO alem do breakeven, via OrderSend TRADE_ACTION_SLTP direto (funciona nos 2 tipos de EA); toggle InpBTTrailingBarra. RECUSAS VIRAM EVENTO: tipo=recusada com lado+motivo via Print + BTvGravarEvento (bt_ev_<magic>) — a BabyMachine passa a registrar as entradas ruins EVITADAS. DEPLOY RITUAL OBRIGATORIO: kill-cache (DELETE FROM mq5_cache) + aquecedor + reenviar bots com nome novo — template mudou. PENDENTE NA SESSAO: (a) marcador BT_DISCIPLINA_V1 no upgrade automatico de frota (hoje o HIT checa so BT_EVT_V2); (b) backend aceitar/registrar tipo=recusada na BabyMachine; (c) aderencia bt_gate; (d) caso EMA-canal. | 7.11 - PROBABILIDADE DE EXECUCAO (definicao do dono, esclarecida: nao e lucro — e a chance de a PROXIMA ENTRADA disparar dado o cenario em construcao diante do Bot e da IA). Formula deterministica coerente com o portao real: parte da media das condicoes (prontidao v7.10), travada pelos CRITICOS (reprovado = teto 10%) e pela existencia de gatilho (sem direcao = teto 25%), e >= 90% quando o portao ja abriu (execucao iminente). Payload ganha prob_execucao (prontidao mantida). Par do front v10.12 (headers mostram EXECUCAO N%). | 7.10 - PRONTIDAO DO CHECKLIST EM TEMPO REAL (desenho do dono: cada item do checklist ganha uma METRICA 0-100% mostrando quanto da condicao ja esta satisfeita, mais um agregado no topo — acompanhar a percepcao ATE o momento da abertura, nao so o veredito binario. Nome melhorado por honestidade: PRONTIDAO DA ENTRADA, nao probabilidade de execucao — e % de condicoes satisfeitas, nunca probabilidade de lucro. Percentuais deterministicos por item: Tendencia = TFs alinhados/total (3 de 4 = 75); Volume = vela fechada/media (cap 100); S/R = corredor livre/0.8 ATR; R:R = rr/2.0; Noticia e Estrategia binarios 100/0; IA = alinhada 100 / neutra 50 / divergente 0; sem dado = null (fora da media). Agregado prontidao = media simples dos itens com dado. Par do front v10.11 (barras por item no cockpit + % no header dos dois checklists). | 7.09 - REGUA DA ESCADA NO ROMPIMENTO (correcao exigida pelo dono apos o caso real de 25/jul no BTCUSD: range de 64.9 pts rompido a favor de 4/4 TFs — gatilho short LOCALIZADO pelo detector, mas alvo = projecao do measured move do range miudo (35 pts) dava R:R 1.03 e o portao barrou o trade limpo, enquanto o proprio BOT narrava os suportes de 60m (12 toques) e 4h como alvos tecnicos a 60-118 pts. IDENTIFICAR SEM EXECUTAR NAO PAGA. FIX: em cenarios de ROMPIMENTO (range e estrutura), o measured move vira PISO, nao teto — alvo1 e promovido ao primeiro nivel da escada alem do risco (take parcial), alvo2 ao segundo nivel quando existe, e o R:R do plano e medido contra o alvo FINAL da cascata (plano de trader real: parcial no 1o nivel, alvo no 2o). Nunca encurta alvo ja calculado, so promove, e so usa nivel que EXISTE na escada medida com toques. Mesma regua aplicada no R:R do card (_chk_leitura le niveis_abaixo/acima do extras). Na cena do dia: alvo2=suporte 4h ~118 pts, R:R ~3.5 — a oportunidade NASCE e passa no portao. PENDENTES DA SESSAO DE CORRECAO (mapeados, ordem firmada): (1) caso EMA-CANAL+PULLBACK como gatilho formal do detector (exige candles/fechamento bem medidos — nao se improvisa de madrugada); (2) ADERENCIA — wrapper do EA (BTBuy/BTSell/BTTrade) consulta o veredito do portao via arquivo bt_gate_<magic> antes de QUALQUER ordem nativa (mexe template+conector+ritual kill-cache/reenvio; entrega dedicada). Regra operacional ate a aderencia: EA de estrategia nao validada FORA do grafico; operar pela plataforma (copiloto/automatico), onde o portao e o unico gatilho. | 7.08 - VOLUME NA VELA FECHADA + TENDENCIA LIMPA (print do dono: grafico com volume visivel mas LED vermelho 133<391. CAUSA CONCEITUAL: o item 3 media a vela EM FORMACAO — volume parcial no comeco da vela perde da media quase sempre. FIX: compara a ultima vela FECHADA com a media das 10 fechadas anteriores (vols[-2] vs vols[-12:-2]) — o LED julga vela completa, como um trader julga. E o detalhe da Tendencia vazava o dict cru do alinhamento ({lado, resumo}) na tela — agora extrai o resumo legivel nos dois pontos (item 2 e derivacao de direcao do card). | 7.07 - PARAMETROS DE ATUACAO SAO DO USUARIO (colocacao do dono antes do deploy: modo/sensibilidade/lote/SL-TP tem que ser escolha do usuario). O que JA era do usuario: modo e sensibilidade (toggles do cockpit v9.66; a sensibilidade agora tambem dita a tolerancia do checklist v7.06); SL/TP/lote do EA COMPILADO (inputs do envio no Editor, re-injetados pelo backend). O que faltava e entrou: (1) POST /monitor/bot/config ja aceitava lote_padrao/drawdown_max_dia/max_operacoes_dia/estrelas_min_auto mas SO o pausado persistia (v7.02) — agora TODOS os campos operacionais persistem no config_operacional (merge JSONB), sobrevivem a restart e valem entre workers; (2) validacao do lote_padrao 0.01-100; (3) o DETECTOR sobrescreve a memoria com a config persistida da linha do bot (verdade unica — o lote do comando automatico e o que o usuario setou); (4) GET config mescla a persistida (cockpit pinta a escolha real pos-restart). PROXIMA ENTREGA (front, desenho dos 3 quadros do dono no header do cockpit): painel Parametros do Bot com campo de LOTE + edicao de SL/TP da oportunidade no copiloto antes de executar. | 7.06 - CHECKLIST FLEXIVEL POR SENSIBILIDADE + LED DA IA (estudo do dono com o grafico Daily: exigir 100% dos itens recusa o trade limpo — a Entrada Perfeita no rompimento do canal teria sido barrada; exigir nada vira achismo. SOLUCAO GRADUADA usando o dial que ja existe: conservador = todos os itens com dado verdes (original); moderado = tolera 1 vermelho NAO-critico; agressivo = tolera 2. CRITICOS que nunca flexibilizam: Noticia de alto impacto e R:R < 1:2 (entrada sem plano nao se negocia em modo nenhum). O payload do checklist ganha sensibilidade/tolerancia/vermelhos/criticos_reprovados. E ITEM 7 NOVO no portao — Leitura da IA favoravel (ideia do dono): a ultima analise cruzada do cockpit vira VOTO — sinal alinhado = verde, divergente = vermelho (flexibilizavel), neutro/observacao ou analise velha (>15min) = cinza nao vota; custo ZERO (le o _ANL_HIST que o cockpit ja gera na cadencia de 5min). Front v10.10 ja renderiza o item novo sozinho (card + cockpit iteram itens[]). PROXIMA ENTREGA REGISTRADA (metodologia do dono, merece entrega propria): detector de ROMPIMENTO DO CANAL EMA20 H/L + PULLBACK + RETOMADA como caso formal de oportunidade — o gatilho classico do dono (duas medias rompidas, pullback nas medias, retomada = entrada) vira padrao registrado no OffMind pra medir no banco e disparar no detector. | 7.05 - FIX ESCADA DO CHECKLIST (print pos-v7.04: Tendencia/R:R/Noticia verdes — mas Volume e S/R cinzas. CAUSA do S/R: o item 4 procurava extras.escada.resistencias/suportes; as chaves REAIS do _extras_cockpit sao niveis_acima/niveis_abaixo na RAIZ (itens com dist_pts). FIX: item 4 le as chaves reais — long confere niveis_acima, short confere niveis_abaixo, corredor >= 0.8 ATR = verde. Volume cinza: depende do campo v nos candles do snapshot (_parse_c) — os candles do monitor chegam como LISTA [o,h,l,c,v] do _parse_c e o item 3 fazia c.get(v) em lista -> excecao engolida -> cinza; agora aceita lista E dict — Volume avalia de verdade (bots pre-v6.72 sem v seguem cinza honesto). | 7.04 - FIX LEITURA DO CHECKLIST NO CARD (print do dono minutos apos o deploy do v7.03: BOTTESTED_28 em XAUUSD com range ROMPIDO pras baixas e 4/4 TFs em baixa na tela, mas LEDs de Tendencia/S-R/R:R vermelhos por SEM DIRECAO e Volume cinza. CAUSA: o _chk_leitura do card lia gatilho.direcao/rr de dentro do _scanner_pacote — que NAO tem gatilho (so tfs/cascata/alinhamento); e os candles nao eram passados. O gate de EXECUCAO no detector estava CORRETO (usa o rr real dos niveis) — o bug era so a leitura informativa do card. FIX: (1) direcao derivada em cascata dos formatos reais: alinhamento (texto alta/baixa) -> cascata_reversao -> range rompido (estado rompeu_cima/baixo) -> regime; (2) R:R derivado da projecao do range quando ha rompimento: entrada=preco agora, alvo=proj_alta/proj_baixa, invalidacao=voltar pro teto/fundo rompido — mesmo calculo do cacador, com dado real ou nada; (3) c_op local do monitor_leitura passado ao helper — o LED de Volume avalia de verdade (bots v6.72+). LED de Estrategia vermelho no 28 e VERDADE, nao bug: bot sem estrategia_nome no config_operacional — reenvio pelo Editor registra; expor o nome na tela segue pendencia conhecida. | 7.03 - CHECKLIST DE ENTRADA COM LEDs (desenho do dono; o NORTE da arquitetura ganha forma: descrever nao e decidir — antes de QUALQUER execucao da plataforma, IA e BOT checam um portao deterministico de 6 itens, todos com dados que a casa JA mede, custo zero de IA): 1 Noticia verificada (calendario_economico, alto impacto na janela -15/+45min pras moedas do simbolo, cache 120s); 2 Tendencia confirmada (regime + alinhamento do scanner a favor da direcao do gatilho); 3 Volume confirmado (ultima vela do TF de operacao >= media das 10 anteriores, candles v6.72+); 4 Suporte/Resistencia analisado (escada de niveis: nivel CONTRARIO colado a <0.8 ATR = vermelho — o caso classico de comprar embaixo de resistencia); 5 Risco:Retorno >= 1:2 (projecao medida; SEM stop/alvo = vermelho, entrada sem plano nao entra); 6 Estrategia ajustada (linhagem registrada no config_operacional). Item sem dado devolve None (cinza, dito na tela, NAO bloqueia — honestidade: so barra com dado). GATE REAL: _checklist_entrada roda no detector ANTES da oportunidade nascer — reprovou, nem UI copiloto nem comando automatico existem (log [checklist] BARRADO com o que faltou); aprovou, o checklist vai CARIMBADO na oportunidade (auditoria). ESCOPO HONESTO: o portao barra tudo que a PLATAFORMA dispara; entradas nativas do EA compilado rodam locais no MT5 (arquitetura) — o caminho deles e o estudo da aderencia ja no roteiro (gerador do EA embutir as regras) + carimbo na BabyMachine (proxima fase). /monitor/leitura ganha campo checklist por bot (par do front v10.07: LEDs no card, verde LIVRE PRA ENTRAR / vermelho AGUARDAR). | 7.02 - BOTAO LIGA/DESLIGA DO BOT (par da app v10.04, pedido do dono via print: cada bot no Monitor ganha, depois do nome, uma TRILHA DE CONEXAO pulsando — MT5 -> Conector -> Plataforma, mesma linguagem visual do Conector v1.31+ — e um botao Ligar/Desligar). BACKEND (2 cirurgias, zero SQL): (1) POST /monitor/bot/{id}/config com pausado agora tambem PERSISTE o flag no config_operacional (JSONB v6.75, merge — padrao do estrategia_nome v6.90): sobrevive a restart do Railway e vira verdade unica entre workers; o /conector/bots (select *) ja devolve a coluna, entao o front le o estado sem endpoint novo; (2) o gate do detector (_analisar_snapshot->_check_pode_operar) passa a ler o pausado da PROPRIA linha do bot carregada do banco — worker com memoria quente que nao recebeu o POST nao opera mais um bot desligado na tela (custo zero: a linha ja estava na mao). ESCOPO HONESTO do botao: desliga o bot NA PLATAFORMA (detector/copiloto/automatico barrados pelo circuit breaker pausado ja existente da v6.74; card marca PAUSADO); o EA continua no grafico do MT5 ate o usuario remove-lo la — leitura read-only segue viva de proposito, o dado nao para de ser coletado. | 7.01 - O PLANO DA ENTRADA NOS ERROS (pedido do dono: a lista de erros tem que mostrar ONDE o bot posicionou o STOP e o TP na entrada — o cálculo de pontos, o objetivo estudado ANTES de entrar; e quando NÃO houver stop registrado, mostrar isso com honestidade, porque entrada sem stop = achismo e é exatamente o comportamento a caçar). FIX: itens de erros do /babymachine/estatisticas ganham stop, alvo (ou alvo_projetado) e projecao {risco_pts, ganho_pts, rr, estrelas, confluencia} — dados que a v6.89/v6.96 JÁ coletam em decisao; faltava expor. Par do front v10.03 (linha PLANO no erroCard). | 7.00 - CADÊNCIA DE 5 MINUTOS DA IA (decisão do dono, retomando o desenho dele: os gráficos operados são 1m/5m/15m e padrão se forma no FECHAMENTO da vela — ler a cada 90s é ler 3x a mesma vela de 5m pela metade, informação repetida com custo triplo; a decisão de entrada se confirma do 5m pro 15m, então UMA leitura de IA por janela de 5 min cobre o que fechou no 1m/5m e o que forma no 15m com alta assertividade). IMPLEMENTAÇÃO: (1) PISO GLOBAL por bot no _analisar_por_gatilho — qualquer gatilho (evento, assinatura, silêncio) respeita 300s desde a última análise; exceção única: visão do operador (ação humana explícita responde na hora); (2) _ANL_SILENCIO_SEG 90->300; (3) freio de evento da v6.99 alinhado em 300s (teto diário de 150 mantido como cinto de segurança); (4) narrador do BOT 120->300s. O DETERMINÍSTICO NÃO MUDA: scanner multi-TF, range, cascata, escada de níveis e detector seguem a cada snapshot de 15s com custo zero — a plataforma nunca fica cega entre as leituras da IA; a IA entra na cadência da vela, como um analista de verdade. Estimativa de custo: ~US$0.50-1/dia com a frota atual (era US$12). | 6.99 - FREIO DE CUSTO DA IA (fatura Anthropic ~US$12/DIA com os bots 24/7 — print do console do dono). CAUSA: a ANÁLISE CRUZADA IMEDIATA por evento (v6.67, sem cooldown de propósito) foi desenhada p/ eventos raros; o bot metralhadora (abre/fecha a cada 1min) disparou uma chamada de IA POR EVENTO a noite inteira. FIX: (1) freio na análise por evento — 1ª sai NA HORA (imediatismo preservado), seguintes respeitam 90s/bot + TETO de 150 análises de evento/bot/dia; (2) narrador do BOT: cache 30->120s (4x menos chamadas Haiku; viradas de estado re-narram pelo fluxo normal). Estimativa: de ~US$12/dia p/ ~US$1-2/dia com a frota atual. Ciclo de silêncio do cockpit intocado. | 6.98 - MEDIÇÃO ÍNTEGRA DA BABYMACHINE (os primeiros históricos reais expuseram medições CRUZANDO instrumentos: XAUUSD entrou 4125 e saiu 65831 = preço de BTC, -61 mil pts falsos; GBPUSD com preços de BTC; direção DESCONHECIDA; placar envenenado). CAUSAS: (a) fechamento casava por FIFO cego por bot, sem conferir ticket nem símbolo; (b) ativo do registro vinha de conector_bots.simbolo (a INTENÇÃO do envio), não do gráfico real. FIX: (1) CASAMENTO v2 — ticket do evento -> símbolo do evento (o EA carimba simbolo=_Symbol) -> FIFO só p/ evento antigo sem símbolo; aberta de outro símbolo NUNCA fecha por engano (drena como órfã via reconciliação); recuperação de órfã também confere símbolo; (2) abertura grava ativo = símbolo VIVO do evento + decisao.simbolo_vivo; órfã criada no fechamento idem; (3) TRAVA DE SANIDADE RETROATIVA no _lm_pts: entrada/saída com razão >3x = medição cruzada -> None (sem_medicao) — o placar se LIMPA SOZINHO das linhas antigas envenenadas, sem SQL; (4) lista de erros do /babymachine/estatisticas: teto 30 -> 200 (mostrar tudo; o front v10.02 rola a página); (5) fix sys.stderr sem import no except dos parâmetros sugeridos. Bots NÃO precisam ser reenviados — tudo backend. | 6.97 - BABYMACHINE FASE 2: EVENTOS POR ARQUIVO DEDICADO (o canal confiavel — fecha a captura vazia). CONTEXTO: eventos aberto/rejeitado/fechado saiam SO por Print no log do MT5, que tem buffer e na RAJADA (bot piramidando) atrasava/perdia eventos -> BabyMachine vazia. AGORA o EA grava CADA evento tambem em arquivo dedicado bt_ev_<magic>.txt (APPEND, flush imediato no FileClose), nos DOIS caminhos: (a) EA da IA — BTAbriu/BTRejeitou/os 2 PositionClose da subclasse BTTrade montam a linha numa var e dao Print + BTvGravarEvento (nova funcao no template raw _BT_VISAO_MQL5, FileOpen READ|WRITE + FileSeek SEEK_END + FileWriteString + FileClose); (b) EA nativo — a propria BTEvento grava autonomamente no arquivo alem do PrintFormat. O Print continua em paralelo como FALLBACK (bots/log antigos). Conector v1.30 F2 le o arquivo via ler_eventos_arquivo (APPEND-CONSUME: rastreia offset em bytes, so linhas novas, nunca reprocessa/perde — 6 smokes incl. rajada de 5) e ignora o evento do LOG quando o arquivo existe (anti-duplicata). POS-DEPLOY: reenviar bots com nome novo pra pegar a instrumentacao + kill-cache. Junto com o fix da metralhadora (bots ja vao ser reenviados). | 6.96 - BABYMACHINE FASE 1: PROJECAO MEDIDA NA ENTRADA (pedido do dono — cada operacao registrada tem que guardar, no momento da entrada, O QUE FOI PROJETADO pra o post-mortem comparar depois planejado x realizado; so backend, bots nao precisam ser reenviados). NOVO helper _bm_projecao_medida(entrada,stop,alvo) calcula em PONTOS: risco_pts=|entrada-stop|, ganho_pts=|alvo-entrada|, rr=ganho/risco — o R:R planejado ANTES de entrar; determinístico, nunca inventa (campo sem dado fica de fora). GRAVADO em decisao.projecao nos DOIS fluxos: (a) detector — de entrada/stop/alvo1 da oportunidade; (b) estrategia_compilada — PRIORIZA o que o CACADOR ja calculou (leitura.scanner.gatilho: alvo/risco/rr/estrelas/confluencia = a projecao ANALISADA no momento do gatilho) e complementa com o calculo dos valores reais da ordem + confirmacao historica do banco (contexto.confirmacao_score). Assim o registro da entrada ja nasce com o dossie completo: por que entrou (sinais+leitura_bot), o que a IA viu (veredito_ia), e a PROJECAO (alvo/stop/risco/ganho/rr/confluencia) — faltava so a projecao, agora fechada. FASE 2 (proxima): eventos por arquivo dedicado bt_ev_<magic>.txt (conector+api.py, exige reenviar bots) pra a captura nunca perder abertura/fechamento. | 6.95 - ESPELHO CORRETO + AQUECEDOR CONFIAVEL (par da app v10.00; avaliacao da validacao da vitrine pedida pelo dono). (1) FIX DO HASH DO ESPELHO: _mq5_hash_geracao usava so os primeiros 6000 chars do codigo ([:6000]) — edicao APOS o char 6000 num codigo custom longo nao mudava o hash e o cache servia o EA ANTIGO como relampago, silenciosamente. Agora o hash cobre o CODIGO INTEIRO; a troca de base foi feita no momento certo porque o upgrade BT_EVT_V2 (v6.89) ja tinha esfriado o cache todo — nada quente foi perdido. (2) AQUECEDOR v2 (_mq5_aquecer_worker, endpoints POST /admin/mq5/aquecer + GET /admin/mq5/cache ja existiam da v6.39): (a) ciente do upgrade de frota — HIT sem o marcador BT_EVT_V2 e apagado e REGENERADO (antes o aquecedor PULAVA a entrada pre-v2 achando o espelho quente, enquanto o /mt5/enviar descartaria a mesma entrada no HIT = aquecimento inutil); (b) sanidade — geracao que reprova na sentinela (_mq5_sanidade) e removida do cache e registrada em erros, nunca fica quente lixo. Front v10.00: botao 🔥 Aquecer espelho na faixa admin (token via prompt/sessionStorage, poll de progresso no /admin/mq5/cache). RITUAL NOVO pos-deploy que mexe em template/cache: rodar o aquecimento. | 6.94 - FIX DA RAIZ DA IA MORTA (par da app v9.95; a faixa de diagnostico v6.93 ENTREGOU a causa em 1 print: 'sem leitura do bot' com Bloco A rico e Zona 2 viva = o snapshot chega, mas _anl_ler_leitura retorna vazio). CAUSA-RAIZ: na v6.88 (Zona 2 completa) o bloco de campos novos entrou TAMBEM na _anl_ler_leitura, referenciando ent_por_nome/ent_por_simbolo — variaveis que so existem dentro do monitor_leitura. NameError em TODA chamada, engolido pelo except silencioso -> None -> 'sem leitura' -> IA morta desde o deploy da v6.88 (por isso o Bloco A seguiu vivo: a narracao dele vem pelo monitor_leitura, que tem as variaveis). FIX: (1) entradas_hoje na _anl_ler_leitura vira None com comentario — o consumidor real do campo e o CARD via monitor_leitura; a IA nao usa entradas_hoje no contexto e nao vale 1 query extra de eventos a cada tick de 6s; (2) o except da _anl_ler_leitura deixa de ser mudo: loga traceback no stderr — um NameError futuro aparece no log do Railway em vez de virar 'sem leitura' misterioso. LICAO: bloco replicado em duas funcoes precisa das dependencias nas duas; a faixa de diagnostico (v6.93) pagou o investimento na primeira utilizacao. | 6.93 - DIAGNOSTICO DA IA NA TELA (par da app v9.94; dono: 'sem funcionamento ja 5min' — Bloco B vazio e a tela nao conta O MOTIVO, obrigando diagnostico as cegas). AGORA a falha vira informacao visivel: (1) _ANL_FALHA por bot + _ANL_ERRO_ULT por worker — TODO ponto de morte do _analisar_ia registra o motivo exato (sem ANTHROPIC_API_KEY / IA respondeu status N, ex. 429 limite ou 404 modelo / resposta vazia / timeout ou classe da excecao) e o tick sem leitura registra 'sem leitura do bot (snapshot nao chegou)'; (2) sucesso limpa a falha; (3) GET /monitor/analise devolve campo falha {motivo, ha_s} quando a falha e mais nova que a ultima analise do historico; (4) front v9.94: o estado vazio do Bloco B vira faixa ambar honesta '⚠ Analise indisponivel — <motivo> · re-tentando sozinho' — o proximo print do dono entrega a causa-raiz na hora, sem cacar log do Railway (mesma filosofia do BT-G01 com resumo_log na tela). | 6.92 - IA INDERRUBAVEL (par da app v9.93; print do dono: 'IA parou de funcionar' — Bloco B preso no estado vazio com o BOT narrando rico). DIAGNOSTICO: 3 fragilidades fatais no _analisar_ia, todas matando a analise EM SILENCIO enquanto o tick re-tentava a cada 6s: (1) TIMEOUT de 10s — o contexto cresceu muito (scanner multi-TF v6.82 + momentum + estrutura + zonas + narracao) e a chamada passou a estourar o relogio; (2) PARSE ESTRITO — json.loads direto no texto: qualquer char fora do JSON (prefixo, sufixo, fence residual) descartava analise INTEIRA e valida; sinal fora da lista tambem descartava tudo; (3) SEM BACKOFF — falha nao gravava _ANL_LAST, entao o tick de 6s re-disparava a geracao pra sempre (loop de queima de API + tela vazia eterna). FIX: (1) timeout 10->25s; (2) PARSE RESILIENTE em camadas: extrai o primeiro {...} balanceado -> se falhar, regex campo a campo -> se ainda falhar mas houver texto, DEGRADA COM HONESTIDADE (analise_ia = texto bruto limpo, veredito = OBSERVACAO + 1a frase, sinal = neutro) — a analise NUNCA morre por formato; sinal invalido normaliza pra neutro em vez de descartar; (3) BACKOFF DE FALHA: _analisar_ia None (API fora/timeout) grava _ANL_LAST mesmo assim — proxima tentativa vem pelo ciclo de silencio (90s), nao a cada 6s; (4) erro loga com traceback no Railway. Zero mudanca visual. | 6.91 - RECONCILIACAO DEFENSIVA + AUTO-RECUPERACAO DE ORFAS (par da app v9.92; print do dono: a reconciliacao v6.89 marcou como ORFA ate as 4 entradas REAIS das 18:38 — historico zerado na Learning Machine). CAUSA: a coluna posicoes_abertas do snapshot chega 0 mesmo com posicao aberta no MT5 (o template BTvPosSimb conta certo por simbolo; a suspeita e o parser do CONECTOR — fora do alcance desta entrega), e a reconciliacao confiava numa UNICA leitura. AGORA: (1) RECONCILIACAO v3 — so marca orfa com 3 LEITURAS CONSECUTIVAS zeradas (contador em memoria por bot), idade minima 10 MIN (era 3) e FLUTUANTE zerado/nulo no mesmo snapshot (lucro != 0 = ha posicao -> aborta e zera o contador); orfas novas ganham resultado.reconciliada=true (marcador estavel). (2) AUTO-RECUPERACAO: (a) leitura com posicoes>0 e bot sem 'aberta' mas com orfa de reconciliacao <2h -> REVERTE a mais recente pra 'aberta' (restaura o registro valido sozinho — inclusive as 4 das 18:38 enquanto a posicao existir); (b) _bm_processar_fechamento sem 'aberta' -> antes de criar orfa nova, procura orfa de reconciliacao <2h do bot e FECHA ela com preco/pnl (o historico volta na saida). Legado casado por substring do motivo. Front v9.92: bloco 'Conectar meu MT5 — 4 passos' REMOVIDO do Monitor (pedido via print; fluxo de envio de 1 clique do Editor e o caminho unico). | 6.90 - O PORQUE DA ENTRADA (par da app v9.91; pedido do dono: cada operacao registrada tem que guardar OS SINAIS que levaram a execucao e QUAL ESTRATEGIA/PADRAO disparou). (1) LINHAGEM DA ESTRATEGIA: /mt5/enviar aceita estrategia_nome (front manda da _estrategiaAtiva) e grava no config_operacional do bot (JSONB v6.75, merge, zero SQL); a BabyMachine grava decisao.estrategia em toda operacao da estrategia compilada — 'qual estrategia' respondido. (2) CONTEXTO v2 (_bm_extrair_contexto): alem de regime/padroes/confirmacao/momentum/veredito, agora captura scanner (alinhamento + cascata de reversao), a NARRACAO DO BOT no momento (ate 450c — a leitura tecnica viva na hora da entrada) e SINAIS LEGIVEIS prontos (_bm_sinais_legiveis: ate 8 frases deterministicas — 'Regime TENDENCIA_ALTA', 'Engolfo de alta no 5m', '3/4 TFs a favor da baixa', 'Reversao: detectada 1m -> confirmando 5m', 'Confirmacao historica 62/100', 'Preco no range N-M', 'Nivel X sendo testado (N toques)'). (3) DEDUP v2 anti-corrida: trava em memoria por worker (_BM_ULT_ABERTURA bot->direcao+ts) ANTES da query — pega duplicatas do mesmo segundo que a janela de 30s via banco perdia por race de commit. Front v9.91: badge da fonte mostra o NOME REAL da estrategia; 'Por Que' ganha secao SINAIS NO MOMENTO (chips) + leitura do BOT na hora (italico). | 6.89 - FIX 2: EVENTOS REAIS DA BABYMACHINE (par da app v9.90; prints do dono: 10 'abertas' com Posicoes 0, duplicatas aos pares, 82 fechadas sem medicao). CAUSAS ENCONTRADAS: (a) o evento 'aberto' era emitido ANTES do trade.Buy/Sell nos DOIS caminhos (wrappers nativos BTBuy/BTSell e clamp BTTrade da IA) — ordem rejeitada gerava registro fantasma, sem preco/ticket; (b) o BTTrade NAO interceptava PositionClose — EA da IA fechava e a BabyMachine nunca via o evento (abertas acumulavam sem fechar). FIX BT_EVT_V2: (1) evento 'aberto' so com execucao CONFIRMADA (retcode DONE/DONE_PARTIAL/PLACED) e RICO: preco real de execucao, ticket, sl, tp, lote; rejeicao vira evento 'rejeitado' com retcode (auditoria); (2) BTTrade ganha PositionClose (symbol e ticket) emitindo 'fechado' com preco e ticket; close nativo tambem pos-execucao com preco; (3) UPGRADE AUTOMATICO DA FROTA: HIT no mq5_cache sem o marcador BT_EVT_V2 e descartado (_mq5_cache_apagar) e regenerado — mesmo mecanismo da auto-cura, cada estrategia migra sozinha no proximo envio; (4) DEDUP na insercao da estrategia compilada: mesma direcao aberta <30s no mesmo bot = ignorada (mata as duplicatas aos pares); (5) decisao da estrategia compilada guarda entrada/sl/tp/ticket/lote vindos do evento; (6) _lm_pts v3 RETROATIVO: le o preco tambem de decisao.detalhe_ea e resultado.detalhe_ea — fechadas antigas com preco nos eventos passam a MEDIR pontos (o aviso de 82 sem medicao encolhe sozinho onde houver dado); (7) RECONCILIACAO: no /monitor/leitura (throttle 120s/bot), snapshot com 0 posicoes + linhas 'aberta' com >3min viram 'orfa' com motivo — drena as fantasmas acumuladas. POS-DEPLOY: reenviar bots com NOME NOVO pra pegar o EA v2 (upgrade do cache faz a 1a geracao demorar ~20-40s por estrategia). | 6.88 - ZONA 2 COMPLETA (par da app v9.89, print do dono: 'Entradas hoje' e 'Drawdown' sempre em tra\u00e7o + vao vazio no card): o front esperava b.entradas_hoje e drawdown desde a v9.53 mas o backend NUNCA enviou. AGORA cada bot do /monitor/leitura ganha: (1) entradas_hoje REAL — 1 query por usuario em agente_eventos (tipo=bot_aberto de hoje UTC, ate 400) contada por bot_nome do detalhe_json com fallback por simbolo (mesma logica do /monitor/eventos v6.66); (2) drawdown_dia_pct — equity atual vs equity_inicio_dia do _CIRCUIT_STATE do detector (telemetria por worker; null quando o estado ainda nao existe — front mostra tra\u00e7o honesto); (3) equity e balance por bot (o front decide usar). Custo: +1 query indexada por chamada. Front v9.89 preenche e ganha linhas novas no vao: 'Pre\u00e7o agora' e 'TFs alinhados' (derivado do scanner v6.82 ja enviado — N/M com seta e cor). | 6.87 - PLACAR NO CARD DO MONITOR (par da app v9.88): o espaco liberado pela voz da IA na Zona 3 (v9.87) ganha a LUCRATIVIDADE MEDIDA do bot — faixa '💰 Placar deste bot (30d)': acertos x erros, %% de acerto, saldo em pontos e medias. Backend: /babymachine/estatisticas passa a agrupar por_bot por BOT_ID (cada item ganha bot_id + nome) pro card casar de forma confiavel, em vez de por nome. Front busca UMA vez por usuario (throttle 60s dentro do ciclo do Monitor) e distribui em todos os cards; bot sem operacoes medidas mostra convite discreto. Sempre pontos medidos, nunca dinheiro prometido — regra da casa. | 6.86 - PLACAR DO APRENDIZADO (Sessao 6C — tela desenhada pelo dono, par da app v9.85): a aba Learning Machine ganha o placar de ACERTOS x ERROS por BOT e por ATIVO + a analise dos erros. ARQUITETURA (deterministica, custo zero de IA): (1) POST /babymachine/estatisticas (user_id, dias, bot_id opcional) le as fechadas do periodo (ate 500) e agrega: geral, por_bot e por_ativo — fechadas, acertos (pts>0; pts==0 conta como acerto de 0 pts = protegeu capital), erros (pts<0), sem_medicao (fechada sem pontos calculaveis — vira aviso na tela; Fix 2 do EA resolve), taxa de acerto sobre as MEDIDAS, pts_ganhos, pts_perdidos, saldo_pts, medias por acerto/erro; (2) _lm_pts prioriza pontos VERDADEIROS (preco_saida - entrada com direcao; o pnl_pts gravado pode ser monetario) e cai pro gravado se faltar dado; (3) _lm_post_mortem — POR QUE ERROU, cruzando contexto_entrada x decisao x resultado em linguagem simples: 'Stop atingido', 'Entrada contra a tendencia do momento', 'Padrao com historico fraco neste ativo (score/100)', 'Poucas confluencias', 'Sinal fraco do detector (N estrelas)', 'O mercado virou logo depois da entrada', 'Analise cruzada DIVERGENTE no momento da entrada'; fallback honesto 'movimento contrario sem aviso previo nos dados registrados'; (4) resposta traz ate 30 erros detalhados: bot/ativo/direcao/cenario/entrada/saida/pnl em pts (negativo) e %/tempo ativo + a ANALISE DE ENTRADA (regime, confirmacao score+veredito, sinal da IA, estrelas/confluencia, explicacao) + motivos. Zero SQL novo (le babymachine_operacoes da v6.76). | 6.85 - FIX DA RAIZ DO ERROR 112 (par da app v9.84): o BT-G01 com resumo_log na tela (BOTTESTED_16) provou que a GERACAO FRESCA tambem saia quebrada — a linha mostrada terminava com aspas abertas ('+ extra + \"'). CAUSA-RAIZ: _BT_VISAO_MQL5 (template injetado com BTvGravarOk/BTLerComando, v6.73) era string Python NAO-RAW — no parse do Python o \\n do FileWriteString colapsava em quebra de linha REAL (error 112: closing quote no MQL5) e os \\\" do BTvJsonNum viravam aspas nuas. Ou seja: TODA geracao pos-v6.73 dessa injecao nascia envenenada; o BOTTESTED_14 compilar foi cache de outra era (conclusao anterior errada, corrigida). FIX: prefixo r no template (r\"\"\") — uso e concatenacao pura (sem format), entao nada mais muda; \\n e \\\" chegam literais no MQL5 como devem. Varredura nos demais templates (_MQL5_AVISO_HEADER, _BT_WRAPPERS_MQL5, _BT_TRADE_CLAMP_MQL5, _BT_PAINEL_MQL5): zero escapes, imunes. Verificado por ast: valor de runtime do template contem \\n de 2 chars e passa na _mq5_sanidade. POS-DEPLOY: reenviar a EMA 9/21 com nome novo — a auto-cura da v6.83 apaga a entrada envenenada de CADA estrategia no proximo envio dela e regenera com o template corrigido (frota inteira se cura sozinha, sem SQL). | 6.84 - SENTINELA v2 (fix do BT-G01 do BOTTESTED_16, par da app v9.83): a sentinela v1 nao conhecia COMENTARIO DE BLOCO /* ... */ — aspas dentro de bloco (cabecalhos que a IA gera) contavam como string aberta e bloqueavam codigo VALIDO, inclusive a geracao fresca pos auto-cura (por isso o BT-G01 na tela). AGORA: (1) _mq5_sanidade v2 rastreia /* */ ATRAVES de linhas (estado em_bloco persistente; aspas e apostrofes dentro de comentario nao contam; */ retoma o scan no meio da linha); mantem tudo da v1: string nao atravessa linha, respeita a barra invertida de escape, // e char literal; (2) TRANSPARENCIA: o retorno BT-G01 do /mt5/enviar agora inclui resumo_log = 'linha N: trecho' da linha que barrou — o modal do front ja renderiza resumo_log, entao o diagnostico aparece NA TELA sem cacar log do Railway; front v9.83 repassa d.resumo_log no handler inicial. Se o BT-G01 voltar a aparecer, a linha mostrada diz na hora se e falso positivo restante ou geracao realmente quebrada. | 6.83 - CODIGOS DE REPROVACAO (BT-xx) + SENTINELA COM AUTO-CURA DO CACHE (par da app v9.82, pedido do dono apos o caso BOTTESTED_15): reprovacao deixa de ser mensagem generica e passa a sair com CODIGO identificavel na hora. CONTEXTO: BOTTESTED_15 (EMA 9/21) reprovou 4x identico — compile.log mostrou error 112 (closing quote) dentro do BTvGravarOk: a entrada da estrategia no mq5_cache estava ENVENENADA (\n colapsado em quebra real, era do bug de escape), e o cache servia a mesma geracao podre em todo reenvio; o template atual esta sao (BOTTESTED_14 gerado fresco compila). AGORA: (1) _mq5_sanidade(texto) — SENTINELA deterministica: varre linha a linha procurando string MQL5 aberta sem fechar (respeita \" escapado, // comentario e char literal '\"'); string nao atravessa linha em MQL5, entao aspas abertas no fim da linha = codigo corrompido; (2) AUTO-CURA no /mt5/enviar: se o .mq5 (novo ou do cache) reprova na sentinela, apaga a entrada do cache via _mq5_cache_apagar(gen_hash) e REGENERA uma vez — usuario final NUNCA precisa de SQL manual; se ainda sair ruim (bug de template), bloqueia ANTES de ir pro MT5 com BT-G01; pre_validado recalculado APOS a cura; (3) _classificar_reprovacao(log) — traduz o log que o conector ja manda no /mt5/veredito em codigo: BT-C01 string/aspas quebrada (error 112, classe do cache envenenado), BT-C02 identificadores MQL4 (Ask/Bid), BT-C03 chaves desbalanceadas/fim inesperado, BT-C04 already defined, BT-C05 cannot convert, BT-C99 compilacao generica, BT-X01 reprovado sem log; primeiro erro decide (o resto e cascata); resumo_log = ate 3 linhas de erro; (4) /mt5/status devolve codigo+resumo_log quando reprovado — o front mostra 'Codigo BT-xx — explicacao direta' + resumo monoespacado; (5) erros do proprio /mt5/enviar tambem codificados: BT-T01 sem token, BT-T02 sem codigo, BT-G02 falha ao gerar, BT-S01 supabase fora, BT-G01 malformado. Zero SQL, EA intocado, Conector intocado (o log ja vinha no veredito). | 6.82 - SCANNER MULTI-TF + ESTADO DO CACADOR (par da app v9.81, pedido do dono): o BOT passa a fazer um PENTE-FINO nos timeframes A PARTIR DO 1M e a narrar o proprio estado de caca. ARQUITETURA (deterministica, custo zero de IA): (1) _scanner_multitf varre 1m/5m/15m/30m/60m/4h (candles do snapshot + zonas + padroes do OffMind por TF) e classifica o comportamento de cada um: tendencia_alta/baixa, inclinando, lateral, reversao_formando (padrao de vela de reversao CONTRA o movimento recente, ou padrao formal do OffMind no TF); TF sem candles e pulado (bots antigos sem c1m comecam a cascata no 5m). (2) _cascata_reversao — o pente-fino de REVERSOES: reversao detectada no menor TF -> 'confirmando' no seguinte (mesmo sinal ou comportamento a favor) -> 'aguardando' no proximo; estados detectada/confirmando/confirmada ate o 15m. (3) _alinhamento_tfs — o pente-fino de TENDENCIAS: 'N de M TFs a favor da baixa (1m, 5m, 15m)'. (4) _estado_gatilho — o BOT narra a propria caca: PROCURANDO GATILHO DE ENTRADA (lista o que esta cacando com precos: fechamento fora da caixa, neckline, reacao em nivel testado) -> GATILHO DE ENTRADA LOCALIZADO (oportunidade recente do detector <15min com entrada/stop/alvo1/estrelas/confluencia, ou rompimento de range/pivo) + CALCULO DE PROBABILIDADE EM PONTOS: alvo provavel (+pts), risco ate a invalidacao (pts), R:R, historico medido (confirmacao contextual). (5) Tudo entra nos fatos do BOT (_bot_narracao_hibrida) com prompt novo (abre com o estado do cacador; cascata e manchete; assinatura do cache ganha estado_gatilho+cascata = re-narra na virada; max_tokens 300->340, ~430 chars) E no campo scanner novo do /monitor/leitura + _anl_ler_leitura -> ctx.momento.scanner da IA do cockpit, com secao SCANNER MULTI-TF no prompt (antecipar padroes em construcao; cascata x alinhamento em conflito E a analise). Numeros medidos, nunca promessa. Zero SQL, zero mq5_cache, EA intocado. | 6.81 - VISAO DO OPERADOR (Sessao C do diferencial de informacao, par da app v9.79): o usuario ganha VOZ dentro da analise cruzada do cockpit. ARQUITETURA: (1) POST /monitor/visao — texto (max 500 chars) e/ou print base64 (max ~5MB); se veio print, UMA chamada de visao Haiku extrai resumo factual do que o operador marcou (~$0.001) e a imagem e DESCARTADA — as analises seguintes usam so o resumo; (2) visao ativa por 2h (TTL) ou ate limpar/substituir (DELETE /monitor/visao), guardada em _VISAO_OP em memoria por worker (mesma classe aceita do _ANL_HIST — interpretacao viva, nao dado regulatorio); (3) injetada em TODA analise cruzada via ctx.visao_operador em _analisar_por_gatilho; prompt da IA ganha secao VISAO DO OPERADOR com regra inegociavel: a IA CRUZA com honestidade — concorda citando numeros quando os dados sustentam, DISCORDA com numeros quando contradizem, nunca obedece cegamente, nunca vira promessa; quando ha visao, o VEREDITO tambem diz ALINHADO/DIVERGENTE com a leitura do operador; (4) enviar visao dispara analise IMEDIATA (mesma prioridade de evento ABRIU/FECHOU — nao espera 90s nem cooldown); (5) /monitor/leitura ganha campo visao_operador por bot (texto, tem_print, resta_min) pro chip do cockpit; (6) DELIMITACAO FIRME: o campo aceita orientacao de LEITURA (vies, niveis a vigiar, contexto, padrao que o usuario enxergou) — NAO executa trade por texto livre; ordem de execucao continua exclusiva dos botoes Copiloto/Automatico com os circuit breakers. Zero SQL, zero mq5_cache, EA intocado. | 6.80 - ESTRUTURA DE SWING (PIVO 1-2-3) + CATALOGO DE PADROES AO VIVO (Sessao B, par da app v9.74): reversao classica entra no radar e no banco. CONTEXTO: BOTTESTED_14 mostrou topo -> fundo -> segundo topo MAIS BAIXO -> perda do fundo (pivo 1-2-3 de baixa, o padrao mais mapeado do trading) 100%% desenhado no grafico — e nem BOT nem IA identificaram, porque o OffMind ao vivo so conhecia 6 padroes de VELA + niveis horizontais; estrutura de SWING nao tinha detector, logo nunca consultava o banco. AGORA: (1) det_pivo_123_baixa/alta REGISTRADOS em PADROES_OFFMIND (categoria estrutura) — com isso a MESMA chave liga tudo automaticamente: _offmind_ao_vivo detecta ao vivo nos TFs de mecanica, /offmind/analisar e _confirm_stats_padrao MEDEM historicamente, e a confirmacao contextual passa a citar 'Pivo 1-2-3 de baixa: medido Nx neste ativo/TF, X%% confirmou'. Deteccao df: swing highs/lows janela 2, 2o extremo mais fraco com margem 0.25xATR, dispara na vela que FECHA alem da neckline, invalida se faz novo extremo alem do 1o. (2) _detectar_estrutura_swing ao vivo nos fatos do BOT: estados FORMANDO (2o topo mais baixo ja feito, neckline segurando) e ROMPEU (fechamento alem da neckline; confirmado com 2 fechamentos), p1/p2, neckline, altura, PROJECAO measured move (neckline -/+ altura), invalidacao no 2o extremo, avanco %% da projecao (>140%% some — virou historia); prompt do BOT ensina a narrar 'segundo topo MAIS BAIXO = compradores perdendo forca' no formando e manchete no rompimento; assinatura do cache re-narra na virada de estado; range tem precedencia quando os dois existem. (3) Detector de oportunidade Caso 0b reversao_estrutura_baixa/alta: entrada no rompimento da neckline, stop de volta pra dentro, ALVO = projecao da estrutura, +15 confluencia. (4) extras do /monitor/leitura ganham swing (estrutura completa) e padroes_catalogo (catalogo INTEIRO do banco) — o front mostra TODOS os padroes disponiveis em tempo real: acesos quando formando (com TF e direcao), 'monitorando' quando nao, estatistica medida anexada quando o banco tem. (5) max_tokens do BOT 250->300 (narracao de range+swing cita mais precos). Zero SQL, zero mq5_cache, EA intocado. | 6.79 - RANGE/LATERALIZACAO + RAIO-X DO COCKPIT (Sessao A do diferencial de informacao, par da app v9.73): o sistema passa a NOMEAR o estado do mercado em vez de so listar pecas. CONTEXTO: BOTTESTED_14 em BTCUSD 15m passou horas numa acumulacao classica (suporte forte x resistencia forte, preco batendo e voltando) e nem BOT nem IA nomearam o estado; quando rompeu pra baixo, nada disparou — existia detector de VELA e de NIVEL, nao existia detector de ESTADO. AGORA: (1) _detectar_range_lateral: detector deterministico de LATERALIZACAO no TF de operacao — dois extremos com 2+ toques cada e >=75% das velas presas entre eles; sai com topo/fundo/altura em pts e em ATRs, velas presas, toques por extremo, posicao %, e PROJECOES de rompimento por measured move (teto+altura / fundo-altura); estados dentro|rompeu_cima|rompeu_baixo (fechamento alem da tolerancia; 'confirmado' com 2 fechamentos fora); range consumido (>140% da projecao) some sozinho. (2) fatos do BOT ganham range_lateral + atr_pts; a assinatura do cache de narracao inclui o estado do range (rompimento re-narra NA HORA); prompt do BOT ensina a ABRIR a leitura nomeando a acumulacao (dentro do range = sem trade; gatilho = fechamento fora; citar fundo/teto/projecoes) e a anunciar ROMPIMENTO como fato principal com invalidacao = voltar pro range. A IA do cockpit (mesmos fatos) herda o range de graca. (3) Detector de oportunidade ganha o Caso 0 rompimento_range_alta/baixa: entrada no preco, stop de volta pro nivel rompido, ALVO1 = projecao da altura (measured move), alvo2 = +50%% da altura; +15 de confluencia 'Rompimento de lateralizacao'. Funciona nos DOIS lados — acumulacao rompe pra onde quiser. (4) RAIO-X DO COCKPIT: /monitor/leitura devolve campo extras por bot via _extras_cockpit — CANAL EMA20 H/L (largura em pts/%%/ATRs, posicao do preco no canal 0-100, distancia pra romper cada lado), ATR do TF em pontos, RANGE completo com projecoes, e ESCADA DE NIVEIS (3 resistencias acima + 3 suportes abaixo com TF, toques e distancia em pts = corredor livre). Tudo deterministico, custo ZERO de IA — a IA so narra por cima de numeros ja calculados. Regra da casa mantida: geometria e historico medidos, nunca promessa. | 6.78 - TREND DAY ADAPTATIVO (v2 TF-aware da Escalonada) + inclui v6.77 (BM dedup): a estrategia Tendencia Diaria Escalonada foi desenhada pra 1d — o BOTTESTED_13 rodando em 5m gerou piramidacao descontrolada. NOVA ESTRATEGIA na Vitrine (tendencia_dia_adaptativa - Trend Day Adaptativo) com regras TF-aware: (1) Trailing adaptativo por TF: M1=0.05%, M5=0.15%, M15=0.35%, M30=0.60%, H1=0.90%, H4=1.20%, D1=1.50%; (2) Cooldown adaptativo por TF: M1=12 barras, M5=6, M15=4, M30=3, H1=2, H4=2, D1=1 — evita piramidacao spam; (3) Espacamento minimo entre entradas: 0.7 x ATR — so piramida se preco realmente avancou; (4) PosicaoFecharTodas() fecha TODAS as posicoes em loop; (5) Trailing coletivo: quando ativa, fecha o pacote INTEIRO; (6) Saida defensiva por 2 velas vermelhas consecutivas; (7) Saida por rompimento canal EMA_L fecha TUDO. Registrada em _mql5_generators + vitrine_catalogo + i18n pt/en/es. v1 continua na Vitrine pra bots existentes. Inclui tambem v6.77: BabyMachine dedup de aberturas por janela de 30s. | 6.76 - BABYMACHINE OPERACOES REAIS (Sessao 6A - coleta pura): a BabyMachine que ate hoje so coletava BACKTESTS ganha novo dominio - OPERACOES REAIS ao vivo do MT5. ARQUITETURA: (1) tabela babymachine_operacoes (SQL v6.76) - irma de backtests_historico, guarda contexto_entrada (JSONB: regime + offmind + momentum_tfs + confirmacao + veredito_ia), decisao (JSONB: direcao + entrada + stop + alvo + cenario + confluencias + explicacao) e resultado (JSONB: preco_saida + pnl + tempo_ativo); (2) 3 fontes alimentam - detector_auto, detector_copiloto, estrategia_compilada; (3) fluxo detector - INSERT pendente + comando_id -> /mt5/comando/confirmar marca aberta + ticket -> /conector/evento tipo=fechado marca fechada (FIFO por bot); (4) fluxo estrategia compilada - /conector/evento tipo=aberto sem pendente cria linha aberta + contexto do ultimo snapshot -> fechado marca fechada; (5) endpoint POST /babymachine/operacoes - filtros e agregados. Base pra aba Learning Machine (Sessao 6B); (6) TTL 90 dias via _bm_operacoes_limpar_antigas() throttle 6h; (7) NON-BLOCKING - falha na coleta nunca quebra detector/comando/evento. Inclui tambem v6.75 (config persistente): coluna config_operacional em conector_bots pra sobreviver Railway restart. ZERO UI - UI vem na Sessao 6B. | 6.74 - DETECTOR AUTOMATICO DE OPORTUNIDADE (Sessao 3 do loop fechado): a inteligencia que construimos passa a produzir sinais executaveis. ARQUITETURA: (1) roda 24/7 no /conector/snapshot (a cada 15s enquanto EA emite) — cockpit fechado nao trava a operacao; (2) detector deterministico: le fatos ja calculados (offmind + regime + momentum + confirmacao) e determina direcao potencial (long/short) por padrao — pullback em tendencia, reversao em nivel testado; (3) niveis reais: entrada, stop, alvos calculados dos niveis S/R clustered do offmind (nao inventa); (4) confluencia 0-100: soma regime alinhado (+20), 4h a favor (+15), padrao formal (+15), toques do nivel (+10/+15), confirmacao historica (+8/+15), momentum recente (+10), veredito alinhado (+10). Traduz em ESTRELAS 1-5; (5) 3 modos por bot (memoria): observar (nada), copiloto (dispara oportunidade pra UI), automatico (cria comando via /mt5/comando); (6) 3 sensibilidades: conservador (>=4 estrelas), moderado (>=3), agressivo (>=2); (7) circuit breakers: pausado, ja tem posicao, cooldown 60s, max operacoes/dia (default 10), drawdown maximo/dia (default 5%); (8) narrador IA (Haiku, ~$0.0005) SO quando oportunidade passa do limiar — barato; ENDPOINTS: GET/POST /monitor/bot/{id}/config, GET /monitor/bot/{id}/oportunidades, POST /monitor/oportunidade/{id}/executar (copiloto). Estado em memoria — Sessao 6 migra pra tabela quando construir dashboard. Fail-safe: se Railway reinicia, todos os bots voltam pra observar. UI de fato vem na Sessao 4. | 6.73 - CANAL DE COMANDO CLOUD->EA (Sessao 1 do loop fechado): a plataforma deixa de ser read-only. ARQUITETURA: (1) tabela mt5_comandos guarda fila de ordens que a cloud manda pro EA (buy/sell/close/close_all/mover_sl/mover_tp/cancelar); (2) POST /mt5/comando cria comando (validacao de bot online, limite 20/dia por bot, retorno com id); (3) GET /mt5/comando/pendente?bot_token=X conector PY faz poll, marca linha como entregue e retorna o mais antigo pendente; (4) POST /mt5/comando/confirmar EA confirma execucao com ticket/preco real ou falha; (5) GET /mt5/comandos?bot_id=X&limite=20 lista pro cockpit mostrar historico. CIRCUIT BREAKERS iniciais: bot deve estar online (ultimo_ping < 45s), max 20 comandos/dia por bot, comandos velhos (>60s pendentes) viram expirado automatico via _mt5_expirar_comandos(). Conector PY ainda nao mexido — contrato documentado, integracao na proxima sessao. EA (MQL5) ganha BTLerComando() injetada que le arquivo bt_cmd_<magic>.txt do conector e executa. ACAO POS-DEPLOY: (1) rodar v6.73_mt5_comandos.sql no Supabase ANTES; (2) invalidar mq5_cache; (3) reenviar bot. | 6.72 - VOLUME NOS CANDLES (preparacao pro grafico com barras de volume no estilo MT5): (1) BTvCandles MQL5 agora inclui tick_volume no fim de cada candle: formato passou de O,H,L,C para O,H,L,C,V. Snapshot cresce ~0.4KB mas continua bem abaixo do limite do Print MQL5 (4KB). (2) _bt_parse_candles e _parse_candles_para_lista ficaram RETROCOMPATIVEIS: aceitam 4 ou 5 valores por candle. Bots velhos (v6.71 e anteriores) continuam funcionando sem volume; bots novos ganham volume automatico. (3) DataFrame do backend ganha coluna Volume quando disponivel; frontend consome via campo v de cada candle. ACAO POS-DEPLOY: invalidar mq5_cache e reenviar 1 bot pra ver volume no grafico do cockpit. Bots antigos continuam com gráfico sem volume ate serem reenviados. | 6.71 - BOT LE OFFMIND (fix arquitetural): a v6.70 do BOT analitico ignorava o offmind ja processado pelo EA (padroes formais engolfo/martelo/estrela cadente/3verdes/3vermelhas + niveis S/R clustered com N toques e marca de testando) e reinventava deteccao mais fraca com swing highs/lows crus de janela 2. AGORA: (1) _analisar_bot_tecnica aceita offmind como parametro; (2) integra padroes_formais e niveis_sendo_testados e niveis_proximos aos fatos passados pra IA; (3) prompt do BOT ensina que niveis com 2+ toques testados AGORA = DUPLO TOPO/FUNDO se formando, e a NARRAR estrutura de mercado antes de swing highs/lows; (4) swing highs/lows antigos permanecem como complemento (nao substitui estrutura formal). Resultado esperado: o BOT vira a citar \"duplo topo se formando em X com 2 toques no 15m\" e \"suporte em Y defendido 3 vezes segurando\". | 6.70 - BOT ANALITICO (Caminho C hibrido): o BOT deixa de ser uma frase-template morta e vira ANALISTA TECNICO real. ARQUITETURA: (1) _analisar_bot_tecnica(candles) DETERMINISTICO — detecta swing highs e swing lows (topos e fundos por janela de 2), padrao da ultima vela (marubozu, doji, martelo, estrela cadente, engolfo de alta/baixa), momentum das ultimas 3 velas, distancia pra romper EMAs, posicao no range das velas visiveis, gatilho concreto de proxima entrada. Nada de opiniao — so o que esta observavel no grafico. (2) _narrar_bot_analitico(fatos) — passa esses fatos pra Haiku formatar em VOZ DE SNIPER (temperatura 0.4, direto, tecnico, sempre com precos concretos). Prompt PROIBE contextualizar com historico ou outros TFs (isso e papel da IA MENTOR no outro quadro). (3) CACHE de 30s por bot_token com assinatura de fatos: se nada relevante mudou, reusa o texto — economiza tokens quando o grafico esta parado. (4) FALLBACK: sem candles ou IA indisponivel cai no _narracao_bot antigo (template morto). Nunca vazio. (5) _bot_narracao_hibrida orquestra tudo, e chamado no lugar de _narracao_bot em /monitor/leitura e _anl_ler_leitura. CUSTO: +1 chamada Haiku por bot ativo por ate 30s = ~$0.001/bot/30s = ~$3/dia por bot rodando 24h. Cache reduz drasticamente quando grafico esta parado. | 6.69 - OFFLINE RAPIDO (fix dos 5 minutos): (1) OFFLINE_APOS_SEGUNDOS caiu de 180s (3 min) para 45s. O EA emite snapshot a cada 15s, entao 45s = 3 ciclos perdidos = com toda certeza offline. Sem essa mudanca, remover EA do MT5 demorava ate 3 min pra card virar offline. (2) BTVisaoDeinit (injetado no OnDeinit do EA) agora EMITE Print(BOTTESTED_FIM|magic=X|motivo=Y) quando o EA e removido do grafico. Serve como sinal explicito de fim de vida — pode ser processado pelo conector PY (se souber) ou fica no log de auditoria. Independente disso, o timeout de 45s ja garante que o bot vira offline rapido. (3) /conector/evento agora aceita tipo=fim: se o conector PY souber processar BOTTESTED_FIM e mandar como evento, o backend zera ultimo_ping IMEDIATAMENTE (bot vira offline em segundos, nem espera os 45s). Cenario tipico: EA removido -> BTVisaoDeinit emite Print BOTTESTED_FIM -> conector PY detecta (se souber) e chama /conector/evento tipo=fim -> backend zera ultimo_ping -> bot offline imediato. Fallback: timeout 45s. | 6.68 - CANDLES REAIS + MOMENTUM SEMANTICO POR TF (par da app v9.56): (1) EA instrumentado agora emite candles OHLC de 5m/15m/60m/4h no snapshot via BTvCandles(tf,18) + CopyRates. Formato compacto O,H,L,C separado por ponto-e-virgula, ate 18 velas por TF. Snapshot cresce ~1.8KB mas Print MQL5 suporta ate 4KB por linha. (2) Backend calcula RESUMO SEMANTICO por TF via _momentum_semantico(candles_str): traduz OHLC crus em texto descritivo — quantas velas de alta/baixa, corpos crescentes/estaveis/encolhendo, posicao do preco no range recente, variacao da ultima vela. Passa isso pra IA como momentum_tfs (nao candles crus — economiza tokens). (3) Prompt IA refinado modo MENTOR HUMANO: reforca linguagem descritiva fluida (o 15m ganhou forca nas ultimas velas mas o 4h ainda esta lateral) em vez de listagem tecnica. (4) Grafico do cockpit volta a mostrar CANDLES REAIS renderizados (canal visual v9.55b continua fallback). ACAO POS-DEPLOY: invalidar mq5_cache e reenviar 1 bot pra testar. | 6.67 - COCKPIT DE ANALISE CRUZADA no Monitor (par da app v9.55): expandir um bot abre o COCKPIT — layout de 3 blocos: (A) LEITURA DO BOT (mecanica, do EA), (B) ANALISE DA IA (contextual + banco absorvido), (C) VEREDITO CONJUNTO (sintese cruzada). Nao e chat de duas vozes conversando; e DOIS PARECERES INDEPENDENTES sobre o mesmo momento e a SINTESE do cruzamento. UMA chamada de IA por rodada devolve JSON com 3 campos: analise_ia, veredito, sinal. Sinal e alinhado|divergente|adicao|neutro — o front pinta a cor. ARQUITETURA: (1) _ANL_HIST em memoria por bot (ate 30 analises); (2) GET /monitor/analise devolve analises novas desde timestamp; (3) POST /monitor/analise/tick decide se analisa: assinatura mudou OU >90s de silencio OU evento ABRIU/FECHOU (imediato, prioridade); respeita cooldown de 30s pra gatilhos de status (evento ignora cooldown). (4) IA ABSORVE o campo confirmacao (banco de padroes medidos v6.48) como conhecimento proprio — narra padroes historicos deste ativo sem citar tabela ou banco. Custo: cockpit fechado = zero. Aberto = ~US 0.001 por analise. | 6.66 - AUDITORIA (par da app v9.52): (1) /conector/evento passa a gravar bot_token e bot_nome DENTRO do detalhe_json do evento em agente_eventos — a partir daqui todo ABRIU/FECHOU no Monitor sai como \"BOTTESTED_10 · ABRIU BUY @ 64020\" em vez de só \"ABRIU @ 64020\" (fim do \"qual bot fez essa operação?\"). (2) /monitor/eventos enriquece cada linha com bot_nome: pega do detalhe_json (eventos novos) ou faz FALLBACK casando por símbolo com os bots do usuário (eventos velhos gravados antes desta versão continuam legíveis). Zero mudança de schema — bot_nome vive dentro do JSONB detalhe_json que já existia. | 6.65 - FONTE ÚNICA DA VERDADE (fim da classe do fantasma de 15/jul): (1) _MQ5_GER_CACHE (camada de memória do cache mq5) MORREU — _mq5_cache_buscar/guardar/aprovar agora falam SÓ com a tabela mq5_cache no Supabase; DELETE FROM mq5_cache mata o cache DE VERDADE, em todos os workers, sem restart (era a memória consultada ANTES do Supabase que manteve a geração envenenada da v6.61 viva o dia inteiro). (2) _MT5_JOBS (dict em memória) MORREU — jobs de validação vivem na tabela NOVA mt5_jobs (criar/pendente/presenca/veredito/status todos via Supabase): elimina o risco multi-worker (job invisível entre workers = F4 do MAPA_PIPELINE) e o job sobrevive a deploy/restart. Limpeza de jobs >1h roda no banco com throttle de 5min. _MT5_POLLS/_VISTO_DB/_MT5_RAISE continuam em memória de propósito (telemetria rápida/throttle; a verdade deles já estava no Supabase). REQUER: rodar o SQL da tabela mt5_jobs ANTES do deploy. | 6.60 - FIX ENUM DO SELO (o último erro de compilação — achado pelo compile.log real do BOTTESTED_05): BTPainelInit usava MQLInfoString(MQL5_PROGRAM_NAME), identificador que não existe no ENUM_MQL_INFO_STRING → error 262 cannot convert enum. Corrigido pra MQL_PROGRAM_NAME. Provavelmente a causa original do 'a injeção quebrava a compilação' da era v6.35. AÇÃO PÓS-DEPLOY: limpar mq5_cache (o envio do BOTTESTED_05 recacheou o selo com o enum errado) e reenviar 1 bot. | 6.59 - FAXINA BRACE-AWARE (fix dos 4x reprovados na compilação): o regex de remoção de função ([^}]*) não atravessa chave aninhada — snapshot da IA com if/FileOpen dentro era cortado no primeiro } e o EA saía com chaves órfãs = não compila; o 1º envio cacheava o .mq5 mutilado e os envios seguintes serviam o mesmo (por isso 4x idêntico). Agora a remoção conta chaves (mesma técnica do bloco OnTimer), protege forward declaration e há sentinela de chaves desbalanceadas no log. AÇÃO PÓS-DEPLOY: invalidar o cache da estratégia (entrada mutilada congelada) e reenviar. | 6.58 - CAUSA-RAIZ FINAL do snapshot mudo: _gerar_mq5_de_codigo (fluxo /mt5/enviar) NUNCA chamava _instrumentar_log_mql5 — comentário da era v6.35 dizia que a injeção quebrava compilação, mas a injeção atual é defensiva e foi validada na v6.56. Por isso v6.55/v6.56 não mudaram nada nesse fluxo (prompt sem snapshot + visão nunca injetada = EA mudo; BOTTESTED_03 provou, EventKillTimer órfão no OnDeinit = faxina nunca rodou). Agora: gera → instrumenta (faxina+SELO+VISÃO) → cacheia neutro instrumentado; HIT sem BTVisaoTick instrumenta e regrava (defesa contra cache legado). | 6.57 - FIX /admin/mq5/invalidar: o handler referenciava _MQ5_CACHE, variável que nunca existiu (o dict real é _MQ5_GER_CACHE, ~linha 8853) — NameError -> 500 em toda invalidação. Corrigido pra _MQ5_GER_CACHE (memória) + delete no Supabase por gen_hash (já certo). | 6.56 - FAXINA DEFENSIVA: v6.55 removeu a INSTRUÇÃO do prompt mas a IA reinventava a função sozinha (ainda saía snapshot mínimo). Agora _instrumentar_log_mql5 REMOVE via regex qualquer função BTEnviarSnapshot/Snapshot/etc inventada pela IA + chamadas + Print direto + EventSetTimer (a instrumentação nossa usa OnTick, não precisa timer). Só BTVisaoTick pode emitir BOTTESTED_SNAPSHOT. Precisa reinvalidar cache e reenviar. | 6.55 - FIX DA RAIZ (loop fechado): o prompt ordenava a IA a definir e chamar uma BTEnviarSnapshot() MÍNIMA (só equity+balance+posicoes+simbolo), que competia com — e vencia — a BTVisaoTick() rica que a instrumentação injeta. Prompt agora PROÍBE a IA de definir/chamar snapshot: a instrumentação faz sozinha em OnInit/OnTick/OnDeinit. EAs regenerados a partir daqui emitem o snapshot RICO. Ação: invalidar caches e reenviar. | 6.54 - INVALIDAR CACHE DO ESPELHO (loop de fechamento — sessão de acabamento): DELETE /admin/mq5/invalidar?estrategia_id=<id>&token=<> remove o .mq5 cacheado (memória + Supabase) de UMA estratégia; próximo envio dela regenera do zero com o PROMPT ATUAL (snapshot rico c/ zonas, regime, offmind, lucro, tfop, canal EMA). Uso: EAs atuais no MT5 emitem esqueleto porque cache é pré-v6.36. Invalidar UMA estratégia + reenviar 1 bot = teste do loop ponta-a-ponta. | 6.53 - FIX SIMBOLO E FLUTUANTE NO MONITOR: (1) o simbolo do card vem do SNAPSHOT (que o EA le do _Symbol do grafico) — nao mais do conector_bots.simbolo (que era o do momento do envio, ex: US30 aparecendo num bot rodando em XAUUSD/BTCUSD); (2) flutuante NULL nao vira mais 0.0 no front (o +0,00 com posicoes>0 era isso); le detalhe.lucro como fallback se o parser antigo nao preencheu a coluna. | 6.52 - PRESENCA EM LOTE (fix de escala do conector). | 6.51 - MONITOR grafico do bot. | 6.50 - DUAS ESTRATEGIAS NOVAS. | 6.49 - MONITOR 2.0. | 6.48 - CONFIRMACAO CONTEXTUAL. | 6.47 - OLHOS DO MONITOR. | 6.46 - VISAO TOTAL. | 6.45 - FIX VITRINE paginacao. | 6.44 - CURADORIA sr_dia_anterior. | 6.43 - VITRINE sem negativo. | 6.42 - ESPELHO POR CODIGO. | 6.41 - VITRINE SEM ACOES. | 6.40 - PREVIA DE VELAS. | 6.39 - CACHE PERSISTENTE. | 6.38 - VALIDACAO RAPIDA. | 6.37 - FIM DE VIDA NO ONDEINIT. | 6.36 - SNAPSHOT EM ARQUIVO. | 6.35 - REVERTE instrumentacao custom. | (historico completo no git)"

BUILD_TAG = "2026-08-16b-bt-cascata-v3"

def _bt_base_ligar(codigo, mq5, ativo):
    """v7.46 — A BASE NASCE LIGADA. Celulas da casa nascem com as chaves do
    Ciclo viradas NO CODIGO do EA gerado (decreto de 10-11/ago: a base e
    soberana; 'dormente por padrao' virou contradicao constitucional depois
    que 3 frotas seguidas nasceram cegas por falha de formulario/ritual).
    Mandatos MEDIDOS por celula (walk-forward 12m):
      - Cruzamento/E2 (XAU e ETH): CicloAtivo + SoFavor (contexto 20/27, cruz 9/9)
      - Donchian BTC: CicloAtivo + CorteCond (7/9) + NivelBloq (B -4966 -> +351)
      - Donchian nao-BTC: so CicloAtivo (NivelBloq reprovou no XAU — lei por celula)
    Stops de desastre por ativo (pior trade dos 6m, v7.27): BTC piso 3000,
    XAU piso 150 — corrige na raiz o InpStopLoss=60 que picou _74 e _83.
    Os inputs continuam visiveis no MT5 pra DESLIGAR conscientemente (laboratorio);
    o PADRAO agora e a constituicao. So mexe nas celulas da casa; EA custom/IA
    intocado. Best-effort: qualquer falha devolve o mq5 original."""
    try:
        if not mq5:
            return mq5
        cod = codigo or ""; atv = str(ativo or "").upper(); txt = mq5
        casa_cruz = ("BT_CRUZAMENTO_CANAL" in cod) or ("BT_ESTRATEGIA2" in cod)
        casa_don  = ("BT_DONCHIAN_20" in cod)
        if not (casa_cruz or casa_don):
            return mq5
        # chaves do Ciclo (texto exato do _BT_WRAPPERS_MQL5)
        txt = txt.replace("input bool InpCicloAtivo   = false;",
                          "input bool InpCicloAtivo   = true;  // v7.46 BASE LIGADA de fabrica")
        if casa_cruz:
            txt = txt.replace("input bool InpCicloSoFavor = false;",
                              "input bool InpCicloSoFavor = true;  // v7.46 mandato SO-FAVOR (medido)")
        if casa_don and "BTC" in atv:
            txt = txt.replace("input bool InpCorteCond    = false;",
                              "input bool InpCorteCond    = true;  // v7.46 mandato corte (Donchian BTC 7/9)")
            import re as _re46
            txt = _re46.sub(r"input\s+bool\s+InpNivelBloq(\s*)=\s*false\s*;",
                            r"input bool InpNivelBloq\g<1>= true;  // v7.46 mandato nivel (BTC H1 B +351)",
                            txt, count=1)
        # piso de desastre por ativo (nunca AFROUXA um stop maior ja enviado)
        import re as _re47
        _piso = None
        if "BTC" in atv: _piso = 3000.0
        elif "XAU" in atv: _piso = 150.0
        if _piso:
            _m = _re47.search(r"(input\s+double\s+InpStopLoss\s*=\s*)([0-9.]+)", txt)
            if _m:
                try: _cur = float(_m.group(2))
                except Exception: _cur = 0.0
                if _cur < _piso:
                    txt = txt[:_m.start()] + _m.group(1) + ("%g" % _piso) + txt[_m.end():]
        return txt
    except Exception:
        return mq5
...
class TutorMomentosParams(BaseModel):
    user_id: str = ""
    bot_id: Optional[int] = None
    dias: int = 3


def _tutor_gerar_momentos(fechadas, orfas_n=0, bots_online=0, dias=3):
    """v7.31 — TUTOR v1: transforma o que a Learning Machine registrou (com a
    razao do BT_OTT_V1) em orientacao ACIONAVEL na tela. Deterministico e de
    custo ZERO de IA — o texto personalizado por IA entra na fase BYOK do Tutor;
    aqui a regua fala com os numeros REAIS do usuario. Funcao PURA de proposito:
    testavel com linhas sinteticas sem Supabase."""
    momentos = []

    def _raz(op):
        try:
            return str(((op.get("resultado") or {}).get("detalhe_ea") or {}).get("razao") or "").lower()
        except Exception:
            return ""

    def _pnl(op):
        try:
            v = (op.get("resultado") or {}).get("pnl_pts")
            return float(v) if v is not None else None
        except Exception:
            return None

    # ── 1) STOP APERTADO: 2+ saidas no stop com prejuizo no MESMO ativo ──
    por_ativo = {}
    for op in fechadas:
        if _raz(op) == "sl" and (_pnl(op) or 0) < 0:
            por_ativo.setdefault(str(op.get("ativo") or "?").upper(), []).append(op)
    for ativo, ops in sorted(por_ativo.items()):
        if len(ops) >= 2:
            exemplo = _pnl(ops[0])
            momentos.append({
                "id": f"stop_apertado:{ativo}:{ops[0].get('id')}",
                "tipo": "stop_apertado", "prioridade": 1, "ancora": "gestao_risco",
                "titulo": "Stop apertado demais?",
                "texto": (f"{len(ops)} saidas no STOP em {ativo} nos ultimos {dias} dias"
                          + (f" (ex.: {exemplo:+.2f} pts)" if exemplo is not None else "")
                          + ". Nas estrategias medidas da casa, a SAIDA e a regra — o stop e "
                            "so o desastre, longe do ruido. Referencia das celulas: XAU 120-150, "
                            "BTC 3000 pontos. Confere a distancia do teu Stop Loss."),
                "acao_label": "Ver Stop Loss"})

    # ── 2) ACERTO: a melhor fechada positiva (catraca ou regra) ──
    ganhos = [op for op in fechadas if (_pnl(op) or 0) > 0]
    if ganhos:
        melhor = max(ganhos, key=lambda o: _pnl(o) or 0)
        p = _pnl(melhor)
        if _raz(melhor) == "sl":
            momentos.append({
                "id": f"catraca:{melhor.get('id')}", "tipo": "acerto_catraca",
                "prioridade": 3, "ancora": "learning", "titulo": "A catraca pagou",
                "texto": (f"Olha essa em {melhor.get('ativo','?')}: saiu pelo stop EM LUCRO "
                          f"de {p:+.2f} pts — e o stop catracado travando ganho, nao derrota. "
                          "E assim que a protecao colhe sem devolver."),
                "acao_label": "Ver operacao"})
        else:
            momentos.append({
                "id": f"acerto:{melhor.get('id')}", "tipo": "acerto_regra",
                "prioridade": 3, "ancora": "learning", "titulo": "Acerto da regra",
                "texto": (f"Melhor operacao recente: {melhor.get('ativo','?')} {p:+.2f} pts "
                          "seguindo a estrategia ate a saida DELA. Um acerto grande paga varios "
                          "erros pequenos — e o desenho das celulas aprovadas."),
                "acao_label": "Ver operacao"})

    # ── 3) FECHAMENTO MANUAL no meio da regra ──
    manuais = [op for op in fechadas if _raz(op) == "manual"]
    if manuais:
        m0 = manuais[0]
        p = _pnl(m0)
        momentos.append({
            "id": f"manual:{m0.get('id')}", "tipo": "fechamento_manual",
            "prioridade": 2, "ancora": "learning",
            "titulo": "Fechamento manual no meio da regra",
            "texto": ("Voce fechou na mao uma posicao que a estrategia ainda geria"
                      + (f" ({p:+.2f} pts)" if p is not None else "")
                      + ". Nas celulas medidas, quem decide a saida e a REGRA — brigar com ela "
                        "no meio do trade descalibra a medicao. Confia na regua ou ajusta a regua."),
            "acao_label": "Ver operacao"})

    # ── 4/5) TELEMETRIA: placar vazio com bot online, ou orfas demais ──
    if not fechadas and orfas_n == 0 and bots_online > 0:
        momentos.append({
            "id": f"sem_telemetria:{dias}", "tipo": "sem_telemetria",
            "prioridade": 2, "ancora": "monitor", "titulo": "Bot online, placar vazio",
            "texto": ("Tem bot conectado, mas nenhuma operacao registrada no periodo. Se houve "
                      "trades no MT5, o EA no grafico e de antes da telemetria nova — reenvia o "
                      "bot (nome novo) pra Learning Machine enxergar tudo, inclusive stops do servidor.")})
    elif orfas_n >= 2:
        momentos.append({
            "id": f"orfas:{orfas_n}", "tipo": "telemetria_parcial",
            "prioridade": 2, "ancora": "monitor", "titulo": "Telemetria pela metade",
            "texto": (f"{orfas_n} fechamentos chegaram sem a abertura registrada (orfas). O EA no "
                      "grafico ainda e da era anterior — reenvia o bot pra completar o registro "
                      "de ponta a ponta.")})

    momentos.sort(key=lambda m: m.get("prioridade", 9))
    return momentos[:5]


@app.post("/tutor/momentos")
def tutor_momentos(params: TutorMomentosParams):
    """v7.31 — TUTOR v1 (aval do dono: 'quero ver isso funcionando mesmo').
    Le as operacoes recentes da Learning Machine e devolve MOMENTOS TUTORAVEIS
    ordenados por prioridade, cada um com ancora de UI, titulo e texto com os
    numeros reais do usuario. O front (v10.26) leva o robo ate a ancora.
    Stateless: dedupe/dispensa e por conta do front (localStorage)."""
    try:
        sb = _sb_admin()
        if sb is None or not (params.user_id or "").strip():
            return {"momentos": []}
        dias = max(1, min(14, int(params.dias or 3)))
        desde = (_dt.now(_tz.utc) - timedelta(days=dias)).isoformat()
        q = (sb.table("babymachine_operacoes")
             .select("id,bot_id,bot_nome,ativo,status,resultado,ts_saida")
        ...
        fechadas = [r for r in rows if r.get("status") == "fechada"]
        orfas_n = sum(1 for r in rows if r.get("status") == "orfa")
        bots_online = 0
        try:
            corte = (_dt.now(_tz.utc) - timedelta(seconds=120)).isoformat()
            bo = (sb.table("conector_bots").select("id")
                  .eq("user_id", params.user_id)
                  .gte("ultimo_ping", corte)
                  .limit(10).execute().data or [])
            bots_online = len(bo)
        except Exception:
            pass
        return {"momentos": _tutor_gerar_momentos(fechadas, orfas_n, bots_online, dias),
                "gerado_em": _dt.now(_tz.utc).isoformat()}
    except Exception as e:
        try:
            print(f"[tutor] {e}")
        except Exception:
...
def babymachine_operacoes(params: BabyMachineOperacoesParams):
    ...
    try:
        ...
        total = len(dados)
        # v7.29 — DIAGNOSTICO NA TELA DA LEARNING MACHINE: quando o periodo vem
        # VAZIO, dizer O MOTIVO em vez de so "nada coletado". Mesma filosofia do
        # BT-G01/IA morta (v6.93): a tela entrega a causa-raiz sem cacar log.
        # So roda no vazio (custo extra so quando ha o que diagnosticar).
        #   ha_fora_periodo  -> tem operacao registrada, mas fora da janela (ampliar filtro)
        #   eventos_sem_linha-> chegou evento aberto mas nenhuma linha gravou (falha backend)
        #   sem_eventos      -> nenhum evento aberto chegou (Conector/EA desatualizado)
        _diag = None
        if total == 0 and params.bot_id:
            try:
                _brow = (sb.table("conector_bots").select("nome,user_id,simbolo")
                         .eq("id", int(params.bot_id)).limit(1).execute().data or [])
                _bnome = str((_brow[0].get("nome") if _brow else "") or "")
                _buid  = (_brow[0].get("user_id") if _brow else None) or params.user_id
                _bsimb = str((_brow[0].get("simbolo") if _brow else "") or "").upper()
                # operacoes deste bot nos ultimos 365d (fora do periodo atual, que e 0)
                _desde_ano = (_dt.now(_tz.utc) - timedelta(days=365)).isoformat()
                _tem_fora = bool(sb.table("babymachine_operacoes").select("id")
                                 .eq("bot_id", int(params.bot_id))
                                 .gte("ts_criada", _desde_ano).limit(1).execute().data)
                # eventos bot_aberto na MESMA janela, casando por nome (fallback simbolo)
                _n_ev = 0
                if _buid:
                    _evs = (sb.table("agente_eventos").select("detalhe_json")
                            .eq("user_id", _buid).eq("tipo", "bot_aberto")
                            .gte("criado_em", desde).limit(400).execute().data or [])
                    for _ev in _evs:
                        _dj = _ev.get("detalhe_json") or {}
                        _nm = _dj.get("bot_nome")
                        if _nm:
                            if _bnome and _nm == _bnome: _n_ev += 1
                        else:
                            _sm = str(_dj.get("simbolo") or "").upper()
                            if _bsimb and _sm == _bsimb: _n_ev += 1
                if _tem_fora:
                    _motivo = "ha_fora_periodo"
                elif _n_ev > 0:
                    _motivo = "eventos_sem_linha"
                else:
                    _motivo = "sem_eventos"
                _diag = {"motivo": _motivo, "eventos_aberto": _n_ev, "tem_fora_periodo": _tem_fora}
            except Exception as _e:
                _diag = None
                try: print(f"[bm diag] {_e}")
...
def _lm_pts(op):
    ...
    try:
        # v6.89 — fallback RETROATIVO: o conector/EA anexa o preço no detalhe
        # do evento; fechadas antigas "sem medição" passam a medir onde há dado.
        entrada = _num(dec, "preco_real_entrada", "entrada")
        if entrada is None:
            entrada = _num(dec.get("detalhe_ea"), "preco", "price")
        saida = _num(res, "preco_saida")
        if saida is None:
            saida = _num(res.get("detalhe_ea"), "preco", "price")
        # v6.98 — TRAVA DE SANIDADE (retroativa): entrada e saída em ordens de
        # grandeza diferentes = medição CRUZOU instrumentos (ex.: 4125 ouro ->
        # 65831 BTC = -61 mil pts falsos). Não mede: None (vira sem_medicao) e
        # o placar se limpa sozinho, inclusive nas linhas antigas envenenadas.
        try:
            if entrada and saida and min(abs(entrada), abs(saida)) > 0:
                if max(abs(entrada), abs(saida)) / min(abs(entrada), abs(saida)) > 3:
                    return None
        except Exception:
            pass
...
def _lm_usd(op):
    """v7.43 — DINHEIRO REAL da operação: o lucro em moeda da conta que o EA
    carimba no evento de fechamento (OTT/BT_EVT). É a única soma honesta entre
    ativos — ponto de BTC vale $0.01 e ponto de ouro $1.00 no lote 0.01, então
    o placar agregado em pts mentia ("−43.8 pts" num dia +$63.59, 07/ago).
    None = fechada sem lucro registrado (linhas antigas)."""
    try:
        det = ((op.get("resultado") or {}).get("detalhe_ea") or {})
        v = det.get("lucro", det.get("profit"))
        if v is None:
            return None
        v = float(v)
        return round(v, 2) if abs(v) < 1e7 else None
    except Exception:
        return None
...
def babymachine_estatisticas(params: LMEstatisticasParams):
    ...
    try:
        ...
        for op in ops:
            ...
            bid = op.get("bot_id")
            grupos = [geral,
                      por_bot.setdefault(bid, dict(_novo(), bot_id=bid,
                                                   nome=op.get("bot_nome") or f"bot {bid}")),
            ...
            for g in grupos:
                if usd is not None:
                    g["usd_soma"] += usd
                    g["usd_medidas"] += 1
            if pts is not None and pts < 0 and len(erros) < 200:
                dec = op.get("decisao") or {}
                ctx = op.get("contexto_entrada") or {}
                res = op.get("resultado") or {}
                via = ctx.get("veredito_ia") or {}
                erros.append({
                    ...
                    # v7.01 — O PLANO DA ENTRADA na tela (pedido do dono): SL,
                    # TP e a projeção calculada ANTES de entrar. Ausência de
                    # stop também é informação: entrada sem stop = achismo.
                    "stop":           dec.get("stop"),
                    "alvo":           dec.get("alvo") or dec.get("alvo_projetado"),
                    "projecao":       dec.get("projecao"),
        ...
        def _fin(g):
            ...
            # v7.43 — saldo em DINHEIRO (moeda da conta) quando há lucro
            # registrado nos eventos; None = sem cobertura (honesto)
            g["saldo_usd"] = round(g["usd_soma"], 2) if g["usd_medidas"] else None
            g["usd_soma"] = round(g["usd_soma"], 2)
            return g

        _fin(geral)
        lista_bot = sorted(
            [_fin(v) for v in por_bot.values()],
...
REGRAS_VERSAO_BIB = "BT_FABRICA_V1"   # regras aplicadas na medicao (R1/R6/R7 do EA)


def _bib_parse_janela(periodo: str):
    """Le rotulos como 'janela 2/3 (5 anos)' e devolve (base, i, n).
    Rotulo comum ('2 anos') devolve (periodo, None, None) — a aba Estudo e
    qualquer chamada antiga seguem funcionando sem fatiar nada."""
    import re as _re
    m = _re.match(r"^\s*janela\s+(\d+)\s*/\s*(\d+)\s*\((.+)\)\s*$", str(periodo or ""))
    if not m:
        return (periodo, None, None)
    return (m.group(3).strip(), int(m.group(1)), int(m.group(2)))


def _bib_fatiar(df, i, n):
    """i-esima de n fatias NAO SOBREPOSTAS. Fatias independentes valem mais que
    periodos aninhados: 2y dentro de 3y dentro de 5y mede quase o mesmo mercado
    tres vezes, enquanto 3 janelas medem tres regimes diferentes."""
    if df is None or i is None or n in (None, 0) or len(df) == 0:
        return df
    tam = len(df) // n
    if tam < 60:
        return df if i == 1 else None    # curto demais pra fatiar: so a 1a vale
    ini = (i - 1) * tam
    fim = ini + tam if i < n else len(df)
    return df.iloc[ini:fim]


def _bib_risco_fabrica(df, k_alvo: float = 1.5, piso_pct: float = 0.005):
    """Traduz as REGRAS DE FABRICA do EA (BT_DISCIPLINA_V2) pras unidades do
    simulador, que le params/10000 como fracao do preco.
      R1+R6 -> stop = max(piso da estrategia, 1.5 x ATR do TF)
      R7    -> alvo = k x risco REAL (o mesmo InpBTAlvoRR do EA)
    Assim a biblioteca passa a medir o sistema que a producao executa, em vez
    do stop fixo de 0,50%% que nunca existiu no MT5."""
    try:
        tr = (df["High"] - df["Low"]).astype(float)
        base = float(df["Close"].astype(float).mean())
        if base <= 0 or len(tr) == 0:
            return (piso_pct * 10000.0, piso_pct * k_alvo * 10000.0)
        atr_pct = float(tr.mean()) / base
        stop_frac = max(piso_pct, 1.5 * atr_pct)
        return (round(stop_frac * 10000.0, 2), round(stop_frac * k_alvo * 10000.0, 2))
    except Exception:
        return (piso_pct * 10000.0, piso_pct * k_alvo * 10000.0)


def _bib_amplitude_pct(df):
    """Quanto o mercado ANDOU na janela, em %% — o denominador da captura."""
    try:
        alta = float(df["High"].astype(float).max())
        baixa = float(df["Low"].astype(float).min())
        if baixa <= 0:
            return None
        return round((alta - baixa) / baixa * 100.0, 4)
    except Exception:
        return None
...
def _matriz_calcular(ativo: str, lista_tfs: list, periodo: str = "2 anos", lang: str = "pt") -> dict:
    import time as _t
    inicio = _t.time()
    # v7.17 — o rotulo pode pedir uma JANELA ('janela 2/3 (5 anos)'): baixa o
    # periodo base UMA vez (o cache de dados serve as 3 janelas) e fatia.
    _base_per, _jan_i, _jan_n = _bib_parse_janela(periodo)
    dfs = {}
    for tf in lista_tfs:
        try:
            _d = baixar_dados(ativo, _base_per, tf)
            dfs[tf] = _bib_fatiar(_d, _jan_i, _jan_n) if _jan_i else _d
        except Exception:
            dfs[tf] = None
    linhas = []
    ranking = []
    for est in ESTRATEGIAS_PRONTAS:
        ...
        for tf in lista_tfs:
            ...
            if df is None or len(df) < 60:
                ...
                continue
            try:
                # v7.17 — REGRAS DE FABRICA: stop com piso de 1.5x ATR do TF e
                # alvo = 1.5 x risco real (R6/R7 do EA). Antes era fixo 50/100
                # = stop 0,50%% e alvo 1,00%%, um sistema que o MT5 nunca rodou.
                _sl_f, _tp_f = _bib_risco_fabrica(df)
                params = BacktestCustom(
                    ativo=ativo, periodo=periodo, timeframe=tf,
                    indicador=est["nome"], ema_period=20,
                    stop_loss=_sl_f, take_profit=_tp_f, capital=10000,
                ...
                # v7.17 — CAPTURA: quanto do movimento disponivel a estrategia
                # levou. Sem esta coluna o banco premiava quem acerta muito e
                # ganha pouco — a raiz do alvo de 36 pts numa perna de 300.
                _amp = _bib_amplitude_pct(df)
                cel["captura_pct"] = (round(cel["retorno"] / _amp * 100.0, 2)
                                      if (_amp and _amp > 0) else None)
                # v7.17 — RECEITA: sem isto o numero do card nao e reproduzivel
                # nem em principio (a estudo_biblioteca nunca guardou a receita).
                cel["parametros"] = {"stop_loss": _sl_f, "take_profit": _tp_f,
                                     "capital": 10000, "max_ops": 5,
                                     "comissao": 0.0002, "ema_period": 20,
                                     "alvo_rr": 1.5, "stop_min_atr": 1.5,
                                     "amplitude_pct": _amp}
                cel["codigo_hash"] = hashlib.sha256(
                    (est.get("codigo") or "").encode("utf-8", "ignore")).hexdigest()[:16]
                cel["regras_versao"] = REGRAS_VERSAO_BIB
                cels[tf] = cel
...
def rodar_codigo_custom(df: pd.DataFrame, params: BacktestCustom) -> dict:
    ...
    # v7.26 — FIX SANDBOX: SAFE_BUILTINS nao tem __import__ (de proposito), mas
    # os codigos dos cards trazem import pandas/from backtesting import Strategy
    # no topo — o exec estourava ImportError SEMPRE e o Teste caia calado no
    # motor padrao, que nao conhece a estrategia do card (resultado: 0 trades,
    # 0 candles). Como o ns JA fornece pd, np, Strategy e crossover, as linhas
    # de import sao redundantes: removidas antes do exec. Import perigoso segue
    # bloqueado pelo AST (verificar_codigo_seguro) antes de chegar aqui.
    _codigo_exec = "\n".join(
        l for l in params.codigo.split("\n")
        if not l.strip().startswith(("import ", "from "))
    )
    exec(_codigo_exec, ns)  # erro no codigo -> excecao sobe -> fallback
...
{{
   ...
   // v6.97 FASE 2 — grava tambem no arquivo dedicado bt_ev_<magic>.txt (APPEND,
   // flush imediato): canal confiavel pra BabyMachine, independente do buffer do
   // log. Mesmo formato do Print -> o parser do conector e o mesmo.
   string _bt_l = "BOTTESTED_EVENTO|" + tipo + "|tipo=" + tipo + "|simbolo=" + _Symbol + "|" + extra;
   string _bt_a = "bt_ev_" + IntegerToString((long)InpMagic) + ".txt";
   int _bt_h = FileOpen(_bt_a, FILE_READ|FILE_WRITE|FILE_TXT|FILE_ANSI|FILE_SHARE_READ|FILE_SHARE_WRITE);
   if(_bt_h != INVALID_HANDLE) {{ FileSeek(_bt_h, 0, SEEK_END); FileWriteString(_bt_h, _bt_l + "\n"); FileClose(_bt_h); }}
}}
void BTSnapshot()
{{
...
_BT_WRAPPERS_MQL5 = """//--- BotTested BT_CICLO_V1 — camada direcional D1xH4 + corte condicional ------
// LEI de 09/ago/2026 (walk-forward 12m, 9 ativos): contexto D1xH4 atravessou a
// metade cega em 20/27 celulas (cruzamento 9/9); corte CONDICIONAL atravessou
// 16/27 (Donchian 7/9; caso-simbolo: Donchian BTC -4966 -> -1803 na cega).
// DORMENTE por padrao (inputs false): o deploy nao muda a frota; a ativacao e
// uma decisao por celula, nao um efeito colateral de release.
input bool InpCicloAtivo   = false; // BT_CICLO: bloqueia entrada NOVA com D1xH4 alinhado CONTRA
input bool InpCicloSoFavor = false; // modo estrito: so entra COM D1xH4 alinhado A FAVOR
input bool InpCorteCond    = false; // corte condicional (contexto contra + quebra M5 sustentada)
input int  InpCorteNJ      = 3;     // fechamentos de M5 sustentando a quebra (evidencia)
int BTvCicloEstado(ENUM_TIMEFRAMES tf)
{
   // canal EMA20 High/Low do TF na ultima barra FECHADA: +1 acima / -1 abaixo / 0 dentro
   int n = 60; double k = 2.0/21.0;
   double eh = iHigh(_Symbol, tf, n), el = iLow(_Symbol, tf, n);
   for(int i = n - 1; i >= 1; i--)
   {
      eh = iHigh(_Symbol, tf, i)*k + eh*(1.0-k);
      el = iLow(_Symbol, tf, i)*k + el*(1.0-k);
   }
...
bool BTvCicloPermite(bool ehCompra, string &mot)
{
   if(!InpCicloAtivo) return(true);
   int d1 = BTvCicloEstado(PERIOD_D1), h4 = BTvCicloEstado(PERIOD_H4);
   int dir = ehCompra ? 1 : -1;
   if(d1 == -dir && h4 == -dir) { mot = "ciclo_contexto_contra";  return(false); }
   if(InpCicloSoFavor && !(d1 == dir && h4 == dir)) { mot = "ciclo_exige_favor"; return(false); }
   return(true);
}
// corte condicional — julgado em barra de M5 FECHADA; nivel congela na 1a quebra
datetime btvCicloM5 = 0; int btvCicloSeq = 0; double btvCicloNivel = 0.0;
void BTCicloTick()
{
   if(!InpCorteCond) return;
   datetime b5 = iTime(_Symbol, PERIOD_M5, 0);
   if(b5 == btvCicloM5) return;
   btvCicloM5 = b5;
   int dir = 0; ulong tk = 0;
   for(int i = PositionsTotal() - 1; i >= 0; i--)
   {
      ulong t = PositionGetTicket(i);
      if(t > 0 && PositionGetString(POSITION_SYMBOL) == _Symbol
         && PositionGetInteger(POSITION_MAGIC) == InpMagic)
      { dir = ((int)PositionGetInteger(POSITION_TYPE) == POSITION_TYPE_BUY) ? 1 : -1; tk = t; break; }
   }
   if(dir == 0) { btvCicloSeq = 0; btvCicloNivel = 0.0; return; }
   // armado SO com o contexto alinhado CONTRA a posicao (a forma que atravessou a cega)
   int d1 = BTvCicloEstado(PERIOD_D1), h4 = BTvCicloEstado(PERIOD_H4);
   if(!(d1 == -dir && h4 == -dir)) { btvCicloSeq = 0; btvCicloNivel = 0.0; return; }
   double c5  = iClose(_Symbol, PERIOD_M5, 1);
   double ext = (dir > 0) ? iLow(_Symbol,  PERIOD_M5, iLowest(_Symbol,  PERIOD_M5, MODE_LOW,  20, 2))
                          : iHigh(_Symbol, PERIOD_M5, iHighest(_Symbol, PERIOD_M5, MODE_HIGH, 20, 2));
   double nivel = (btvCicloSeq > 0) ? btvCicloNivel : ext;
   bool sustenta = (dir > 0) ? (c5 < nivel) : (c5 > nivel);
   if(sustenta)
   {
      if(btvCicloSeq == 0) btvCicloNivel = ext;
      btvCicloSeq++;
      if(btvCicloSeq >= InpCorteNJ)
      {
         if(trade.PositionClose(tk))
         {
            BTEvento("fechado", "preco=" + DoubleToString(trade.ResultPrice(), _Digits)
                     + "|motivo=corte_ciclo|nj=" + IntegerToString(InpCorteNJ));
            BTCasCorteReg(dir);
         }
         btvCicloSeq = 0; btvCicloNivel = 0.0;
      }
   }
   else { btvCicloSeq = 0; btvCicloNivel = 0.0; }
}
//--- BotTested BT_NIVEIS_V1 — mapa de teto/chao por TOQUES (decreto 09/ago) ---
// Detector deterministico da CAMADA NIVEIS (professor v4.6/v4.7, walk-forward
// 12m): pivos fractais (k=2) nas ultimas InpNivelLook barras FECHADAS, agrupados
// por 0.3xATR14; nivel VIVO = 2+ toques. MEDIDO: autopsia por estado nas 3
// celulas x 3 ativos; unica regra que atravessou a metade cega = BLOQUEIO no
// Donchian cripto (BTC H1: -4966 -> +351). Espera-confirmacao reprovou 0/6 e
// rompe-degrau reprovou nas versoes SO-FAVOR — atrasar entrada de rompimento
// destroi celula de rompimento (4a medicao da mesma lei). Por isso:
// InpNivelBloq DORMENTE por padrao (liga SO na celula promulgada); telemetria
// SEMPRE ativa nos eventos (nv= no aberto) — a LM ganha os olhos de nivel.
input bool InpNivelBloq = false; // BT_NIVEIS: bloqueia entrada colada/contra nivel 2+ toques (mandato: Donchian BTC H1)
input int  InpNivelLook = 60;    // barras fechadas do mapa de niveis (decreto do dono)
double BTNivAtr(ENUM_TIMEFRAMES tf)
{
   double soma = 0.0;
   for(int i = 1; i <= 14; i++)
   {
      double tr = iHigh(_Symbol, tf, i) - iLow(_Symbol, tf, i);
      double pc = iClose(_Symbol, tf, i + 1);
      double a = MathAbs(iHigh(_Symbol, tf, i) - pc);
      double b = MathAbs(iLow(_Symbol, tf, i) - pc);
      if(a > tr) tr = a;
      if(b > tr) tr = b;
      soma += tr;
   }
...
ENUM_TIMEFRAMES BTNivTfMenor()
{
   ENUM_TIMEFRAMES p = (ENUM_TIMEFRAMES)Period();
   if(p == PERIOD_D1)  return(PERIOD_H4);
   if(p == PERIOD_H4)  return(PERIOD_H1);
   if(p == PERIOD_H1)  return(PERIOD_M30);
   if(p == PERIOD_M30) return(PERIOD_M15);
   if(p == PERIOD_M15) return(PERIOD_M5);
   return(p);
}
double BTNivPerto(ENUM_TIMEFRAMES tf, bool tetos, double px, double atr, int &tqOut)
{
   // pivo fractal confirmado: extremo numa janela de 5 barras FECHADAS (shift>=3
   // garante que as 2 barras a frente do pivo tambem ja fecharam — zero lookahead)
   tqOut = 0;
   if(atr <= 0.0) return(0.0);
   double piv[];
   int cap = InpNivelLook + 2;
   int disp = (int)iBars(_Symbol, tf) - 3;
   if(disp < 8) return(0.0);
   if(cap > disp) cap = disp;
   ArrayResize(piv, 0, 128);
   for(int s = 3; s <= cap; s++)
   {
      bool ok = true;
      if(tetos)
      {
         ...
         for(int j = s - 2; j <= s + 2; j++)
            if(j != s && iHigh(_Symbol, tf, j) > h) { ok = false; break; }
      }
      else
      {
         double l = iLow(_Symbol, tf, s);
         for(int j = s - 2; j <= s + 2; j++)
            if(j != s && iLow(_Symbol, tf, j) < l) { ok = false; break; }
      }
      if(ok)
      {
         int n = ArraySize(piv);
         ArrayResize(piv, n + 1, 128);
         piv[n] = tetos ? iHigh(_Symbol, tf, s) : iLow(_Symbol, tf, s);
      }
   }
   ...
   ArraySort(piv);
   double tol = 0.3 * atr;
   double best = 0.0, bestDist = 0.0; int bestTq = 0;
   int i = 0;
   while(i < np)
   {
      double soma = piv[i]; int c = 1; int j = i + 1;
      while(j < np && (piv[j] - piv[j - 1]) <= tol) { soma += piv[j]; c++; j++; }
      if(c >= 2)
      {
         double m = soma / c, d = MathAbs(m - px);
         if(bestTq == 0 || d < bestDist) { best = m; bestDist = d; bestTq = c; }
      }
      i = j;
   }
   tqOut = bestTq;
...
string BTNivEstado(bool ehCompra, double px, double &nvPx, int &nvTq, double &nvDist)
{
   // estado do preco ante o nivel mais proximo NO CAMINHO da ordem (compra: tetos;
   // venda: chaos). Rotulos identicos aos do professor: NO_NIVEL (±0.3 ATR) ·
   // NIVEL_A_FRENTE (0.3-1.5 a frente) · ROMPEU_RECENTE (0.3-1.0 atras) · LIVRE.
   nvPx = 0.0; nvTq = 0; nvDist = 0.0;
   double atr = BTNivAtr(PERIOD_CURRENT);
   if(atr <= 0.0) return("LIVRE");
   int tq = 0;
   double n = BTNivPerto(PERIOD_CURRENT, ehCompra, px, atr, tq);
   if(n <= 0.0 || tq < 2) return("LIVRE");
   double dist = ehCompra ? (n - px) / atr : (px - n) / atr;
   if(dist < -1.0 || dist > 1.5) return("LIVRE");
   nvPx = n; nvTq = tq; nvDist = dist;
   if(MathAbs(dist) <= 0.3) return("NO_NIVEL");
   if(dist > 0.0) return("NIVEL_A_FRENTE");
   return("ROMPEU_RECENTE");
}
string BTNivTag(string rot, double nvPx, int nvTq, double nvDist, bool ehCompra, double px)
{
   // telemetria carimbada no evento aberto: estado no TF da celula + nivel mais
   // proximo do DEGRAU DE BAIXO (nv2, so registro — estudo multi-TF do dono)
   string t = "|nv=" + rot;
   if(nvTq >= 2)
      t += "|nv_px=" + DoubleToString(nvPx, _Digits) + "|nv_tq=" + IntegerToString(nvTq)
         + "|nv_atr=" + DoubleToString(nvDist, 2);
   ENUM_TIMEFRAMES tfm = BTNivTfMenor();
   if(tfm != (ENUM_TIMEFRAMES)Period())
   {
      double a2 = BTNivAtr(tfm);
      int q2 = 0;
      double n2 = BTNivPerto(tfm, ehCompra, px, a2, q2);
      if(n2 > 0.0 && q2 >= 2 && a2 > 0.0)
      {
         double d2 = ehCompra ? (n2 - px) / a2 : (px - n2) / a2;
         if(d2 >= -1.0 && d2 <= 1.5)
            t += "|nv2_px=" + DoubleToString(n2, _Digits) + "|nv2_tq=" + IntegerToString(q2)
               + "|nv2_atr=" + DoubleToString(d2, 2);
      }
   }
   return(t);
}
//--- BotTested: ajuste de stops + wrappers de entrada (injetado) ------
...
bool BTvExecOk()
{
   // BT_EVT_V2 — evento so sai com execucao CONFIRMADA pela corretora
   uint rc = trade.ResultRetcode();
   return (rc == TRADE_RETCODE_DONE || rc == TRADE_RETCODE_DONE_PARTIAL || rc == TRADE_RETCODE_PLACED);
}
string BTvExecExtra(string lado, double sl, double tp, double lote)
{
   return lado + "|preco=" + DoubleToString(trade.ResultPrice(), _Digits)
        + "|ticket=" + IntegerToString((long)trade.ResultOrder())
        + "|sl=" + DoubleToString(sl, _Digits) + "|tp=" + DoubleToString(tp, _Digits)
        + "|lote=" + DoubleToString(lote, 2);
}
//--- BotTested BT_CASCATA_V1 — OS 2 CICLOS DO DONO (Fase 1: TELEMETRIA PURA) --
// Desenho do Adriano (12/ago/2026, diagrama dos 2 Ciclos): cada TF le a
// SEQUENCIA do movimento em formacao e a confirmacao vertical. Ciclo 1
// (baixo->cima, M1..H1): antecipacao — a quebra nasce no 1m/5m antes do 30m.
// Ciclo 2 (cima->baixo, MN1..H4): contexto macro. FASE 1 = so LE e CARIMBA
// (zero mandato, zero mudanca de comportamento); corte/entrada viram mandato
// apos o Professor v5 medir o desenho FIEL nos 12m e o dono bater o martelo.
// Codigos por TF: U=impulso alta D=impulso baixa L=lateral; sufixos:
// ! = estrutura QUEBRADA (fechou alem do topo/fundo das 20 fechadas)
// ^ = quebra IMINENTE (preco a <=0.3xATR da estrutura no sentido do impulso)
// x = candidato a EXAUSTAO (range >=2x media10 + corpo <=35% do range)
string BTCasTF(ENUM_TIMEFRAMES tf)
{
   int n = 20;
   if(iBars(_Symbol, tf) < n + 3) return("?");
   // impulso: 5 fechadas
   int up = 0, dn = 0;
   for(int i = 1; i <= 5; i++)
   {
      double c = iClose(_Symbol, tf, i), o = iOpen(_Symbol, tf, i);
      if(c > o) up++; else if(c < o) dn++;
   }
   string s = "L";
   if(up >= 4) s = "U"; else if(dn >= 4) s = "D";
   else
   {
      double c1 = iClose(_Symbol, tf, 1), c5 = iClose(_Symbol, tf, 5);
      double rng = 0.0;
      for(int i = 1; i <= 5; i++) rng += (iHigh(_Symbol, tf, i) - iLow(_Symbol, tf, i));
      rng /= 5.0;
      if(rng > 0.0 && MathAbs(c1 - c5) >= 1.5 * rng) s = (c1 > c5 ? "U" : "D");
   }
   // estrutura: topo/fundo das 20 fechadas ANTES da ultima fechada (2..21)
   double hh = iHigh(_Symbol, tf, 2), ll = iLow(_Symbol, tf, 2);
   for(int i = 3; i <= n + 1; i++)
   {
   ...
   double c1f = iClose(_Symbol, tf, 1);
   double atr = 0.0;
   for(int i = 1; i <= 14; i++) atr += (iHigh(_Symbol, tf, i) - iLow(_Symbol, tf, i));
   atr /= 14.0;
   if(c1f > hh) s += "!";           // quebrou pra cima (fechada)
   else if(c1f < ll) s += "!";      // quebrou pra baixo (fechada)
   else if(atr > 0.0)
   {
      double px = iClose(_Symbol, tf, 0);
      if((s == "U" && (hh - px) <= 0.3 * atr && (hh - px) >= 0.0) ||
         (s == "D" && (px - ll) <= 0.3 * atr && (px - ll) >= 0.0)) s += "^"; // iminente
   }
   // exaustao na ultima fechada
   double h1 = iHigh(_Symbol, tf, 1), l1 = iLow(_Symbol, tf, 1);
   double o1 = iOpen(_Symbol, tf, 1);
   double rg = h1 - l1, corpo = MathAbs(c1f - o1), med = 0.0;
   for(int i = 2; i <= 11; i++) med += (iHigh(_Symbol, tf, i) - iLow(_Symbol, tf, i));
   med /= 10.0;
   if(med > 0.0 && rg >= 2.0 * med && rg > 0.0 && corpo <= 0.35 * rg) s += "x";
   return(s);
}
string BTCasTag()
{
   // telemetria dos 2 Ciclos carimbada no evento aberto (Fase 1)
   ENUM_TIMEFRAMES c1[5] = {PERIOD_M1, PERIOD_M5, PERIOD_M15, PERIOD_M30, PERIOD_H1};
   ENUM_TIMEFRAMES c2[4] = {PERIOD_MN1, PERIOD_W1, PERIOD_D1, PERIOD_H4};
   string e1[5], t1 = "", t2 = "";
   for(int i = 0; i < 5; i++) { e1[i] = BTCasTF(c1[i]); t1 += (i > 0 ? "." : "") + e1[i]; }
   for(int j = 0; j < 4; j++) { t2 += (j > 0 ? "." : "") + BTCasTF(c2[j]); }
   // confirmado_ate: maior andar do Ciclo 1 contando a MESMA historia do M1
   string tfn[5] = {"1M", "5M", "15M", "30M", "1H"};
   string base = StringSubstr(e1[0], 0, 1);
   string conf = "0";
   if(base != "L" && base != "?")
      for(int k = 0; k < 5; k++)
      {
         if(StringSubstr(e1[k], 0, 1) != base) break;
         conf = tfn[k];
      }
   // quebra antecipada: 1m E 5m com estrutura quebrada (!) e 30m ainda nao
   int qa = 0;
   if(StringFind(e1[0], "!") >= 0 && StringFind(e1[1], "!") >= 0 && StringFind(e1[3], "!") < 0) qa = 1;
   return("|cas1=" + t1 + "|cas2=" + t2 + "|casconf=" + conf + "|casqa=" + IntegerToString(qa));
}
//--- BotTested BT_CASCATA_V2 — DECISOES DA CASCATA (Fase 2, DORMENTE) --------
// LEI DE CONFIRMACAO DE ENTRADA (Adriano, 12/ago/2026, inegociavel): rompimento
// de topo/fundo NUNCA entra sem a BARRA SEGUINTE romper a extremidade do candle
// que rompeu; reversao em exaustao identica — o candle do topo/fundo ARMA, a
// entrada so ocorre quando o proximo candle rompe a extremidade dele. Maquina de
// estado por barra FECHADA do TF da celula: quebra de estrutura (fechamento alem
// do topo/fundo das 20) ou candidato a EXAUSTAO no extremo ARMA o gatilho na
// extremidade do candle-gatilho (validade InpCasValid barras); confirmou = a
// barra seguinte rompeu essa extremidade. TUDO DORMENTE (inputs false): sem
// numero do Professor v5, nenhum mandato liga — ligar e decisao do dono, por
// celula. Telemetria casop=/casgat=/casorig= SEMPRE ativa (acervo pro v5).
input bool InpCasEntrada  = false; // BT_CASCATA: gate de entrada (so entra com gatilho CONFIRMADO)
input int  InpCasValid    = 4;     // validade do gatilho armado (barras do TF da celula)
input bool InpCasCorte    = false; // corte-5m: M1 E M5 quebrados CONTRA, sustentado
input int  InpCasCorteNJ  = 2;     // barras de M5 sustentando a quebra dupla (evidencia)
int      btCasDir   = 0;      // 0 nada · 1 compra armada · -1 venda armada
double   btCasGat   = 0.0;    // extremidade do candle-gatilho (a linha da lei)
string   btCasOrig  = "";     // quebra | exaustao
int      btCasRest  = 0;      // validade restante (barras)
bool     btCasConf  = false;  // barra seguinte rompeu a extremidade = confirmado
datetime btCasBarra = 0;      // uma decisao por barra FECHADA
void BTCasEstado()
{
   datetime b = iTime(_Symbol, PERIOD_CURRENT, 0);
   if(b == btCasBarra) return;
   btCasBarra = b;
   if(iBars(_Symbol, PERIOD_CURRENT) < 25) return;
   ...
   // 1) gatilho armado aguardando: a barra que ACABOU de fechar e a "seguinte" da lei
   if(btCasDir != 0 && !btCasConf)
   {
      if(btCasDir == 1 && h1 > btCasGat)       btCasConf = true;
      else if(btCasDir == -1 && l1 < btCasGat) btCasConf = true;
      else
      {
         btCasRest--;
         if(btCasRest <= 0) { btCasDir = 0; btCasGat = 0.0; btCasOrig = ""; }
      }
      if(btCasConf) return;
   }
   // 2) oportunidade NOVA na barra fechada: estrutura das 20 ANTES da ultima (2..21)
   double hh = iHigh(_Symbol, PERIOD_CURRENT, 2), ll = iLow(_Symbol, PERIOD_CURRENT, 2);
   for(int i = 3; i <= 21; i++)
   {
   ...
   double rg = h1 - l1, corpo = MathAbs(c1 - o1), med = 0.0;
   for(int i = 2; i <= 11; i++) med += (iHigh(_Symbol, PERIOD_CURRENT, i) - iLow(_Symbol, PERIOD_CURRENT, i));
   med /= 10.0;
   bool exaust = (med > 0.0 && rg >= 2.0 * med && rg > 0.0 && corpo <= 0.35 * rg);
   double faixa = hh - ll;
   int dirNova = 0; string origem = "";
   if(c1 > hh)      { dirNova = 1;  origem = "quebra"; }     // rompeu topo -> compra
   else if(c1 < ll) { dirNova = -1; origem = "quebra"; }     // rompeu fundo -> venda
   else if(exaust && faixa > 0.0)
   {
      // exaustao formando FUNDO -> arma COMPRA de reversao; TOPO -> arma VENDA
      if(l1 <= ll + 0.10 * faixa)      { dirNova = 1;  origem = "exaustao"; }
      else if(h1 >= hh - 0.10 * faixa) { dirNova = -1; origem = "exaustao"; }
   }
   if(dirNova != 0)
   {
      btCasDir  = dirNova;
      btCasGat  = (dirNova == 1) ? h1 : l1;   // a extremidade do candle-gatilho
      btCasOrig = origem;
      btCasRest = InpCasValid;
      btCasConf = false;
   }
}
string BTCasOpTag()
{
   // telemetria SEMPRE ativa (ligado ou dormente): estado da maquina no aberto
   BTCasEstado();
   string op = "0";
   if(btCasDir == 1)  op = btCasConf ? "Cconf" : "Carm";
   if(btCasDir == -1) op = btCasConf ? "Vconf" : "Varm";
   string t = "|casop=" + op + "|casses=" + BTCasSessao();
   if(btCasDir != 0) t += "|casgat=" + DoubleToString(btCasGat, _Digits) + "|casorig=" + btCasOrig;
   return(t);
}
bool BTCasPermite(bool ehCompra, string &mot)
{
   if(!InpCasEntrada) return(true);   // DORMENTE: gate desligado = comportamento identico
   BTCasEstado();
   int dir = ehCompra ? 1 : -1;
   if(btCasDir != dir) { mot = "cascata_sem_oportunidade"; return(false); }
   if(!btCasConf)      { mot = "cascata_sem_confirmacao";  return(false); }
   btCasDir = 0; btCasGat = 0.0; btCasOrig = ""; btCasConf = false;  // consumo do gatilho
   return(true);
}
// corte-5m — relogio PROPRIO, independente do corte_ciclo: M1 E M5 com estrutura
// quebrada CONTRA a posicao, sustentado InpCasCorteNJ barras de M5 fechadas
bool BTCasQuebraContra(ENUM_TIMEFRAMES tf, int dirPos)
{
   if(iBars(_Symbol, tf) < 25) return(false);
   double hh = iHigh(_Symbol, tf, 2), ll = iLow(_Symbol, tf, 2);
   for(int i = 3; i <= 21; i++)
   {
      double h = iHigh(_Symbol, tf, i), l = iLow(_Symbol, tf, i);
   ...
   if(dirPos > 0) return(c1 < ll);
   return(c1 > hh);
}
//--- BotTested BT_CASCATA_V3 — CICLO COMPLETO (itens 1-4 do desenho, DORMENTES)
// Decreto do dono 16/ago: nada fica pra tras — o desenho inteiro vira estrutura
// ANTES dos testes. TUDO dormente (nada sobe sem medicao; ligar = decisao do
// dono por celula). (1) REENTRADA POS-CORTE: apos corte, mesma direcao aguarda;
// direcao NOVA so entra com reversao CONFIRMADA no 15m (o quadro verde do
// desenho). (2) ANDARES VOTAM: 30m confirma a tendencia; 1h da contexto/forca
// maior e bloqueia em exaustao. (3) CORTE DO CICLO 2: barra FECHADA de
// H4/D1/W1/MN1 com estrutura quebrada CONTRA a posicao corta. (4) FILTROS
// ADICIONAIS: sessao (GMT), noticia alto impacto (calendario MT5, degrada
// honesto se indisponivel), volume anormal, piso de ATR, correlacao entre
// ativos. CONFLITO REGISTRADO: volume-como-gate ja REPROVOU medido (volume e
// telemetria, nao voto) — o input existe por ordem do dono, nasce false, e so
// liga com numero novo do Professor v5 na mao.
input bool   InpCasReentrada  = false; // (1) reentrada pos-corte governada pelo 15m
input int    InpCasReentValid = 8;     // validade da janela pos-corte (barras de 15m)
input bool   InpCas30Conf     = false; // (2) 30m precisa CONFIRMAR a tendencia da entrada
input bool   InpCas1hCtx      = false; // (2) 1h contexto/forca: bloqueia contra e exaustao
input bool   InpCas2Corte     = false; // (3) corte de reversao do Ciclo 2 (H4/D1/W1/MN1)
input int    InpCasSessao     = 0;     // (4) 0=off 1=Asia 2=Londres 3=NY 4=Londres+NY
input bool   InpCasNoticia    = false; // (4) bloqueia janela -15/+45min de noticia ALTA
input bool   InpCasVolAnormal = false; // (4) bloqueia volume anormal (>=3x media20) [conflito registrado]
input double InpCasAtrPiso    = 0.0;   // (4) piso de ATR14 do TF (0 = off; mercado morto nao entra)
input string InpCasCorrSimbolo = "";   // (4) simbolo correlacionado ("" = off)
input bool   InpCasCorrAlinha  = true; // (4) true = exige impulso M15 ALINHADO; false = contrario
int      btCasReDir = 0;   // direcao da posicao CORTADA (janela de reentrada viva)
datetime btCasReM15 = 0;   // barra de 15m do momento do corte
void BTCasCorteReg(int dirPos)
{
   btCasReDir = dirPos;
   btCasReM15 = iTime(_Symbol, PERIOD_M15, 0);
}
string BTCasSessao()
{
   MqlDateTime g; TimeToStruct(TimeGMT(), g);
   int h = g.hour;
   if(h < 7)  return("A");   // Asia
   if(h < 12) return("L");   // Londres
   if(h < 16) return("X");   // sobreposicao Londres+NY
   if(h < 21) return("N");   // NY
   return("F");              // fora de sessao
}
bool BTCasSessaoOk()
{
   if(InpCasSessao == 0) return(true);
   string s = BTCasSessao();
   if(InpCasSessao == 1) return(s == "A");
   if(InpCasSessao == 2) return(s == "L" || s == "X");
   if(InpCasSessao == 3) return(s == "N" || s == "X");
   if(InpCasSessao == 4) return(s == "L" || s == "X" || s == "N");
   return(true);
}
bool BTCasNoticiaBloqueia()
{
   if(!InpCasNoticia) return(false);
   string m0 = SymbolInfoString(_Symbol, SYMBOL_CURRENCY_BASE);
   string m1 = SymbolInfoString(_Symbol, SYMBOL_CURRENCY_PROFIT);
   MqlCalendarValue vals[];
   datetime de = TimeTradeServer() - 900, ate = TimeTradeServer() + 2700;
   if(!CalendarValueHistory(vals, de, ate)) return(false);   // sem calendario = degrada honesto
   for(int i = 0; i < ArraySize(vals); i++)
   {
      ...
      if(!CalendarCountryById(ev.country_id, pais)) continue;
      if(pais.currency == m0 || pais.currency == m1) return(true);
   }
   return(false);
}
bool BTCasVolAnormal()
{
   if(!InpCasVolAnormal) return(false);
   if(iBars(_Symbol, PERIOD_CURRENT) < 25) return(false);
   double v1 = (double)iVolume(_Symbol, PERIOD_CURRENT, 1), med = 0.0;
   for(int i = 2; i <= 21; i++) med += (double)iVolume(_Symbol, PERIOD_CURRENT, i);
   med /= 20.0;
   return(med > 0.0 && v1 >= 3.0 * med);
}
double BTCasAtr14()
{
   double a = 0.0;
   for(int i = 1; i <= 14; i++) a += (iHigh(_Symbol, PERIOD_CURRENT, i) - iLow(_Symbol, PERIOD_CURRENT, i));
   return(a / 14.0);
}
bool BTCasCorrOk(bool ehCompra)
{
   if(StringLen(InpCasCorrSimbolo) == 0) return(true);
   if(iBars(InpCasCorrSimbolo, PERIOD_M15) < 7) return(true);   // sem dado = nao bloqueia
   int up = 0, dn = 0;
   for(int i = 1; i <= 5; i++)
   {
      double c = iClose(InpCasCorrSimbolo, PERIOD_M15, i), o = iOpen(InpCasCorrSimbolo, PERIOD_M15, i);
      if(c > o) up++; else if(c < o) dn++;
   }
   int dirC = 0;
   if(up >= 4) dirC = 1; else if(dn >= 4) dirC = -1;
   if(dirC == 0) return(true);   // correlacionado lateral nao vota
   int dir = ehCompra ? 1 : -1;
   if(InpCasCorrAlinha) return(dirC == dir);
   return(dirC == -dir);
}
bool BTCasGate2(bool ehCompra, string &mot)
{
   int dir = ehCompra ? 1 : -1;
   // (1) janela pos-corte: 15m governa a reentrada
   if(InpCasReentrada && btCasReDir != 0)
   {
      int idade = iBarShift(_Symbol, PERIOD_M15, btCasReM15);
      if(idade < 0 || idade > InpCasReentValid) btCasReDir = 0;   // janela expirou
      else
      {
         if(dir == btCasReDir) { mot = "cascata_pos_corte_aguarda"; return(false); }
         string b15 = StringSubstr(BTCasTF(PERIOD_M15), 0, 1);
         bool conf15 = (dir == 1 && b15 == "U") || (dir == -1 && b15 == "D");
         if(!conf15) { mot = "cascata_reentrada_sem_15m"; return(false); }
         btCasReDir = 0;   // reversao confirmada no 15m: janela consumida, a favor liberado
      }
   }
   // (2) andares 30m e 1h
   if(InpCas30Conf)
   {
      string b30 = StringSubstr(BTCasTF(PERIOD_M30), 0, 1);
      if(!((dir == 1 && b30 == "U") || (dir == -1 && b30 == "D")))
         { mot = "cascata_30m_nao_confirma"; return(false); }
   }
   if(InpCas1hCtx)
   {
      string e1h = BTCasTF(PERIOD_H1);
      string b1h = StringSubstr(e1h, 0, 1);
      if((dir == 1 && b1h == "D") || (dir == -1 && b1h == "U"))
         { mot = "cascata_1h_contra"; return(false); }
      if(StringFind(e1h, "x") >= 0) { mot = "cascata_1h_exaustao"; return(false); }
   }
   // (4) filtros adicionais
   if(!BTCasSessaoOk())      { mot = "filtro_sessao";         return(false); }
   if(BTCasNoticiaBloqueia()){ mot = "filtro_noticia";        return(false); }
   if(BTCasVolAnormal())     { mot = "filtro_volume_anormal"; return(false); }
   if(InpCasAtrPiso > 0.0 && BTCasAtr14() < InpCasAtrPiso)
                             { mot = "filtro_atr_piso";       return(false); }
   if(!BTCasCorrOk(ehCompra)){ mot = "filtro_correlacao";     return(false); }
   return(true);
}
datetime btCasCorteB5 = 0; int btCasCorteSeq = 0;
void BTCasTick()
{
   if(!InpCasCorte && !InpCas2Corte) return;
   int dir = 0; ulong tk = 0;
   for(int i = PositionsTotal() - 1; i >= 0; i--)
   {
      ulong t = PositionGetTicket(i);
   ...
   if(dir == 0) { btCasCorteSeq = 0; return; }
   // (3) corte de reversao do CICLO 2 — barra FECHADA de cada TF macro
   if(InpCas2Corte)
   {
      ENUM_TIMEFRAMES c2tf[4] = {PERIOD_H4, PERIOD_D1, PERIOD_W1, PERIOD_MN1};
      string c2nm[4] = {"H4", "D1", "W1", "MN1"};
      static datetime c2b[4] = {0, 0, 0, 0};
      for(int f = 0; f < 4; f++)
      {
         datetime bt = iTime(_Symbol, c2tf[f], 0);
         if(bt == c2b[f]) continue;
         c2b[f] = bt;
         if(BTCasQuebraContra(c2tf[f], dir))
         {
            if(trade.PositionClose(tk))
            {
               BTEvento("fechado", "preco=" + DoubleToString(trade.ResultPrice(), _Digits)
                        + "|ticket=" + IntegerToString((long)tk)
                        + "|motivo=corte_ciclo2|tf=" + c2nm[f]);
               BTCasCorteReg(dir);
            }
            return;
         }
   ...
   if(!InpCasCorte) return;
   datetime b5 = iTime(_Symbol, PERIOD_M5, 0);
   if(b5 == btCasCorteB5) return;
   btCasCorteB5 = b5;
   bool q1 = BTCasQuebraContra(PERIOD_M1, dir);
   bool q5 = BTCasQuebraContra(PERIOD_M5, dir);
   if(q1 && q5) btCasCorteSeq++; else btCasCorteSeq = 0;
   if(btCasCorteSeq >= InpCasCorteNJ)
   {
      if(trade.PositionClose(tk))
         BTEvento("fechado", "preco=" + DoubleToString(trade.ResultPrice(), _Digits)
                  + "|ticket=" + IntegerToString((long)tk)
                  + "|motivo=corte_cascata|nj=" + IntegerToString(InpCasCorteNJ));
      BTCasCorteReg(dir);
      btCasCorteSeq = 0;
   }
}
...
{
   string btMot = "";
   if(!BTvDisciplina(true, preco, sl, tp, lote, btMot)) { BTvRecusada("BUY", btMot); return false; }
   if(!BTvCicloPermite(true, btMot))                    { BTvRecusada("BUY", btMot); return false; }
   string btCasT = BTCasOpTag();
   if(!BTCasPermite(true, btMot))                       { BTvRecusada("BUY", btMot); return false; }
   if(!BTCasGate2(true, btMot))                         { BTvRecusada("BUY", btMot); return false; }
   double btNvPx; int btNvTq; double btNvD;
   string btNvRot = BTNivEstado(true, preco, btNvPx, btNvTq, btNvD);
   if(InpNivelBloq && (btNvRot == "NO_NIVEL" || btNvRot == "NIVEL_A_FRENTE"))
      { BTvRecusada("BUY", "nivel_bloqueio_" + btNvRot); return false; }
   BTAjustaStops(true, preco, sl, tp);
   bool ok = trade.Buy(lote, sym, preco, sl, tp, com);
   if(ok && BTvExecOk()) BTEvento("aberto", BTvExecExtra("lado=BUY", sl, tp, lote) + BTNivTag(btNvRot, btNvPx, btNvTq, btNvD, true, preco) + BTCasTag() + btCasT);
   else BTEvento("rejeitado", "lado=BUY|retcode=" + IntegerToString((long)trade.ResultRetcode()));
   return ok;
}
bool BTSell(double lote, string sym, double preco, double sl, double tp, string com="")
{
   string btMot = "";
   if(!BTvDisciplina(false, preco, sl, tp, lote, btMot)) { BTvRecusada("SELL", btMot); return false; }
   if(!BTvCicloPermite(false, btMot))                    { BTvRecusada("SELL", btMot); return false; }
   string btCasT = BTCasOpTag();
   if(!BTCasPermite(false, btMot))                      { BTvRecusada("SELL", btMot); return false; }
   if(!BTCasGate2(false, btMot))                        { BTvRecusada("SELL", btMot); return false; }
   double btNvPx; int btNvTq; double btNvD;
   string btNvRot = BTNivEstado(false, preco, btNvPx, btNvTq, btNvD);
   if(InpNivelBloq && (btNvRot == "NO_NIVEL" || btNvRot == "NIVEL_A_FRENTE"))
      { BTvRecusada("SELL", "nivel_bloqueio_" + btNvRot); return false; }
   BTAjustaStops(false, preco, sl, tp);
   bool ok = trade.Sell(lote, sym, preco, sl, tp, com);
   if(ok && BTvExecOk()) BTEvento("aberto", BTvExecExtra("lado=SELL", sl, tp, lote) + BTNivTag(btNvRot, btNvPx, btNvTq, btNvD, false, preco) + BTCasTag() + btCasT);
   else BTEvento("rejeitado", "lado=SELL|retcode=" + IntegerToString((long)trade.ResultRetcode()));
   return ok;
}
//----------------------------------------------------------------------"""
...
{
public:
   // BT_EVT_V2 — evento so na execucao CONFIRMADA, com preco/ticket/sl/tp reais
   bool BTOk()
   {
      uint rc = ResultRetcode();
      return (rc == TRADE_RETCODE_DONE || rc == TRADE_RETCODE_DONE_PARTIAL || rc == TRADE_RETCODE_PLACED);
   }
   void BTAbriu(string lado, double sl, double tp, double lote)
   {
      string _ev = "BOTTESTED_EVENTO|aberto|tipo=aberto|simbolo=" + _Symbol + "|lado=" + lado
            + "|preco=" + DoubleToString(ResultPrice(), _Digits)
            + "|ticket=" + IntegerToString((long)ResultOrder())
            + "|sl=" + DoubleToString(sl, _Digits) + "|tp=" + DoubleToString(tp, _Digits)
            + "|lote=" + DoubleToString(lote, 2);
      Print(_ev);
      BTvGravarEvento(_ev);   // v6.97 — canal confiavel pra BabyMachine
   }
   void BTRejeitou(string lado)
   {
      string _ev = "BOTTESTED_EVENTO|rejeitado|tipo=rejeitado|simbolo=" + _Symbol + "|lado=" + lado
            + "|retcode=" + IntegerToString((long)ResultRetcode());
      Print(_ev);
      BTvGravarEvento(_ev);   // v6.97
   }
   bool Buy(double volume, const string symbol=NULL, double price=0.0, double sl=0.0, double tp=0.0, const string comment="")
   {
      ...
      string btMot = "";
      if(!BTvDisciplina(true, p, sl, tp, volume, btMot)) { BTvRecusada("BUY", btMot); return false; }
      BTAjustaStops(true, p, sl, tp);
      bool ok = CTrade::Buy(volume, symbol, price, sl, tp, comment);
      if(ok && BTOk()) BTAbriu("BUY", sl, tp, volume); else BTRejeitou("BUY");
      return ok;
   }
   bool Sell(double volume, const string symbol=NULL, double price=0.0, double sl=0.0, double tp=0.0, const string comment="")
   {
      ...
      string btMot = "";
      if(!BTvDisciplina(false, p, sl, tp, volume, btMot)) { BTvRecusada("SELL", btMot); return false; }
      BTAjustaStops(false, p, sl, tp);
      bool ok = CTrade::Sell(volume, symbol, price, sl, tp, comment);
      if(ok && BTOk()) BTAbriu("SELL", sl, tp, volume); else BTRejeitou("SELL");
      return ok;
   }
   ...
   bool PositionOpen(const string symbol, ENUM_ORDER_TYPE order_type, double volume, double price, double sl, double tp, const string comment="")
   {
      ...
      string btMot = "";
      if(!BTvDisciplina(ehCompra, p, sl, tp, volume, btMot)) { BTvRecusada(ehCompra ? "BUY" : "SELL", btMot); return false; }
      BTAjustaStops(ehCompra, p, sl, tp);
      bool ok = CTrade::PositionOpen(symbol, order_type, volume, price, sl, tp, comment);
      if(ok && BTOk()) BTAbriu((ehCompra ? "BUY" : "SELL"), sl, tp, volume);
      else BTRejeitou((ehCompra ? "BUY" : "SELL"));
      return ok;
   }
   // FECHAMENTO no caminho da IA: sem isto o EA fechava via PositionClose e a
   // BabyMachine nunca via o evento (por isso "abertas" acumulavam sem fechar).
   bool PositionClose(const string symbol, const ulong deviation=ULONG_MAX)
   {
      bool ok = CTrade::PositionClose(symbol, deviation);
      if(ok) { string _ev = "BOTTESTED_EVENTO|fechado|tipo=fechado|simbolo=" + _Symbol
                   + "|preco=" + DoubleToString(ResultPrice(), _Digits)
                   + "|ticket=" + IntegerToString((long)ResultOrder());
               Print(_ev); BTvGravarEvento(_ev); }   // v6.97
      return ok;
   }
...
//  BT_PAINEL_MAGIC_V1 (v7.41): contadores filtram por InpMagic — o selo
//  mostra SO as posicoes DESTE bot (posicao manual/legada no simbolo nao
//  pinta mais Posicoes/Lucro/COMPRADO no card de um bot zerado).
//----------------------------------------------------------------------
...
int BTposSimb()
{
   ...
   for(int i=PositionsTotal()-1;i>=0;i--)
      if(PositionGetSymbol(i)==_Symbol
         && PositionGetInteger(POSITION_MAGIC)==(long)InpMagic) c++;
   return c;
}
double BTlucroSimb()
{
   ...
   for(int i=PositionsTotal()-1;i>=0;i--)
      if(PositionGetSymbol(i)==_Symbol
         && PositionGetInteger(POSITION_MAGIC)==(long)InpMagic) L+=PositionGetDouble(POSITION_PROFIT);
   return L;
}
int BTladoSimb()
{
...
_BT_VISAO_MQL5 = r"""//--- BotTested v7.19 — EA DISCIPLINADO (BT_DISCIPLINA_V3): regras de fabrica
//    que NENHUMA estrategia desliga (incidente 24-25/jul: metralhadora por
//    tick + ordens sem stop). Definido UMA vez aqui (a visao e injetada em
//    TODO EA) e consultado pelos wrappers nativos (BTBuy/BTSell) e pelo
//    clamp da IA (BTTrade.Buy/Sell/PositionOpen).
input int    InpBTMaxPosicoes   = 2;     // BT Disciplina: max posicoes simultaneas deste bot
input double InpBTLoteMax       = 1.0;   // BT Disciplina: teto de lote por ordem
input double InpBTStopMinATR    = 1.5;   // BT Disciplina: piso do stop em x ATR(14) do TF
input bool   InpBTTrailingBarra = true;  // BT Disciplina: trailing por barra fechada
input double InpBTAlvoRR        = 1.5;   // BT Disciplina: alvo minimo em x o RISCO real (R:R)
input double InpBTAlvoTendRR    = 3.0;   // BT Disciplina: alvo em x o risco quando o TF esta em tendencia a favor
...
bool BTvTendencia(bool ehCompra)
{
   // Proxy de tendencia do TF do grafico, sem indicador nem handle: a escada
   // de fechamentos 1 > 5 > 10 (compra) ou 1 < 5 < 10 (venda). Serve so pra
   // decidir o TAMANHO do alvo (R7); nunca autoriza nem barra entrada.
   double c1 = iClose(_Symbol, _Period, 1);
   double c5 = iClose(_Symbol, _Period, 5);
   double c10 = iClose(_Symbol, _Period, 10);
   if(c1 <= 0.0 || c5 <= 0.0 || c10 <= 0.0) return false;
   if(ehCompra) return (c1 > c5 && c5 > c10);
   return (c1 < c5 && c5 < c10);
}
bool BTvDisciplina(bool ehCompra, double preco, double &sl, double &tp, double &lote, string &motivo)
{
   // R4 — teto de lote: clamp no maximo do usuario (nunca acima)
   if(InpBTLoteMax > 0.0 && lote > InpBTLoteMax) lote = InpBTLoteMax;
   // R3 — teto de posicoes simultaneas deste bot (por magic)
   if(BTvPosDoBot() >= InpBTMaxPosicoes) { motivo = "teto_posicoes"; return false; }
   // R2 — CADENCIA POR BARRA (desenho do dono): UMA decisao por barra do TF
   static datetime btUltBarra = 0;
   datetime barra = iTime(_Symbol, _Period, 0);
   if(barra == btUltBarra) { motivo = "cadencia_barra"; return false; }
   // R2b — adicao (ja ha posicao) so com a barra anterior fechada A FAVOR
   if(BTvPosDoBot() > 0)
   {
      double o1 = iOpen(_Symbol, _Period, 1), c1 = iClose(_Symbol, _Period, 1);
      if(ehCompra && c1 <= o1)  { motivo = "barra_anterior_contra"; return false; }
      if(!ehCompra && c1 >= o1) { motivo = "barra_anterior_contra"; return false; }
   }
   // R1 + R6 — stop OBRIGATORIO com piso de volatilidade (1.5x ATR do TF):
   // sem SL = aplica automatico; SL mais curto que o piso = alarga. O stop
   // fica fora do ruido de 1 vela (SL fixo 36pts < passo medio matou 14 ops).
   double piso = InpBTStopMinATR * BTvATR14();
   if(piso > 0.0)
   {
      if(sl <= 0.0)                             sl = ehCompra ? (preco - piso) : (preco + piso);
      else if(ehCompra  && (preco - sl) < piso) sl = preco - piso;
      else if(!ehCompra && (sl - preco) < piso) sl = preco + piso;
   }
   // R7 — ALVO PROPORCIONAL AO RISCO (26/jul: o R6 alargou o SL ate 1.5x ATR e
   // o TP ficou no valor fixo do input -> R:R 0.58 nas 5 operacoes do dia; com
   // 40% de acerto o minimo pra empatar e 1.47). O alvo passa a ser multiplo do
   // risco REAL, ja depois de o R1/R6 fecharem o SL definitivo. So ESTICA o
   // alvo, NUNCA encurta (mesma regra da v7.09: promove, nao corta).
   if(InpBTAlvoRR > 0.0 && sl > 0.0)
   {
      double risco = ehCompra ? (preco - sl) : (sl - preco);
      if(risco > 0.0)
      {
         double k = InpBTAlvoRR;
         // Em tendencia a favor o alvo sai da frente: quem manda e o trailing
         // por barra fechada (R6 gestao), que ja segura o lucro sozinho.
         if(InpBTAlvoTendRR > InpBTAlvoRR && BTvTendencia(ehCompra)) k = InpBTAlvoTendRR;
         double alvoMin = k * risco;
         if(ehCompra)
         {
            double alvo = preco + alvoMin;
            if(tp <= 0.0 || tp < alvo) tp = alvo;
         }
         else
         {
            double alvo = preco - alvoMin;
   ...
   btUltBarra = barra;   // decisao desta barra consumida (recusa nao queima a barra)
   return true;
}
void BTvRecusada(string lado, string motivo)
{
   // v7.19 ANTI-FLOOD: com o teto de posicoes atingido a estrategia tenta
   // entrar a CADA TICK e cada recusa virava Print + FileOpen/Close no
   // bt_ev_ (dezenas por segundo) — o Conector afogava drenando recusadas
   // e o SNAPSHOT chegava com 45-60s de atraso -> card OFFLINE intermitente
   // (caso real BOTTESTED_40, 26/jul). A recusa vira evento UMA vez por
   // barra por lado+motivo; repeticao na mesma barra e silenciosa (a
   // informacao e identica e a BabyMachine nao perde nada).
   static datetime btRecBarra = 0;
   static string   btRecChave = "";
   datetime _rb = iTime(_Symbol, _Period, 0);
   string   _rc = lado + "|" + motivo;
   if(_rb == btRecBarra && _rc == btRecChave) return;
   btRecBarra = _rb;
   btRecChave = _rc;
   string _ev = "BOTTESTED_EVENTO|recusada|tipo=recusada|simbolo=" + _Symbol
              + "|lado=" + lado + "|motivo=" + motivo;
   Print(_ev);
   BTvGravarEvento(_ev);
}
void BTvTrailingBarra()
{
   // R6 gestao — trailing por barra fechada (metodologia TrailingBot): so
   // APERTA o stop, so alem do breakeven, na virada de cada barra do TF.
   if(!InpBTTrailingBarra) return;
   static datetime btTrBarra = 0;
   datetime barra = iTime(_Symbol, _Period, 0);
   if(barra == btTrBarra) return;
   btTrBarra = barra;
   double lo1 = iLow(_Symbol, _Period, 1), hi1 = iHigh(_Symbol, _Period, 1);
   for(int i = PositionsTotal() - 1; i >= 0; i--)
   {
      ulong tk = PositionGetTicket(i);
      ...
      if(PositionGetInteger(POSITION_MAGIC) != (long)InpMagic) continue;
      long tipo = PositionGetInteger(POSITION_TYPE);
      double slAtu = PositionGetDouble(POSITION_SL);
      double abre  = PositionGetDouble(POSITION_PRICE_OPEN);
      double alvo  = PositionGetDouble(POSITION_TP);
      double novo  = 0.0;
      if(tipo == POSITION_TYPE_BUY)
      { novo = NormalizeDouble(lo1, _Digits); if(novo <= abre || (slAtu > 0.0 && novo <= slAtu)) continue; }
      else if(tipo == POSITION_TYPE_SELL)
      { novo = NormalizeDouble(hi1, _Digits); if(novo >= abre || (slAtu > 0.0 && novo >= slAtu)) continue; }
      else continue;
...
int BTvPosSimb()
{
   // BT_MAGIC_ISO_V1: snapshot conta SO as posicoes deste bot (simbolo+magic)
   int c=0;
   for(int i=PositionsTotal()-1;i>=0;i--)
      if(PositionGetSymbol(i)==_Symbol
         && PositionGetInteger(POSITION_MAGIC)==(long)InpMagic) c++;
...
// v6.97 — FASE 2 da BabyMachine: grava CADA evento (aberto/rejeitado/fechado)
// numa linha do arquivo dedicado bt_ev_<magic>.txt, em modo APPEND com flush
// imediato (FileClose). O conector le e CONSOME esse arquivo — canal confiavel,
// independente do buffer do log do MT5 (que atrasava/perdia eventos na rajada).
// O Print continua em paralelo como fallback (bots/log antigos). A linha ja vem
// no MESMO formato do log (BOTTESTED_EVENTO|...), entao o parser do conector e o
// mesmo. Nunca lanca: se o arquivo estiver travado, so perde ESTE flush (o Print
// cobre) e tenta no proximo evento.
void BTvGravarEvento(const string linha_evento)
{
   string arq = "bt_ev_" + IntegerToString((long)InpMagic) + ".txt";
   int h = FileOpen(arq, FILE_READ|FILE_WRITE|FILE_TXT|FILE_ANSI|FILE_SHARE_READ|FILE_SHARE_WRITE);
   if(h == INVALID_HANDLE) return;
   FileSeek(h, 0, SEEK_END);        // APPEND: nunca sobrescreve eventos anteriores
   FileWriteString(h, linha_evento + "\n");
   FileClose(h);                    // flush imediato pro disco
}
void BTLerComando()
{
   ...
   {
      // BT_MAGIC_ISO_V1: fecha SO as posicoes DESTE bot (magic). Dois bots no
      // mesmo simbolo = veto/Guardiao de um nunca mata a posicao do outro.
      int fechados = 0;
      for(int i = PositionsTotal()-1; i >= 0; i--)
      {
         if(PositionGetSymbol(i) != _Symbol) continue;
...
void BTVisaoTick()
{
   BTvTrailingBarra();   // v7.12 — disciplina R6: gestao por barra fechada
...
def _instrumentar_log_mql5(codigo: str) -> str:
    ...
    if "//__BT_INJECT_WRAPPERS__" in codigo:
        ...
        codigo = codigo.replace("trade.PositionClose(_Symbol);",
                                "if(trade.PositionClose(_Symbol)) BTEvento(\"fechado\", \"preco=\" + DoubleToString(trade.ResultPrice(), _Digits));")
        ...
        # BT_CICLO_V1: monitor do corte condicional por barra de M5 — vive nos
        # wrappers, entao a chamada tambem so entra nos EAs NATIVOS (IA = V1.1)
        _mtk = _re.search(r"void\s+OnTick\s*\([^)]*\)", codigo)
        if _mtk:
            _itk = codigo.find("{", _mtk.end())
            if _itk >= 0:
                codigo = codigo[:_itk + 1] + "\n   BTCicloTick();\n   BTCasTick();" + codigo[_itk + 1:]
    else:
    ...
    # ── v7.30 BT_OTT_V1 → v7.41 BT_OTT_V2: TELEMETRIA POR TRANSACAO
    # V2 (item 2 da fila 06/ago): o deal de ENTRADA seleciona a posicao por
    # ticket e carimba |sl=|tp= no evento "aberto" — fecha na raiz o gap
    # SEM STOP que o v7.36/v7.37 contornavam por estimativa.
    # Buraco fechado: fechamento por SL/TP acontece NO SERVIDOR — o EA nunca via
    # e nunca emitia "fechado"; e os EAs deterministicos (executor/cruzamento/
    # donchian) nao tinham BTEvento, entao nem "aberto" emitiam. OnTradeTransaction
    # captura TODO deal do simbolo+magic (aberto, fechado por expert, SL, TP,
    # stopout, manual) e grava no MESMO formato/arquivo que o Conector ja le
    # (BOTTESTED_EVENTO|... em bt_ev_<magic>.txt). O backend ja casa por
    # ticket -> simbolo -> FIFO (v6.98); carimbamos ticket= e preco=.
    # So injeta onde NAO existe BTEvento (EAs nativos de preambulo seguem com o
    # canal proprio — zero duplicata) e onde ha InpMagic pra assinar o arquivo.
    if ("void BTEvento" not in codigo and "OnTradeTransaction" not in codigo
            and "InpMagic" in codigo):
        _bt_ott = (
            "\n// ── BT_OTT_V2 — telemetria por transacao (BotTested v7.30, SL no aberto v7.41) ──\n"
            "void BTOttGravar(string tipo, string extra)\n"
            "  {\n"
            "   string _l = \"BOTTESTED_EVENTO|\" + tipo + \"|tipo=\" + tipo + \"|simbolo=\" + _Symbol + \"|\" + extra;\n"
            "   Print(_l);\n"
            "   string _a = \"bt_ev_\" + IntegerToString((long)InpMagic) + \".txt\";\n"
            "   int _h = FileOpen(_a, FILE_READ|FILE_WRITE|FILE_TXT|FILE_ANSI|FILE_SHARE_READ|FILE_SHARE_WRITE);\n"
            "   if(_h != INVALID_HANDLE)\n"
            "     {\n"
            "      FileSeek(_h, 0, SEEK_END);\n"
            "      FileWriteString(_h, _l + \"\\n\");\n"
            "      FileClose(_h);\n"
            "     }\n"
            "  }\n"
            ...
            "   ulong _deal = trans.deal;\n"
            "   if(_deal == 0) return;\n"
            "   if(!HistoryDealSelect(_deal)) return;\n"
            "   if(HistoryDealGetString(_deal, DEAL_SYMBOL) != _Symbol) return;\n"
            "   if((long)HistoryDealGetInteger(_deal, DEAL_MAGIC) != (long)InpMagic) return;\n"
            "   long _tipoNeg = HistoryDealGetInteger(_deal, DEAL_TYPE);\n"
            "   if(_tipoNeg != DEAL_TYPE_BUY && _tipoNeg != DEAL_TYPE_SELL) return;\n"
            "   long _entrada = HistoryDealGetInteger(_deal, DEAL_ENTRY);\n"
            "   string _lado = (_tipoNeg == DEAL_TYPE_BUY) ? \"buy\" : \"sell\";\n"
            "   double _preco = HistoryDealGetDouble(_deal, DEAL_PRICE);\n"
            "   double _lucro = HistoryDealGetDouble(_deal, DEAL_PROFIT);\n"
            "   long _rz = HistoryDealGetInteger(_deal, DEAL_REASON);\n"
            "   string _razao = \"expert\";\n"
            "   if(_rz == DEAL_REASON_SL) _razao = \"sl\";\n"
            "   else if(_rz == DEAL_REASON_TP) _razao = \"tp\";\n"
            "   else if(_rz == DEAL_REASON_SO) _razao = \"stopout\";\n"
            "   else if(_rz == DEAL_REASON_CLIENT || _rz == DEAL_REASON_MOBILE || _rz == DEAL_REASON_WEB) _razao = \"manual\";\n"
            "   string _ex = \"preco=\" + DoubleToString(_preco, _Digits)\n"
            "              + \"|lado=\" + _lado\n"
            "              + \"|ticket=\" + IntegerToString((long)trans.position)\n"
            "              + \"|deal=\" + IntegerToString((long)_deal)\n"
            "              + \"|razao=\" + _razao\n"
            "              + \"|lucro=\" + DoubleToString(_lucro, 2);\n"
            "   // v7.41: SL/TP da posicao recem-nascida no evento aberto (raiz do\n"
            "   // gap SEM STOP). Posicao sem SL ou select falhou = campo omitido.\n"
            "   string _slx = \"\";\n"
            "   if(_entrada == DEAL_ENTRY_IN || _entrada == DEAL_ENTRY_INOUT)\n"
            "     {\n"
            "      if(PositionSelectByTicket((ulong)trans.position))\n"
            "        {\n"
            "         double _psl = PositionGetDouble(POSITION_SL);\n"
            "         double _ptp = PositionGetDouble(POSITION_TP);\n"
            "         if(_psl > 0.0) _slx += \"|sl=\" + DoubleToString(_psl, _Digits);\n"
            "         if(_ptp > 0.0) _slx += \"|tp=\" + DoubleToString(_ptp, _Digits);\n"
            "        }\n"
            "     }\n"
            "   if(_entrada == DEAL_ENTRY_IN)\n"
            "      BTOttGravar(\"aberto\", _ex + _slx);\n"
            "   else if(_entrada == DEAL_ENTRY_INOUT)\n"
            "     {\n"
            "      // reversao num deal so: fecha a antiga e abre a nova (nova leva o SL)\n"
            "      BTOttGravar(\"fechado\", _ex);\n"
            "      BTOttGravar(\"aberto\", _ex + _slx);\n"
            "     }\n"
            "   else\n"
            "      BTOttGravar(\"fechado\", _ex);   // OUT e OUT_BY: inclui SL/TP do servidor\n"
            "  }\n")
        codigo = codigo + _bt_ott
...
ESTRATEGIAS_PRONTAS = [
    {
        "id": "estrategia2_reversao", "casa": True, "emoji": "\U0001f3af",
        "cor_destaque": "#f59e0b",
        "nome": "Reversao no Extremo (Estrategia 2)",
        "desc": "A LEITURA DO FUNDADOR MEDIDA EM DADO REAL DO MT5 (6 meses, IC Markets): XAUUSD M30 fez PF 1.88 — e com o filtro de fase de mercado ligado, PF 2.55, o melhor numero ja medido na plataforma. A regra nasceu de leituras anotadas ao vivo: o mercado forma um FUNDO (minima das ultimas 12 barras), e a entrada e o ROMPIMENTO DA MAXIMA da barra que formou o fundo — a primeira prova de que os compradores assumiram. Stop estrutural no proprio fundo (com folga de 0.3xATR); saida quando o preco fecha de volta abaixo do canal EMA20 — estrutural, sem alvo fixo. Vale nos dois lados (topo espelhado pra venda). Tambem positiva no XAU M5 (PF 1.42/1.77 com fase). Recomendada no M30 e no XAUUSD; BTC M15 no limite (1.32 com fase), BTC M30 abaixo da regua. Stop de desastre obrigatorio como teto.",
        "tags": ["REVERSAO", "ESTRUTURAL"], "nivel": "intermedi\u00e1rio",
        "mercados": ["XAU/USD (Ouro)"],
        "celula_real": {"ativo": "XAU/USD (Ouro)", "periodo": "6 meses", "timeframe": "30m", "faixa_acerto": [35, 45], "fonte": "MT5 IC Markets"},
        "codigo": '''BT_MARCADOR = "BT_ESTRATEGIA2"   # EA nativo deterministico (recomendado: grafico M30)
# Estrategia da casa nascida da leitura do fundador, medida em 6 meses de dado
# real do MT5 (XAUUSD M30: PF 1.88; com veto de fase 2.55 — recorde da casa).
# FUNDO = minima das 12 barras; entrada no rompimento da MAXIMA da barra do
# fundo (ate 4 barras de validade); stop estrutural no fundo - 0.3xATR;
# saida no fechamento de volta abaixo do canal EMA20-Low. Lados espelhados.
# Nota: pd, Strategy e crossover ja sao fornecidos pelo motor do Teste.

def EMA(serie, n):
    return pd.Series(serie).ewm(span=n, adjust=False).mean()

class ReversaoNoExtremo(Strategy):
    lookback = 12
    validade = 4
    def init(self):
        self.ema_high = self.I(EMA, self.data.High, 20)
        self.ema_low  = self.I(EMA, self.data.Low, 20)
        self.arm = None
    def next(self):
        ...
        # saida estrutural
        if self.position.is_long and c[-1] < self.ema_low[-1]:
            self.position.close()
        elif self.position.is_short and c[-1] > self.ema_high[-1]:
            self.position.close()
        if self.position:
            return
        # dispara gatilho armado
        if self.arm:
            lado, gat, stp, val = self.arm
            if i > val:
                self.arm = None
            elif lado == 1 and h[-1] > gat:
                self.buy(sl=stp); self.arm = None; return
            elif lado == -1 and l[-1] < gat:
                self.sell(sl=stp); self.arm = None; return
        # arma na barra anterior sendo o extremo das 12
        trs = [max(h[-j] - l[-j], abs(h[-j] - c[-j - 1]), abs(l[-j] - c[-j - 1])) for j in range(2, 16)]
        atr = sum(trs) / len(trs)
        js = list(range(2, 2 + self.lookback))
        if l[-2] == min(l[-j] for j in js):
            self.arm = (1, h[-2], l[-2] - 0.3 * atr, i + self.validade)
        elif h[-2] == max(h[-j] for j in js):
            self.arm = (-1, l[-2], h[-2] + 0.3 * atr, i + self.validade)
'''
    },
    {
        "id": "cruzamento_canal", "casa": True, "emoji": "\u26a1",
        "cor_destaque": "#8b5cf6",
        "nome": "Cruzamento do Canal",
        ...
        "codigo": '''BT_MARCADOR = "BT_CRUZAMENTO_CANAL"   # EA nativo deterministico (recomendado: grafico M30)
...
# sai e INVERTE no cruzamento oposto. Sempre posicionada. Sem take profit;
# stop de desastre configuravel (campo Stop Loss = distancia em pontos de preco).
# Nota: pd, Strategy e crossover ja sao fornecidos pelo motor do Teste — sem imports.

def EMA(serie, n):
    return pd.Series(serie).ewm(span=n, adjust=False).mean()

class CruzamentoCanal(Strategy):
    n = 20
    def init(self):
        self.ema_high = self.I(EMA, self.data.High, self.n)
        self.ema_low  = self.I(EMA, self.data.Low,  self.n)
    def next(self):
        fechamento = self.data.Close[-1]
        if fechamento > self.ema_high[-1]:
            if self.position.is_short:
                self.position.close()
            if not self.position.is_long:
                self.buy()
        elif fechamento < self.ema_low[-1]:
...
'''
    ...
    {
        "id": "rompimento_donchian", "casa": True, "emoji": "💥",
        "cor_destaque": "#06b6d4",
        "nome": "Rompimento Donchian 20",
        "desc": "MEDIDA EM DADO REAL DO MT5 (6 meses, IC Markets, liquida de spread): XAUUSD positiva em TODOS os timeframes (H1 PF 2.08, M30 1.57, H4 1.71) e BTCUSD aprovada em H1 (PF 1.49, +33 mil pts) e M30 (1.34, 6 de 7 meses positivos). Regra dos dois lados: rompeu a MAXIMA das 20 barras anteriores = compra; rompeu a MINIMA = vende e INVERTE. US500 foi medido e REPROVADO em todos os TFs; BTC em H4 tambem reprova — timeframe e metade da estrategia. Sem take profit; stop de desastre por ativo.",
        "tags": ["ROMPIMENTO", "DONCHIAN"], "nivel": "intermediário",
        "mercados": ["BTC/USD", "XAU/USD (Ouro)"],
        "celula_real": {"ativo": "BTC/USD", "ativos": ["BTC/USD", "XAU/USD (Ouro)"], "periodo": "6 meses", "timeframe": "1h", "faixa_acerto": [40, 47], "fonte": "MT5 IC Markets"},
        "codigo": '''BT_MARCADOR = "BT_DONCHIAN_20"   # EA nativo deterministico (BTC: grafico H1; XAU: H1 ou M30)
# Medida em 6 meses de dado real do MT5: XAUUSD H1 PF 2.08 / BTCUSD H1 PF 1.49.
# Rompeu a MAXIMA das 20 barras anteriores = compra; rompeu a MINIMA = vende e inverte.
# Sempre posicionada apos o primeiro rompimento. Sem take profit; stop de desastre
# configuravel (campo Stop Loss = distancia em pontos de preco).
# Nota: pd e Strategy ja sao fornecidos pelo motor do Teste — sem imports.

def MAX_ANT(serie, n):
    return pd.Series(serie).rolling(n).max().shift(1)

def MIN_ANT(serie, n):
    return pd.Series(serie).rolling(n).min().shift(1)

class Donchian20DoisLados(Strategy):
    n = 20
    def init(self):
        self.topo  = self.I(MAX_ANT, self.data.High, self.n)
        self.fundo = self.I(MIN_ANT, self.data.Low,  self.n)
    def next(self):
        topo = self.topo[-1]; fundo = self.fundo[-1]
        if topo != topo or fundo != fundo:
            return  # aquecimento (NaN)
        if self.data.High[-1] > topo:
            if self.position.is_short:
                self.position.close()
            if not self.position.is_long:
                self.buy()
        elif self.data.Low[-1] < fundo:
            if self.position.is_long:
                self.position.close()
            if not self.position.is_short:
                self.sell()
...
ESTRATEGIAS_I18N = {
  "cruzamento_canal": {
    "en": {"nome": "Channel Cross", "desc": "MEASURED ON REAL MT5 DATA (6 months, 35k bars from IC Markets): XAUUSD M30 delivered PF 1.44 over 312 trades, positive in 6 of 7 months. The simplest house rule: close BELOW the EMA20 High/Low channel = sell; close ABOVE = buy; exit and REVERSE on the opposite cross. Always in the market — wins ~32% of the time, but the average win (61 pts) is 3x the average loss (20 pts). Recommended on M30 and ONLY on XAUUSD: BTC/USD (PF 0.98, spread eats the rest) and S&P500 (PF 0.85) were measured over the SAME 6 months and REPROVED — measurement honesty beats a pretty market list. Disaster stop required; no take profit by design."},
    "es": {"nome": "Cruce del Canal", "desc": "MEDIDA EN DATOS REALES DE MT5 (6 meses, 35 mil velas de IC Markets): XAUUSD M30 hizo PF 1.44 con 312 operaciones, positiva en 6 de 7 meses. La regla mas simple de la casa: cierre DEBAJO del canal EMA20 High/Low = venta; cierre ENCIMA = compra; sale e INVIERTE en el cruce opuesto. Siempre posicionada — acierta ~32% de las veces, pero la ganancia media (61 pts) es 3x la perdida media (20 pts). Recomendada en M30 y SOLO en XAUUSD: BTC/USD (PF 0.98, el spread se come el resto) y S&P500 (PF 0.85) fueron medidos en los MISMOS 6 meses y REPROBADOS — la honestidad de la medicion vale mas que una lista bonita de mercados. Stop de desastre obligatorio; sin take profit por diseno."}},
  "canal_ema20_hl": {
  ...
  "rompimento_donchian": {
    "en": {"nome": "Donchian 20 Breakout", "desc": "MEASURED ON REAL MT5 DATA (6 months, IC Markets, net of spread): XAUUSD positive on ALL timeframes (H1 PF 2.08, M30 1.57, H4 1.71) and BTCUSD approved on H1 (PF 1.49, +33k pts) and M30 (1.34, 6 of 7 positive months). Two-sided rule: break of the previous 20-bar HIGH = buy; break of the LOW = sell and REVERSE. US500 was measured and REPROVED on every TF; BTC on H4 also fails — the timeframe is half the strategy. No take profit; disaster stop per asset."},
    "es": {"nome": "Ruptura Donchian 20", "desc": "MEDIDA EN DATOS REALES DE MT5 (6 meses, IC Markets, neta de spread): XAUUSD positiva en TODOS los timeframes (H1 PF 2.08, M30 1.57, H4 1.71) y BTCUSD aprobada en H1 (PF 1.49, +33 mil pts) y M30 (1.34, 6 de 7 meses positivos). Regla de dos lados: ruptura del MAXIMO de las 20 velas anteriores = compra; ruptura del MINIMO = venta e INVIERTE. US500 fue medido y REPROBADO en todos los TF; BTC en H4 tambien reprueba — el timeframe es la mitad de la estrategia. Sin take profit; stop de desastre por activo."}},
  "macd_tendencia": {
...
_ANL_FALHA = {}         # v6.93 — bot_id -> {ts, motivo}: por que a última geração morreu
_ANL_ERRO_ULT = {}      # v6.93 — {ts, motivo} do último erro do _analisar_ia (por worker)
_ANL_HIST_MAX = 30      # 30 análises = ~30-90 min de cockpit aberto
_ANL_COOLDOWN_SEG = 30  # entre análises de status (eventos ignoram)
_ANL_SILENCIO_SEG = 300  # v7.00: cadência de 5 MIN (decisão do dono) — padrão se forma no FECHAMENTO da vela; ler a cada 90s = ler 3x a mesma vela de 5m pela metade
...
def _analisar_ia(ctx, gatilho="status"):
    ...
    chave = os.environ.get("ANTHROPIC_API_KEY", "").strip()
    if not chave:
        _ANL_ERRO_ULT.update({"ts": _time_anl.time(), "motivo": "ANTHROPIC_API_KEY ausente no ambiente"})
        return None
    ...
    try:
        ...
        r = httpx.post(
            ...
            timeout=25.0,   # v6.92 — o contexto cresceu (scanner/momentum/estrutura); 10s estourava
        )
        if r.status_code != 200:
            print(f"ANALISE IA status {r.status_code}: {r.text[:200]}", file=sys.stderr)
            _mot = f"IA respondeu status {r.status_code}"
            if r.status_code == 429: _mot += " (limite de requisições — normaliza sozinho)"
            elif r.status_code == 404: _mot += " (modelo não encontrado — conferir RADAR_IA_MODELO)"
            elif r.status_code in (500, 502, 503, 529): _mot += " (instabilidade do provedor)"
            _ANL_ERRO_ULT.update({"ts": _time_anl.time(), "motivo": _mot})
            return None
        texto = "".join(b.get("text", "") for b in r.json().get("content", []))
        texto = (texto or "").strip()
        if not texto:
            _ANL_ERRO_ULT.update({"ts": _time_anl.time(), "motivo": "resposta vazia da IA"})
            return None
        # v6.92 — PARSE RESILIENTE EM CAMADAS: a análise NUNCA morre por formato.
        # Camada 1: primeiro bloco {...} do texto (ignora prefixo/sufixo/fences).
        obj = None
        try:
            _i = texto.find("{")
            _f = texto.rfind("}")
            if _i >= 0 and _f > _i:
                obj = json.loads(texto[_i:_f + 1])
        except Exception:
            obj = None
        # Camada 2: regex campo a campo (JSON truncado no meio ainda rende os
        # campos que chegaram inteiros).
        if not isinstance(obj, dict):
            import re as _re_anl
            obj = {}
            for _campo in ("analise_ia", "veredito", "sinal"):
                _m = _re_anl.search('"' + _campo + '"\\s*:\\s*"((?:[^"\\\\]|\\\\.)*)"', texto)
                if _m:
                    try:
                        obj[_campo] = json.loads('"' + _m.group(1) + '"')
                    except Exception:
                        obj[_campo] = _m.group(1)
        an = str((obj or {}).get("analise_ia") or "").strip()
        vr = str((obj or {}).get("veredito") or "").strip()
        sn = str((obj or {}).get("sinal") or "").strip().lower()
        # Camada 3: degrada com honestidade — entrega o texto bruto limpo em
        # vez de sumir da tela (melhor uma análise sem moldura que tela vazia).
        if not an:
            _limpo = texto.replace("```json", " ").replace("```", " ").strip()
            if len(_limpo) < 30:
                print("ANALISE IA vazia demais pra degradar", file=sys.stderr)
                _ANL_ERRO_ULT.update({"ts": _time_anl.time(), "motivo": "resposta curta demais pra aproveitar"})
                return None
            an = _limpo
        if not vr:
            vr = "OBSERVAÇÃO — " + an.split(".")[0][:110]
        if sn not in ("alinhado", "divergente", "adicao", "neutro"):
            sn = "neutro"
        return {"analise_ia": an[:600], "veredito": vr[:200], "sinal": sn}
    except Exception as e:
        import traceback
        print(f"ANALISE IA erro: {e}\n{traceback.format_exc()}", file=sys.stderr)
        _mot = type(e).__name__
        if "Timeout" in _mot: _mot = "sem resposta da IA dentro do tempo (timeout)"
        _ANL_ERRO_ULT.update({"ts": _time_anl.time(), "motivo": _mot})
        return None
...
def _analisar_por_gatilho(sb, bot_id, user_id, gatilho,
    ...
    try:
        # v7.00 — CADÊNCIA DE 5 MIN (decisão do dono): o mercado que os bots
        # operam fecha vela em 1m/5m/15m — padrão se forma no FECHAMENTO, não
        # no meio da vela. UMA análise de IA por janela de 5 min lê o que
        # fechou no 1m/5m e o que está formando no 15m; mais que isso é a
        # mesma informação repetida com custo triplo. PISO GLOBAL por bot:
        # qualquer gatilho (evento, assinatura, silêncio) respeita 300s desde
        # a última análise. Exceção única: visão do operador (ação humana
        # explícita merece resposta na hora). O determinístico (scanner,
        # range, cascata, narração-cache) segue a cada snapshot, custo zero.
        if gatilho != "visao_operador":
            _piso = globals().setdefault("_ANL_PISO_ULT", {})
            _ag_p = _time_anl.time()
            if (_ag_p - float(_piso.get(bot_id) or 0)) < 300:
                return
            _piso[bot_id] = _ag_p
        ctx = {
        ...
        if not result:
            # v6.92 — BACKOFF DE FALHA: sem análise (API fora/timeout), marca o
            # momento mesmo assim — a próxima tentativa vem pelo ciclo de
            # silêncio (90s), não a cada tick de 6s. Fim do loop de queima.
            _ANL_LAST[bot_id] = {
                "assinatura": _anl_assinatura(leitura_atual) if leitura_atual else "",
                "ts": _time_anl.time(),
            }
            # v6.93 — a falha vira INFORMAÇÃO na tela (motivo exato no Bloco B)
            _mot = (_ANL_ERRO_ULT.get("motivo") or "geração sem retorno") \
                if (_time_anl.time() - float(_ANL_ERRO_ULT.get("ts") or 0)) < 60 else "geração sem retorno"
            _ANL_FALHA[bot_id] = {"ts": _time_anl.time(), "motivo": _mot}
            return
        entrada = {
            "gatilho": gatilho,
        ...
        _ANL_FALHA.pop(bot_id, None)   # v6.93 — sucesso limpa a falha da tela
        _ANL_LAST[bot_id] = {
            "assinatura": _anl_assinatura(leitura_atual) if leitura_atual else "",
            "ts": _time_anl.time(),
        }
...
def _bm_projecao_medida(entrada, stop, alvo, direcao=None):
    """v6.96 — FASE 1 da BabyMachine completa (pedido do dono: gravar na ENTRADA
    o que foi PROJETADO, pra o post-mortem comparar 'projetei X, andou Y').
    A partir de entrada/stop/alvo já capturados, calcula em PONTOS:
      - risco_pts  = |entrada - stop|      (quanto se perde se bater o stop)
      - ganho_pts  = |alvo - entrada|      (quanto foi analisado que ia andar)
      - rr         = ganho_pts / risco_pts (o R:R planejado ANTES de entrar)
    Determinístico, custo zero. Retorna dict só com o que dá pra calcular —
    campos faltando ficam de fora (nunca inventa número)."""
    out = {}
    def _n(v):
        try:
            f = float(v)
            ...
            return None
    e, s, a = _n(entrada), _n(stop), _n(alvo)
    if e is not None and s is not None:
        out["risco_pts"] = round(abs(e - s), 2)
    if e is not None and a is not None:
        out["ganho_pts"] = round(abs(a - e), 2)
    if out.get("risco_pts") and out.get("ganho_pts"):
        try:
            out["rr"] = round(out["ganho_pts"] / out["risco_pts"], 2)
        except Exception:
            pass
    return out
...
def _bm_extrair_contexto(bot, leitura, veredito):
    ...
    # v6.90 — O PORQUÊ DA ENTRADA: scanner + narração do BOT + sinais legíveis
    if leitura and isinstance(leitura, dict):
        try:
            sc = leitura.get("scanner") or {}
            if sc:
                ctx["scanner"] = {
                    "alinhamento": (sc.get("alinhamento") or {}).get("resumo"),
                    "cascata": (sc.get("cascata_reversao") or {}).get("resumo") or sc.get("cascata_reversao"),
                }
                # v7.34 — cada operacao nasce sabendo EM QUE FASE entrou: e a
                # chave que deixa a Learning Machine responder "entradas em
                # RETOMADA pagam X por cento; em RESPIRO contra, Y" por celula
                _fz = sc.get("fase") or {}
                if _fz:
                    ctx["fase"] = {"fase": _fz.get("fase"), "transicao": _fz.get("transicao"),
                                   "detalhe": _fz.get("detalhe")}
        except Exception:
            pass
        try:
            nb = leitura.get("narracao_bot")
            if nb:
                ctx["leitura_bot"] = str(nb)[:450]
        except Exception:
            pass
    ctx["sinais"] = _bm_sinais_legiveis(ctx, leitura)
    return ctx


def _bm_sinais_legiveis(ctx, leitura):
    """v6.90 — traduz o contexto do momento em FRASES prontas (o material de
    trabalho que o dono pediu: por que essa entrada, quais sinais no gráfico).
    Determinístico, custo zero. Até 8 sinais."""
    sinais = []
    try:
        if ctx.get("regime"):
            sinais.append(f"Regime {ctx['regime']}")
        for p in (ctx.get("padroes_formais") or [])[:3]:
            try:
                nome = p.get("nome") or p.get("padrao") or "?"
                tf = p.get("tf") or "?"
                d = str(p.get("direcao") or "").lower()
                sinais.append(f"{nome} no {tf}" + (f" ({d})" if d else ""))
            except Exception:
                pass
        scn = ctx.get("scanner") or {}
        if scn.get("alinhamento"):
            sinais.append(str(scn["alinhamento"]))
        casc = scn.get("cascata")
        if casc:
            sinais.append("Reversão em cascata: " + str(casc)[:90])
        if ctx.get("confirmacao_score") is not None:
            sinais.append(f"Confirmação histórica {ctx['confirmacao_score']}/100")
        if leitura and isinstance(leitura, dict):
            try:
                ex = leitura.get("extras") or {}
                rg = ex.get("range") or {}
                if rg.get("fundo") is not None and rg.get("topo") is not None:
                    est = str(rg.get("estado") or "")
                    marca = " (rompeu ▲)" if "cima" in est else (" (rompeu ▼)" if "baixa" in est or "baixo" in est else "")
                    sinais.append(f"Range {rg['fundo']}–{rg['topo']}{marca}")
            except Exception:
                pass
            try:
                for e in (leitura.get("estrutura") or [])[:2]:
                    if e.get("preco") is not None:
                        sinais.append(f"Nível {e['preco']} sendo testado ({e.get('toques', '?')} toques, {e.get('tf', '?')})")
            except Exception:
                pass
    except Exception:
        pass
...
def _bm_registrar_do_detector(sb, bot, oportunidade, leitura, veredito, comando_id, fonte):
    ...
    try:
        ...
        # v6.96 — PROJEÇÃO MEDIDA na entrada (o planejado, pro post-mortem):
        # risco/ganho em pts + R:R calculados de entrada/stop/alvo1.
        try:
            _proj = _bm_projecao_medida(oportunidade.get("entrada"),
                                        oportunidade.get("stop"),
                                        oportunidade.get("alvo1"),
                                        oportunidade.get("direcao"))
            if _proj:
                decisao["projecao"] = _proj
        except Exception:
            pass
        linha = {
...
_BM_RECON_TS = {}
_BM_RECON_ZEROS = {}    # v6.91 — bot_id -> leituras consecutivas com 0 posições
_BM_ULT_ABERTURA = {}   # v6.90 — bot_id -> (direcao, ts) trava anti-corrida do dedup


def _bm_eh_orfa_reconciliada(res):
    """Órfã criada pela reconciliação (não pelo fluxo normal de fechamento)."""
    try:
        if not isinstance(res, dict):
            return False
        if res.get("reconciliada") is True:
            return True
        return "pré-eventos-confirmados" in str(res.get("motivo") or "")
    except Exception:
        return False


def _bm_reverter_orfas(sb, bot_id):
    """v6.91 — AUTO-RECUPERAÇÃO: o MT5 mostra posição aberta mas o bot não tem
    'aberta' na BabyMachine e tem órfã de RECONCILIAÇÃO recente (<2h) — a
    reconciliação cortou registro válido (snapshot mentiu posições=0). Reverte
    a órfã mais recente pra 'aberta'. Caso real: as 4 entradas das 18:38."""
    try:
        tem_aberta = (sb.table("babymachine_operacoes")
                      .select("id").eq("bot_id", bot_id).eq("status", "aberta")
                      .limit(1).execute().data or [])
        if tem_aberta:
            return
        corte = (_dt.now(_tz.utc) - timedelta(hours=2)).isoformat()
        orfas = (sb.table("babymachine_operacoes")
                 .select("id,resultado,ts_saida").eq("bot_id", bot_id)
                 .eq("status", "orfa").gte("ts_saida", corte)
                 .order("ts_saida", desc=True).limit(10).execute().data or [])
        alvo = next((o for o in orfas if _bm_eh_orfa_reconciliada(o.get("resultado"))), None)
        if not alvo:
            return
        sb.table("babymachine_operacoes").update({
            "status": "aberta", "ts_saida": None, "resultado": None,
        }).eq("id", alvo["id"]).execute()
        print(f"[bm-reverter] bot {bot_id}: órfã {alvo['id']} restaurada pra 'aberta' (posição existe no MT5)")
    except Exception as _e:
        try: print(f"[bm-reverter] {_e}")
        except Exception: pass


def _bm_reconciliar_abertas(sb, bot_id, posicoes_abertas, flutuante=None):
    """v6.91 — RECONCILIAÇÃO DEFENSIVA: o snapshot pode reportar posições=0
    MESMO com posição aberta (parser do conector; o template BTvPosSimb conta
    certo). Uma leitura mentirosa não pode apagar histórico: só marca órfã com
    3 LEITURAS CONSECUTIVAS zeradas + idade >10min + flutuante zerado/nulo.
    Se posições>0, tenta REVERTER órfãs cortadas por engano. Throttle 120s/bot."""
    try:
        if bot_id is None or posicoes_abertas is None:
            return
        if int(posicoes_abertas) != 0:
            _BM_RECON_ZEROS[bot_id] = 0
            _bm_reverter_orfas(sb, bot_id)
            return
        # flutuante diferente de zero = HÁ posição, o campo posições mentiu
        try:
            if flutuante is not None and abs(float(flutuante)) > 0.0001:
                _BM_RECON_ZEROS[bot_id] = 0
                return
        except Exception:
            pass
        _BM_RECON_ZEROS[bot_id] = _BM_RECON_ZEROS.get(bot_id, 0) + 1
        if _BM_RECON_ZEROS[bot_id] < 3:
            return
        agora = _time_anl.time()
        if agora - _BM_RECON_TS.get(bot_id, 0) < 120:
            return
        _BM_RECON_TS[bot_id] = agora
        corte = (_dt.now(_tz.utc) - timedelta(minutes=10)).isoformat()
        abertas = (sb.table("babymachine_operacoes")
                   .select("id").eq("bot_id", bot_id).eq("status", "aberta")
                   .lt("ts_entrada", corte).limit(50).execute().data or [])
        if not abertas:
            return
        ...
        sb.table("babymachine_operacoes").update({
            ...
            "resultado": {"reconciliada": True,
                          "motivo": "aberta sem posição correspondente no MT5 "
                                    "(provável ordem rejeitada — era pré-eventos-confirmados)"},
        }).in_("id", ids).execute()
        print(f"[bm-reconciliar] bot {bot_id}: {len(ids)} aberta(s) fantasma(s) viraram órfãs (3 leituras zeradas)")
    except Exception as _e:
        try: print(f"[bm-reconciliar] {_e}")
        except Exception: pass
...
def _bm_registrar_estrategia_compilada(sb, bot, det, evento_id):
    ...
    try:
        ...
        # v6.98 — símbolo VIVO do gráfico (o evento carimba simbolo=_Symbol);
        # o de conector_bots é a intenção do envio e pode divergir do MT5 real.
        _dl_ab = {str(k).lower(): v for k, v in (det or {}).items()}
        _simb_vivo = str(_dl_ab.get("simbolo") or _dl_ab.get("symbol") or "").strip().upper()
        # v6.90 — DEDUP v2 ANTI-CORRIDA: trava em memória (por worker) pega as
        # duplicatas do MESMO segundo que a janela via banco perdia (o insert da
        # 1ª ainda não tinha commitado quando a 2ª consultava).
        # v7.46 — DEDUP POR TICKET: a reentrada REAL de 20s do BTC (stop-out +
        # reentrada no mesmo sinal, 10/ago 21:36) foi engolida pelo dedup por
        # direcao+janela. Ticket diferente = operacao NOVA, nunca duplicata.
        _tk46 = str(_dl_ab.get("ticket") or "").strip()
        try:
            _ag = _time_anl.time()
            _ult = _BM_ULT_ABERTURA.get(bot.get("id"))
            if _ult and _ult[0] == direcao and (_ag - _ult[1]) < 30:
                _tk_ant = _ult[2] if len(_ult) > 2 else ""
                if _tk46 and _tk_ant and _tk46 != _tk_ant:
                    pass  # tickets distintos = reentrada legitima, registra
                else:
                    print(f"[bm dedup-mem] abertura duplicada ignorada (bot {bot.get('id')} {direcao})")
                    return
            _BM_ULT_ABERTURA[bot.get("id")] = (direcao, _ag, _tk46)
        except Exception:
            pass
        # v6.89 — DEDUP: mesma direção aberta há <30s no mesmo bot = duplicata
        # (EA/conector emitindo em dobro). Ignora antes de inserir.
        try:
            corte = (_dt.now(_tz.utc) - timedelta(seconds=30)).isoformat()
            rec = (sb.table("babymachine_operacoes")
                   .select("id,decisao").eq("bot_id", bot.get("id"))
                   .eq("status", "aberta").gte("ts_entrada", corte)
                   .limit(3).execute().data or [])
            for r0 in rec:
                _d0 = (r0.get("decisao") or {})
                if str(_d0.get("direcao") or "") == direcao:
                    # v7.46 — ticket diferente = reentrada real, NAO e duplicata
                    _tk0 = str(_d0.get("ticket") or "").strip()
                    if _tk46 and _tk0 and _tk46 != _tk0:
                        continue
                    print(f"[bm dedup] abertura duplicada ignorada (bot {bot.get('id')} {direcao} <30s)")
                    return
        except Exception:
            pass
        decisao = {
        ...
        if _simb_vivo:
            decisao["simbolo_vivo"] = _simb_vivo
        # v6.90 — LINHAGEM: qual estratégia gerou este bot (config_operacional)
        try:
            _en = (bot.get("config_operacional") or {}).get("estrategia_nome")
            if _en:
                decisao["estrategia"] = _en
        except Exception:
            pass
        # v6.89 — EA v2 (BT_EVT_V2) manda preço real de execução + sl/tp/ticket/lote
        for _k_ev, _k_dec in (("preco", "entrada"), ("price", "entrada"),
                              ("sl", "stop"), ("tp", "alvo"),
                              ("ticket", "ticket"), ("lote", "lote")):
            _v = (det or {}).get(_k_ev)
            if _v is not None and _k_dec not in decisao:
                try:
                    decisao[_k_dec] = float(_v) if _k_dec != "ticket" else _v
                except Exception:
                    pass
        # v6.96 — PROJEÇÃO MEDIDA na entrada (pedido do dono). Duas fontes, em
        # ordem: (a) o CAÇADOR já calculou alvo/risco/rr quando localizou o
        # gatilho (leitura.scanner.gatilho) — a projeção ANALISADA antes de
        # entrar; (b) fallback: calcula de entrada/stop/alvo reais da ordem.
        try:
            _proj = {}
            _gat = ((leitura or {}).get("scanner") or {}).get("gatilho") or {}
            if isinstance(_gat, dict):
                for _kg, _kp in (("alvo_pts", "ganho_pts"), ("ganho_pts", "ganho_pts"),
                                 ("risco_pts", "risco_pts"), ("rr", "rr"),
                                 ("r_r", "rr"), ("estrelas", "estrelas"),
                                 ("confluencia", "confluencia")):
                    _vg = _gat.get(_kg)
                    if _vg is not None and _kp not in _proj:
                        _proj[_kp] = _vg
                if _gat.get("alvo") and "alvo" not in decisao:
                    decisao["alvo_projetado"] = _gat.get("alvo")
            # fallback / complemento: calcula dos valores reais da ordem
            _calc = _bm_projecao_medida(decisao.get("entrada"),
                                        decisao.get("stop"),
                                        decisao.get("alvo") or (_gat.get("alvo") if isinstance(_gat, dict) else None),
                                        direcao)
            for _k, _v in (_calc or {}).items():
                _proj.setdefault(_k, _v)
            # confirmação histórica do banco (o "medido" do momento)
            _cf = decisao.get("confluencia")
            if _cf is None:
                _cf = (contexto or {}).get("confirmacao_score")
                if _cf is not None:
                    _proj.setdefault("confirmacao_score", _cf)
            if _proj:
                decisao["projecao"] = _proj
        except Exception:
            pass
        linha = {
            ...
            "bot_id":           bot.get("id"),
            "bot_nome":         bot.get("nome") or "",
            "ativo":            (_simb_vivo or bot.get("simbolo") or ""),
            "timeframe":        bot.get("timeframe") or "",
...
def _bm_processar_fechamento(sb, bot, det, evento_id):
    ...
    if not sb or not bot: return
    try:
        # v6.98 — CASAMENTO v2 (medição íntegra): o FIFO cego cruzava
        # instrumentos. Ordem nova: TICKET do evento -> SÍMBOLO do evento
        # (o EA carimba simbolo=_Symbol) -> FIFO puro só p/ evento antigo
        # sem símbolo. Aberta de outro símbolo NUNCA fecha por engano.
        _dl0 = {str(k).lower(): v for k, v in (det or {}).items()}
        _ev_simb = str(_dl0.get("simbolo") or _dl0.get("symbol") or "").strip().upper()
        _ev_tick = str(_dl0.get("ticket") or "").strip()
        _cands = (sb.table("babymachine_operacoes")
                  ...
                  .execute().data or [])
        def _op_simb(o):
            _d0 = o.get("decisao") or {}
            return str(_d0.get("simbolo_vivo") or o.get("ativo") or "").strip().upper()
        abertas = []
        if _ev_tick:
            abertas = [o for o in _cands
                       if str(((o.get("decisao") or {}).get("ticket")) or "").strip() == _ev_tick]
        if not abertas and _ev_simb:
            abertas = [o for o in _cands if _op_simb(o) in ("", _ev_simb)]
        if not abertas and not _ev_simb:
            abertas = _cands
        abertas = abertas[:1]
        if not abertas:
            # v6.91 — AUTO-RECUPERAÇÃO na saída: antes de criar órfã nova,
            # procura órfã de RECONCILIAÇÃO recente (<2h) — se a reconciliação
            # cortou o registro por engano, o fechamento real devolve ele pro
            # histórico COM medição (preço/pnl), em vez de duplicar lixo.
            try:
                corte_r = (_dt.now(_tz.utc) - timedelta(hours=2)).isoformat()
                orfas_r = (sb.table("babymachine_operacoes")
                           .select("id,decisao,ts_entrada,resultado,ativo").eq("bot_id", bot.get("id"))
                           .eq("status", "orfa").gte("ts_saida", corte_r)
                           .order("ts_saida", desc=True).limit(10).execute().data or [])
                alvo_r = next((o for o in orfas_r
                               if _bm_eh_orfa_reconciliada(o.get("resultado"))
                               and (not _ev_simb or _op_simb(o) in ("", _ev_simb))), None)
                if alvo_r:
                    abertas = [alvo_r]
                    print(f"[bm-recuperar] bot {bot.get('id')}: fechamento recuperou órfã {alvo_r['id']} pro histórico")
            except Exception:
                pass
        if not abertas:
            ...
            try:
                sb.table("babymachine_operacoes").insert({
                    ...
                    "bot_id":          bot.get("id"),
                    "bot_nome":        bot.get("nome") or "",
                    "ativo":           (_ev_simb or bot.get("simbolo") or ""),
                    "fonte":           "estrategia_compilada",
                    "status":          "orfa",
...
def _dd_dia_pct(bot_id, equity):
    """v6.88 — drawdown DO DIA: equity atual vs equity de início do dia
    registrado pelo circuit breaker do detector (_CIRCUIT_STATE, memória por
    worker). None quando o estado ainda não existe — o front mostra traço
    honesto em vez de número inventado."""
    try:
        st = _CIRCUIT_STATE.get(bot_id) or {}
        ini = float(st.get("equity_inicio_dia") or 0)
        eq = float(equity or 0)
        if ini > 0 and eq > 0:
            return round(max(0.0, (ini - eq) / ini * 100), 2)
    except Exception:
        pass
    return None
_OPORT_MAX_HIST = 30
...
def _detectar_direcao_potencial(fatos, leitura):
    ...
    if rng.get("estado") == "rompeu_baixo" and rng.get("fundo"):
        return {"direcao": "short", "cenario": "rompimento_range_baixa",
                ...
                "range": rng}
    # Caso 0c (v7.18): CANAL EMA 20 H/L + BOLLINGER — metodologia do dono.
    # Entra ANTES do range/estrutura porque e a estrategia da casa e ela ja
    # traz entrada e stop calculados pela propria geometria (exaustao ->
    # confirmacao -> rompimento do extremo da barra B). Sem alvo de proposito:
    # a saida e a volta pro canal, nao um numero.
    ce = fatos.get("canal_ema_setup") or {}
    if ce.get("gatilho") == "G1" and ce.get("direcao") and not ce.get("veto"):
        return {"direcao": ce["direcao"],
                "cenario": ("canal_ema_reversao_alta" if ce["direcao"] == "long"
                            else "canal_ema_reversao_baixa"),
                "gatilho_ref": {"tipo": "exaustao_bollinger",
                                "nivel": ce.get("entrada_em"),
                                "stop": ce.get("stop"),
                                "volume_ok": ce.get("volume_ok"),
                                "armado": ce.get("armado")},
                "canal_ema": ce}
...
def _calcular_niveis_operacao(direcao_info, fatos):
    ...
    if str(direcao_info.get("cenario") or "").startswith("reversao_estrutura") and _sw_mm.get("proj") is not None:
        ...
        if direcao == "long":
            ...
            alvo1 = float(_sw_mm["proj"]); alvo2 = alvo1 - _alt_sw * 0.5
    # v7.09 — REGUA DA ESCADA (caso real do dono, 25/jul BTCUSD: range de 64.9
    # pts rompido a favor de 4/4 TFs; a projecao miuda dava alvo 35 pts e
    # R:R 1.03 — trade limpo barrado pela regua curta, enquanto o proprio BOT
    # narrava os suportes de 60m/4h como alvos tecnicos a 60-118 pts).
    # Em cenario de ROMPIMENTO, o measured move e PISO, nao teto: o alvo do
    # plano e a escada de niveis medidos na direcao — alvo1 = primeiro nivel
    # alem do risco (take parcial), alvo2 = segundo nivel quando existe, e o
    # R:R do plano e medido contra o alvo FINAL da cascata. So promove
    # (nunca encurta o que ja foi calculado) e so usa nivel que EXISTE.
    _alvo_rr = None
    if str(direcao_info.get("cenario") or "").startswith(("rompimento_range", "reversao_estrutura")):
        try:
            _escada_rr = suportes if direcao == "short" else resistencias
            _alem = []
            for _nv in (_escada_rr or [])[:4]:
                try:
                    _pnv = float(_nv.get("nivel"))
                    _dnv = (entrada - _pnv) if direcao == "short" else (_pnv - entrada)
                    if _dnv > risco:
                        _alem.append(_pnv)
                except Exception:
                    pass
            if _alem:
                if abs(_alem[0] - entrada) > abs(alvo1 - entrada):
                    alvo1 = _alem[0]
                if len(_alem) > 1 and abs(_alem[1] - entrada) > abs(alvo1 - entrada):
                    alvo2 = _alem[1]
                    _alvo_rr = alvo2   # plano em cascata: parcial no 1o, final no 2o
                else:
                    _alvo_rr = alvo1
        except Exception:
            _alvo_rr = None
    rr = abs((_alvo_rr if _alvo_rr is not None else alvo1) - entrada) / risco if risco > 0 else 0
    if rr < 1.0:
...
def _chk_ia_ultima(bot_id):
    """v7.06 — a ultima analise cruzada do cockpit (_ANL_HIST) vira voto no
    checklist: sinal alinhado/divergente/neutro + idade. Custo zero."""
    try:
        import time as _t
        hist = (_ANL_HIST.get(bot_id) or [])
        if not hist:
            return None
        h = hist[-1] or {}
        ts = h.get("ts")
        ha = (_t.time() - float(ts)) if ts else None
        return {"sinal": h.get("sinal"), "ha_s": ha}
    except Exception:
        return None


def _chk_leitura(det, s, b, c_op=None):
    """v7.04 — monta o checklist do card com os formatos REAIS da leitura
    (fix do print do dono: 4/4 baixa na tela e o LED de tendencia vermelho).
    Direcao: alinhamento -> cascata -> range rompido -> regime. R:R: derivado
    da projecao do range (proj_alta/proj_baixa vs nivel rompido) quando ha
    rompimento — o gate de EXECUCAO no detector segue usando o rr dos niveis."""
    try:
        pac = _scanner_pacote(det) or {}
        ex = _extras_cockpit(det, s) or {}
        _alp = pac.get("alinhamento")
        ali = str((_alp.get("resumo") or _alp.get("lado")) if isinstance(_alp, dict) else (_alp or "")).lower()
        casc = str(pac.get("cascata_reversao") or "").lower()
        regj = det.get("regime")
        reg = str((regj.get("estado") if isinstance(regj, dict) else regj) or "").upper()
        rng = ex.get("range") or {}
        est_r = str(rng.get("estado") or "")
        direcao = None
        if "baixa" in ali: direcao = "short"
        elif "alta" in ali: direcao = "long"
        elif "baixa" in casc: direcao = "short"
        elif "alta" in casc: direcao = "long"
        elif est_r == "rompeu_baixo" or "BAIXA" in reg: direcao = "short"
        elif est_r == "rompeu_cima" or "ALTA" in reg: direcao = "long"
        # R:R do rompimento do range: entrada=preco agora, alvo=projecao,
        # invalidacao=voltar pro nivel rompido. So calcula com dado real.
        rr = None
        try:
            preco = float(det.get("preco"))
            alvo = inval = None
            if est_r == "rompeu_baixo":
                alvo, inval = rng.get("proj_baixa"), rng.get("fundo")
            elif est_r == "rompeu_cima":
                alvo, inval = rng.get("proj_alta"), rng.get("teto")
            if alvo is not None and inval is not None:
                risco = abs(preco - float(inval))
                ganho = abs(float(alvo) - preco)
                # v7.09 — REGUA DA ESCADA no card: o alvo do rompimento e o
                # melhor entre a projecao e os 2 primeiros niveis da escada
                # na direcao (dist_pts alem do risco). Mesma regua do detector.
                try:
                    _esc_c = ex.get("niveis_abaixo") if est_r == "rompeu_baixo" else ex.get("niveis_acima")
                    for _nvc in (_esc_c or [])[:2]:
                        _dc = abs(float(_nvc.get("dist_pts")))
                        if _dc > risco and _dc > ganho:
                            ganho = _dc
                except Exception:
                    pass
                if risco > 0:
                    rr = round(ganho / risco, 2)
        except Exception:
            pass
        # v7.20 — R:R CONDICIONAL DO PLANO EM FORMACAO: sem rompimento de
        # range, deriva o R:R da ESTRUTURA DE SWING (o BOT ja narra o plano;
        # o card ignorava). formando: entrada=neckline, risco=|neck-P2|,
        # alvo=projecao. rompeu: entrada=preco, invalidacao=neckline,
        # alvo=projecao. Mesma regua da escada v7.09 (promove alvo pros
        # niveis medidos alem do risco). Rotulado como plano condicional.
        rr_nota = None
        try:
            if rr is None:
                sw = ex.get("swing") or {}
                sw_est = str(sw.get("estado") or "")
                sw_dir = str(sw.get("direcao") or "").lower()
                neck = sw.get("neckline")
                proj = sw.get("projecao")
                inv = sw.get("invalidacao")
                preco2 = float(det.get("preco"))
                if sw_est in ("formando", "rompeu") and neck is not None and proj is not None:
                    if sw_est == "formando" and inv is not None:
                        ent2 = float(neck)
                        risco2 = abs(ent2 - float(inv))
                        rr_nota = f"plano condicional — no rompimento da neckline {float(neck):g}"
                    else:
                        ent2 = preco2
                        risco2 = abs(ent2 - float(neck))
                        rr_nota = "plano do rompimento da estrutura"
                    ganho2 = abs(float(proj) - ent2)
                    try:
                        _esc2 = ex.get("niveis_abaixo") if sw_dir == "baixa" else ex.get("niveis_acima")
                        for _nv2 in (_esc2 or [])[:2]:
                            _d2 = abs(float(_nv2.get("dist_pts")))
                            if _d2 > risco2 and _d2 > ganho2:
                                ganho2 = _d2
                    except Exception:
                        pass
                    if risco2 > 0 and ganho2 > 0:
                        rr = round(ganho2 / risco2, 2)
                    else:
                        rr_nota = None
                if direcao is None and sw_dir in ("alta", "baixa"):
                    direcao = "long" if sw_dir == "alta" else "short"
        except Exception:
            rr_nota = None
        candles = c_op if c_op is not None else []
        return _checklist_entrada(direcao=direcao, scanner=pac, extras=ex,
                                  candles_op=candles, rr=rr, rr_nota=rr_nota,
                                  atr_pts=ex.get("atr_pts"),
                                  simbolo=det.get("simbolo") or (s or {}).get("simbolo") or "",
                                  bot=b,
                                  sensibilidade=((b or {}).get("config_operacional") or {}).get("modo_sensibilidade"),
                                  ia=_chk_ia_ultima((b or {}).get("id")))
    except Exception:
        return None


_CHK_CAL_CACHE = {}   # v7.03: simbolo -> (ts, evento_quente|None) — cache 120s

def _chk_moedas_do_simbolo(simbolo):
    """Moedas relevantes pro checklist de noticia. Deterministico e defensivo."""
    sim = str(simbolo or "").upper()
    ms = set()
    for m in ("USD", "EUR", "GBP", "JPY", "AUD", "CAD", "CHF", "NZD"):
        if m in sim:
            ms.add(m)
    if any(k in sim for k in ("XAU", "BTC", "US500", "NAS", "US30", "SPX")):
        ms.add("USD")
    return ms or {"USD"}


def _chk_noticia_quente(simbolo):
    """Item 1 do checklist: evento de ALTO impacto na janela (-15min..+45min)
    pras moedas do simbolo, lendo calendario_economico. Cache 120s por simbolo.
    Retorna (ok, detalhe): True=sem evento quente; False=evento quente;
    None=sem dado (tabela vazia/erro) — nunca inventa."""
    import time as _t
    from datetime import datetime as _dt2, timedelta as _td2, timezone as _tz2
    sim = str(simbolo or "").upper()
    hit = _CHK_CAL_CACHE.get(sim)
    if hit and (_t.time() - hit[0]) < 120:
        return hit[1]
    res = (None, "calendario sem dado — verificacao manual")
    try:
        sb = _sb_admin()
        if sb is not None:
            agora = _dt2.now(_tz2.utc)
            ini = (agora - _td2(minutes=15)).isoformat()
            fim = (agora + _td2(minutes=45)).isoformat()
            r = (sb.table("calendario_economico").select("*")
                 .gte("data_evento", ini).lte("data_evento", fim)
                 .limit(60).execute())
            linhas = r.data or []
            moedas = _chk_moedas_do_simbolo(sim)
            quente = None
            for ev in linhas:
                if str(ev.get("impacto") or "").lower() != "alto":
                    continue
                if str(ev.get("moeda") or "").upper() in moedas:
                    quente = ev
                    break
            if quente is not None:
                res = (False, f"noticia de alto impacto na janela: {str(quente.get('titulo') or quente.get('evento') or '?')[:60]}")
            else:
                res = (True, "sem noticia de alto impacto na janela (-15/+45min)")
    except Exception:
        res = (None, "calendario indisponivel — verificacao manual")
    _CHK_CAL_CACHE[sim] = (_t.time(), res)
    return res


def _checklist_entrada(direcao=None, scanner=None, extras=None, candles_op=None,
                       rr=None, atr_pts=None, simbolo="", bot=None,
                       sensibilidade=None, ia=None, rr_nota=None):
    """v7.03 — CHECKLIST DE ENTRADA (desenho do dono, LEDs verde/vermelho):
    o portao deterministico que a IA e o BOT checam ANTES de qualquer execucao
    da plataforma. So dados que a casa JA mede — custo zero de IA, nunca inventa:
      1 noticia     — calendario_economico (alto impacto na janela)
      2 tendencia   — regime/alinhamento do scanner a favor da direcao
      3 volume      — ultima vela do TF de operacao vs media das 10 anteriores
      4 sr          — corredor livre na escada de niveis (>=0.8 ATR sem nivel contrario)
      5 rr          — projecao medida: risco:retorno >= 1:2
      6 estrategia  — linhagem registrada (config_operacional.estrategia_nome)
    ok True=verde, False=vermelho, None=sem dado (cinza; nao bloqueia, e dito).
    pode_entrar = TODOS os itens com dado em verde E ha direcao definida."""
    itens = []
    d = str(direcao or "").lower()
    alvo = "alta" if d in ("long", "buy", "alta") else ("baixa" if d in ("short", "sell", "baixa") else "")

    # 1) NOTICIA
    ok1, det1 = _chk_noticia_quente(simbolo)
    itens.append({"id": "noticia", "nome": "Noticia verificada", "ok": ok1, "detalhe": det1,
                  "pct": (100 if ok1 is True else (0 if ok1 is False else None))})

    # 2) TENDENCIA CONFIRMADA
    ok2, det2 = None, "sem dado do scanner"
    try:
        sc = scanner or {}
        _al = sc.get("alinhamento")
        # v7.08: alinhamento real e um dict {lado, resumo} — extrai o texto
        alin = str((_al.get("resumo") or _al.get("lado")) if isinstance(_al, dict) else (_al or ""))
        regime = ""
        try:
            regime = str((sc.get("regime") or {}).get("estado") or sc.get("regime") or "").upper()
        except Exception:
            regime = str(sc.get("regime") or "").upper()
        if not alvo:
            ok2, det2 = False, "sem direcao definida — aguardando gatilho de entrada"
        elif alin or regime:
            a_favor = (alvo in alin.lower()) or (alvo.upper() in regime)
            contra = (("baixa" if alvo == "alta" else "alta") in alin.lower()) and not a_favor
            if a_favor and "divid" not in alin.lower():
                ok2, det2 = True, (alin or regime)[:80]
            elif contra or "divid" in alin.lower():
                ok2, det2 = False, (alin or regime)[:80] or "TFs divididos/contra"
            else:
                ok2, det2 = False, "tendencia nao confirma a direcao"
    except Exception:
        pass
    # v7.32 — CICLO CIRCULAR no portao de tendencia (decisao do dono 04/ago,
    # caso do Brent: barra gigante de reversao bloqueada como contra-tendencia
    # enquanto 30 barras de M1 contavam a historia). CONTRA A TENDENCIA e
    # rotulo falso quando o sistema distingue reversao de pullback: se os
    # criterios estruturais dizem REVERSAO_EM_CURSO na direcao proposta, a
    # entrada contra e a PRIMEIRA da tendencia nova — item 2 vira verde com a
    # marca contra_tendencia na ficha (telemetria: a BabyMachine mede se paga).
    # PULLBACK ou INDEFINIDO = bloqueio original mantido.
    ciclo = None
    item2_contra = False
    try:
        if alvo and candles_op:
            ciclo = _ciclo_reversao_pullback(candles_op, scanner=scanner,
                                             alvo=alvo, atr_pts=atr_pts)
        if (ok2 is False and ciclo and "sem direcao" not in str(det2)
                and ciclo.get("veredicto") == "REVERSAO_EM_CURSO"):
            ok2 = True
            item2_contra = True
            det2 = ("reversao confirmada (" + str(ciclo.get("n_ok")) +
                    "/5 criterios) — primeira entrada da tendencia nova")
        elif (ok2 is False and ciclo and "sem direcao" not in str(det2)
                and ciclo.get("veredicto") == "INDEFINIDO"):
            det2 = (str(det2)[:60] + " · ciclo " + str(ciclo.get("n_ok")) +
                    "/5 criterios de reversao — aguardando")
    except Exception:
        pass
    pct2 = None
    try:
        if item2_contra and ciclo:
            pct2 = round(100 * float(ciclo.get("n_ok") or 0) / 5.0)
            raise Exception("pct2 do ciclo")
        _m2 = re.search(r"(\d+)\s*de\s*(\d+)", alin)
        if _m2 and int(_m2.group(2)) > 0:
            pct2 = round(100 * int(_m2.group(1)) / int(_m2.group(2)))
        elif ok2 is not None:
            pct2 = 100 if ok2 else 0
    except Exception:
        pct2 = 100 if ok2 else (0 if ok2 is False else None)
    _it2 = {"id": "tendencia", "nome": "Tendencia confirmada", "ok": ok2, "detalhe": det2,
            "pct": pct2}
    if item2_contra:
        _it2["contra_tendencia"] = True   # v7.32 — telemetria: reversao liberou o portao
    itens.append(_it2)

    # 3) VOLUME CONFIRMADO
    ok3, det3 = None, "EA sem volume nos candles (reenvie pra atualizar)"
    try:
        cs = candles_op or []
        # v7.05: candles chegam como LISTA [o,h,l,c,v] (_parse_c) ou dict {v}
        vols = []
        for c in cs:
            try:
                if isinstance(c, dict):
                    vols.append(float(c.get("v") or 0))
                elif isinstance(c, (list, tuple)) and len(c) >= 5:
                    vols.append(float(c[4] or 0))
            except Exception:
                pass
        # v7.08: a ULTIMA vela esta EM FORMACAO (volume parcial) — medir ela
        # contra a media deixava o LED vermelho injustamente. Compara a ultima
        # vela FECHADA com a media das 10 fechadas anteriores a ela.
        if len(vols) >= 7 and any(v > 0 for v in vols):
            ult = vols[-2]
            base = [v for v in vols[-12:-2] if v > 0]
            med = (sum(base) / len(base)) if base else 0
            if med > 0:
                pct3 = min(100, round(100 * ult / med))
                if ult >= med:
                    ok3, det3 = True, f"volume da ultima vela fechada {ult:.0f} >= media {med:.0f}"
                else:
                    ok3, det3 = False, f"volume fraco na ultima fechada: {ult:.0f} < media {med:.0f}"
    except Exception:
        pass
    itens.append({"id": "volume", "nome": "Volume confirmado", "ok": ok3, "detalhe": det3,
                  "pct": locals().get("pct3")})

    # 4) SUPORTE/RESISTENCIA ANALISADO
    ok4, det4 = None, "escada de niveis sem dado"
    try:
        esc = extras or {}   # v7.05: niveis_acima/niveis_abaixo vivem na RAIZ do extras
        atr = float(atr_pts or (extras or {}).get("atr_pts") or 0)
        if not alvo:
            ok4, det4 = False, "sem direcao definida — aguardando gatilho"
        else:
            contra = esc.get("niveis_acima") if alvo == "alta" else esc.get("niveis_abaixo")
            if isinstance(contra, list) and atr > 0:
                dists = []
                for n in contra:
                    try:
                        dv = n.get("dist_pts", n.get("dist", n.get("distancia")))
                        if dv is not None:
                            dists.append(abs(float(dv)))
                    except Exception:
                        pass
                if dists:
                    prox = min(dists)
                    pct4 = min(100, round(100 * prox / (0.8 * atr)))
                    if prox >= 0.8 * atr:
                        ok4, det4 = True, f"corredor livre: nivel contrario a {prox:.0f} pts (>= {0.8*atr:.0f})"
                    else:
                        ok4, det4 = False, f"nivel contrario COLADO a {prox:.0f} pts (< 0.8 ATR) — entrada do lado errado da barreira"
                else:
                    ok4, det4, pct4 = True, "sem nivel contrario mapeado no caminho", 100
            elif isinstance(contra, list):
                ok4, det4 = (True, "sem nivel contrario mapeado") if not contra else (None, "sem ATR pra medir o corredor")
    except Exception:
        pass
    itens.append({"id": "sr", "nome": "Suporte/Resistencia analisado", "ok": ok4, "detalhe": det4,
                  "pct": locals().get("pct4")})

    # 5) RELACAO RISCO:RETORNO 1:2+
    ok5, det5 = False, "sem projecao (stop/alvo) — entrada sem plano nao entra"
    try:
        if rr is not None and str(rr) != "":
            rrf = float(rr)
            if rrf >= 2.0:
                ok5, det5 = True, f"R:R {rrf:.2f} (>= 1:2)"
            else:
                ok5, det5 = False, f"R:R {rrf:.2f} abaixo de 1:2 — aguardar"
    except Exception:
        pass
    # v7.20 — rotulo do plano condicional (honestidade: R:R do plano que so
    # vale se o gatilho disparar, nao de posicao aberta)
    try:
        if rr_nota and rr is not None and str(rr) != "":
            det5 = f"{det5} · {rr_nota}"
    except Exception:
        pass
    pct5 = 0
    try:
        if rr is not None and str(rr) != "":
            pct5 = min(100, round(100 * float(rr) / 2.0))
    except Exception:
        pass
    itens.append({"id": "rr", "nome": "Risco:Retorno 1:2+", "ok": ok5, "detalhe": det5, "pct": pct5})

    # 6) ESTRATEGIA AJUSTADA
    ok6, det6 = False, "estrategia nao registrada — reenvie pelo Editor"
    try:
        en = ((bot or {}).get("config_operacional") or {}).get("estrategia_nome")
        if en:
            ok6, det6 = True, f"estrategia: {str(en)[:60]}"
    except Exception:
        pass
    itens.append({"id": "estrategia", "nome": "Estrategia ajustada", "ok": ok6, "detalhe": det6,
                  "pct": (100 if ok6 else 0)})

    # 7) LEITURA DA IA FAVORAVEL (pedido do dono: a IA tambem vota no portao —
    #    custo ZERO novo: le a ultima analise cruzada que o cockpit ja gerou)
    ok7, det7 = None, "sem analise recente da IA (nao bloqueia)"
    try:
        if ia and ia.get("sinal"):
            sn = str(ia.get("sinal")).lower()
            hs = ia.get("ha_s")
            fresca = (hs is None) or (float(hs) <= 900)
            sufixo = (f" (ha {int(float(hs))}s)" if hs is not None else "")
            if fresca and sn == "alinhado":
                ok7, det7 = True, "IA ALINHADA com a leitura do BOT" + sufixo
            elif fresca and sn == "divergente":
                ok7, det7 = False, "IA DIVERGENTE da leitura do BOT" + sufixo
            elif fresca:
                ok7, det7 = None, f"IA neutra/observacao ({sn}) — nao vota"
    except Exception:
        pass
    pct7 = None
    if ok7 is True: pct7 = 100
    elif ok7 is False: pct7 = 0
    elif "neutra" in str(det7): pct7 = 50
    itens.append({"id": "ia", "nome": "Leitura da IA favoravel", "ok": ok7, "detalhe": det7, "pct": pct7})

    avaliados = [i for i in itens if i["ok"] is not None]
    vermelhos = [i for i in itens if i["ok"] is False]
    # v7.06 — TOLERANCIA POR SENSIBILIDADE (decisao do dono: exigir 100% de tudo
    # recusa o trade limpo; o meio-termo e graduado e usa o dial que JA existe):
    #   conservador = todos os itens com dado verdes (comportamento original)
    #   moderado    = tolera 1 vermelho NAO-critico
    #   agressivo   = tolera 2 vermelhos NAO-criticos
    # CRITICOS (nunca flexibilizam): Noticia quente e R:R < 1:2 — noticia de
    # alto impacto e entrada sem plano nao se negociam em nenhum modo.
    sens = str(sensibilidade or "conservador").lower()
    tol = {"conservador": 0, "moderado": 1, "agressivo": 2}.get(sens, 0)
    criticos = [i for i in vermelhos if i["id"] in ("noticia", "rr")]
    pode = (bool(alvo) and bool(avaliados) and not criticos
            and len(vermelhos) <= tol)
    faltando = [i["nome"] for i in itens if i["ok"] is False]
    # v7.10 — PRONTIDAO DA ENTRADA (desenho do dono, com nome honesto: e o %
    # de condicoes satisfeitas em tempo real, NAO probabilidade de lucro —
    # a casa nao promete retorno). Media simples dos itens com dado.
    _pcts = [i.get("pct") for i in itens if i.get("pct") is not None]
    prontidao = round(sum(_pcts) / len(_pcts)) if _pcts else None
    # v7.11 — PROBABILIDADE DE EXECUCAO (definicao do dono): a chance de a
    # PROXIMA ENTRADA disparar, dado o cenario em construcao diante do Bot e
    # da IA — nada a ver com lucro. Formula deterministica coerente com o
    # comportamento real do portao: parte da media das condicoes, e travada
    # pelos itens CRITICOS (reprovado = portao nao abre, teto 10%) e pela
    # existencia de gatilho (sem direcao localizada = entrada nao e iminente,
    # teto 25%), e sobe pro topo quando o portao JA esta aberto (>= 90%).
    prob = prontidao if prontidao is not None else 0
    if not alvo:
        prob = min(prob, 25)
    if criticos:
        prob = min(prob, 10)
    if pode:
        prob = max(prob, 90)
    return {"itens": itens, "pode_entrar": pode, "faltando": faltando,
            "direcao": alvo or None, "sensibilidade": sens, "tolerancia": tol,
            "vermelhos": len(vermelhos), "prontidao": prontidao,
            "prob_execucao": prob,
            "ciclo": ciclo,   # v7.32 — veredicto reversao x pullback (ficha)
            "contra_tendencia": item2_contra,
            "criticos_reprovados": [i["nome"] for i in criticos]}
...
def _detectar_oportunidade(sb, bot, fatos, leitura, veredito=None, snap_dict=None):
    ...
    # v7.07 — a linha do bot (config_operacional) e a verdade entre workers e
    # restarts: sobrescreve a memoria pros campos do detector que o usuario setou.
    try:
        _co = bot.get("config_operacional") or {}
        for _k in _DETECTOR_DEFAULT_CONFIG:
            if _k in _co:
                config[_k] = _co[_k]
    except Exception:
        pass
    # v7.02 — LIGA/DESLIGA cross-worker: o pausado persistido no
    # config_operacional (Supabase) e a VERDADE — a memoria do worker que nao
    # recebeu o POST /config nao pode deixar o detector operar um bot que o
    # usuario DESLIGOU na tela (a linha `bot` ja veio carregada do banco, custo zero).
    try:
        _pz = (bot.get("config_operacional") or {}).get("pausado")
        if _pz is not None:
            config = {**config, "pausado": bool(_pz)}
    except Exception:
        pass
    ...
    if estrelas < limiar:
        return None
    # v7.03 — CHECKLIST DE ENTRADA (portao do dono): TODOS os LEDs com dado em
    # VERDE, senao a oportunidade NAO nasce — nem UI copiloto, nem automatico.
    try:
        chk = _checklist_entrada(
            direcao=direcao_info.get("direcao"),
            scanner=(leitura or {}).get("scanner"),
            extras=(leitura or {}).get("extras"),
            candles_op=(leitura or {}).get("candles_op"),
            rr=niveis.get("rr"),
            atr_pts=(fatos or {}).get("atr_pts"),
            simbolo=(leitura or {}).get("simbolo") or bot.get("simbolo") or "",
            bot=bot,
            sensibilidade=(config or {}).get("modo_sensibilidade"),
            ia=_chk_ia_ultima(bot_id))
        if not chk.get("pode_entrar"):
            try:
                print(f"[checklist] bot {bot_id} BARRADO: {', '.join(chk.get('faltando') or ['sem dado'])}")
            except Exception:
                pass
            # v7.32 — ESPERA CARIMBADA (fecha a pendencia (b) da v7.12): toda
            # recusa da PLATAFORMA vira evento auditavel com o motivo exato e o
            # veredicto do ciclo — acaba a autopsia por print. Throttle de 1
            # evento por bot+motivo por barra do TF (anti-flood, mesma filosofia
            # do debounce v7.19 do EA). Non-blocking: falha aqui nunca trava.
            try:
                import time as _t_rec
                _mot_lst = chk.get("faltando") or ["sem dado"]
                _mot_key = ",".join(sorted(_mot_lst))[:180]
                _tfop_rec = str((leitura or {}).get("tf_op") or (leitura or {}).get("timeframe") or "").lower()
                _seg_rec = _CICLO_TF_SEG.get(_tfop_rec, 300)
                _k_rec = (str(bot_id), _mot_key)
                if _t_rec.time() - float(_REC_PLAT_STATE.get(_k_rec) or 0) >= _seg_rec:
                    _REC_PLAT_STATE[_k_rec] = _t_rec.time()
                    if len(_REC_PLAT_STATE) > 4000:
                        _REC_PLAT_STATE.clear()
                        _REC_PLAT_STATE[_k_rec] = _t_rec.time()
                    _ciclo_rec = (chk.get("ciclo") or {})
                    sb.table("agente_eventos").insert({
                        "user_id": bot.get("user_id"),
                        "tipo": "bot_recusada_plataforma", "regra": None,
                        "detalhe_json": {
                            "bot_nome": bot.get("nome") or "",
                            "bot_token": bot.get("bot_token") or "",
                            "simbolo": (leitura or {}).get("simbolo") or bot.get("simbolo") or "",
                            "direcao": direcao_info.get("direcao"),
                            "motivo": _mot_lst,
                            "ciclo_veredicto": _ciclo_rec.get("veredicto"),
                            "ciclo_n_ok": _ciclo_rec.get("n_ok"),
                            "prontidao": chk.get("prontidao"),
                            "estrelas": estrelas,
                        },
                    }).execute()
            except Exception:
        ...
        try: print(f"[checklist] erro (nao barra por falha propria): {_e_chk}")
        except Exception: pass
        chk = None
    ...
    oport = {
        ...
        "status":         "detectada",
        "checklist":      chk,   # v7.03 — o portao que a entrada passou (auditoria)
    }
...
# ════════════════════════════════════════════════════════════════════════════
#  v7.21 — PROTETOR DE POSICAO
#  O complemento do detector: o detector so nasce ENTRADA e para quando ha
#  posicao ("ja tem posicao" no _check_pode_operar); o protetor SO age quando
#  ha posicao. Deterministico, zero IA, usa o canal de comando v6.73 (o EA
#  ja executa close_all e mover_sl sem reenvio). Regras e freios: ver a nota
#  da v7.21 no API_VERSAO. Toggle: config_operacional.protecao (default ON).
# ════════════════════════════════════════════════════════════════════════════

_PROT_STATE = {}   # bot_id -> {"barra_acao": bucket_m5 da ultima acao}


def _prot_ema(vals, p=20):
    """EMA autocontida (seed = SMA do warmup). Retorna lista alinhada a vals
    ou None se dado insuficiente (< p//2 amostras = sem estimativa honesta)."""
    try:
        ...
        k = 2.0 / (p + 1.0)
        seed_n = min(p, len(vs))
        ema = sum(vs[:seed_n]) / seed_n
        out = []
        for i, v in enumerate(vs):
            if i == 0:
                ema = v if seed_n == 1 else ema
            ema = v * k + ema * (1 - k)
            out.append(ema)
        return out
    except Exception:
...
def _prot_atr(cs, n=14):
    """ATR simples sobre candles fechados (dicts o/h/l/c). None sem dado."""
    try:
        fechados = (cs or [])[:-1]
        if len(fechados) < 5:
            return None
        ...
        for i in range(1, len(fechados)):
            h, l = float(fechados[i]["h"]), float(fechados[i]["l"])
            pc = float(fechados[i - 1]["c"])
            trs.append(max(h - l, abs(h - pc), abs(l - pc)))
        janela = trs[-n:] if len(trs) >= n else trs
        atr = sum(janela) / len(janela)
        return atr if atr > 0 else None
    except Exception:
        return None


def _prot_pin_contra(c, lado_pos):
    """Pin bar CONTRA a posicao na barra dada (dict o/h/l/c).
    long -> estrela cadente/pin de baixa (pavio superior dominante);
    short -> martelo/pin de alta (pavio inferior dominante)."""
    try:
        o, h, l, c_ = float(c["o"]), float(c["h"]), float(c["l"]), float(c["c"])
        rng = h - l
        if rng <= 0:
            return False
        corpo = abs(c_ - o)
        if lado_pos == "long":
            pavio = h - max(o, c_)
            return pavio >= 2 * corpo and pavio >= 0.5 * rng
        else:
            pavio = min(o, c_) - l
            return pavio >= 2 * corpo and pavio >= 0.5 * rng
    except Exception:
...
def _prot_fib_local(cs, preco, atr):
    """LOCALIZACAO (doutrina R9c): preco a <=0.2xATR de um nivel de Fibonacci
    38.2/50/61.8 do swing recente dos candles fechados. Nome do nivel ou None."""
    try:
        f = (cs or [])[:-1]
        if len(f) < 10 or preco is None:
            return None
        hi = max(float(c["h"]) for c in f)
        lo = min(float(c["l"]) for c in f)
        rng = hi - lo
        if rng <= 0:
            return None
        tol = 0.2 * (atr if atr else rng * 0.1)
        for nome, k in (("38.2", 0.382), ("50", 0.5), ("61.8", 0.618)):
            for nivel in (hi - rng * k, lo + rng * k):
                if abs(float(preco) - nivel) <= tol:
                    return nome
        return None
    except Exception:
        return None
...
def _prot_vol_alterado(f1):
    """VOLUME (doutrina R9b): media das 6 ultimas M1 fechadas >= 1.4x a media
    das anteriores. True/False, ou None sem dado de volume utilizavel."""
    try:
        vols = [float(c.get("v") or 0) for c in (f1 or [])]
        if len(vols) < 9:
            return None
        jan, base = vols[-6:], vols[:-6]
        if sum(base) <= 0 or sum(jan) <= 0:
            return None
        return (sum(jan) / len(jan)) >= 1.4 * (sum(base) / len(base))
    except Exception:
        return None


def _prot_reversao_identificada(det, lado_pos, preco, cs5, cs_op):
    """R9 da doutrina (27/jul): janela de 6 barras de M1 confirmando pro M5,
    SEM 15m; identificar nao e esperar 6 minutos — retorna True assim que os
    TRES confirmadores batem contra a posicao. (True/False, quadro legivel)."""
    try:
        cs1 = _parse_candles_para_lista(det.get("c1m"))
        f1 = (cs1 or [])[:-1]
        if len(f1) < 7:
            return False, ""
        jan = f1[-6:]
        # (a) ESTRUTURA: pin no M5 fechado OU cluster de M1 contra
        estrutura, det_est = False, ""
        try:
            if cs5 and len(cs5) >= 2 and _prot_pin_contra(cs5[-2], lado_pos):
                estrutura, det_est = True, "pin M5"
        except Exception:
            pass
        if not estrutura:
            contra = 0
            for i in range(1, len(jan)):
                c0, c1 = float(jan[i - 1]["c"]), float(jan[i]["c"])
                if (lado_pos == "long" and c1 < c0) or (lado_pos == "short" and c1 > c0):
                    contra += 1
            if contra >= 4:
                estrutura, det_est = True, f"cluster {contra}/5 M1 contra"
        if not estrutura:
            return False, ""
        # (b) VOLUME alterado na janela
        if _prot_vol_alterado(f1) is not True:
            return False, ""
        # (c) LOCALIZACAO em Fibonacci do swing do M5
        atr5 = _prot_atr(cs5) or _prot_atr(cs_op)
        loc = _prot_fib_local(cs5, preco, atr5)
        if loc is None:
            return False, ""
        return True, f"{det_est} + volume alterado + fib {loc}"
    except Exception:
        return False, ""


def _prot_sl_ja_enviado(sb, bot_id):
    """Anti-afrouxo cross-restart: mapa ticket -> ultimo SL que o protetor (ou
    qualquer origem) ja mandou via mover_sl pra este bot. 1 query por ACAO
    (nao por snapshot). Best-effort: erro = mapa vazio (os guards de
    breakeven-ou-melhor seguram o pior caso)."""
    mapa = {}
    try:
        r = (sb.table("mt5_comandos").select("params,tipo,criado_em")
             .eq("bot_id", bot_id).eq("tipo", "mover_sl")
             .order("criado_em", desc=True).limit(30).execute())
        for ln in (r.data or []):
            p = ln.get("params") or {}
            t = p.get("ticket")
            if t is None or t in mapa:
                continue   # desc: o primeiro visto e o mais recente
            try:
                mapa[t] = float(p.get("sl"))
            except Exception:
                pass
    except Exception:
        pass
...
def _prot_cmd(sb, bot, tipo, params, motivo):
    """Cria o comando no canal v6.73 (insert direto, padrao do detector
    automatico). Respeita bot online e o teto diario. Retorna True se criou."""
    try:
        if not _mt5_bot_online(bot):
            return False
        if _mt5_contar_comandos_dia(sb, bot.get("id")) >= MAX_COMANDOS_DIA_POR_BOT:
            print(f"[protetor] {bot.get('bot_nome')}: teto diario de comandos atingido — {motivo} NAO enviado")
            return False
        pr = dict(params or {})
        pr["motivo"] = str(motivo)[:140]
        linha = {"bot_token": bot.get("bot_token"), "user_id": bot.get("user_id"),
                 "bot_id": bot.get("id"), "tipo": tipo, "params": pr,
                 "status": "pendente", "origem": "auto_detector"}
        sb.table("mt5_comandos").insert(linha).execute()
        print(f"[protetor] {bot.get('bot_nome')}: {tipo} -> {motivo}")
        return True
    except Exception as _e:
        try:
            import sys as _sys
            print(f"[protetor cmd] {_e}", file=_sys.stderr)
        except Exception:
            pass
        return False


def _protetor_posicao(sb, bot, snap, det):
    """v7.21 — roda a cada snapshot, mas SO faz algo com posicao aberta e
    protecao ligada. Prioridade: R2 saida estrutural M5 (close_all) ->
    R3 sinal de reversao (aperta SL) -> R1 breakeven (SL pra entrada)."""
    import time as _t
    try:
        pos = int(snap.posicoes_abertas or 0)
    except Exception:
        pos = 0
    try:
        lucro = float(snap.lucro_flutuante) if snap.lucro_flutuante is not None else None
    except Exception:
        lucro = None
    # posicoes_abertas pode chegar 0 com posicao viva (parser, caso v6.91) —
    # flutuante nao-zero tambem conta como "ha posicao"
    if pos <= 0 and not (lucro is not None and abs(lucro) > 1e-9):
        return
    cfg = (bot or {}).get("config_operacional") or {}
    # v7.33 — o gate de protecao DESCEU: o GUARDIAO DE ADERENCIA roda mesmo com
    # protecao=false (protecao desliga a GESTAO — catraca/breakeven, que no BTC
    # custa spread —, nao a legitimidade da ENTRADA). Toggle proprio: guardiao.
    guardiao_on = cfg.get("guardiao") is not False
    protecao_on = cfg.get("protecao") is not False
    if not guardiao_on and not protecao_on:
        return
    bot_id = bot.get("id")
    bucket = int(_t.time() // 300)   # barra de M5
    st = _PROT_STATE.setdefault(bot_id, {})
    if st.get("barra_acao") == bucket:
        return   # 1 acao por barra de M5 — freio inegociavel

    # ── dados ──
    cs5 = _parse_candles_para_lista(det.get("c5m"))
    tf_op = str(det.get("tfop") or "").lower()
    chave_op = {"1m": "c1m", "5m": "c5m", "15m": "c15m", "30m": "c30m",
                "1h": "c60m", "60m": "c60m", "4h": "c4h"}.get(tf_op, "c15m")
    cs_op = _parse_candles_para_lista(det.get(chave_op)) or cs5
    preco = None
    try:
        preco = float(det.get("preco"))
    except Exception:
        try:
            preco = float(cs5[-1]["c"]) if cs5 else None
        except Exception:
            preco = None

    # ── posicoes registradas (entrada + ticket) da BabyMachine ──
    abertas = []
    try:
        r = (sb.table("babymachine_operacoes").select("id,decisao,fonte,created_at")
             .eq("bot_id", bot_id).eq("status", "aberta").limit(6).execute())
        for ln in (r.data or []):
            d = ln.get("decisao") or {}
            lado = str(d.get("direcao") or d.get("lado") or "").lower()
            lado = "long" if lado in ("long", "buy", "compra") else ("short" if lado in ("short", "sell", "venda") else "")
            ent = None
            try:
                ent = float(d.get("entrada"))
            except Exception:
                pass
            # v7.33 — idade da linha (pro Guardiao julgar so entradas FRESCAS)
            idade_s = None
            try:
                from datetime import datetime, timezone
                _ca = str(ln.get("created_at") or "")
                if _ca:
                    _dt = datetime.fromisoformat(_ca.replace("Z", "+00:00"))
                    idade_s = (datetime.now(timezone.utc) - _dt).total_seconds()
            except Exception:
                idade_s = None
            _slr = None
            try:
                ...
                _slr = None
            abertas.append({"lado": lado, "entrada": ent, "ticket": d.get("ticket"),
                            "sl": _slr, "id_bm": ln.get("id"),
                            "fonte": str(ln.get("fonte") or ""), "idade_s": idade_s})
    except Exception:
        pass
    lados = {a["lado"] for a in abertas if a["lado"]}
    lado_pos = lados.pop() if len(lados) == 1 else None

    # v7.22 — fechamento da ultima M5 fechada + lucro do SIMBOLO em pontos
    # (o flutuante da conta mistura outros simbolos e abriu o guard indevidamente
    # nas 05:31:49 de 27/jul)
    fech5 = None
    try:
        if cs5 and len(cs5) >= 2:
            fech5 = float(cs5[-2]["c"])
    except Exception:
        fech5 = None
    ref = fech5 if fech5 is not None else preco
    lucro_sym = None
    try:
        _pts = [((ref - a["entrada"]) if a["lado"] == "long" else (a["entrada"] - ref))
                for a in abertas if a["entrada"] is not None and a["lado"] and ref is not None]
        if _pts:
            lucro_sym = sum(_pts)
    except Exception:
        lucro_sym = None
    if lucro_sym is None:
        lucro_sym = lucro   # fallback: sem posicoes registradas, usa o flutuante

    # ════ GUARDIAO DE ADERENCIA (v7.33 — ordem do dono 05/ago, apos o dia dos
    # tres episodios: 11 entradas nativas em lateral no BTC que o proprio cerebro
    # nomeou, giro cego de 16s comprando dentro de cascata de baixa confirmada,
    # e saida atrasada devolvendo 20 pts que o M1/M5 tinham anunciado. A frase:
    # ELIMINE DE UMA VEZ A INTERFERENCIA DO GATILHO NATIVO. O backend nao impede
    # a ordem nativa de NASCER — mas a mata em segundos: posicao FRESCA (<10min)
    # aberta pelo EA compilado (fonte estrategia_compilada) e julgada contra a
    # leitura ATUAL do cerebro; entrada ilegitima = close_all MESMO NEGATIVA
    # (perda de segundos e menor que stop de 60 pts depois). Criterios:
    #   (a) LATERAL — range detector diz "dentro" = entrada em lateralizacao;
    #   (b) CASCATA CONTRA — cascata de reversao confirmando contra o lado;
    #   (c) CONTRA SEM REVERSAO — alinhamento contra o lado E o classificador
    #       do Ciclo NAO diz REVERSAO_EM_CURSO na direcao da posicao (= era
    #       pullback/indefinido; comprar faca caindo sem confirmacao).
    # Entradas da PLATAFORMA (fonte detector_*) passaram no portao — imunes.
    # Carimbo em agente_eventos tipo bot_guardiao_aderencia. Toggle
    # config_operacional.guardiao (default LIGADO). ════
    try:
        if guardiao_on and abertas and preco is not None:
            _frescas = [a for a in abertas
                        if (a.get("idade_s") is not None and a["idade_s"] <= 600
                            and (not a.get("fonte") or "compilada" in str(a.get("fonte"))))]
            _lados_f = {a["lado"] for a in _frescas if a["lado"]}
            _lado_f = _lados_f.pop() if len(_lados_f) == 1 else None
            if _lado_f:
                _dir_pos = "alta" if _lado_f == "long" else "baixa"
                _dir_contra = "baixa" if _lado_f == "long" else "alta"
                _motivo_g = None
                _pac_g = None
                try:
                    _pac_g = _scanner_pacote(det)
                except Exception:
                    _pac_g = None
                # (a) LATERAL
                try:
                    _atr_g = _prot_atr(cs_op) or _prot_atr(cs5)
                    _rng = _detectar_range_lateral(cs_op, _atr_g, preco)
                    if _rng and str(_rng.get("estado") or "") == "dentro":
                        _motivo_g = ("entrada nativa DENTRO de lateralizacao de "
                                     + str(_rng.get("altura_pts") or "?") + " pts — dentro do range nao ha trade")
                except Exception:
                    pass
                # (b) CASCATA CONTRA
                if not _motivo_g:
                    try:
                        _casc = (_pac_g or {}).get("cascata_reversao") or {}
                        if (str(_casc.get("direcao") or "").lower() == _dir_contra
                                and "confirm" in str(_casc.get("estado") or "").lower()):
                            _motivo_g = ("entrada nativa CONTRA cascata de reversao "
                                         + str(_casc.get("estado")) + " (detectada no "
                                         + str(_casc.get("detectada_no")) + ")")
                    except Exception:
                        pass
                # (c) CONTRA A TENDENCIA SEM REVERSAO CONFIRMADA
                # v7.37b — ISENCAO DE FASE (1o mandato da fase, DEFENSIVO — achado
                # pelo smoke: em RESPIRO de tendencia, o alinhamento de janela
                # curta le o pullback como direcao contraria e o criterio (c)
                # condenava entrada LEGITIMA de retomada; a fase le a cena certa):
                # RESPIRO/RETOMADA ou IMPULSO na direcao da posicao = pullback
                # de continuacao, entrada legitima — (c) nao condena. Mandato
                # defensivo apenas: impede fechamento indevido, nao veta entrada.
                _fz_g = str(((_pac_g or {}).get("fase") or {}).get("fase") or "")
                _fase_ok = (_fz_g in ("RESPIRO", "RETOMADA")
                            or _fz_g == ("IMPULSO_ALTA" if _dir_pos == "alta" else "IMPULSO_BAIXA"))
                if not _motivo_g and not _fase_ok:
                    try:
                        _ali = (_pac_g or {}).get("alinhamento") or {}
                        if str(_ali.get("lado") or "").lower() == _dir_contra:
                            _cic = _ciclo_reversao_pullback(cs_op, scanner=_pac_g, alvo=_dir_pos)
                            _ver = (_cic or {}).get("veredicto")
                            if _ver != "REVERSAO_EM_CURSO":
                                _motivo_g = ("entrada nativa contra o alinhamento ("
                                             + str(_ali.get("resumo") or _dir_contra)[:60]
                                             + ") sem reversao confirmada — ciclo "
                                             + str(_ver or "sem dado"))
                    except Exception:
                        pass
                # (d) v7.39 — VETO DE FASE COM MANDATO (a colheita do Professor,
                # 05/ago: XAU M30 Cruzamento re-simulado nos 6 meses de CSV real
                # com esta politica: PF 1.44 -> 1.52, mesmo liquido com 63 trades
                # a menos = APROVADO; BTC M30 0.95 -> 0.96 = reprovado, nao liga.
                # Por isso o mandato e POR CELULA: so age quando o dono gravou
                # veto_fase=true no config_operacional do bot — hoje, apenas a
                # celula aprovada. A politica identica a testada:
                #   - fase LATERAL -> entrada nativa fora de historia = condena;
                #   - entrada CONTRA IMPULSO oposto -> condena (a fase REVERSAO
                #     na direcao da entrada e outra fase, entao a excecao do
                #     Professor esta embutida na propria gramatica).
                if not _motivo_g and bool(cfg.get("veto_fase")):
                    if _fz_g == "LATERAL":
                        _motivo_g = ("VETO DE FASE (mandato medido pelo Professor): "
                                     "entrada nativa em fase LATERAL — fora de historia direcional")
                    elif _fz_g == ("IMPULSO_BAIXA" if _lado_f == "long" else "IMPULSO_ALTA"):
                        _motivo_g = ("VETO DE FASE (mandato medido pelo Professor): "
                                     "entrada nativa CONTRA " + _fz_g + " sem reversao confirmada")
                if _motivo_g:
                    if _prot_cmd(sb, bot, "close_all", {},
                                 "GUARDIAO DE ADERENCIA: " + _motivo_g):
                        st["barra_acao"] = bucket
                        try:
                            sb.table("agente_eventos").insert({
                                "user_id": bot.get("user_id"),
                                "tipo": "bot_guardiao_aderencia", "regra": None,
                                "detalhe_json": {"bot_nome": bot.get("nome") or "",
                                                 "lado": _lado_f, "motivo": _motivo_g,
                                                 "preco": preco},
                            }).execute()
                        except Exception:
                            pass
    ...
    # v7.33 — daqui pra baixo e GESTAO: respeita o toggle protecao original
    if not protecao_on:
        return

    # ── R0b (v7.35 — cobranca do dono 05/ago, print do 54 nascendo com stop
    # geometrico ~60 pts abaixo, ate embaixo do topo rompido: o EA calcula o
    # SL inicial por multiplo de ATR, cego a estrutura — e o ATR inflado num
    # dia de 4 por cento fabrica risco maximo. O CEREBRO conhece a estrutura E
    # conhece o SL registrado da entrada (v6.89 grava decisao.sl): posicao
    # FRESCA (<=15 min) legitima ganha STOP ESTRUTURAL — o fundo das ultimas
    # 8 fechadas (long) / topo (short) com folga de 0.3xATR — via mover_sl,
    # SO SE APERTA o stop registrado (catraca: nunca afrouxa, nunca cola a
    # menos de 0.5xATR do preco). Uma vez por posicao (_PROT_STATE). ──
    try:
        if abertas and cs_op and len(cs_op) >= 10 and preco is not None:
            _atr0 = _prot_atr(cs_op) or _prot_atr(cs5) or 0
            if _atr0 > 0:
                _f0 = cs_op[:-1][-8:]
                movidos0 = 0
                for a in abertas:
                    if movidos0 >= 3:
                        break
                    if not a.get("lado") or a.get("idade_s") is None or a["idade_s"] > 900:
                        continue
                    if not a.get("ticket"):
                        continue
                    _ja = st.setdefault("sl_estrutural", set())
                    if a.get("id_bm") in _ja:
                        continue
                    if a["lado"] == "long":
                        nivel = min(float(b["l"]) for b in _f0) - 0.3 * _atr0
                        cola = nivel > preco - 0.5 * _atr0
                    else:
                        nivel = max(float(b["h"]) for b in _f0) + 0.3 * _atr0
                        cola = nivel < preco + 0.5 * _atr0
                    # v7.36 — APERTA COM OU SEM SL REGISTRADO: (a) sl gravado ->
                    # compara direto (nunca afrouxa); (b) sl AUSENTE (gap do OTT:
                    # o evento aberto nao carrega SL, o caso real dos 54/55) ->
                    # usa a ENTRADA gravada como referencia e SO move se o nivel
                    # estrutural for mais justo que o PISO DE FABRICA do EA
                    # (1.5xATR do R6): distancia entrada->nivel < 1.2xATR garante
                    # aperto matematico sem conhecer o stop. Sem sl E sem entrada
                    # = pula com honestidade.
                    aperta = False
                    _sl_ref = a.get("sl")
                    if _sl_ref is None and a.get("entrada") is not None:
                        # v7.37 — sl ausente no registro (gap do OTT): ESTIMA do
                        # stop do ENVIO persistido (sl_envio_pts, verdade do
                        # /mt5/enviar): sl_estimado = entrada -/+ pontos. Vale
                        # pro XAU largo (60 -> aperta) E pro BTC apertado
                        # (3000? nivel mais fundo que o stop -> nao mexe).
                        try:
                            _pts_env = float(cfg.get("sl_envio_pts"))
                            _sl_ref = (float(a["entrada"]) - _pts_env) if a["lado"] == "long" else (float(a["entrada"]) + _pts_env)
                        except Exception:
                            _sl_ref = None
                    if _sl_ref is not None:
                        aperta = (nivel > float(_sl_ref) + 1e-9) if a["lado"] == "long" else (nivel < float(_sl_ref) - 1e-9)
                    elif a.get("entrada") is not None:
                        _dist = abs(float(a["entrada"]) - nivel)
                        aperta = _dist < 1.2 * _atr0
                    if aperta and not cola:
                        if _prot_cmd(sb, bot, "mover_sl",
                                     {"ticket": a["ticket"], "sl": round(nivel, 2)},
                                     f"stop ESTRUTURAL de entrada: {round(nivel,2)} (fundo/topo das 8 fechadas +-0.3xATR) aperta o geometrico {a['sl']}"):
                            _ja.add(a.get("id_bm"))
                            movidos0 += 1
                if movidos0:
                    st["barra_acao"] = bucket
    ...
    # ── R2c (v7.33 — exigencia do dono: reversao confirmada no 5m = CORTA,
    # sem esperar 15/30min; e barra gigante que ATRAVESSA o canal e fecha do
    # lado oposto tambem corta — o R2 original so pegava a transicao suave
    # de-fora-pra-dentro). Com lucro: cascata confirmando contra OU M5 fechado
    # ALEM do canal oposto -> close_all. Negativa: exige as DUAS (cascata
    # confirmada E fechamento alem do canal) — reversao identificada corta
    # mesmo no vermelho, mas ruido isolado nao. ──
    try:
        if lado_pos and cs5 and len(cs5) >= 8:
            _f5 = cs5[:-1]
            _eh5 = _prot_ema([c["h"] for c in _f5], 20)
            _el5 = _prot_ema([c["l"] for c in _f5], 20)
            if _eh5 and _el5:
                _cu = _f5[-1]
                _alem = ((lado_pos == "long" and float(_cu["c"]) < _el5[-1])
                         or (lado_pos == "short" and float(_cu["c"]) > _eh5[-1]))
                _dir_c = "baixa" if lado_pos == "long" else "alta"
                _casc2 = {}
                try:
                    _casc2 = ((_scanner_pacote(det) or {}).get("cascata_reversao") or {})
                except Exception:
                    _casc2 = {}
                _casc_contra = (str(_casc2.get("direcao") or "").lower() == _dir_c
                                and "confirm" in str(_casc2.get("estado") or "").lower())
                _casc_forte = (str(_casc2.get("direcao") or "").lower() == _dir_c
                               and "confirmada" in str(_casc2.get("estado") or "").lower())
                _corta = False
                if lucro_sym is not None and lucro_sym > 0 and (_alem or _casc_contra):
                    _corta = True
                elif _alem and _casc_forte:
                    _corta = True   # negativa: dupla condicao
                if _corta:
                    _mot_c = ("saida pela CASCATA (R2c): M5 "
                              + ("fechou ALEM do canal contra" if _alem else "")
                              + (" + " if (_alem and _casc_contra) else "")
                              + (("cascata " + str(_casc2.get("estado"))) if _casc_contra else ""))
                    if lucro_sym is not None:
                        _mot_c += f" — lucro simbolo {lucro_sym:+.1f} pts"
                    if _prot_cmd(sb, bot, "close_all", {}, _mot_c):
                        st["barra_acao"] = bucket
                    return
    except Exception:
        pass

    # ── R9 (doutrina 27/jul): REVERSAO IDENTIFICADA -> FECHA, mesmo negativa.
    # Prioridade maxima: se o mercado provou a reversao, nao ha o que proteger.
    try:
        if lado_pos and abertas:
            _rev, _quadro = _prot_reversao_identificada(det, lado_pos, preco, cs5, cs_op)
            if _rev:
                if _prot_cmd(sb, bot, "close_all", {},
                             f"reversao identificada contra a posicao ({_quadro}) — aborto da doutrina"):
                    st["barra_acao"] = bucket
                return
    except Exception:
        pass

    # ── R2: SAIDA ESTRUTURAL M5 (fechamento de VOLTA pro canal contra) ──
    # Exige transicao real: barra anterior FORA do canal, ultima fechada DENTRO.
    # So com flutuante positivo — o protetor realiza lucro, nao corta perda
    # (a perda e territorio do stop do EA).
    try:
        if lado_pos and lucro_sym is not None and lucro_sym > 0 and cs5 and len(cs5) >= 8:
            fechados5 = cs5[:-1]
            eh = _prot_ema([c["h"] for c in fechados5], 20)
            el = _prot_ema([c["l"] for c in fechados5], 20)
            if eh and el and len(fechados5) >= 3:
                c_ult, c_ant = fechados5[-1], fechados5[-2]
                if lado_pos == "long" and float(c_ant["c"]) > eh[-2] and float(c_ult["c"]) <= eh[-1]:
                    if _prot_cmd(sb, bot, "close_all", {},
                                 f"saida estrutural M5: fechamento {float(c_ult['c']):g} de volta pro canal contra a compra (lucro simbolo {lucro_sym:+.1f} pts)"):
                        st["barra_acao"] = bucket
                    return
                if lado_pos == "short" and float(c_ant["c"]) < el[-2] and float(c_ult["c"]) >= el[-1]:
                    if _prot_cmd(sb, bot, "close_all", {},
                                 f"saida estrutural M5: fechamento {float(c_ult['c']):g} de volta pro canal contra a venda (lucro simbolo {lucro_sym:+.1f} pts)"):
                        st["barra_acao"] = bucket
                    return
    except Exception:
        pass

    if preco is None or not abertas:
        return
    atr = _prot_atr(cs_op) or _prot_atr(cs5)

    # ── R3: SINAL DE REVERSAO no TF de operacao -> aperta SL pro extremo
    #        da barra do sinal, SO alem da entrada (breakeven-ou-melhor
    #        garantido sem conhecer o SL atual) ──
    enviados = None
    acao = False
    try:
        if cs_op and len(cs_op) >= 3:
            sinal = cs_op[-2]   # ultima barra FECHADA
            movidos = 0
            for a in abertas:
                if movidos >= 3:
                    break
                if not a["lado"] or a["entrada"] is None or not a.get("ticket"):
                    continue
                if not _prot_pin_contra(sinal, a["lado"]):
                    continue
                if a["lado"] == "long":
                    alvo_sl = float(sinal["l"])
                    if alvo_sl <= a["entrada"] or preco <= alvo_sl:
                        continue
                else:
                    alvo_sl = float(sinal["h"])
                    if alvo_sl >= a["entrada"] or preco >= alvo_sl:
                        continue
                if enviados is None:
                    enviados = _prot_sl_ja_enviado(sb, bot_id)
                ja = enviados.get(a["ticket"])
                if ja is not None and ((a["lado"] == "long" and alvo_sl <= ja) or
                                       (a["lado"] == "short" and alvo_sl >= ja)):
                    continue   # nunca afrouxa o que ja foi apertado
                if _prot_cmd(sb, bot, "mover_sl", {"ticket": a["ticket"], "sl": round(alvo_sl, 5)},
                             f"reversao no {tf_op or 'TF op'}: pin bar contra — SL pro extremo da barra do sinal"):
                    enviados[a["ticket"]] = alvo_sl
                    movidos += 1
                    acao = True
        if acao:
            st["barra_acao"] = bucket
    ...
    # ── R1: BREAKEVEN — avanco >= 0.6 x ATR a favor -> SL na entrada ──
    try:
        if atr is None:
            return
        movidos = 0
        for a in abertas:
            ...
            if not a["lado"] or a["entrada"] is None or not a.get("ticket"):
                continue
            _base_be = fech5 if fech5 is not None else preco
            avanco = (_base_be - a["entrada"]) if a["lado"] == "long" else (a["entrada"] - _base_be)
            if avanco < 0.6 * atr:
                continue
            alvo_sl = a["entrada"]
            if enviados is None:
            ...
            if ja is not None and ((a["lado"] == "long" and alvo_sl <= ja) or
                                   (a["lado"] == "short" and alvo_sl >= ja)):
                continue   # SL ja esta na entrada ou melhor — nunca afrouxa
            if _prot_cmd(sb, bot, "mover_sl", {"ticket": a["ticket"], "sl": round(alvo_sl, 5)},
                         f"breakeven no fechamento M5: avanco {avanco:.1f} pts >= 0.6 x ATR ({0.6*atr:.1f}) — SL pra entrada"):
                enviados[a["ticket"]] = alvo_sl
                movidos += 1
                acao = True
        if acao:
    ...
    # ── R5 (doutrina 27/jul, degrau 2 da escada): PERSEGUICAO ESTRUTURAL —
    # depois do breakeven, o SL cola no canal EMA20 do M5 (low pra compra,
    # high pra venda) e sobe a cada vela fechada a favor. Catraca mantida:
    # so anda na direcao do lucro e nunca cruza o preco.
    try:
        if st.get("barra_acao") == bucket:
            return
        if not cs5 or len(cs5) < 8 or preco is None:
            return
        _f5 = cs5[:-1]
        _eh = _prot_ema([c["h"] for c in _f5], 20)
        _el = _prot_ema([c["l"] for c in _f5], 20)
        if not _eh or not _el:
            return
        movidos = 0
        acao2 = False
        for a in abertas:
            ...
            ja = enviados.get(a["ticket"])
            if ja is None:
                continue   # degrau 2 so DEPOIS do degrau 1 (breakeven feito)
            if a["lado"] == "long":
                if ja < a["entrada"]:
                    continue
                alvo_sl = float(_el[-1])
                if alvo_sl <= ja or alvo_sl >= preco:
                    continue
            else:
                if ja > a["entrada"]:
                    continue
                ...
                if alvo_sl >= ja or alvo_sl <= preco:
                    continue
            if _prot_cmd(sb, bot, "mover_sl", {"ticket": a["ticket"], "sl": round(alvo_sl, 5)},
                         "escada degrau 2: SL perseguindo o canal EMA do M5"):
                enviados[a["ticket"]] = alvo_sl
                movidos += 1
                acao2 = True
        if acao2:
...
def conector_snapshot(snap: ConectorSnapshot):
    ...
    # v7.21 — PROTETOR DE POSICAO: o complemento do detector. So faz algo com
    # posicao aberta e protecao ligada; nunca quebra o snapshot.
    try:
        _protetor_posicao(sb, bot, snap, det)
    except Exception as _e:
        try:
            import sys as _sys
            print(f"[protetor] {_e}", file=_sys.stderr)
        except Exception:
            pass
...
def monitor_bot_config_get(bot_id: int):
    ...
    v7.07: mescla a config PERSISTIDA da linha do bot — pos-restart o cockpit
    pinta o que o usuario realmente escolheu, nao o default da memoria."""
    cfg = _bot_config_get(bot_id)
    try:
        _sbg = _sb_admin()
        if _sbg is not None:
            _rowg = (_sbg.table("conector_bots").select("config_operacional")
                     .eq("id", bot_id).limit(1).execute())
            if _rowg.data:
                _cog = _rowg.data[0].get("config_operacional") or {}
                for _k in _DETECTOR_DEFAULT_CONFIG:
                    if _k in _cog:
                        cfg[_k] = _cog[_k]
    except Exception:
...
def monitor_bot_config_set(bot_id: int, inp: BotConfigIn):
    ...
    if "lote_padrao" in novo and not (0.01 <= float(novo["lote_padrao"]) <= 100):
        raise HTTPException(status_code=400, detail="lote_padrao deve ser 0.01-100")
    cfg = _bot_config_set(bot_id, novo)
    # v7.02 — LIGA/DESLIGA persistente: o pausado tambem vai pro
    # config_operacional (JSONB v6.75, merge — mesmo padrao do estrategia_nome
    # v6.90). Sobrevive a restart do Railway e vale pra TODOS os workers: o
    # /conector/bots devolve a coluna pro front pintar o botao, e o gate do
    # detector le a coluna da linha do bot (verdade unica). Best-effort.
    # v7.07 — PERSISTENCIA TOTAL (pedido do dono: os parametros de atuacao sao
    # do USUARIO): TODOS os campos operacionais vao pro config_operacional
    # (JSONB v6.75, merge) — modo, sensibilidade, lote, breakers, pausado.
    # Sobrevive a restart e vale pra todos os workers (generaliza o v7.02).
    if novo:
        try:
            _sbp = _sb_admin()
            if _sbp is not None:
                _row = (_sbp.table("conector_bots").select("id,config_operacional")
                        .eq("id", bot_id).limit(1).execute())
                if _row.data:
                    _cfgp = dict(_row.data[0].get("config_operacional") or {})
                    for _k, _v in novo.items():
                        _cfgp[_k] = _v
                    _sbp.table("conector_bots").update(
                        {"config_operacional": _cfgp}).eq("id", bot_id).execute()
        except Exception:
            pass
...
def monitor_oportunidade_executar(oport_id: str, inp: OportExecutarIn):
    ...
    # v7.07 — mesma verdade unica do detector: config persistida da linha
    # do bot sobrescreve a memoria (o lote do Executar e o do usuario).
    try:
        _co2 = bot.get("config_operacional") or {}
        for _k2 in _DETECTOR_DEFAULT_CONFIG:
            if _k2 in _co2:
                config[_k2] = _co2[_k2]
    except Exception:
        pass
...
def conector_evento(ev: ConectorEvento):
    ...
    # v6.99 — FREIO DE CUSTO: a "análise imediata sem cooldown" (v6.67) foi
    # desenhada pra eventos RAROS; um bot metralhadora (abre/fecha a cada 1min)
    # disparava uma chamada de IA POR EVENTO a noite toda = ~US$12/dia na conta
    # da Anthropic. Agora: 1ª análise de evento sai NA HORA (imediatismo
    # preservado), as seguintes respeitam 90s por bot + TETO de 150 análises de
    # evento por bot por dia (rajada infinita nunca mais fura o bolso). O
    # ciclo de silêncio (90s, cockpit aberto) segue como está.
    try:
        ...
        if _gatilho.startswith("evento_") and bot.get("id"):
            _evt_ult = globals().setdefault("_ANL_EVT_ULT", {})
            _evt_dia = globals().setdefault("_ANL_EVT_DIA", {})
            _bid = bot.get("id")
            _agora_ev = _time_anl.time()
            _hoje_ev = _dt.now(_tz.utc).date().isoformat()
            _cont = _evt_dia.get(_bid) or {}
            if _cont.get("dia") != _hoje_ev:
                _cont = {"dia": _hoje_ev, "n": 0}
            if (_agora_ev - float(_evt_ult.get(_bid) or 0)) < 300:
                raise RuntimeError("freio: analise de evento <5min (cadencia v7.00)")
            if _cont["n"] >= 150:
                raise RuntimeError("freio: teto diario de analises de evento")
            _evt_ult[_bid] = _agora_ev
            _cont["n"] += 1
            _evt_dia[_bid] = _cont
...
def _cv(c, i):
    """Le o campo i de um candle que pode vir LISTA [o,h,l,c,v] ou DICT."""
    try:
        if isinstance(c, dict):
            return float(c.get(("o", "h", "l", "c", "v")[i]) or 0.0)
        return float(c[i])
    except Exception:
        return 0.0


def _ema_serie(vals, periodo):
    """EMA simples sobre uma lista de floats. Devolve lista do mesmo tamanho
    (None enquanto nao ha dado suficiente)."""
    if not vals or len(vals) < periodo:
        return [None] * len(vals or [])
    k = 2.0 / (periodo + 1.0)
    out = [None] * (periodo - 1)
    media = sum(vals[:periodo]) / periodo
    out.append(media)
...
def _detectar_canal_ema_exaustao(cs, atr=None):
    """CANAL EMA 20 H/L + BOLLINGER — metodologia do dono (v7.18).

    Uma operacao com DUAS entradas, e as duas nascem aqui:
      G1 REVERSAO   barra A fecha com o extremo tocando a banda de Bollinger
                    (exaustao) -> barra B fecha confirmando o sentido ->
                    ENTRADA no rompimento do extremo da barra B (intrabarra),
                    stop no extremo da exaustao.
      G2 DOUBLE     fechamento alem do canal EMA20 H/L no mesmo sentido =
                    continuidade confirmada, dobra a posicao.
    PERMANENCIA: enquanto nao fechar de volta pra DENTRO do canal, permanece.
    Sem alvo — a saida e estrutural, nao numerica. Foi o alvo fixo de 36 pts
    que capturou 18%% de um movimento de 200 no caso real de 26/jul.

    Indices: cs[-1] e a barra EM FORMACAO; cs[-2] e a ultima FECHADA (barra B);
    cs[-3] e a barra A. Mesma convencao do item 3 do checklist (v7.08).
    """
    if not cs or len(cs) < 25:
        return None
    try:
        fech = [_cv(c, 3) for c in cs]
        alta = [_cv(c, 1) for c in cs]
        baixa = [_cv(c, 2) for c in cs]
        vol = [_cv(c, 4) for c in cs]
        if min(fech[-25:]) <= 0:
            return None

        ema_h = _ema_serie(alta, 20)
        ema_l = _ema_serie(baixa, 20)
        if ema_h[-2] is None or ema_l[-2] is None:
            return None

        # Bollinger(20,2) sobre os fechamentos
        jan = fech[-21:-1]                     # 20 fechadas ate a barra B
        media = sum(jan) / len(jan)
        var = sum((x - media) ** 2 for x in jan) / len(jan)
        desv = var ** 0.5
        bb_sup, bb_inf = media + 2 * desv, media - 2 * desv

        A_h, A_l = alta[-3], baixa[-3]         # barra A (exaustao)
        B_o, B_h, B_l, B_c = fech[-2], alta[-2], baixa[-2], fech[-2]
        B_o = _cv(cs[-2], 0)
        preco = fech[-1] or B_c

        largura = ema_h[-2] - ema_l[-2]
        # v7.18b — o canal EMA20 H/L mede, por construcao, ~1 ATR (media das
        # maximas menos media das minimas). Vetar em 1.0x barraria condicao
        # NORMAL; 0.6x pega so o canal genuinamente espremido, que e o caso
        # em que o rompimento nao paga o spread.
        comprimido = bool(atr and largura < 0.6 * float(atr))
        dentro_canal = (ema_l[-2] <= preco <= ema_h[-2])

        out = {"ema_high": round(ema_h[-2], 5), "ema_low": round(ema_l[-2], 5),
               "bb_sup": round(bb_sup, 5), "bb_inf": round(bb_inf, 5),
               "largura_canal": round(largura, 5),
               "comprimido": comprimido, "dentro_canal": bool(dentro_canal)}

        # ── VETO: dentro do canal e lateral; canal comprimido nao paga ──
        if comprimido:
            out["veto"] = "canal_comprimido"
            return out

        # ── G1: exaustao (A) + confirmacao de sentido (B) ──
        vol_ok = True
        if len(vol) >= 13 and max(vol[-13:]) > 0:
            base = [v for v in vol[-13:-3] if v > 0]
            vol_ok = bool(base) and vol[-2] >= (sum(base) / len(base))

        if A_h >= bb_sup and B_c < B_o:        # topo exaurido + barra vermelha
            out.update({"gatilho": "G1", "direcao": "short",
                        "entrada_em": round(B_l, 5),      # rompimento da minima da B
                        "stop": round(A_h, 5),
                        "risco_pts": round(abs(A_h - B_l), 5),
                        "volume_ok": vol_ok,
                        "armado": bool(preco > B_l)})     # ainda nao rompeu = armado
        elif A_l <= bb_inf and B_c > B_o:      # fundo exaurido + barra verde
            out.update({"gatilho": "G1", "direcao": "long",
                        "entrada_em": round(B_h, 5),      # rompimento da maxima da B
                        "stop": round(A_l, 5),
                        "risco_pts": round(abs(B_h - A_l), 5),
                        "volume_ok": vol_ok,
                        "armado": bool(preco < B_h)})

        # ── G2: fechamento alem do canal = continuidade (autoriza o DOUBLE) ──
        if B_c < ema_l[-2] and fech[-3] >= ema_l[-3]:
            out["g2"] = {"direcao": "short", "cruzou_em": round(ema_l[-2], 5)}
        elif B_c > ema_h[-2] and fech[-3] <= ema_h[-3]:
            out["g2"] = {"direcao": "long", "cruzou_em": round(ema_h[-2], 5)}

        # ── PERMANENCIA: fechou de volta pra dentro = tese morreu ──
        out["voltou_ao_canal"] = bool(ema_l[-2] <= B_c <= ema_h[-2])
        return out
    except Exception:
        return None
...
def _analisar_bot_tecnica(candles_str, ema_h, ema_l, preco, regime, posicoes, offmind=None):
    ...
    # v7.18 — CANAL EMA 20 H/L + BOLLINGER: a metodologia do dono.
    ce = _detectar_canal_ema_exaustao(cs, atr_v)
    if ce:
        fatos["canal_ema_setup"] = ce
    return fatos
...
_REC_PLAT_STATE = {}   # v7.32 — throttle da espera carimbada: (bot_id, motivo) -> ts
_CICLO_TF_SEG = {"1m": 60, "5m": 300, "15m": 900, "30m": 1800,
                 "60m": 3600, "1h": 3600, "4h": 14400, "1d": 86400}


def _ciclo_norm_candles(cs):
    """Normaliza candles em dicts o/h/l/c — aceita lista [o,h,l,c,(v)] ou dict."""
    out = []
    for c in (cs or []):
        try:
            if isinstance(c, dict):
                out.append({"o": float(c.get("o")), "h": float(c.get("h")),
                            "l": float(c.get("l")), "c": float(c.get("c"))})
            elif isinstance(c, (list, tuple)) and len(c) >= 4:
                out.append({"o": float(c[0]), "h": float(c[1]),
                            "l": float(c[2]), "c": float(c[3])})
        except Exception:
            pass
    return out


def _ciclo_ema(vals, n=20):
    if not vals:
        return None
    k = 2.0 / (n + 1)
    e = vals[0]
...
def _ciclo_reversao_pullback(candles_op, scanner=None, alvo=None, atr_pts=None):
    """v7.32 — CICLO CIRCULAR: classificador deterministico REVERSAO x PULLBACK.
    Responde a pergunta que o portao de tendencia nunca fez: o movimento CONTRA
    a tendencia dominante e correcao (pullback — entrada contra segue bloqueada)
    ou quebra (reversao — a entrada contra e a PRIMEIRA da tendencia nova)?
    5 criterios estruturais, todos mensuraveis por regra, custo ZERO de IA:
      1 estrutura    — o fechamento contra quebrou o ultimo swing relevante?
      2 esforco      — barra contra com range >= 1.8x o passo medio (impulso)?
      3 canal        — fechou ALEM do canal EMA20 H/L no sentido contra?
      4 cascata      — cascata de reversao dos TFs menores confirmando (scanner)?
      5 profundidade — retrace do swing > 62 por cento (alem do territorio de pullback)?
    Veredicto: REVERSAO_EM_CURSO (n>=4, ou n>=3 com quebra de estrutura),
    TENDENCIA_INTACTA (n<=1), senao INDEFINIDO. Calibracao futura: a ficha
    carimba os criterios na BabyMachine — os thresholds se ajustam com dado.
    alvo = direcao do movimento contra proposto: alta ou baixa."""
    try:
        d = str(alvo or "").lower()
        if d not in ("alta", "baixa"):
            return None
        cs = _ciclo_norm_candles(candles_op)
        if len(cs) < 14:
            return None
        fechadas = cs[:-1]          # ultima esta em formacao
        ult = fechadas[-1]          # ultima barra FECHADA
        prev = fechadas[:-1]
        crit = {}

        # 1) ESTRUTURA — swing anterior ao impulso (exclui as 2 ultimas fechadas)
        base = fechadas[-14:-2] if len(fechadas) >= 14 else fechadas[:-2]
        ok1, det1 = False, "estrutura preservada"
        try:
            if base:
                if d == "baixa":
                    sw = min(b["l"] for b in base)
                    ok1 = ult["c"] < sw
                    det1 = ("quebrou o fundo " + str(round(sw, 2))) if ok1 else ("fundo " + str(round(sw, 2)) + " segurando")
                else:
                    sw = max(b["h"] for b in base)
                    ok1 = ult["c"] > sw
                    det1 = ("quebrou o topo " + str(round(sw, 2))) if ok1 else ("topo " + str(round(sw, 2)) + " segurando")
        except Exception:
            pass
        crit["estrutura"] = {"ok": ok1, "detalhe": det1}

        # 2) ESFORCO — range da barra contra vs passo medio das 12 anteriores
        ok2, det2 = False, "barra dentro do passo medio"
        try:
            rng = ult["h"] - ult["l"]
            passos = [(b["h"] - b["l"]) for b in prev[-12:] if (b["h"] - b["l"]) > 0]
            med = (sum(passos) / len(passos)) if passos else 0
            dir_ok = (ult["c"] < ult["o"]) if d == "baixa" else (ult["c"] > ult["o"])
            if med > 0 and dir_ok and rng >= 1.8 * med:
                ok2 = True
                det2 = "impulso: barra de " + str(round(rng, 2)) + " = " + str(round(rng / med, 1)) + "x o passo medio"
            elif med > 0:
                det2 = "barra " + str(round(rng, 2)) + " vs passo medio " + str(round(med, 2))
        except Exception:
            pass
        crit["esforco"] = {"ok": ok2, "detalhe": det2}

        # 3) CANAL — fechamento alem do canal EMA20 H/L no sentido contra
        ok3, det3 = False, "dentro do canal EMA20 H/L"
        try:
            highs = [b["h"] for b in fechadas]
            lows = [b["l"] for b in fechadas]
            emaH = _ciclo_ema(highs, 20)
            emaL = _ciclo_ema(lows, 20)
            if emaH is not None and emaL is not None:
                if d == "baixa" and ult["c"] < emaL:
                    ok3, det3 = True, "fechou ABAIXO do canal (EMA_L " + str(round(emaL, 2)) + ")"
                elif d == "alta" and ult["c"] > emaH:
                    ok3, det3 = True, "fechou ACIMA do canal (EMA_H " + str(round(emaH, 2)) + ")"
        except Exception:
            pass
        crit["canal"] = {"ok": ok3, "detalhe": det3}

        # 4) CASCATA — os TFs menores virando em sequencia (scanner v6.82)
        ok4, det4 = False, "sem cascata de reversao nos TFs menores"
        try:
            casc = (scanner or {}).get("cascata_reversao") or {}
            if str(casc.get("direcao") or "").lower() == d and "confirm" in str(casc.get("estado") or "").lower():
                ok4 = True
                det4 = "cascata " + str(casc.get("estado")) + " (detectada no " + str(casc.get("detectada_no")) + ")"
        except Exception:
            pass
        crit["cascata"] = {"ok": ok4, "detalhe": det4}

        # 5) PROFUNDIDADE — retrace do swing alem de 62 por cento
        ok5, det5 = False, "retrace em territorio de pullback"
        try:
            look = fechadas[-30:] if len(fechadas) >= 30 else fechadas
            hi = max(b["h"] for b in look)
            lo = min(b["l"] for b in look)
            amp = hi - lo
            if amp > 0:
                ret = (hi - ult["c"]) / amp if d == "baixa" else (ult["c"] - lo) / amp
                pct = round(100 * ret)
                if ret >= 0.62:
                    ok5, det5 = True, "retrace de " + str(pct) + " por cento do swing — alem do pullback"
                else:
                    det5 = "retrace de " + str(pct) + " por cento (<62)"
        except Exception:
            pass
        crit["profundidade"] = {"ok": ok5, "detalhe": det5}

        n_ok = sum(1 for c in crit.values() if c.get("ok"))
        if n_ok >= 4 or (n_ok >= 3 and crit["estrutura"]["ok"]):
            ver = "REVERSAO_EM_CURSO"
        elif n_ok <= 1:
            ver = "TENDENCIA_INTACTA"
        else:
            ver = "INDEFINIDO"
        return {"veredicto": ver, "direcao": d, "n_ok": n_ok, "total": 5,
                "criterios": crit,
                "resumo": str(n_ok) + "/5 criterios de reversao (" + ver.lower().replace("_", " ") + ")"}
    except Exception:
        return None


_FASE_STATE = {}   # v7.34 — memoria narrativa por chave (magic/simbolo+tf): impulso_ref + fase anterior

_FASE_I18N = {
    "IMPULSO_ALTA":  {"pt": "Impulso de alta",  "en": "Bullish impulse",   "es": "Impulso alcista"},
    "IMPULSO_BAIXA": {"pt": "Impulso de baixa", "en": "Bearish impulse",   "es": "Impulso bajista"},
    "RESPIRO":       {"pt": "Respiro",          "en": "Breather (pullback)", "es": "Respiro (retroceso)"},
    "RETOMADA":      {"pt": "Retomada",         "en": "Resumption",        "es": "Reanudacion"},
    "LATERAL":       {"pt": "Lateralizacao",    "en": "Range-bound",       "es": "Lateralizacion"},
    "EXAUSTAO":      {"pt": "Exaustao",         "en": "Exhaustion",        "es": "Agotamiento"},
    "REVERSAO_ALTA": {"pt": "Reversao pra alta","en": "Reversal up",       "es": "Reversion alcista"},
    "REVERSAO_BAIXA":{"pt": "Reversao pra baixa","en": "Reversal down",    "es": "Reversion bajista"},
    "INDEFINIDA":    {"pt": "Sem fase definida","en": "No defined phase",  "es": "Sin fase definida"},
}


def _fase_mercado(det, sc=None, casc=None):
    """v7.34 — MAQUINA DE FASES (a sensibilidade do analista virando gramatica
    do sistema, pedido do dono 05/ago apos o caso do respiro): o mercado deixa
    de ser descrito por PECAS (engolfo, range, cascata) e passa a ser narrado
    por FASES DE UMA HISTORIA, com MEMORIA de sequencia — porque respiro so
    significa algo em contraste com o impulso que veio antes. Deterministico,
    custo zero de IA. Fases e definicoes mensuraveis:
      LATERAL   — range detector diz dentro, ou canal comprimido (<0.6 ATR)
      IMPULSO   — 4+ de 5 fechadas na direcao OU barra >= 1.8x o passo, com
                  deslocamento real; grava IMPULSO_REF na memoria (direcao,
                  passo das barras do impulso, extremo)
      RESPIRO   — movimento contra o impulso com barras PEQUENAS (<=0.7x o
                  passo do impulso), retrace 5-61 por cento, estrutura preservada
      RETOMADA  — vindo de RESPIRO, fechada na direcao do impulso rompendo o
                  extremo da barra anterior (a entrada classica do dono)
      EXAUSTAO  — impulso esticado (>2x ATR do canal) + fechada de indecisao
      REVERSAO  — o classificador do Ciclo v7.32 confirma contra o impulso
    TELEMETRIA PRIMEIRO: esta versao NARRA e CARIMBA — mandato so liga depois
    da re-simulacao do Professor nos CSVs (regra da casa). Saida com codigo
    (universal, decide/mede igual nos 3 idiomas) + rotulos i18n pt/en/es."""
    try:
        tf_op = str((det or {}).get("tfop") or "").lower()
        ck = {"1m": "c1m", "5m": "c5m", "15m": "c15m", "30m": "c30m",
              "1h": "c60m", "60m": "c60m", "4h": "c4h"}.get(tf_op, "c15m")
        cs = _parse_candles_para_lista(det.get(ck)) or _parse_candles_para_lista(det.get("c15m"))
        if not cs or len(cs) < 16:
            return None
        fechadas = cs[:-1]
        ult = fechadas[-1]
        chave = str(det.get("magic") or "") + "|" + str(det.get("simbolo") or "") + "|" + tf_op
        mem = _FASE_STATE.setdefault(chave, {})
        fase_ant = mem.get("fase")
        atr = _prot_atr(fechadas) or 0
        passos = [(b["h"] - b["l"]) for b in fechadas[-20:] if (b["h"] - b["l"]) > 0]
        passo = (sum(passos) / len(passos)) if passos else 0
        preco = float(ult["c"])
        fase, detalhe, nums = None, "", {}

        # 1) LATERAL — mata tudo: dentro do range nao ha historia direcional
        try:
            rng = _detectar_range_lateral(fechadas, atr, preco)
            eh = _prot_ema([b["h"] for b in fechadas], 20)
            el = _prot_ema([b["l"] for b in fechadas], 20)
            larg = (eh[-1] - el[-1]) if (eh and el) else None
            if (rng and str(rng.get("estado")) == "dentro") or (larg is not None and atr > 0 and larg < 0.6 * atr):
                fase = "LATERAL"
                detalhe = "dentro do range" + (f" de {rng.get('altura_pts')} pts" if rng else " (canal comprimido)")
                nums = {"range_pts": (rng or {}).get("altura_pts"), "canal_pts": round(larg, 2) if larg else None}
        except Exception:
            pass

        # 2) IMPULSO novo na janela recente
        if fase is None and passo > 0:
            j = fechadas[-5:]
            altas = sum(1 for b in j if b["c"] > b["o"])
            baixas = sum(1 for b in j if b["c"] < b["o"])
            desloc = abs(float(j[-1]["c"]) - float(j[0]["o"]))
            barra_ult = ult["h"] - ult["l"]
            dir_ult = "alta" if ult["c"] > ult["o"] else ("baixa" if ult["c"] < ult["o"] else "")
            imp_dir = None
            if desloc >= 1.5 * passo:
                if altas >= 4:
                    imp_dir = "alta"
                    ...
                    imp_dir = "baixa"
            if imp_dir is None and barra_ult >= 1.8 * passo and dir_ult:
                imp_dir = dir_ult
            if imp_dir:
                # v7.34b — a MEMORIA tem precedencia semantica sobre o impulso cru:
                # (a) impulso forte na direcao do IMPULSO_REF vindo de RESPIRO = RETOMADA
                #     (a entrada classica do dono), nao um impulso novo do nada;
                # (b) impulso forte CONTRA o IMPULSO_REF vivo: consulta o Ciclo —
                #     estrutura quebrada = REVERSAO nomeada; senao segue impulso novo.
                _imp_mem = mem.get("impulso")
                if (_imp_mem and fase_ant == "RESPIRO" and imp_dir == _imp_mem.get("dir")):
                    fase = "RETOMADA"
                    detalhe = "retomada com forca na direcao do impulso — respiro concluido"
                    _imp_mem["extremo"] = (max(float(_imp_mem["extremo"]), ult["h"]) if imp_dir == "alta"
                                           else min(float(_imp_mem["extremo"]), ult["l"]))
                    _imp_mem["barras"] = 0
                    nums = {"desloc": round(desloc, 2)}
                elif _imp_mem and imp_dir != _imp_mem.get("dir"):
                    try:
                        _scn2 = {"cascata_reversao": casc} if casc else None
                        _cic2 = _ciclo_reversao_pullback(cs, scanner=_scn2, alvo=imp_dir)
                    except Exception:
                        _cic2 = None
                    if _cic2 and _cic2.get("veredicto") == "REVERSAO_EM_CURSO":
                        fase = "REVERSAO_ALTA" if imp_dir == "alta" else "REVERSAO_BAIXA"
                        detalhe = ("impulso contra quebrando a estrutura ("
                                   + str(_cic2.get("n_ok")) + "/5 criterios) — reversao")
                        nums = {"criterios": _cic2.get("n_ok"), "desloc": round(desloc, 2)}
                        mem.pop("impulso", None)
                if fase is None and _imp_mem and imp_dir != _imp_mem.get("dir") and int(_imp_mem.get("barras") or 99) <= 10:
                    # virada VIOLENTA contra impulso RECENTE = a propria reversao,
                    # mesmo antes do Ciclo fechar os criterios — e o nome que o
                    # trader da; nas barras seguintes vira IMPULSO na direcao nova
                    fase = "REVERSAO_ALTA" if imp_dir == "alta" else "REVERSAO_BAIXA"
                    detalhe = f"virada violenta ({desloc:.1f} pts) contra impulso recente de {_imp_mem.get('dir')}"
                    nums = {"desloc": round(desloc, 2)}
                    mem["impulso"] = {"dir": imp_dir, "passo": passo, "extremo": (ult["h"] if imp_dir == "alta" else ult["l"]), "barras": 0}
                if fase is None:
                    barras_dir = [(b["h"] - b["l"]) for b in j if ((b["c"] > b["o"]) if imp_dir == "alta" else (b["c"] < b["o"]))]
                    passo_imp = (sum(barras_dir) / len(barras_dir)) if barras_dir else passo
                    extremo = max(b["h"] for b in j) if imp_dir == "alta" else min(b["l"] for b in j)
                    mem["impulso"] = {"dir": imp_dir, "passo": passo_imp, "extremo": extremo, "barras": 0}
                    fase = "IMPULSO_ALTA" if imp_dir == "alta" else "IMPULSO_BAIXA"
                    detalhe = f"deslocamento de {desloc:.1f} pts ({desloc/passo:.1f}x o passo)" if passo else "impulso"
                    nums = {"desloc": round(desloc, 2), "passo_impulso": round(passo_imp, 2), "extremo": extremo}

        # 3) memoria: a historia depois do impulso
        imp = mem.get("impulso")
        if imp:
            imp["barras"] = int(imp.get("barras") or 0) + 1
            if imp["barras"] > 60:
                mem.pop("impulso", None)
                imp = None
        if fase is None and imp:
            d = imp["dir"]
            contra = "baixa" if d == "alta" else "alta"
            ext = float(imp["extremo"])
            # 3a) REVERSAO — o Ciclo confirma contra o impulso
            try:
                scn = {"cascata_reversao": casc} if casc else None
                cic = _ciclo_reversao_pullback(cs, scanner=scn, alvo=contra)
                if cic and cic.get("veredicto") == "REVERSAO_EM_CURSO":
                    fase = "REVERSAO_ALTA" if contra == "alta" else "REVERSAO_BAIXA"
                    detalhe = "reversao confirmada (" + str(cic.get("n_ok")) + "/5 criterios) contra o impulso"
                    nums = {"criterios": cic.get("n_ok")}
                    mem.pop("impulso", None)
            except Exception:
                pass
            # 3b) RETOMADA — vindo de respiro, fechada na direcao rompendo a barra anterior
            if fase is None and fase_ant == "RESPIRO" and len(fechadas) >= 2:
                ant = fechadas[-2]
                rompeu = (ult["c"] > ult["o"] and ult["c"] > ant["h"]) if d == "alta" else (ult["c"] < ult["o"] and ult["c"] < ant["l"])
                if rompeu:
                    fase = "RETOMADA"
                    detalhe = "fechada a favor do impulso rompendo a barra anterior — respiro concluido"
                    imp["extremo"] = max(ext, ult["h"]) if d == "alta" else min(ext, ult["l"])
                    nums = {"direcao": d}
            # 3c) RESPIRO — recuo raso com barras pequenas
            if fase is None:
                recuo = (ext - preco) if d == "alta" else (preco - ext)
                base = fechadas[-30:]
                amp = (max(b["h"] for b in base) - min(b["l"] for b in base)) or 1
                retr = recuo / amp
                j3 = fechadas[-3:]
                barras_contra = [(b["h"] - b["l"]) for b in j3 if ((b["c"] < b["o"]) if d == "alta" else (b["c"] > b["o"]))]
                pequenas = barras_contra and max(barras_contra) <= 0.7 * float(imp["passo"] or passo)
                if recuo > 0 and 0.05 <= retr <= 0.61 and pequenas:
                    fase = "RESPIRO"
                    detalhe = (f"recuo de {recuo:.1f} pts ({retr*100:.0f} por cento do swing), barras contra a "
                               + f"{(max(barras_contra)/float(imp['passo'])):.1f}x do passo do impulso — estrutura preservada")
                    nums = {"recuo": round(recuo, 2), "retrace_pct": round(retr * 100), "extremo_ref": ext}
            # 3d) EXAUSTAO — esticado + indecisao
            if fase is None and atr > 0:
                try:
                    eh = _prot_ema([b["h"] for b in fechadas], 20)
                    el = _prot_ema([b["l"] for b in fechadas], 20)
                    afast = (preco - eh[-1]) if d == "alta" else (el[-1] - preco)
                    corpo = abs(ult["c"] - ult["o"])
                    rng_u = (ult["h"] - ult["l"]) or 1
                    if afast > 2 * atr and corpo <= 0.35 * rng_u:
                        fase = "EXAUSTAO"
                        detalhe = f"impulso esticado {afast/atr:.1f}x ATR do canal + fechada de indecisao"
                        nums = {"afastamento_atr": round(afast / atr, 1)}
                except Exception:
                    pass
            # 3e) impulso ainda vivo sem evento novo
            if fase is None and imp["barras"] <= 6:
                fase = "IMPULSO_ALTA" if d == "alta" else "IMPULSO_BAIXA"
                detalhe = "impulso recente sem contramovimento relevante"
        if fase is None:
            fase = "INDEFINIDA"
            detalhe = "sem fase dominante nos dados"
        transicao = (str(fase_ant) + "->" + fase) if (fase_ant and fase_ant != fase) else None
        mem["fase"] = fase
        rot = _FASE_I18N.get(fase, {})
        return {"fase": fase, "anterior": fase_ant, "transicao": transicao,
                "detalhe": detalhe, "numeros": nums,
                "rotulos": {"pt": rot.get("pt"), "en": rot.get("en"), "es": rot.get("es")}}
    except Exception:
        return None
...
def _scanner_pacote(det):
    ...
    if ali:
        pac["alinhamento"] = ali
    # v7.34 — MAQUINA DE FASES: a narrativa do mercado viaja no pacote — flui
    # de graca pro /monitor/leitura, pra IA do cockpit e pro carimbo da BabyMachine
    try:
        fz = _fase_mercado(det, sc=sc, casc=pac.get("cascata_reversao"))
        if fz:
            pac["fase"] = fz
    except Exception:
        pass
...
#
#  Cache com assinatura curta: se nada mudou nos últimos 300s, reutiliza o
#  texto — evita gastar tokens quando o gráfico ainda não avançou.
# ════════════════════════════════════════════════════════════════════════════
_NARR_BOT_CACHE = {}   # bot_token -> {"assinatura": str, "ts": float, "texto": str}
_NARR_BOT_MIN_INTERVAL = 300  # v7.00: cadência de 5 min (decisão do dono) — narração acompanha o fechamento da vela
# v7.28 — ECONOMIA DE IA (decisão do dono 28/jul: ~US$12/dia de leitura inviabiliza
# o custo por assinante). Regras novas da leitura:
#   1. NADA mudou na assinatura => NUNCA re-chama a IA (a leitura antiga continua
#      verdadeira; antes re-chamava a cada 300s mesmo sem mudança = 288+/dia/bot).
#   2. Mudança GROSSA (regime, posições, range, gatilho, cascata, swing) => re-narra
#      na hora — é o que o usuário precisa ver mudar.
#   3. Mudança só FINA (padrão da vela, momentum, % do range) => re-narra no máximo
#      1x por barra do TF de operação (15m => 900s), piso 300s, teto 3600s.
#   4. Kill-switch: env BT_IA_LEITURA=off => leitura IA desligada, painel cai no
#      texto determinístico (_narracao_bot), custo zero, sem redeploy de código.
_NARR_BOT_TETO = 3600

def _narr_tf_seg(tf_op):
    """Duração da barra do TF de operação em segundos (piso 300, teto 3600)."""
    try:
        t = str(tf_op or "").strip().lower()
        mapa = {"1m": 60, "5m": 300, "15m": 900, "30m": 1800, "m1": 60, "m5": 300,
                "m15": 900, "m30": 1800, "1h": 3600, "h1": 3600, "4h": 14400, "h4": 14400,
                "60m": 3600, "1d": 86400, "d1": 86400}
        seg = mapa.get(t, 900)
        return max(_NARR_BOT_MIN_INTERVAL, min(seg, _NARR_BOT_TETO))
    except Exception:
        return 900

def _bot_assinatura_grossa(fatos):
    """Só o que muda de verdade o quadro: regime, posições, range, gatilho,
    cascata e estrutura de swing. Preço/momentum/padrão de vela ficam de fora."""
    if not fatos: return ""
...
def _narrar_bot_analitico(fatos, tf_op, bot_token):
    ...
    if not fatos:
        return None
    # v7.28 — kill-switch de emergencia (Railway: BT_IA_LEITURA=off, sem redeploy de codigo)
    if os.environ.get("BT_IA_LEITURA", "on").strip().lower() in ("off", "0", "nao", "não"):
        return None  # painel cai no texto deterministico; custo zero
    ass_agora = _bot_assinatura(fatos)
    agora = time.time()
    cache = _NARR_BOT_CACHE.get(bot_token) or {}
    # v7.28 regra 1 — NADA mudou: reusa PRA SEMPRE (sem expiracao por tempo)
    if cache.get("assinatura") == ass_agora and cache.get("texto"):
        return cache.get("texto")
    # v7.28 regra 3 — mudanca so FINA dentro da mesma barra do TF: segura a re-narracao
    grossa = _bot_assinatura_grossa(fatos)
    idade = agora - float(cache.get("ts") or 0)
    if (cache.get("texto") and cache.get("grossa") == grossa
            and idade < _narr_tf_seg(tf_op)):
        return cache.get("texto")
    chave = os.environ.get("ANTHROPIC_API_KEY", "").strip()
    if not chave:
        return None
    try:
        ...
        if not texto:
            return cache.get("texto")
        _NARR_BOT_CACHE[bot_token] = {"assinatura": ass_agora, "grossa": grossa, "ts": agora, "texto": texto[:500]}
        return texto[:500]
    except Exception as e:
...
#
#  O TICK decide se vale analisar agora (assinatura mudou, silêncio > 300s,
...
def _anl_ler_leitura(sb, user_id, bot_id):
    ...
    try:
        ...
        return {
            ...
            # v6.88 — Zona 2 completa: dados que o card sempre esperou
            "equity": s.get("equity"),
            "balance": s.get("balance"),
            # v6.94 — FIX RAIZ: aqui NÃO existem ent_por_nome/ent_por_simbolo
            # (são locais do monitor_leitura); a referência da v6.88 dava
            # NameError em toda chamada -> except -> None -> "sem leitura" ->
            # IA morta. O consumidor real do campo é o CARD via monitor_leitura;
            # a IA não usa entradas_hoje no contexto — None honesto aqui.
            "entradas_hoje": None,
            "drawdown_dia_pct": _dd_dia_pct(b.get("id"), s.get("equity")),
        ...
        # v6.94 — nunca mais engolir a causa: NameError aqui virou "sem leitura"
        # misterioso por dias. Loga traceback e devolve None (fluxo preservado).
        try:
            import sys as _sys_al, traceback as _tb_al
            print("ANL LER LEITURA erro:\n" + _tb_al.format_exc(), file=_sys_al.stderr)
        except Exception:
            pass
        return None
...
def monitor_analise(bot_id: int, desde: Optional[str] = None):
    ...
    resp = {"analises": hist}
    # v6.93 — DIAGNÓSTICO NA TELA: se a última geração morreu e não há análise
    # mais nova que a falha, o front mostra o motivo em faixa âmbar honesta.
    try:
        f = _ANL_FALHA.get(bot_id)
        if f:
            hist_all = list(_anl_hist(bot_id))
            ult_ts = 0.0
            if hist_all:
                try:
                    ult_ts = _dt.fromisoformat(str(hist_all[-1].get("ts"))).timestamp()
                except Exception:
                    ult_ts = 0.0
            if float(f.get("ts") or 0) > ult_ts:
                resp["falha"] = {"motivo": f.get("motivo"),
                                 "ha_s": int(_time_anl.time() - float(f.get("ts") or 0))}
    except Exception:
        pass
    return resp
...
def monitor_analise_tick(inp: AnlTickIn):
    """O cockpit chama a cada 6s. Verifica se vale analisar:
      ...
      - senão, silêncio > 300s -> analisa observação
    ...
    if not leit:
        _ANL_FALHA[inp.bot_id] = {"ts": _time_anl.time(),
                                  "motivo": "sem leitura do bot (snapshot não chegou do MT5)"}
...
def monitor_leitura(user_id: str):
    ...
    # v6.88 — ENTRADAS HOJE por bot (o front esperava isso desde a v9.53 e o
    # backend nunca mandou): conta os eventos bot_aberto de HOJE (UTC) do
    # usuário, casando por bot_nome do detalhe_json com fallback por símbolo
    # (mesma lógica do /monitor/eventos). 1 query indexada por chamada.
    ent_por_nome, ent_por_simbolo = {}, {}
    try:
        hoje0 = _dt.now(_tz.utc).replace(hour=0, minute=0, second=0, microsecond=0).isoformat()
        evs_hoje = (sb.table("agente_eventos")
                    .select("detalhe_json")
                    .eq("user_id", user_id).eq("tipo", "bot_aberto")
                    .gte("criado_em", hoje0)
                    .limit(400).execute().data or [])
        for ev in evs_hoje:
            dj = ev.get("detalhe_json") or {}
            nm = dj.get("bot_nome")
            if nm:
                ent_por_nome[nm] = ent_por_nome.get(nm, 0) + 1
            else:
                sm = str(dj.get("simbolo") or "").upper()
                if sm:
                    ent_por_simbolo[sm] = ent_por_simbolo.get(sm, 0) + 1
    except Exception as _e:
        try: print(f"[leitura] entradas_hoje falhou: {_e}")
        except Exception: pass
    # v6.89 — reconciliação das abertas fantasmas (throttle interno 120s/bot)
    for _b0 in bots:
        try:
            _s0 = ultimo.get(_b0.get("bot_token")) or {}
            _bm_reconciliar_abertas(sb, _b0.get("id"), _s0.get("posicoes_abertas"),
                                    _s0.get("lucro_flutuante"))
        except Exception:
            pass
    saida = []
    for b in bots:
        ...
        saida.append({
            ...
            "scanner": _scanner_pacote(det),
            # v7.03 — CHECKLIST DE ENTRADA (LEDs do card): mesmo portao que o
            # detector usa pra executar; direcao vem do gatilho do cacador.
            "checklist": _chk_leitura(det, s, b, c_op),
        })
    return {"bots": saida}
...
# v7.17 — JANELA MOVEL: em vez de periodos (que se repetiam no intradiario e se
# aninhavam no diario), 3 janelas NAO SOBREPOSTAS fatiadas do mesmo download.
# Todo timeframe passa a ter n=3 medicoes independentes.
_BIB_JANELAS_N = 3
_BIB_BASE_PERIODO = {"5m": "6 meses", "15m": "6 meses", "30m": "6 meses",
                     "1h": "2 anos", "4h": "2 anos", "1d": "5 anos"}
_BIB_COMBOS_LEGADO = {
    "5m":  ["6 meses"],
    # v7.16 — 15m/30m tinham DOIS periodos aqui, mas o yfinance corta
    # intradiario em ~60 dias: "6 meses" e "1 ano" baixavam o MESMO arquivo e
    # gravavam duas linhas identicas (594 de 594 celulas, medido em 26/jul).
    # Um periodo so: mesma evidencia, metade do tempo de esquenta.
    "15m": ["6 meses"],
    "30m": ["6 meses"],
    "1h":  ["1 ano", "2 anos"],
    "4h":  ["1 ano", "2 anos"],
...
_BIB_COMBOS = {
    tf: [f"janela {i}/{_BIB_JANELAS_N} ({base})" for i in range(1, _BIB_JANELAS_N + 1)]
    for tf, base in _BIB_BASE_PERIODO.items()
}
    ...
    for linha in resultado.get("linhas", []):
        ...
        for tf, cel in (linha.get("cels") or {}).items():
            ...
            linhas.append({
                ...
                "forca": cel.get("forca"),
                "medido_em": medido,
                # v7.17 — receita + captura (colunas novas da refundacao)
                "parametros": cel.get("parametros"),
                "codigo_hash": cel.get("codigo_hash"),
                "regras_versao": cel.get("regras_versao"),
                "captura_pct": cel.get("captura_pct"),
                "janela": periodo,
            })
    if not linhas:
        return 0
...
def _vit_faixa(wrs):
    """Faixa de acerto min-max arredondada em 5 (ex. [65, 75]). Numero cravado
    com 1 casa em cima de 2 medicoes e falsa precisao — a faixa diz a verdade
    sobre o tamanho da amostra sem precisar explicar amostra."""
    wrs = [w for w in wrs if w is not None]
    if not wrs:
        return None
    lo = max(0, int(min(wrs) // 5 * 5))
    hi = min(100, int(-(-max(wrs) // 5) * 5))
    if hi <= lo:
        hi = min(100, lo + 5)
    return [lo, hi]


def _vit_periodos_tf(tf):
    """Quantas medicoes o motor da biblioteca CONSEGUE fazer neste timeframe.
    Vem do proprio _BIB_COMBOS: 1d tem 3 periodos, 15m/30m/1h/4h tem 2 e o 5m
    tem 1 — limitacao de historico intradiario do provedor, nao escolha. Serve
    pra cobrar completude do que e obtenivel em vez de um numero fixo."""
    try:
        return max(1, len(_BIB_COMBOS.get(str(tf or "").strip().lower(), []) or []))
    except Exception:
        return 3


def _vit_estrelas(sh_max, bons, n, min_trades_ok, tf=None):
    """Piso ABSOLUTO de qualidade (o rank relativo entre TFs vem depois). Sem
    isso, um card ruim ganharia 5 estrelas so por ter o 'menos pior' dos TFs."""
    if not min_trades_ok:
        return 1
    aprov = (bons / n) if n else 0.0
    # v7.15 — completude exigida RELATIVA ao obtenivel naquele TF (ver
    # _vit_periodos_tf). Piso de 2 mantido: uma medicao unica nao e evidencia.
    _n_min = max(2, min(3, _vit_periodos_tf(tf))) if tf else 3
    if sh_max >= 2.5 and bons == n and n >= _n_min:
        e = 5
    elif sh_max >= 1.5 and aprov >= 0.66 and bons >= 1:
        e = 4
    elif sh_max >= 1.0 and bons >= 1:
        e = 3
    elif sh_max > 0:
        e = 2
    else:
        e = 1
    if n < _n_min:
        e = min(e, 4)   # 5 estrelas se conquista com medicao, nao com sorte
    return e


def _vit_escada_tf(combos, min_trades):
    """Escada de timeframes de UM ativo: cada TF com estrelas e faixa de acerto.
    COLAPSADA DE PROPOSITO — no maximo 3 TFs individuais + uma linha 'demais'.
    A vitrine vende direcao, nao entrega o banco de estudo (decisao do dono)."""
    porf = {}
    _visto = set()
    for c in combos:
        tf = c.get("tf") or ""
        if not tf:
            continue
        # v7.15b — ANTI-EVIDENCIA DUPLICADA: o yfinance corta intradiario em
        # ~60 dias, entao pedir "6 meses" e "1 ano" em 15m/30m devolve a MESMA
        # janela e grava duas linhas identicas com periodo diferente. Contar as
        # duas como 2 medicoes inflaria o n e daria estrela por evidencia que
        # nao existe. Medicoes com numeros identicos valem UMA.
        _assin = (tf, round(float(c.get("sh") or 0), 6), round(float(c.get("wr") or 0), 6),
                  int(c.get("trades") or 0), round(float(c.get("ret") or 0), 6))
        if _assin in _visto:
            continue
        _visto.add(_assin)
        d = porf.setdefault(tf, {"n": 0, "bons": 0, "sh": [], "wr": [], "trades": 0})
        d["n"] += 1
        d["sh"].append(c.get("sh") or 0.0)
        d["wr"].append(c.get("wr"))
        d["trades"] = max(d["trades"], int(c.get("trades") or 0))
        if (c.get("ret") or 0) > 0 and (c.get("sh") or 0) >= 1.0 \
           and int(c.get("trades") or 0) >= min_trades:
            d["bons"] += 1
    if not porf:
        return []
    linhas = []
    for tf, d in porf.items():
        sh_max = max(d["sh"]) if d["sh"] else 0.0
        linhas.append({
            "tf": tf,
            "estrelas": _vit_estrelas(sh_max, d["bons"], d["n"], d["trades"] >= min_trades, tf),
            "faixa": _vit_faixa(d["wr"]),
            "n": d["n"],
            "_sh": round(sh_max, 2),
            "_aprov": (d["bons"] / d["n"]) if d["n"] else 0.0,
        })
    linhas.sort(key=lambda x: (x["estrelas"], x["_aprov"], x["_sh"]), reverse=True)
    fortes = [l for l in linhas if l["estrelas"] >= 2][:3]
    resto = [l for l in linhas if l not in fortes]
    out = []
    for l in fortes:
        out.append({"tf": l["tf"], "estrelas": l["estrelas"], "faixa": l["faixa"], "n": l["n"]})
    if resto:
        out.append({"tf": "demais", "estrelas": 1, "faixa": None,
                    "n": sum(l["n"] for l in resto), "agrupado": len(resto)})
    return out



def _vit_celulas_aprovadas(ativos, min_trades):
    """v7.23 — CURADORIA: celulas (ativo x TF) desta estrategia que passam a
    REGUA DE APROVACAO: estrelas >= 4 (mesma _vit_estrelas da escada, com o
    dedup anti-evidencia da v7.15b) + melhor combo do TF com retorno positivo,
    PF >= 1.3 e sharpe > 0. Saida COLAPSADA (estrelas + faixa, nunca os
    numeros crus por celula) — o anti-vazamento do banco continua valendo."""
    out = []
    try:
        for ativo_nome, d in (ativos or {}).items():
            ...
            for c in combos:
                ...
                if not tf:
                    continue
                assin = (tf, round(float(c.get("sh") or 0), 6), round(float(c.get("wr") or 0), 6),
                         int(c.get("trades") or 0), round(float(c.get("ret") or 0), 6))
                if assin in visto:
                    continue
                visto.add(assin)
                g = porf.setdefault(tf, {"n": 0, "bons": 0, "sh": [], "wr": [],
                                         "pf": [], "ret": [], "trades": 0})
                g["n"] += 1
                g["sh"].append(c.get("sh") or 0.0)
                g["wr"].append(c.get("wr"))
                g["pf"].append(c.get("pf") or 0.0)
                g["ret"].append(c.get("ret") or 0.0)
                g["trades"] = max(g["trades"], int(c.get("trades") or 0))
                if (c.get("ret") or 0) > 0 and (c.get("sh") or 0) >= 1.0 \
                   and int(c.get("trades") or 0) >= min_trades:
                    g["bons"] += 1
            for tf, g in porf.items():
                sh_max = max(g["sh"]) if g["sh"] else 0.0
                estrelas = _vit_estrelas(sh_max, g["bons"], g["n"], g["trades"] >= min_trades, tf)
                pf_max = max(g["pf"]) if g["pf"] else 0.0
                ret_max = max(g["ret"]) if g["ret"] else 0.0
                if estrelas >= 4 and pf_max >= 1.3 and sh_max > 0 and ret_max > 0:
                    out.append({"ativo": ativo_nome, "tf": tf, "estrelas": estrelas,
                                "faixa": _vit_faixa(g["wr"]), "_sh": round(sh_max, 2)})
        out.sort(key=lambda x: (x["estrelas"], x["_sh"]), reverse=True)
    except Exception:
        pass
    return out[:3]
...
def estrategias_vitrine(lang: str = "pt"):
        ...
        try:
            ...
            while True:
                lote = (sb.table("estudo_biblioteca")
                        .select("estrategia_id,ativo,sharpe,profit_factor,retorno,win_rate,trades,forca,timeframe,periodo,regras_versao,captura_pct")
            ...
            # ── v7.16: DEDUP NA RAIZ (medido no banco em 26/jul) ────────────
            # 100% das celulas de 15m e 30m (594 de 594 = 18 estrategias x 33
            # ativos) tinham DUAS linhas com numeros IDENTICOS e periodo
            # diferente: o yfinance corta intradiario em ~60 dias, entao pedir
            # "6 meses" e "1 ano" baixa o MESMO arquivo. Eram ~1.188 linhas de
            # eco num banco de ~7.100 (17%), entrando com PESO DOBRADO em
            # sharpe_medio/winrate_medio/pf_medio dos cards e inflando o n das
            # estrelas. Medicao identica vale UMA — a partir daqui, em tudo.
            # ── v7.17: MIGRACAO SUAVE ENTRE ERAS DE MEDICAO ────────────────
            # Durante a remedicao o banco tem linhas velhas (stop fixo 0,50%) e
            # novas (regras de fabrica). Misturar as duas numa media so seria
            # pior que qualquer uma sozinha. Por CELULA: se ja existe medicao da
            # versao atual, usa SO ela; senao mantem a antiga. Card nenhum fica
            # vazio e media nenhuma mistura eras — a vitrine migra sozinha,
            # celula por celula, conforme o esquenta avanca.
            _tem_novo = set()
            for _r in rows:
                if (_r.get("regras_versao") or "") == REGRAS_VERSAO_BIB:
                    _tem_novo.add((_r.get("estrategia_id"), _r.get("ativo"), _r.get("timeframe")))
            if _tem_novo:
                _antes = len(rows)
                rows = [_r for _r in rows
                        if (_r.get("estrategia_id"), _r.get("ativo"), _r.get("timeframe")) not in _tem_novo
                        or (_r.get("regras_versao") or "") == REGRAS_VERSAO_BIB]
                print(f"[vitrine] eras: {len(_tem_novo)} celulas ja remedidas "
                      f"({_antes} -> {len(rows)} linhas apos preferir a versao atual)")
            _dedup, _vistas = [], set()
            for _r in rows:
                _k = (_r.get("estrategia_id"), _r.get("ativo"), _r.get("timeframe"),
                      _r.get("sharpe"), _r.get("win_rate"), _r.get("trades"), _r.get("retorno"))
                if _k in _vistas:
                    continue
                _vistas.add(_k)
                _dedup.append(_r)
            if len(_dedup) != len(rows):
                print(f"[vitrine] dedup: {len(rows)} -> {len(_dedup)} linhas "
                      f"({len(rows) - len(_dedup)} medicoes repetidas descartadas)")
            rows = _dedup
            ...
            for eid, ativos in por_ativo.items():
                ...
                for _nome in _prefer:
                    ...
                    if mc:
                        _wr_tf = [c["wr"] for c in ativos.get(_nome, {}).get("combos", [])
                                  if c.get("tf") == mc["tf"]]
                        _MELHOR_COMBO_EST[eid] = {"ativo": _nome, "periodo": mc["periodo"],
                                                  "timeframe": mc["tf"],
                                                  "retorno": round(mc["ret"], 2),
                                                  "win_rate": round(mc["wr"], 1),
                                                  "faixa_acerto": _vit_faixa(_wr_tf)}
                        break
                # ── ESCADA DE TIMEFRAMES por ativo do card (v7.14) ──
                # So dos ativos que o card MOSTRA (3 de 40) — a vitrine nunca
                # devolve a matriz inteira, nem pra quem chamar a API direto.
                _esc = {}
                for _nome in list(dict.fromkeys(_ATIVOS_FIXOS_VITRINE.get(eid, []) + auto))[:3]:
                    _d = ativos.get(_nome)
                    if not _d:
                        continue
                    _linhas = _vit_escada_tf(_d["combos"], _MIN_TRADES_COMBO)
                    if _linhas:
                        _esc[_nome] = _linhas
                if _esc:
                    _ESCADA_EST[eid] = _esc
                # v7.23 — CURADORIA: celulas aprovadas desta estrategia (todos
                # os ativos com combos suficientes, nao so os 3 do card)
                _cels = _vit_celulas_aprovadas(
                    {n: dd for n, dd in ativos.items() if len(dd.get("combos") or []) >= _MIN_BACKTESTS_ATIVO},
                    _MIN_TRADES_COMBO)
                if _cels:
                    _APROV_EST[eid] = _cels
            ...
            for eid, a in acc.items():
                medias[eid] = {
                    ...
                    "escada_tf": _ESCADA_EST.get(eid, {}),
                    "celulas_aprovadas": _APROV_EST.get(eid, []),
                }
        except Exception as e:
    ...
    for est in ESTRATEGIAS_PRONTAS:
        ...
        itens.append({
            ...
            "escada_tf": m.get("escada_tf", {}),
            "celulas_aprovadas": [{k: v for k, v in c.items() if not k.startswith("_")}
                                  for c in (m.get("celulas_aprovadas") or [])],
            "aprovacao": ("aprovada" if m.get("celulas_aprovadas") else
        ...
        if _cr:
            # v7.27 — celula_real pode listar mais de um ativo medido
            itens[-1]["top_ativos"] = (_cr.get("ativos") or [_cr.get("ativo")])
    ...
    melhores = []
    for _it in itens:
        for _c in (medias.get(_it["id"], {}).get("celulas_aprovadas") or []):
            melhores.append({"estrategia_id": _it["id"], "nome": _it["nome"],
                             "emoji": _it.get("emoji", ""), "ativo": _c["ativo"],
                             "tf": _c["tf"], "estrelas": _c["estrelas"],
                             "faixa": _c["faixa"], "_sh": _c.get("_sh", 0)})
    melhores.sort(key=lambda x: (x["estrelas"], x["_sh"]), reverse=True)
    for _m in melhores:
        _m.pop("_sh", None)
    out = {"total": len(itens), "estrategias": itens, "melhores_celulas": melhores[:6]}
...
def _mq5_hash_geracao(codigo_python, sl=None, tp=None) -> str:
    ...
    # v6.95 — CÓDIGO INTEIRO no hash: o [:6000] antigo deixava edições após o
    # caractere 6000 INVISÍVEIS pro espelho — num custom longo, o cache servia
    # o EA ANTIGO como "relâmpago" depois da edição, silenciosamente. Corrigido
    # no momento certo: o upgrade BT_EVT_V2 (v6.89) já esfriou o cache inteiro,
    # então trocar a base do hash não invalida nada que estivesse quente.
    base = (codigo_python or "")
...
def _gerar_mq5_de_codigo(codigo_python, params, idioma="pt"):
    # ── v7.24 — EA EXECUTOR PURO (remessa 2 da doutrina) ─────────────────────
    # Marcador BT_EXECUTOR_PURO no codigo = EA sem estrategia nativa, gerado
    # DETERMINISTICAMENTE (sem IA): esqueleto vazio + instrumentacao completa
    # (visao, canal de comando, disciplina, painel). A entrada e 100 por cento
    # da plataforma (detector modo automatico com a leitura do dono); a gestao
    # e do Protetor. Nao precisa de ANTHROPIC_API_KEY nem mexe no template.
    if "BT_EXECUTOR_PURO" in (codigo_python or ""):
        try:
            ...
            texto = (hit or {}).get("mq5") or ""
            if not texto or "BT_EVT_V2" not in texto or "BT_DISCIPLINA_V3" not in texto or "BT_OTT_V2" not in texto or "BT_MAGIC_ISO_V1" not in texto or "BT_CICLO_V1" not in texto or "BT_NIVEIS_V1" not in texto or "BT_CASCATA_V1" not in texto or "BT_CASCATA_V2" not in texto or "BT_CASCATA_V3" not in texto:
                esqueleto = (
                    "//+------------------------------------------------------------------+\n"
                    "//| BotTested — EXECUTOR PURO (chassi da doutrina, v7.24)             |\n"
                    "//| Este EA NAO tem estrategia nativa: nenhuma entrada local.         |\n"
                    "//| A ENTRADA vem da plataforma (portao + detector, modo automatico); |\n"
                    "//| a GESTAO vem do Protetor; este EA executa comandos e emite visao. |\n"
                    "//+------------------------------------------------------------------+\n"
                    "#include <Trade\\Trade.mqh>\n"
                    "input long   InpMagic      = 0;\n"
                    "input double InpLote       = 0.01;\n"
                    "input int    InpStopLoss   = 60;    // pontos (referencia; o comando traz o SL real)\n"
                    "input int    InpTakeProfit = 120;   // pontos (referencia)\n"
                    "CTrade trade;\n\n"
                    "int OnInit()\n"
                    "  {\n"
                    "   trade.SetExpertMagicNumber(InpMagic);\n"
                    ...
                    "void OnTick()\n"
                    "  {\n"
                    "   // EXECUTOR PURO: sem entrada nativa. Visao e canal de comando sao\n"
                    "   // injetados pela instrumentacao BotTested (BTVisaoTick/BTLerComando).\n"
                    "  }\n")
                texto = _instrumentar_log_mql5(esqueleto)
                _mq5_cache_guardar(gen_hash, _neutralizar_magic_mql5(texto))
                print(f"[executor-puro] EA deterministico gerado e cacheado ({gen_hash[:10]})")
            if magic:
                texto = _forcar_magic_mql5(texto, magic)
            texto = _forcar_sl_tp_mql5(texto, sl, tp)
            return texto
        except Exception as _e:
            try:
                import sys as _sys
                print(f"[executor-puro] {_e}", file=_sys.stderr)
            except Exception:
                pass
            return ""
    # ── v7.40 — ESTRATEGIA 2: REVERSAO NO EXTREMO (a leitura do dono medida) ──
    # Marcador BT_ESTRATEGIA2 = EA NATIVO deterministico (sem IA): FUNDO =
    # minima das 12 fechadas; ARMA gatilho = maxima da barra do fundo (validade
    # 4 barras); DISPARA no TICK ao romper o gatilho (entrada intrabarra, fiel
    # a leitura e ao simulador do Professor); stop NASCE ESTRUTURAL (fundo -
    # 0.3xATR14) com InpStopLoss de TETO DE DESASTRE (BT_STOPE_V1 por definicao
    # — nesta estrategia o stop estrutural E a regra, nao um ajuste); saida no
    # fechamento de volta alem do canal EMA20 oposto. Lados espelhados. Medido
    # 6 meses IC Markets: XAU M30 PF 1.88 (2.55 com veto de fase — recorde).
    if "BT_ESTRATEGIA2" in (codigo_python or ""):
        try:
            magic = int(params.get("magic", 0) or 0)
            sl = params.get("stop_loss", 150)
            tp = params.get("take_profit", 0)
            gen_hash = _mq5_hash_geracao(codigo_python, sl, tp)
            hit = _mq5_cache_buscar(gen_hash)
            texto = (hit or {}).get("mq5") or ""
            if not texto or "BT_EVT_V2" not in texto or "BT_DISCIPLINA_V3" not in texto or "BT_OTT_V2" not in texto or "BT_MAGIC_ISO_V1" not in texto or "BT_CICLO_V1" not in texto or "BT_NIVEIS_V1" not in texto or "BT_CASCATA_V1" not in texto or "BT_CASCATA_V2" not in texto or "BT_CASCATA_V3" not in texto or "BT_STOPE_V1" not in texto:
                esqueleto = (
                    "//+------------------------------------------------------------------+\n"
                    "//| BotTested — REVERSAO NO EXTREMO / Estrategia 2 (v7.40)           |\n"
                    "//| A leitura do fundador medida: 6 meses IC Markets, XAU M30        |\n"
                    "//| PF 1.88 (2.55 com veto de fase — recorde da casa).               |\n"
                    "//| FUNDO = minima das 12 fechadas; entrada no rompimento da MAXIMA  |\n"
                    "//| da barra do fundo (tick, validade 4 barras); stop estrutural no  |\n"
                    "//| fundo - 0.3xATR (BT_STOPE_V1 nativo); saida no fechamento alem   |\n"
                    "//| do canal EMA20 oposto. Lados espelhados. InpStopLoss = teto.     |\n"
                    "//+------------------------------------------------------------------+\n"
                    "#include <Trade\\\\Trade.mqh>\n"
                    "input long   InpMagic      = 0;\n"
                    "input double InpLote       = 0.01;\n"
                    "input int    InpStopLoss   = 150;   // TETO de desastre em pontos de preco (0 = sem teto)\n"
                    "input int    InpTakeProfit = 0;     // sem alvo por desenho: a saida e estrutural\n"
                    "input int    InpLookback   = 12;    // barras do extremo (fundo/topo)\n"
                    "input int    InpValidade   = 4;     // barras de validade do gatilho armado\n"
                    "CTrade trade;\n"
                    "int    btE2Dir   = 0;      // 1=gatilho long armado, -1=short, 0=nada\n"
                    "double btE2Gat   = 0.0;\n"
                    "double btE2Stop  = 0.0;\n"
                    "int    btE2Val   = 0;      // barras restantes de validade\n"
                    "\n"
                    "int OnInit()\n"
                    "  {\n"
                    "   trade.SetExpertMagicNumber(InpMagic);\n"
                    ...
                    "  }\n"
                    "\n"
                    "// BT_STOPE_V1 nativo: nesta estrategia o stop estrutural E a regra —\n"
                    "// nasce no extremo que definiu a entrada, com folga de 0.3xATR14.\n"
                    "double BTE2Atr14()\n"
                    "  {\n"
                    "   double atr=0.0; int n=14;\n"
                    "   for(int i=1;i<=n;i++)\n"
                    "     {\n"
                    "      double h=iHigh(_Symbol,_Period,i), l=iLow(_Symbol,_Period,i);\n"
                    "      double cp=iClose(_Symbol,_Period,i+1);\n"
                    ...
                    "  }\n"
                    "\n"
                    "void BTE2EmaHL(double &eh, double &el)\n"
                    "  {\n"
                    "   int n=60; double k=2.0/21.0;\n"
                    "   eh=iHigh(_Symbol,_Period,n); el=iLow(_Symbol,_Period,n);\n"
                    "   for(int i=n-1;i>=1;i--)\n"
                    "     {\n"
                    "      eh=iHigh(_Symbol,_Period,i)*k+eh*(1.0-k);\n"
                    "      el=iLow(_Symbol,_Period,i)*k+el*(1.0-k);\n"
                    "     }\n"
                    "  }\n"
                    "\n"
                    "void OnTick()\n"
                    "  {\n"
                    "   // ── DISPARO NO TICK (rompimento intrabarra do gatilho armado) ──\n"
                    "   if(btE2Dir!=0 && !BTE2Tem())\n"
                    "     {\n"
                    "      double ask=SymbolInfoDouble(_Symbol,SYMBOL_ASK);\n"
                    "      double bid=SymbolInfoDouble(_Symbol,SYMBOL_BID);\n"
                    "      if(btE2Dir==1 && ask>btE2Gat)\n"
                    "        {\n"
                    "         double slv=btE2Stop;\n"
                    "         if(InpStopLoss>0) slv=MathMax(slv, ask-InpStopLoss);   // teto de desastre\n"
                    "         if(slv<ask) trade.Buy(InpLote,_Symbol,0.0,slv,0.0,\"BT E2 fundo rompido\");\n"
                    "         btE2Dir=0;\n"
                    "        }\n"
                    "      else if(btE2Dir==-1 && bid<btE2Gat)\n"
                    "        {\n"
                    "         double slv=btE2Stop;\n"
                    "         if(InpStopLoss>0) slv=MathMin(slv, bid+InpStopLoss);   // teto de desastre\n"
                    "         if(slv>bid) trade.Sell(InpLote,_Symbol,0.0,slv,0.0,\"BT E2 topo rompido\");\n"
                    "         btE2Dir=0;\n"
                    "        }\n"
                    "     }\n"
                    "   // ── DECISOES DE BARRA FECHADA ──\n"
                    "   static datetime btE2Barra=0;\n"
                    "   datetime b=iTime(_Symbol,_Period,0);\n"
                    "   if(b==btE2Barra) return;\n"
                    "   btE2Barra=b;\n"
                    "   if(Bars(_Symbol,_Period)<80) return;\n"
                    "   double eh=0.0, el=0.0;\n"
                    "   BTE2EmaHL(eh, el);\n"
                    "   double c1=iClose(_Symbol,_Period,1);\n"
                    "   // saida estrutural: fechou alem do canal oposto\n"
                    "   if(BTE2Tem())\n"
                    "     {\n"
                    "      bool temBuy=false, temSell=false;\n"
                    "      for(int i=PositionsTotal()-1;i>=0;i--)\n"
                    "        {\n"
                    "         ulong tk=PositionGetTicket(i);\n"
                    "         if(tk>0 && PositionGetString(POSITION_SYMBOL)==_Symbol\n"
                    "            && PositionGetInteger(POSITION_MAGIC)==InpMagic)\n"
                    "           {\n"
                    "            if((int)PositionGetInteger(POSITION_TYPE)==POSITION_TYPE_BUY) temBuy=true;\n"
                    "            else temSell=true;\n"
                    "           }\n"
                    "        }\n"
                    "      if(temBuy && c1<el)  BTE2FecharTudo();\n"
                    "      if(temSell && c1>eh) BTE2FecharTudo();\n"
                    "      return;   // com posicao nao arma gatilho novo\n"
                    "     }\n"
                    "   // validade do gatilho armado\n"
                    "   if(btE2Dir!=0)\n"
                    "     {\n"
                    "      btE2Val--;\n"
                    "      if(btE2Val<=0) btE2Dir=0;\n"
                    "     }\n"
                    "   // arma: a barra fechada [1] e o extremo das ultimas InpLookback?\n"
                    "   double atr=BTE2Atr14();\n"
                    "   if(atr<=0.0) return;\n"
                    "   double lo1=iLow(_Symbol,_Period,1), hi1=iHigh(_Symbol,_Period,1);\n"
                    "   double loMin=lo1, hiMax=hi1;\n"
                    "   for(int i=2;i<=InpLookback;i++)\n"
                    "     {\n"
                    "      loMin=MathMin(loMin,iLow(_Symbol,_Period,i));\n"
                    "      hiMax=MathMax(hiMax,iHigh(_Symbol,_Period,i));\n"
                    "     }\n"
                    "   if(lo1<=loMin)\n"
                    "     { btE2Dir=1;  btE2Gat=hi1; btE2Stop=lo1-0.3*atr; btE2Val=InpValidade; }\n"
                    "   else if(hi1>=hiMax)\n"
                    "     { btE2Dir=-1; btE2Gat=lo1; btE2Stop=hi1+0.3*atr; btE2Val=InpValidade; }\n"
                    "  }\n")
                texto = _instrumentar_log_mql5(esqueleto)
                _mq5_cache_guardar(gen_hash, _neutralizar_magic_mql5(texto))
                print(f"[estrategia2] EA deterministico gerado e cacheado ({gen_hash[:10]})")
            if magic:
                texto = _forcar_magic_mql5(texto, magic)
            texto = _forcar_sl_tp_mql5(texto, sl, tp)
            return texto
            ...
            try:
                import sys as _sys
                print(f"[estrategia2] {_e}", file=_sys.stderr)
            except Exception:
                pass
            return ""
    # ── v7.25 — CRUZAMENTO DO CANAL (estrategia da casa, medida no MT5) ─────
    # Marcador BT_CRUZAMENTO_CANAL no codigo = EA NATIVO deterministico (sem IA):
    # fechou abaixo do canal EMA20 High/Low = vende; fechou acima = compra; sai e
    # INVERTE no cruzamento oposto. Decisao SO no fechamento de barra do grafico
    # (recomendado M30). Sem take profit por desenho; InpStopLoss = stop de
    # DESASTRE em pontos de preco (0 = sem stop). Mesma instrumentacao da frota.
    if "BT_CRUZAMENTO_CANAL" in (codigo_python or ""):
        try:
            magic = int(params.get("magic", 0) or 0)
            sl = params.get("stop_loss", 60)
            tp = params.get("take_profit", 120)
            ...
            if not texto or "BT_EVT_V2" not in texto or "BT_DISCIPLINA_V3" not in texto or "BT_OTT_V2" not in texto or "BT_MAGIC_ISO_V1" not in texto or "BT_CICLO_V1" not in texto or "BT_NIVEIS_V1" not in texto or "BT_CASCATA_V1" not in texto or "BT_CASCATA_V2" not in texto or "BT_CASCATA_V3" not in texto or "BT_STOPE_V1" not in texto:
                esqueleto = (
                    "//+------------------------------------------------------------------+\n"
                    "//| BotTested — CRUZAMENTO DO CANAL (estrategia da casa, v7.25)       |\n"
                    "//| Medida em 6 meses de dado real do MT5: XAUUSD M30 PF 1.44.       |\n"
                    "//| Fechou ABAIXO do canal EMA20 H/L = vende; ACIMA = compra;        |\n"
                    "//| sai e INVERTE no cruzamento oposto. Decide so em barra fechada.  |\n"
                    "//| Sem take profit por desenho; InpStopLoss = stop de DESASTRE em   |\n"
                    "//| pontos de preco (0 = sem stop). Recomendado: grafico M30.        |\n"
                    "//+------------------------------------------------------------------+\n"
                    "#include <Trade\\\\Trade.mqh>\n"
                    "input long   InpMagic      = 0;\n"
                    "input double InpLote       = 0.01;\n"
                    "input int    InpStopLoss   = 60;    // stop de DESASTRE em pontos de preco (0 = sem)\n"
                    "input int    InpTakeProfit = 0;     // sem alvo por desenho: a saida e o cruzamento oposto\n"
                    "int hEmaAlta=INVALID_HANDLE, hEmaBaixa=INVALID_HANDLE;\n"
                    "CTrade trade;\n\n"
                    "int OnInit()\n"
                    "  {\n"
                    "   trade.SetExpertMagicNumber(InpMagic);\n"
                    "   hEmaAlta =iMA(_Symbol,_Period,20,0,MODE_EMA,PRICE_HIGH);\n"
                    "   hEmaBaixa=iMA(_Symbol,_Period,20,0,MODE_EMA,PRICE_LOW);\n"
                    "   if(hEmaAlta==INVALID_HANDLE || hEmaBaixa==INVALID_HANDLE)\n"
                    "      return(INIT_FAILED);\n"
                    "   return(INIT_SUCCEEDED);\n"
                    "  }\n\n"
                    "void OnDeinit(const int reason)\n"
                    "  {\n"
                    "   if(hEmaAlta!=INVALID_HANDLE)  IndicatorRelease(hEmaAlta);\n"
                    "   if(hEmaBaixa!=INVALID_HANDLE) IndicatorRelease(hEmaBaixa);\n"
                    "  }\n\n"
                    "bool BTCruzTem(int tipo)\n"
                    "  {\n"
                    "   for(int i=PositionsTotal()-1;i>=0;i--)\n"
                    "     {\n"
                    "      ulong tk=PositionGetTicket(i);\n"
                    "      if(tk>0 && PositionGetString(POSITION_SYMBOL)==_Symbol\n"
                    "         && PositionGetInteger(POSITION_MAGIC)==InpMagic\n"
                    "         && (int)PositionGetInteger(POSITION_TYPE)==tipo)\n"
                    "         return(true);\n"
                    "     }\n"
                    "   return(false);\n"
                    "  }\n\n"
                    "void BTCruzFecharTudo()\n"
                    "  {\n"
                    "   for(int i=PositionsTotal()-1;i>=0;i--)\n"
                    "     {\n"
                    "      ulong tk=PositionGetTicket(i);\n"
                    ...
                    "     }\n"
                    "  }\n\n"
                    "// BT_STOPE_V1 — STOP ESTRUTURAL DE FABRICA (v7.38): o stop inicial\n"
                    "// nasce no fundo/topo do pullback (8 fechadas com folga de 0.3xATR14).\n"
                    "// InpStopLoss vira TETO DE DESASTRE: o stop nunca nasce alem dele.\n"
                    "double BTStopEstrutural(bool compra)\n"
                    "  {\n"
                    "   double atr=0.0; int n=14;\n"
                    "   for(int i=1;i<=n;i++)\n"
                    "     {\n"
                    ...
                    "   if(compra)\n"
                    "     {\n"
                    "      double fundo=iLow(_Symbol,_Period,1);\n"
                    "      for(int i=2;i<=8;i++) fundo=MathMin(fundo,iLow(_Symbol,_Period,i));\n"
                    "      return(fundo-0.3*atr);\n"
                    "     }\n"
                    "   double topo=iHigh(_Symbol,_Period,1);\n"
                    "   for(int i=2;i<=8;i++) topo=MathMax(topo,iHigh(_Symbol,_Period,i));\n"
                    "   return(topo+0.3*atr);\n"
                    "  }\n"
                    "\n"
                    "void OnTick()\n"
                    "  {\n"
                    "   static datetime btCruzBarra=0;\n"
                    "   datetime tAtual=iTime(_Symbol,_Period,0);\n"
                    "   if(tAtual==btCruzBarra) return;   // decide SO no fechamento de barra\n"
                    "   btCruzBarra=tAtual;\n"
                    "   double emaA[1], emaB[1];\n"
                    "   if(CopyBuffer(hEmaAlta,0,1,1,emaA)<1)  return;\n"
                    "   if(CopyBuffer(hEmaBaixa,0,1,1,emaB)<1) return;\n"
                    "   double fechamento=iClose(_Symbol,_Period,1);\n"
                    "   if(fechamento>emaA[0])\n"
                    "     {\n"
                    "      // cruzou pra CIMA do canal: zera venda e fica comprado\n"
                    "      if(BTCruzTem(POSITION_TYPE_SELL)) BTCruzFecharTudo();\n"
                    "      if(!BTCruzTem(POSITION_TYPE_BUY))\n"
                    "        {\n"
                    "         double ask=SymbolInfoDouble(_Symbol,SYMBOL_ASK);\n"
                    "         double slGeo=(InpStopLoss>0)?(ask-InpStopLoss):0.0;\n"
                    "         double slEst=BTStopEstrutural(true);\n"
                    "         double sl=slGeo;   // BT_STOPE_V1: mais justo entre estrutural e desastre\n"
                    "         if(slEst>0.0 && slEst<ask) sl=(slGeo>0.0)?MathMax(slEst,slGeo):slEst;\n"
                    "         trade.Buy(InpLote,_Symbol,0.0,sl,0.0,\"BT cruzamento cima\");\n"
                    "        }\n"
                    "     }\n"
                    "   else if(fechamento<emaB[0])\n"
                    "     {\n"
                    "      // cruzou pra BAIXO do canal: zera compra e fica vendido\n"
                    "      if(BTCruzTem(POSITION_TYPE_BUY)) BTCruzFecharTudo();\n"
                    "      if(!BTCruzTem(POSITION_TYPE_SELL))\n"
                    "        {\n"
                    "         double bid=SymbolInfoDouble(_Symbol,SYMBOL_BID);\n"
                    "         double slGeo=(InpStopLoss>0)?(bid+InpStopLoss):0.0;\n"
                    "         double slEst=BTStopEstrutural(false);\n"
                    "         double sl=slGeo;   // BT_STOPE_V1: mais justo entre estrutural e desastre\n"
                    "         if(slEst>0.0 && slEst>bid) sl=(slGeo>0.0)?MathMin(slEst,slGeo):slEst;\n"
                    "         trade.Sell(InpLote,_Symbol,0.0,sl,0.0,\"BT cruzamento baixo\");\n"
                    "        }\n"
                    "     }\n"
                    "  }\n")
                texto = _instrumentar_log_mql5(esqueleto)
                _mq5_cache_guardar(gen_hash, _neutralizar_magic_mql5(texto))
                print(f"[cruzamento-canal] EA deterministico gerado e cacheado ({gen_hash[:10]})")
            if magic:
                texto = _forcar_magic_mql5(texto, magic)
            texto = _forcar_sl_tp_mql5(texto, sl, tp)
            return texto
            ...
            try:
                import sys as _sys
                print(f"[cruzamento-canal] {_e}", file=_sys.stderr)
            except Exception:
                pass
            return ""
    # ── v7.27 — ROMPIMENTO DONCHIAN 20 (dois lados, medido no MT5) ──────────
    # Marcador BT_DONCHIAN_20 = EA NATIVO deterministico (sem IA): rompeu a
    # MAXIMA das 20 barras FECHADAS anteriores = compra; rompeu a MINIMA =
    # vende e inverte. Bandas recalculadas por barra; gatilho no tick (fiel a
    # medicao de 28/jul: entrada no nivel da banda). Sem TP; InpStopLoss =
    # stop de DESASTRE em pontos de preco (0 = sem). Instrumentacao da frota.
    if "BT_DONCHIAN_20" in (codigo_python or ""):
        try:
            magic = int(params.get("magic", 0) or 0)
            sl = params.get("stop_loss", 60)
            tp = params.get("take_profit", 120)
            ...
            if not texto or "BT_EVT_V2" not in texto or "BT_DISCIPLINA_V3" not in texto or "BT_OTT_V2" not in texto or "BT_MAGIC_ISO_V1" not in texto or "BT_CICLO_V1" not in texto or "BT_NIVEIS_V1" not in texto or "BT_CASCATA_V1" not in texto or "BT_CASCATA_V2" not in texto or "BT_CASCATA_V3" not in texto or "BT_STOPE_V1" not in texto:
                esqueleto = (
                    "//+------------------------------------------------------------------+\n"
                    "//| BotTested — ROMPIMENTO DONCHIAN 20 dois lados (v7.27)             |\n"
                    "//| Medido em 6 meses de dado real do MT5: XAU H1 PF 2.08 / BTC 1.49 |\n"
                    "//| Rompeu a MAXIMA das 20 barras fechadas = compra; a MINIMA =      |\n"
                    "//| vende e INVERTE. Bandas por barra, gatilho no tick.              |\n"
                    "//| Sem take profit; InpStopLoss = stop de DESASTRE em pontos de     |\n"
                    "//| preco (0 = sem). BTC: grafico H1. XAU: H1 ou M30.                |\n"
                    "//+------------------------------------------------------------------+\n"
                    "#include <Trade\\\\Trade.mqh>\n"
                    "input long   InpMagic      = 0;\n"
                    "input double InpLote       = 0.01;\n"
                    "input int    InpStopLoss   = 60;    // stop de DESASTRE em pontos de preco (0 = sem)\n"
                    "input int    InpTakeProfit = 0;     // sem alvo por desenho: a saida e o rompimento oposto\n"
                    "input int    InpDonchianN  = 20;\n"
                    "CTrade trade;\n\n"
                    "int OnInit()\n"
                    "  {\n"
                    "   trade.SetExpertMagicNumber(InpMagic);\n"
                    ...
                    "void OnTick()\n"
                    "  {\n"
                    "   static datetime btDonBarra=0;\n"
                    "   static double btDonTopo=0.0, btDonFundo=0.0;\n"
                    "   datetime tAtual=iTime(_Symbol,_Period,0);\n"
                    "   if(tAtual!=btDonBarra)\n"
                    "     {\n"
                    "      // barra nova: recalcula as bandas das N barras FECHADAS anteriores\n"
                    "      int ih=iHighest(_Symbol,_Period,MODE_HIGH,InpDonchianN,1);\n"
                    "      int il=iLowest(_Symbol,_Period,MODE_LOW,InpDonchianN,1);\n"
                    "      if(ih<0 || il<0) return;\n"
                    "      btDonTopo=iHigh(_Symbol,_Period,ih);\n"
                    "      btDonFundo=iLow(_Symbol,_Period,il);\n"
                    "      btDonBarra=tAtual;\n"
                    "     }\n"
                    "   if(btDonTopo<=0.0 || btDonFundo<=0.0) return;\n"
                    "   double ask=SymbolInfoDouble(_Symbol,SYMBOL_ASK);\n"
                    "   double bid=SymbolInfoDouble(_Symbol,SYMBOL_BID);\n"
                    "   if(ask>btDonTopo)\n"
                    "     {\n"
                    "      // rompeu a maxima: zera venda e fica comprado\n"
                    "      if(BTDonTem(POSITION_TYPE_SELL)) BTDonFecharTudo();\n"
                    "      if(!BTDonTem(POSITION_TYPE_BUY))\n"
                    "        {\n"
                    "         double slGeo=(InpStopLoss>0)?(ask-InpStopLoss):0.0;\n"
                    "         double slEst=BTStopEstrutural(true);\n"
                    "         double sl=slGeo;   // BT_STOPE_V1: mais justo entre estrutural e desastre\n"
                    "         if(slEst>0.0 && slEst<ask) sl=(slGeo>0.0)?MathMax(slEst,slGeo):slEst;\n"
                    "         trade.Buy(InpLote,_Symbol,0.0,sl,0.0,\"BT donchian cima\");\n"
                    "        }\n"
                    "     }\n"
                    "   else if(bid<btDonFundo)\n"
                    "     {\n"
                    "      // rompeu a minima: zera compra e fica vendido\n"
                    "      if(BTDonTem(POSITION_TYPE_BUY)) BTDonFecharTudo();\n"
                    "      if(!BTDonTem(POSITION_TYPE_SELL))\n"
                    "        {\n"
                    "         double slGeo=(InpStopLoss>0)?(bid+InpStopLoss):0.0;\n"
                    "         double slEst=BTStopEstrutural(false);\n"
                    "         double sl=slGeo;   // BT_STOPE_V1: mais justo entre estrutural e desastre\n"
                    "         if(slEst>0.0 && slEst>bid) sl=(slGeo>0.0)?MathMin(slEst,slGeo):slEst;\n"
                    "         trade.Sell(InpLote,_Symbol,0.0,sl,0.0,\"BT donchian baixo\");\n"
                    "        }\n"
                    "     }\n"
                    "  }\n")
                texto = _instrumentar_log_mql5(esqueleto)
                _mq5_cache_guardar(gen_hash, _neutralizar_magic_mql5(texto))
                print(f"[donchian-20] EA deterministico gerado e cacheado ({gen_hash[:10]})")
            if magic:
                texto = _forcar_magic_mql5(texto, magic)
            texto = _forcar_sl_tp_mql5(texto, sl, tp)
            return texto
            ...
            try:
                import sys as _sys
                print(f"[donchian-20] {_e}", file=_sys.stderr)
            except Exception:
                pass
            return ""
    chave = os.environ.get("ANTHROPIC_API_KEY", "").strip()
    ...
    try:
        ...
        if hit and hit.get("mq5"):
            texto = hit["mq5"]
            # v6.89 — UPGRADE DE FROTA: entrada sem os eventos v2 (BT_EVT_V2 =
            # evento só na execução confirmada, com preço/ticket) é DESCARTADA
            # e regenerada — mesma filosofia da auto-cura da sentinela; cada
            # estratégia migra sozinha no próximo envio, sem SQL manual.
            if "BT_EVT_V2" not in texto or "BT_DISCIPLINA_V3" not in texto or "BT_OTT_V2" not in texto or "BT_MAGIC_ISO_V1" not in texto or "BT_CICLO_V1" not in texto or "BT_NIVEIS_V1" not in texto or "BT_CASCATA_V1" not in texto or "BT_CASCATA_V2" not in texto or "BT_CASCATA_V3" not in texto:
                print(f"[mq5-cache] HIT {gen_hash[:10]} sem BT_EVT_V2/BT_DISCIPLINA_V3 — descartado pra regenerar com anti-flood de recusada")
                _mq5_cache_apagar(gen_hash)
            else:
...
class MT5EnviarReq(BaseModel):
    ...
    timeframe: str = "1d"
    idioma: str = "pt"
    estrategia_nome: str = ""   # v6.90 — linhagem: qual estratégia gerou este bot
...
def mt5_enviar(req: MT5EnviarReq):
    ...
    # v6.90 — LINHAGEM DA ESTRATÉGIA: grava no config_operacional do bot (JSONB
    # v6.75, merge) qual estratégia gerou este envio. A BabyMachine lê daqui pra
    # responder "qual estratégia fez esta operação" em cada registro. Best-effort.
    # v7.24 — linhagem automatica do EXECUTOR PURO: marcador sem nome vindo do
    # front => registra "Executor BotTested" (linhagem nunca vazia, item 6 verde)
    if not req.estrategia_nome and "BT_EXECUTOR_PURO" in (req.codigo or ""):
        req.estrategia_nome = "Executor BotTested"
    # v7.25 — linhagem automatica do CRUZAMENTO DO CANAL (mesma protecao)
    if not req.estrategia_nome and "BT_ESTRATEGIA2" in (req.codigo or ""):
        req.estrategia_nome = "Reversao no Extremo (Estrategia 2)"
    if not req.estrategia_nome and "BT_CRUZAMENTO_CANAL" in (req.codigo or ""):
        req.estrategia_nome = "Cruzamento do Canal"
    # v7.27 — linhagem automatica do DONCHIAN 20 (mesma protecao)
    if not req.estrategia_nome and "BT_DONCHIAN_20" in (req.codigo or ""):
        req.estrategia_nome = "Rompimento Donchian 20"
    if req.estrategia_nome:
        try:
            _sbl = _sb_admin()
            if _sbl is not None:
                _blin = _bot_por_token(_sbl, req.bot_token)
                if _blin:
                    _cfg = dict(_blin.get("config_operacional") or {})
                    _cfg["estrategia_nome"] = str(req.estrategia_nome)[:120]
                    _sbl.table("conector_bots").update({"config_operacional": _cfg}).eq("id", _blin["id"]).execute()
        except Exception:
            pass
    # v7.37 — O STOP DO ENVIO VIRA CONHECIMENTO DO CEREBRO: persiste
    # InpStopLoss/TP em config_operacional (merge best-effort). Fecha o gap do
    # OTT sem tocar no EA: o Protetor passa a ESTIMAR o SL inicial de qualquer
    # posicao (sl_estimado = entrada -/+ sl_envio_pts) e comparar direto.
    try:
        _sbsl = _sb_admin()
        if _sbsl is not None and req.stop_loss is not None:
            _bsl = _bot_por_token(_sbsl, req.bot_token)
            if _bsl:
                _csl = dict(_bsl.get("config_operacional") or {})
                _csl["sl_envio_pts"] = float(req.stop_loss)
                if req.take_profit is not None:
                    _csl["tp_envio_pts"] = float(req.take_profit)
                _sbsl.table("conector_bots").update({"config_operacional": _csl}).eq("id", _bsl["id"]).execute()
    except Exception:
        pass
    # v7.46 — MANDATO MEDIDO GRAVADO NO ENVIO: o /mt5/enviar grava sozinho os
    # flags do Supabase que antes eram ritual manual (e falharam 3 frotas):
    # veto_fase=true nas celulas XAU (Cruzamento/E2, mandato +0.08/+0.67);
    # protecao=false no Donchian BTC (decisao registrada da celula). ETH fica
    # sem veto (nunca medido la — lei por celula). Best-effort, non-blocking.
    try:
        _cod46 = req.codigo or ""; _atv46 = str(req.ativo or "").upper()
        _fl46 = {}
        if ("BT_CRUZAMENTO_CANAL" in _cod46 or "BT_ESTRATEGIA2" in _cod46) and "XAU" in _atv46:
            _fl46["veto_fase"] = True
        if "BT_DONCHIAN_20" in _cod46 and "BTC" in _atv46:
            _fl46["protecao"] = False
        if _fl46:
            _sbm46 = _sb_admin()
            if _sbm46 is not None:
                _bm46 = _bot_por_token(_sbm46, req.bot_token)
                if _bm46:
                    _cm46 = dict(_bm46.get("config_operacional") or {})
                    _cm46.update(_fl46)
                    _sbm46.table("conector_bots").update({"config_operacional": _cm46}).eq("id", _bm46["id"]).execute()
                    print(f"[v7.46 mandato] {_fl46} gravado no bot {_bm46.get('id')}")
    except Exception:
        pass
    ...
    # v7.46 — A BASE NASCE LIGADA: chaves do Ciclo viradas no EA final das
    # celulas da casa + piso de desastre por ativo. Vale pro fresco E pro cache.
    mq5 = _bt_base_ligar(req.codigo, mq5, req.ativo)
    import uuid
...
def _mq5_aquecer_worker():
    try:
        ...
        for est in alvos:
            ...
            hit = _mq5_cache_buscar(h)
            if hit and hit.get("mq5"):
                # v6.95 — AQUECEDOR CIENTE DO UPGRADE DE FROTA: entrada sem o
                # marcador BT_EVT_V2 seria DESCARTADA pelo /mt5/enviar no HIT
                # (v6.89); pular aqui deixaria o espelho FALSAMENTE quente.
                # Mesmo tratamento do envio: apaga e regenera com o template
                # atual — o aquecedor sempre entrega cache que o envio aceita.
                if "BT_EVT_V2" in (hit.get("mq5") or "") and "BT_DISCIPLINA_V3" in (hit.get("mq5") or ""):
                    _MQ5_AQUECER["feitas"] += 1
                    print(f"[mq5-aquecer] {est.get('id')} já no cache ({h[:10]})")
                    continue
                _mq5_cache_apagar(h)
                print(f"[mq5-aquecer] {est.get('id')} pré-BT_EVT_V2/BT_DISCIPLINA_V3 no cache — regenerando")
            ...
            if not mq5:
                ...
                # v6.95 — SANIDADE NO AQUECEDOR: geração reprovada na sentinela
                # não pode ficar "quente" (viraria BT-G01 pro primeiro usuário).
                _ok_aq, _ln_aq, _tr_aq = _mq5_sanidade(mq5)
                if not _ok_aq:
                    _mq5_cache_apagar(h)
                    _MQ5_AQUECER["erros"].append(f"{est.get('id')} (sentinela linha {_ln_aq})")
                    print(f"[mq5-aquecer] SENTINELA reprovou {est.get('id')} linha {_ln_aq} — cache limpo")
                else:
