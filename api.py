# ============================================================
#  BotTested API — v6.18  (a versão REAL está em API_VERSAO/BUILD_TAG, ~linha 604, e no /versao)
        ...
        if bots:
            ...
            for b in bots:
                if b.get("ultimo_ping"):
                    ...
                    # ping RECENTE = bot está no gráfico emitindo snapshots = OPERANDO
                    try:
                        ping = _dt.fromisoformat(str(b["ultimo_ping"]).replace("Z", "+00:00"))
                        if (agora - ping).total_seconds() < OFFLINE_APOS_SEGUNDOS:
                            etapa = max(etapa, 6)  # no gráfico, lendo o mercado = operar
                    except Exception:
                        pass
    except Exception:
        pass
