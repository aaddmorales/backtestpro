# ============================================================
#  BotTested API — v7.50 | Build: 2026-08-16c-cicloverif-v1 | Deploy: Railway
...
//--- BotTested BT_CICLOVERIF_V1 — CICLO DE VERIFICACAO POR TIME FRAME --------
// NOME OFICIAL registrado pelo dono (16/ago/2026): CICLO DE VERIFICACAO POR
// TIME FRAME. Dois Ciclos em funcionamento continuo: Ciclo 1 de baixo pra cima
// (M1->H1, antecipacao e resposta rapida) e Ciclo 2 de cima pra baixo (MN1->H4,
// contexto e confirmacao). LEIS DO DONO (16/ago): o CICLO 2 NUNCA CORTA — dele
// o sistema extrai direcao, tendencia e continuidade (leitura pura); quem corta
// e o Ciclo 1 de baixo pra cima. Toda decisao usa a leitura do PROPRIO Ciclo
// (cada frame sabe o que acontece nos outros) — nunca numero/janela inventada.
// TUDO DORMENTE (nada sobe sem medicao; ligar = decisao do dono, por celula).
// Leitura por TF (barras FECHADAS): U/D/L impulso; ! estrutura quebrada;
// ^ quebra iminente (<=0.3xATR no sentido); x candidato a exaustao.
string BTCVTF(ENUM_TIMEFRAMES tf)
{
   int n = 20;
...
string BTCVTag()
{
   // telemetria dos 2 Ciclos carimbada no evento aberto (leitura continua)
   ...
   string e1[5], t1 = "", t2 = "";
   for(int i = 0; i < 5; i++) { e1[i] = BTCVTF(c1[i]); t1 += (i > 0 ? "." : "") + e1[i]; }
   for(int j = 0; j < 4; j++) { t2 += (j > 0 ? "." : "") + BTCVTF(c2[j]); }
   ...
   return("|cv1=" + t1 + "|cv2=" + t2 + "|cvconf=" + conf + "|cvqa=" + IntegerToString(qa));
}
// ── DECISOES (todas dormentes; ligar = decisao do dono por celula) ──────────
input bool   InpCVEntrada    = false; // gate: gatilho CONFIRMADO (Lei 12/ago) + Ciclo confirmando a direcao
input int    InpCVValid      = 4;     // validade do gatilho armado (barras do TF da celula)
input double InpCVExZona     = 0.10;  // zona do extremo p/ exaustao armar reversao (PARAMETRO DE TESTE)
input bool   InpCVCorte      = false; // verificador continuo em posicao: 1m janela -> 5m corta -> 15m julga
input int    InpCVJanela1m   = 6;     // janela de barras de 1m do verificador (doutrina do dono)
input bool   InpCVReentrada  = false; // pos-corte governado pelo 15m (SEM relogio — o Ciclo governa)
input bool   InpCVLateral    = false; // lateralizacao bloqueia de baixo pra cima (5m identifica + 15m confirma)
input bool   InpCVExaustao   = false; // exaustao lida pela escada (5m confirmada acima) bloqueia continuacao
input bool   InpCVDirecional = false; // DIA DIRECIONAL: ontem no D1 + confirmacao descendo os frames
input int    InpCVDiaConf    = 3;     // andares do Ciclo 1 confirmando o dia (3 = ate 15M; PARAMETRO DE TESTE)
input int    InpCVSessao     = 0;     // 0=off 1=Asia 2=Londres 3=NY 4=Lon+NY (referencia profissional GMT)
input bool   InpCVNoticia    = false; // bloqueia janela -15/+45min de noticia ALTA (calendario MT5)
input double InpCVAtrPiso    = 0.0;   // piso de ATR14 do TF (0=off; mercado morto nao entra)
// ── maquina de estado do gatilho (Lei de Confirmacao de Entrada, 12/ago) ────
int      btCVDir   = 0;
double   btCVGat   = 0.0;
string   btCVOrig  = "";
int      btCVRest  = 0;
bool     btCVConf  = false;
datetime btCVBarra = 0;
void BTCVEstado()
{
   datetime b = iTime(_Symbol, PERIOD_CURRENT, 0);
   if(b == btCVBarra) return;
   btCVBarra = b;
   ...
   if(btCVDir != 0 && !btCVConf)
   {
      if(btCVDir == 1 && h1 > btCVGat)       btCVConf = true;
      else if(btCVDir == -1 && l1 < btCVGat) btCVConf = true;
      else
      {
         btCVRest--;
         if(btCVRest <= 0) { btCVDir = 0; btCVGat = 0.0; btCVOrig = ""; }
      }
      if(btCVConf) return;
   }
   ...
   if(c1 > hh)      { dirNova = 1;  origem = "quebra"; }
   else if(c1 < ll) { dirNova = -1; origem = "quebra"; }
   else if(exaust && faixa > 0.0)
   {
      if(l1 <= ll + InpCVExZona * faixa)      { dirNova = 1;  origem = "exaustao"; }
      else if(h1 >= hh - InpCVExZona * faixa) { dirNova = -1; origem = "exaustao"; }
   }
   if(dirNova != 0)
   {
      btCVDir  = dirNova;
      btCVGat  = (dirNova == 1) ? h1 : l1;
      btCVOrig = origem;
      btCVRest = InpCVValid;
      btCVConf = false;
   }
}
// ── DIA DIRECIONAL (Ciclo 2 le de cima pra baixo; NUNCA corta) ──────────────
int BTCVDia()
{
   // barra de ONTEM no Diario da a hipotese; a confirmacao DESCE os frames
   // durante o dia (H4 + escada do Ciclo 1 ate InpCVDiaConf andares)
   string d1 = StringSubstr(BTCVTF(PERIOD_D1), 0, 1);
   if(d1 != "U" && d1 != "D") return(0);
   if(StringSubstr(BTCVTF(PERIOD_H4), 0, 1) != d1) return(0);
   ENUM_TIMEFRAMES c1[5] = {PERIOD_M1, PERIOD_M5, PERIOD_M15, PERIOD_M30, PERIOD_H1};
   int prof = 0;
   for(int k = 0; k < 5; k++)
   {
      if(StringSubstr(BTCVTF(c1[k]), 0, 1) != d1) break;
      prof = k + 1;
   }
   if(prof < InpCVDiaConf) return(0);
   return(d1 == "U" ? 1 : -1);
}
// ── sessoes (referencia profissional GMT: Toquio 00-09, Londres 08-17,
//    NY 13-22, sobreposicao Londres+NY 13-17) ────────────────────────────────
string BTCVSessao()
{
...
bool BTCVSessaoOk()
{
   if(InpCVSessao == 0) return(true);
   MqlDateTime g; TimeToStruct(TimeGMT(), g);
   int h = g.hour;
   if(InpCVSessao == 1) return(h >= 0 && h < 9);
   if(InpCVSessao == 2) return(h >= 8 && h < 17);
   if(InpCVSessao == 3) return(h >= 13 && h < 22);
   if(InpCVSessao == 4) return(h >= 8 && h < 22);
   return(true);
}
bool BTCVNoticiaBloqueia()
{
   if(!InpCVNoticia) return(false);
   ...
   if(!CalendarValueHistory(vals, de, ate)) return(false);
...
string BTCVVolTag()
{
   // volume SO DEMONSTRATIVO (lei da casa: telemetria, nunca voto)
   if(iBars(_Symbol, PERIOD_CURRENT) < 25) return("");
   double v1 = (double)iVolume(_Symbol, PERIOD_CURRENT, 1), med = 0.0;
   for(int i = 2; i <= 21; i++) med += (double)iVolume(_Symbol, PERIOD_CURRENT, i);
   med /= 20.0;
   if(med <= 0.0) return("");
   return("|cvvol=" + DoubleToString(v1 / med, 1));
}
string BTCVOpTag()
{
   BTCVEstado();
   string op = "0";
   if(btCVDir == 1)  op = btCVConf ? "Cconf" : "Carm";
   if(btCVDir == -1) op = btCVConf ? "Vconf" : "Varm";
   string t = "|cvop=" + op + "|cvses=" + BTCVSessao();
   if(btCVDir != 0) t += "|cvgat=" + DoubleToString(btCVGat, _Digits) + "|cvorig=" + btCVOrig;
   int d = BTCVDia();
   t += "|cvdia=" + (d == 1 ? "U" : (d == -1 ? "D" : "0"));
   t += BTCVVolTag();
   return(t);
}
// ── porteiro 1: gatilho confirmado + o proprio Ciclo confirmando a direcao ──
bool BTCVPermite(bool ehCompra, string &mot)
{
   if(!InpCVEntrada) return(true);
   BTCVEstado();
   int dir = ehCompra ? 1 : -1;
   if(btCVDir != dir) { mot = "cicloverif_sem_oportunidade"; return(false); }
   if(!btCVConf)      { mot = "cicloverif_sem_confirmacao";  return(false); }
   // nunca no escuro: a historia do 1m na direcao, confirmada pelo 5m (a
   // confirmacao mais profunda — 15m/30m — vai carimbada em cvconf p/ o v5)
   string letra = ehCompra ? "U" : "D";
   if(StringSubstr(BTCVTF(PERIOD_M1), 0, 1) != letra ||
      StringSubstr(BTCVTF(PERIOD_M5), 0, 1) != letra)
      { mot = "cicloverif_ciclo_nao_confirma"; return(false); }
   btCVDir = 0; btCVGat = 0.0; btCVOrig = ""; btCVConf = false;
   return(true);
}
// ── porteiro 2: reentrada pelo 15m, lateral, exaustao, dia, filtros ─────────
int btCVReDir = 0;   // direcao da posicao CORTADA (caso vivo; SEM relogio)
void BTCVCorteReg(int dirPos) { btCVReDir = dirPos; }
bool BTCVGate2(bool ehCompra, string &mot)
{
   int dir = ehCompra ? 1 : -1;
   if(InpCVReentrada && btCVReDir != 0)
   {
      string l15 = StringSubstr(BTCVTF(PERIOD_M15), 0, 1);
      string velha = (btCVReDir == 1) ? "U" : "D";
      string nova  = (btCVReDir == 1) ? "D" : "U";
      if(l15 == velha) btCVReDir = 0;   // 15m re-confirmou a direcao antiga: caso encerrado
      else if(dir == btCVReDir) { mot = "cicloverif_pos_corte_aguarda"; return(false); }
      else if(l15 != nova)      { mot = "cicloverif_reentrada_sem_15m"; return(false); }
      else btCVReDir = 0;               // reversao CONFIRMADA no 15m: a favor liberado
   }
   if(InpCVLateral)
   {
      // identificada no 5m + confirmada no 15m (barras fechadas — sem esperar relogio)
      if(StringSubstr(BTCVTF(PERIOD_M5), 0, 1) == "L" &&
         StringSubstr(BTCVTF(PERIOD_M15), 0, 1) == "L")
         { mot = "cicloverif_lateral"; return(false); }
   }
   if(InpCVExaustao)
   {
      // exaustao lida pela ESCADA: 5m exausto na direcao da entrada, confirmado acima
      string e5 = BTCVTF(PERIOD_M5), e15 = BTCVTF(PERIOD_M15);
      string letra = ehCompra ? "U" : "D";
      bool ex5    = (StringSubstr(e5, 0, 1) == letra && StringFind(e5, "x") >= 0);
      bool conf15 = (StringSubstr(e15, 0, 1) == letra || StringFind(e15, "x") >= 0);
      if(ex5 && conf15) { mot = "cicloverif_exaustao"; return(false); }
   }
   if(InpCVDirecional)
   {
      int d = BTCVDia();
      if(d != 0 && d != dir) { mot = "cicloverif_contra_dia"; return(false); }
   }
   if(!BTCVSessaoOk())       { mot = "filtro_sessao";   return(false); }
   if(BTCVNoticiaBloqueia()) { mot = "filtro_noticia";  return(false); }
   if(InpCVAtrPiso > 0.0 && BTCVAtr14() < InpCVAtrPiso)
                             { mot = "filtro_atr_piso"; return(false); }
   return(true);
}
// ── VERIFICADOR CONTINUO EM POSICAO (doutrina do dono; Ciclo 1 corta) ───────
// 1m identifica (janela de InpCVJanela1m barras) -> 5m confirma = CORTA NA
// HORA -> nao identificado embaixo, o 15m julga. Sempre dentro da operacao.
bool BTCVQuebraContra(ENUM_TIMEFRAMES tf, int dirPos)
{
...
datetime btCVb1 = 0, btCVb15 = 0; int btCVJan = 0;
void BTCVTick()
{
   if(!InpCVCorte) return;
   datetime b1 = iTime(_Symbol, PERIOD_M1, 0);
   if(b1 == btCVb1) return;   // decisao por barra de 1m FECHADA
   btCVb1 = b1;
   int dir = 0; ulong tk = 0;
   for(int i = PositionsTotal() - 1; i >= 0; i--)
   {
   ...
   if(dir == 0) { btCVJan = 0; return; }
   if(BTCVQuebraContra(PERIOD_M1, dir)) btCVJan = InpCVJanela1m;   // 1m identificou: abre/renova a janela
   else if(btCVJan > 0) btCVJan--;
   if(btCVJan > 0 && BTCVQuebraContra(PERIOD_M5, dir))             // 5m confirmou: corta NA HORA
   {
      if(trade.PositionClose(tk))
      {
         BTEvento("fechado", "preco=" + DoubleToString(trade.ResultPrice(), _Digits)
                  + "|ticket=" + IntegerToString((long)tk) + "|motivo=corte_verificacao|via=5m");
         BTCVCorteReg(dir);
      }
      btCVJan = 0;
      return;
   }
   // nao identificado/confirmado embaixo: o 15m julga (por barra de 15m FECHADA)
   datetime b15 = iTime(_Symbol, PERIOD_M15, 0);
   if(b15 == btCVb15) return;
   btCVb15 = b15;
   if(BTCVQuebraContra(PERIOD_M15, dir))
   {
      if(trade.PositionClose(tk))
      {
         BTEvento("fechado", "preco=" + DoubleToString(trade.ResultPrice(), _Digits)
                  + "|ticket=" + IntegerToString((long)tk) + "|motivo=corte_verificacao|via=15m");
         BTCVCorteReg(dir);
      }
      btCVJan = 0;
   }
...
bool BTBuy(double lote, string sym, double preco, double sl, double tp, string com="")
{
   ...
   string btCVT = BTCVOpTag();
   if(!BTCVPermite(true, btMot))                        { BTvRecusada("BUY", btMot); return false; }
   if(!BTCVGate2(true, btMot))                          { BTvRecusada("BUY", btMot); return false; }
   ...
   if(ok && BTvExecOk()) BTEvento("aberto", BTvExecExtra("lado=BUY", sl, tp, lote) + BTNivTag(btNvRot, btNvPx, btNvTq, btNvD, true, preco) + BTCVTag() + btCVT);
...
bool BTSell(double lote, string sym, double preco, double sl, double tp, string com="")
{
   ...
   string btCVT = BTCVOpTag();
   if(!BTCVPermite(false, btMot))                       { BTvRecusada("SELL", btMot); return false; }
   if(!BTCVGate2(false, btMot))                         { BTvRecusada("SELL", btMot); return false; }
   double btNvPx; int btNvTq; double btNvD;
   ...
   if(ok && BTvExecOk()) BTEvento("aberto", BTvExecExtra("lado=SELL", sl, tp, lote) + BTNivTag(btNvRot, btNvPx, btNvTq, btNvD, false, preco) + BTCVTag() + btCVT);
...
def _gerar_mq5_de_codigo(codigo_python, params, idioma="pt"):
    ...
    if "BT_EXECUTOR_PURO" in (codigo_python or ""):
        try:
            ...
            if not texto or "BT_EVT_V2" not in texto or "BT_DISCIPLINA_V3" not in texto or "BT_OTT_V2" not in texto or "BT_MAGIC_ISO_V1" not in texto or "BT_CICLO_V1" not in texto or "BT_NIVEIS_V1" not in texto or "BT_CICLOVERIF_V1" not in texto:
                esqueleto = (
                    "//+------------------------------------------------------------------+\n"
    ...
    if "BT_ESTRATEGIA2" in (codigo_python or ""):
        try:
            ...
            hit = _mq5_cache_buscar(gen_hash)
            texto = (hit or {}).get("mq5") or ""
            if not texto or "BT_EVT_V2" not in texto or "BT_DISCIPLINA_V3" not in texto or "BT_OTT_V2" not in texto or "BT_MAGIC_ISO_V1" not in texto or "BT_CICLO_V1" not in texto or "BT_NIVEIS_V1" not in texto or "BT_CICLOVERIF_V1" not in texto or "BT_STOPE_V1" not in texto:
                esqueleto = (
                    "//+------------------------------------------------------------------+\n"
    ...
    try:
        ...
        if hit and hit.get("mq5"):
            ...
            if "BT_EVT_V2" not in texto or "BT_DISCIPLINA_V3" not in texto or "BT_OTT_V2" not in texto or "BT_MAGIC_ISO_V1" not in texto or "BT_CICLO_V1" not in texto or "BT_NIVEIS_V1" not in texto or "BT_CICLOVERIF_V1" not in texto:
