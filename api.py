...
_BT_VISAO_MQL5 = r"""//--- BotTested v7.13 — EA DISCIPLINADO (BT_DISCIPLINA_V2): regras de fabrica
...
def _gerar_mq5_de_codigo(codigo_python, params, idioma="pt"):
    ...
    try:
        ...
        if hit and hit.get("mq5"):
            ...
            if "BT_EVT_V2" not in texto or "BT_DISCIPLINA_V2" not in texto:
                print(f"[mq5-cache] HIT {gen_hash[:10]} sem BT_EVT_V2/BT_DISCIPLINA_V2 — descartado pra regenerar com eventos reais + regras de fabrica")
    ...
    try:
        ...
        for est in alvos:
            ...
            if hit and hit.get("mq5"):
                ...
                if "BT_EVT_V2" in (hit.get("mq5") or "") and "BT_DISCIPLINA_V2" in (hit.get("mq5") or ""):
                    _MQ5_AQUECER["feitas"] += 1
                    print(f"[mq5-aquecer] {est.get('id')} já no cache ({h[:10]})")
                    continue
                _mq5_cache_apagar(h)
                print(f"[mq5-aquecer] {est.get('id')} pré-BT_EVT_V2/BT_DISCIPLINA_V2 no cache — regenerando")
