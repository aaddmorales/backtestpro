# ============================================================
#  BotTested API — v7.45  (a versão REAL está em API_VERSAO/BUILD_TAG, ~linha 640, e no /versao)
#  Build: 2026-08-11a-base-nasce-ligada | Deploy: Railway
...
bool BTBuy(double lote, string sym, double preco, double sl, double tp, string com="")
{
   ...
   if(ok && BTvExecOk()) BTEvento("aberto", BTvExecExtra("lado=BUY", sl, tp, lote) + BTNivTag(btNvRot, btNvPx, btNvTq, btNvD, true, preco));
...
bool BTSell(double lote, string sym, double preco, double sl, double tp, string com="")
{
   ...
   if(ok && BTvExecOk()) BTEvento("aberto", BTvExecExtra("lado=SELL", sl, tp, lote) + BTNivTag(btNvRot, btNvPx, btNvTq, btNvD, false, preco));
...
def _gerar_mq5_de_codigo(codigo_python, params, idioma="pt"):
    ...
    if "BT_EXECUTOR_PURO" in (codigo_python or ""):
        try:
            ...
            if not texto or "BT_EVT_V2" not in texto or "BT_DISCIPLINA_V3" not in texto or "BT_OTT_V2" not in texto or "BT_MAGIC_ISO_V1" not in texto or "BT_CICLO_V1" not in texto or "BT_NIVEIS_V1" not in texto:
                esqueleto = (
                    "//+------------------------------------------------------------------+\n"
    ...
    if "BT_ESTRATEGIA2" in (codigo_python or ""):
        try:
            ...
            hit = _mq5_cache_buscar(gen_hash)
            texto = (hit or {}).get("mq5") or ""
            if not texto or "BT_EVT_V2" not in texto or "BT_DISCIPLINA_V3" not in texto or "BT_OTT_V2" not in texto or "BT_MAGIC_ISO_V1" not in texto or "BT_CICLO_V1" not in texto or "BT_NIVEIS_V1" not in texto or "BT_STOPE_V1" not in texto:
                esqueleto = (
                    "//+------------------------------------------------------------------+\n"
    ...
    try:
        ...
        if hit and hit.get("mq5"):
            ...
            if "BT_EVT_V2" not in texto or "BT_DISCIPLINA_V3" not in texto or "BT_OTT_V2" not in texto or "BT_MAGIC_ISO_V1" not in texto or "BT_CICLO_V1" not in texto or "BT_NIVEIS_V1" not in texto:
