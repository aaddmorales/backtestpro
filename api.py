# ============================================================
#  BotTested API — v7.03  (a versão REAL está em API_VERSAO/BUILD_TAG, ~linha 640, e no /versao)
...
def _chk_leitura(det, s, b):
    """v7.03 — monta o checklist do card a partir do que o monitor ja tem."""
    try:
        sc = _scanner_pacote(det) or {}
        gat = sc.get("gatilho") or {}
        direcao = gat.get("direcao") or gat.get("dir") or None
        rr = gat.get("rr")
        ex = _extras_cockpit(det, s)
        c_op = []
        try:
            c_op = _parse_candles_para_lista(det.get("c_op") or det.get("candles_op") or "")
        except Exception:
            pass
        return _checklist_entrada(direcao=direcao, scanner=sc, extras=ex,
                                  candles_op=c_op, rr=rr,
                                  atr_pts=(ex or {}).get("atr_pts"),
...
def monitor_leitura(user_id: str):
    ...
    for b in bots:
        ...
        saida.append({
            ...
            "checklist": _chk_leitura(det, s, b),
        })
    return {"bots": saida}
