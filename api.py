# ============================================================
#  BotTested API — v7.40  (a versão REAL está em API_VERSAO/BUILD_TAG, ~linha 640, e no /versao)
#  Build: 2026-08-06b-estrategia2 | Deploy: Railway
...
def _instrumentar_log_mql5(codigo: str) -> str:
    ...
    # ── v7.30 — BT_OTT_V1: TELEMETRIA POR TRANSACAO (a cura da Learning Machine cega)
    ...
    if ("void BTEvento" not in codigo and "OnTradeTransaction" not in codigo
        ...
        _bt_ott = (
            "\n// ── BT_OTT_V1 — telemetria por transacao (BotTested v7.30) ──\n"
            ...
            "      BTOttGravar(\"aberto\", _ex);\n"
            "   else if(_entrada == DEAL_ENTRY_INOUT)\n"
            "     {\n"
            "      // reversao num deal so: fecha a antiga e abre a nova\n"
...
def _gerar_mq5_de_codigo(codigo_python, params, idioma="pt"):
    ...
    if "BT_EXECUTOR_PURO" in (codigo_python or ""):
        try:
            ...
            if not texto or "BT_EVT_V2" not in texto or "BT_DISCIPLINA_V3" not in texto or "BT_OTT_V1" not in texto:
                esqueleto = (
                    "//+------------------------------------------------------------------+\n"
    ...
    if "BT_ESTRATEGIA2" in (codigo_python or ""):
        try:
            ...
            hit = _mq5_cache_buscar(gen_hash)
            texto = (hit or {}).get("mq5") or ""
            if not texto or "BT_EVT_V2" not in texto or "BT_DISCIPLINA_V3" not in texto or "BT_OTT_V1" not in texto or "BT_STOPE_V1" not in texto:
                esqueleto = (
                    "//+------------------------------------------------------------------+\n"
    ...
    try:
        ...
        if hit and hit.get("mq5"):
            ...
            if "BT_EVT_V2" not in texto or "BT_DISCIPLINA_V3" not in texto or "BT_OTT_V1" not in texto:
