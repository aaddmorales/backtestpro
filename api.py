# ============================================================
#  BotTested API — v7.41  (a versão REAL está em API_VERSAO/BUILD_TAG, ~linha 640, e no /versao)
#  Build: 2026-08-06c-ott-v2-painel-magic | Deploy: Railway
...
def _gerar_mq5_de_codigo(codigo_python, params, idioma="pt"):
    ...
    if "BT_EXECUTOR_PURO" in (codigo_python or ""):
        try:
            ...
            if not texto or "BT_EVT_V2" not in texto or "BT_DISCIPLINA_V3" not in texto or "BT_OTT_V2" not in texto:
                esqueleto = (
                    "//+------------------------------------------------------------------+\n"
    ...
    if "BT_ESTRATEGIA2" in (codigo_python or ""):
        try:
            ...
            hit = _mq5_cache_buscar(gen_hash)
            texto = (hit or {}).get("mq5") or ""
            if not texto or "BT_EVT_V2" not in texto or "BT_DISCIPLINA_V3" not in texto or "BT_OTT_V2" not in texto or "BT_STOPE_V1" not in texto:
                esqueleto = (
                    "//+------------------------------------------------------------------+\n"
    ...
    try:
        ...
        if hit and hit.get("mq5"):
            ...
            if "BT_EVT_V2" not in texto or "BT_DISCIPLINA_V3" not in texto or "BT_OTT_V2" not in texto:
