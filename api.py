# ============================================================
#  BotTested API — v7.12  (a versão REAL está em API_VERSAO/BUILD_TAG, ~linha 640, e no /versao)
#  Build: 2026-07-25g-ea-disciplinado | Deploy: Railway
...
{
   string btMot = "";
   if(!BTvDisciplina(true, preco, sl, lote, btMot)) { BTvRecusada("BUY", btMot); return false; }
...
{
   string btMot = "";
   if(!BTvDisciplina(false, preco, sl, lote, btMot)) { BTvRecusada("SELL", btMot); return false; }
...
public:
   ...
   bool Buy(double volume, const string symbol=NULL, double price=0.0, double sl=0.0, double tp=0.0, const string comment="")
   {
      ...
      string btMot = "";
      if(!BTvDisciplina(true, p, sl, volume, btMot)) { BTvRecusada("BUY", btMot); return false; }
   ...
   bool Sell(double volume, const string symbol=NULL, double price=0.0, double sl=0.0, double tp=0.0, const string comment="")
   {
      ...
      string btMot = "";
      if(!BTvDisciplina(false, p, sl, volume, btMot)) { BTvRecusada("SELL", btMot); return false; }
   ...
   bool PositionOpen(const string symbol, ENUM_ORDER_TYPE order_type, double volume, double price, double sl, double tp, const string comment="")
   {
      ...
      string btMot = "";
      if(!BTvDisciplina(ehCompra, p, sl, volume, btMot)) { BTvRecusada(ehCompra ? "BUY" : "SELL", btMot); return false; }
...
_BT_VISAO_MQL5 = r"""//--- BotTested v7.12 — EA DISCIPLINADO (BT_DISCIPLINA_V1): regras de fabrica
...
bool BTvDisciplina(bool ehCompra, double preco, double &sl, double &lote, string &motivo)
{
...
def _gerar_mq5_de_codigo(codigo_python, params, idioma="pt"):
    ...
    try:
        ...
        if hit and hit.get("mq5"):
            ...
            if "BT_EVT_V2" not in texto:
                print(f"[mq5-cache] HIT {gen_hash[:10]} sem BT_EVT_V2 — descartado pra regenerar com eventos reais")
    ...
    try:
        ...
        for est in alvos:
            ...
            if hit and hit.get("mq5"):
                ...
                if "BT_EVT_V2" in (hit.get("mq5") or ""):
                    _MQ5_AQUECER["feitas"] += 1
                    print(f"[mq5-aquecer] {est.get('id')} já no cache ({h[:10]})")
                    continue
                _mq5_cache_apagar(h)
                print(f"[mq5-aquecer] {est.get('id')} pré-BT_EVT_V2 no cache — regenerando")
