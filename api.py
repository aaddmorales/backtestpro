# ============================================================
#  BotTested API — v7.07  (a versão REAL está em API_VERSAO/BUILD_TAG, ~linha 640, e no /versao)
...
def _chk_leitura(det, s, b, c_op=None):
    ...
    try:
        ...
        ali = str(pac.get("alinhamento") or "").lower()
...
def _checklist_entrada(direcao=None, scanner=None, extras=None, candles_op=None,
    ...
    try:
        sc = scanner or {}
        alin = str(sc.get("alinhamento") or "")
        regime = ""
        try:
    ...
    try:
        ...
        for c in cs:
            try:
                ...
                pass
        if len(vols) >= 6 and any(v > 0 for v in vols):
            ult = vols[-1]
            base = [v for v in vols[-11:-1] if v > 0]
            med = (sum(base) / len(base)) if base else 0
            if med > 0:
                if ult >= med:
                    ok3, det3 = True, f"volume da vela {ult:.0f} >= media {med:.0f}"
                else:
                    ok3, det3 = False, f"volume fraco: {ult:.0f} < media {med:.0f}"
    except Exception:
        pass
