# ============================================================
#  BotTested API — v6.90  (a versão REAL está em API_VERSAO/BUILD_TAG, ~linha 640, e no /versao)
#  Build: 2026-07-19o-porque-da-entrada | Deploy: Railway
...
def _bm_reconciliar_abertas(sb, bot_id, posicoes_abertas):
    """v6.89 — abertas FANTASMAS viram órfãs: MT5 reporta 0 posições mas a
    BabyMachine tem 'aberta' com mais de 3 min = evento da era pré-BT_EVT_V2
    (emitido antes da execução; ordem provavelmente rejeitada) ou fechamento
    perdido. Nunca vão fechar sozinhas — drena com motivo. Throttle 120s/bot."""
    try:
        ...
        corte = (_dt.now(_tz.utc) - timedelta(minutes=3)).isoformat()
        ...
        sb.table("babymachine_operacoes").update({
            ...
            "resultado": {"motivo": "aberta sem posição correspondente no MT5 "
                                    "(provável ordem rejeitada — era pré-eventos-confirmados)"},
        }).in_("id", ids).execute()
        print(f"[bm-reconciliar] bot {bot_id}: {len(ids)} aberta(s) fantasma(s) viraram órfãs")
    except Exception as _e:
