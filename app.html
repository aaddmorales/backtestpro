...
:root{
  ...
  --text:#dde3ee;--muted:#5a6478;--muted2:#8a94a8;
...
.radar-sep{display:flex;align-items:center;gap:8px;margin:4px 0;font-size:10px;color:var(--muted);font-family:var(--mono)}
...
.radar-panel{position:fixed;bottom:86px;right:22px;width:360px;max-width:92vw;max-height:520px;
...
.radar-msg{background:var(--bg3);border:1px solid var(--border);border-left:3px solid var(--green);
  border-radius:0 10px 10px 10px;padding:10px 12px;font-size:12px;color:var(--text);line-height:1.6;max-width:95%}
...
.radar-act{flex:1;min-width:100px;padding:8px 10px;border-radius:8px;border:1px solid var(--border2);
  background:var(--bg3);color:var(--text);font-size:11px;font-weight:600;cursor:pointer;text-align:center;transition:all .12s}
...
.radar-toast{position:fixed;bottom:86px;right:86px;max-width:300px;background:var(--bg2);
  border:1px solid rgba(0,208,132,0.4);border-radius:12px 12px 2px 12px;padding:11px 14px;font-size:11.5px;
...
body.light select,body.light input[type=number],body.light input[type=text]{background:#f8f9fb;border-color:rgba(0,0,0,0.1);color:var(--text)}
...
<div class="topbar">
  ...
  <button class="tbtn" id="tab-monitor-btn" onclick="switchTab('monitor')">📡 Monitor</button>
  <div class="spacer"></div>
  <div class="top-right">
  <div style="position:relative">
    <button class="theme-icon" id="sino-btn" onclick="toggleSino()" title="Sugestões do agente">🔔<span id="sino-badge" style="display:none;position:absolute;top:-3px;right:-3px;background:var(--red);color:#fff;font-size:9px;font-weight:700;min-width:15px;height:15px;border-radius:8px;line-height:15px;text-align:center;padding:0 3px">0</span></button>
    <div id="sino-dropdown" style="display:none;position:absolute;top:40px;right:0;width:330px;max-height:420px;overflow-y:auto;background:var(--bg3);border:1px solid var(--border2);border-radius:10px;z-index:200;box-shadow:0 10px 35px rgba(0,0,0,0.5)">
      <div style="padding:11px 14px;border-bottom:1px solid var(--border);font-size:12px;font-weight:700;color:var(--text)" id="sino-titulo">🤖 Sugestões do agente</div>
      <div id="sino-lista" style="padding:6px"><div style="padding:18px;text-align:center;font-size:12px;color:var(--muted)">Nenhuma sugestão por enquanto</div></div>
    </div>
  </div>
...
<div class="main">
  ...
  <div class="col-left">
    <div class="bt-counter" id="bt-counter" title="Backtests rodados">
      <span class="pulse"></span><span id="bt-counter-num">0</span>&nbsp;backtests rodados
    </div>
    <div class="ps">
...
#                 self.position.close()"></textarea>
          ...
          <div id="tab-overview" style="display:block">
            <div id="ia-hero" style="margin:16px 16px 0;padding:18px 20px;background:linear-gradient(135deg,rgba(157,111,255,0.10),rgba(0,208,132,0.07));border:1px solid rgba(157,111,255,0.30);border-radius:12px">
              <div style="font-size:14px;font-weight:700;color:var(--text);margin-bottom:3px">✨ Descreva sua estratégia em português</div>
              ...
              <div style="display:flex;gap:8px;flex-wrap:wrap">
                <div style="flex:1;min-width:240px;position:relative;display:flex">
                  <input type="text" id="ia-hero-desc" autocomplete="off" name="bb_nl_q_x7" spellcheck="false" readonly onfocus="this.removeAttribute('readonly')" placeholder='Descreva ("canal EMA 20 high/low") ou escolha uma pronta no botão ▾' style="flex:1;background:var(--bg2);border:1px solid var(--border2);border-radius:8px 0 0 8px;border-right:none;color:var(--text);font-size:12.5px;padding:11px 13px;outline:none" onkeydown="if(event.key==='Enter')gerarHeroIA()">
                  <button id="hero-lista-btn" onclick="toggleHeroLista(event)" title="Escolher estratégia pronta" style="width:42px;background:var(--bg2);border:1px solid var(--border2);border-radius:0 8px 8px 0;color:var(--green);font-size:14px;cursor:pointer;font-weight:700">▾</button>
                  <div id="hero-lista" style="display:none;position:absolute;top:46px;left:0;right:0;max-height:340px;overflow-y:auto;background:var(--bg3);border:1px solid var(--border2);border-radius:10px;z-index:120;box-shadow:0 12px 35px rgba(0,0,0,0.55)">
                ...
                <button class="tbtn tbtn-purple" id="ia-hero-btn" onclick="gerarHeroIA()" style="padding:11px 18px;font-size:12.5px">✨ Gerar estratégia</button>
              </div>
            </div>
    ...
    <div class="metrics-bar" id="metrics-bar">
      ...
      <div class="mb-card">
        ...
        <div class="sem" id="m-pf-sem"></div>
            ...
            <div id="ov-sample" style="padding:18px 16px">
...
function switchTab(tab) {
  activeTab = tab;
  ['editor','overview','templates','rank','planos','otimizar','monitor'].forEach(t => {
...
const _radarFrases = [
  ...
  'Consultando o banco coletivo de backtests...',
...
async function radarAnalisarAtivo(d){
  ...
  try{
    ...
    (resp.mensagens || []).forEach(msg => radarFala(msg));
    if(resp.aplicar && resp.aplicar.indicador){
      radarSugestaoBox(resp.aplicar);
    }
...
function radarSugestaoBox(ap){
  ...
  let ganho = '';
  if(ap.pf && ap.pf_atual) ganho = `<br><span style="color:var(--green);font-size:11px">PF histórico <b>${ap.pf}</b> vs seu teste <b>${ap.pf_atual}</b> — melhora validada no coletivo</span>`;
  else if(ap.pf) ganho = `<br><span style="color:var(--green);font-size:11px">PF histórico <b>${ap.pf}</b> no coletivo deste ativo</span>`;
  const div = document.createElement('div');
  div.className = 'radar-msg';
  div.innerHTML = `🎯 <b>Encontrei uma configuração historicamente mais forte:</b><br>${partes.join(' · ')}${ganho}
    <div style="display:flex;margin-top:9px">
      <button class="radar-act radar-btn-rodar" style="flex:1;border-color:rgba(0,208,132,0.5);color:var(--green);font-size:12px;padding:10px">▶ Aplicar e rodar</button>
    </div>`;
...
function radarCoach(d){
  try{
    ...
    if(nt > 0 && nt < 10){
      radarFala(`Só <b>${nt} trades</b> nesse teste — amostra pequena demais pra confiar. Aumente o período ou teste um timeframe menor antes de tirar conclusão.`, 'atencao');
    }
    if(!isNaN(pf) && pf < 1 && nt >= 10){
      radarFala(`Profit factor <b>${pf.toFixed(2)}</b> — essa configuração perde no histórico. Antes de descartar: use o <b>⚙ Otimizar</b> pra varrer stop/take e período, ou teste a mesma lógica em outro ativo.`, 'alerta');
    }
    if(!isNaN(pf) && pf >= 1.3 && !isNaN(dd) && dd > 15){
      radarFala(`Lucrativa (PF ${pf.toFixed(2)}), mas drawdown de <b>${dd.toFixed(1)}%</b> é pesado. Reduza exposição (Máx. Ops) ou aperte o stop — sobreviver ao pior momento importa mais que o retorno.`, 'atencao');
    }
    if(!isNaN(wr) && wr >= 45 && !isNaN(pf) && pf >= 1.3 && nt >= 20){
      radarFala(`Resultado sólido: win rate <b>${wr.toFixed(1)}%</b>, PF <b>${pf.toFixed(2)}</b> em ${nt} trades. Próximo passo profissional: validação <b>out-of-sample</b> — se segurar nos dados que a estratégia nunca viu, aí sim vale automatizar no MT5. 📡`);
    }
    if(!isNaN(wr) && wr < 35 && !isNaN(pf) && pf >= 1.2){
      radarFala(`Win rate baixo (${wr.toFixed(1)}%) mas PF ${pf.toFixed(2)} positivo = estratégia que <b>erra pequeno e acerta grande</b>. É um perfil válido — exige disciplina pra aguentar sequências de perda.`);
    }
...
async function atualizarContador(){
  try{
    ...
    if(el && typeof d.total === 'number') el.textContent = d.total.toLocaleString('pt-BR');
  }catch(e){}
}
...
document.addEventListener('DOMContentLoaded', ()=>{
  ...
  const st = document.getElementById('sino-titulo');
  if(st) st.textContent = '🤖 ' + AGENT_NAME + ' — sugestões';
...
async function gerarHeroIA(){
  const v = document.getElementById('ia-hero-desc').value.trim();
  if(!v){ document.getElementById('ia-hero-desc').focus(); return; }
  document.getElementById('ia-desc').value = v;
  const btn = document.getElementById('ia-hero-btn');
  btn.textContent='⏳ Gerando...'; btn.disabled=true;
  try{
    await gerarBotIA();                     // gera e cola no editor
    if(lastIACode){
      btn.textContent='▶ Testando...';
      await rodarBacktest();                // roda na hora — resultado na Overview
      switchTab('overview');
    } else {
      switchTab('editor');                  // sem codigo? mostra o editor
    }
  }catch(e){ console.log('heroIA', e); }
  btn.textContent='✨ Gerar estratégia'; btn.disabled=false;
}
...
async function carregarGaleria(){
  try{
    const r = await fetch(API+'/estrategias/prontas');
    const d = await r.json();
...
// ── Sino do agente ──
let _sinoAberto = false;
function toggleSino(){
  _sinoAberto = !_sinoAberto;
  document.getElementById('sino-dropdown').style.display = _sinoAberto ? 'block' : 'none';
  if(_sinoAberto) atualizarSino(true);
}
async function atualizarSino(render){
  ...
  try{
    ...
    if(d.nao_lidas>0){ badge.style.display='block'; badge.textContent=d.nao_lidas; }
    else badge.style.display='none';
    if(render||_sinoAberto) renderSugestoes(d.sugestoes, 'sino-lista');
...
<div class="radar-loading" id="radar-loading">
  <div class="radar-loading-card">
    ...
    <div class="radar-loading-sub">OffMind + histórico coletivo trabalhando nos seus parâmetros</div>
...
<div class="radar-panel" id="radar-panel">
  <div class="radar-head">
    <span style="font-size:20px">📡</span>
    <div>
      <div style="font-size:13px;font-weight:800;color:var(--text)" id="radar-titulo">Radar</div>
      <div style="font-size:10px;color:var(--muted2)">Seu copiloto de validação — fala de disciplina, nunca prevê mercado</div>
    </div>
  </div>
  <div class="radar-body" id="radar-body">
    <div class="radar-msg">Olá! Rode um backtest que eu te digo o que os números estão contando — e o melhor próximo passo. 🎯</div>
  </div>
  <div class="radar-acts">
    <button class="radar-act" onclick="abrirOtimizar();toggleRadar()">⚙ Otimizar</button>
    <button class="radar-act" onclick="switchTab('monitor');toggleRadar()">📡 Conectar MT5</button>
    <button class="radar-act" onclick="switchTab('templates');toggleRadar()">🏛️ Estratégias</button>
  </div>
</div>
</body>
</html>
